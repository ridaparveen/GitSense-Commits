...
*   **Subsequent Minor Refinements (11/5/2025, 2:11:31 PM - 2:45:57 PM):** Following the major refactor, numerous minor changes occurred, primarily focused on improving readability, adding comments, and refining console output (e.g., adding more descriptive messages and emojis like "📄 Final ref_type:", "⚠️ File path missing:", "❌ Error saving document:"). The core logic for `ref_type` handling and path determination remained consistent after the 2:09:40 PM update. A check for `!file.path` was consistently present to handle missing file paths.
...
## 10:38:41 PM
This log details several frontend code changes focusing on UI, state management, and data handling within a T360 application. Key updates revolve around a dashboard page, sidebar navigation, user authentication, analytics tooling, and the introduction of a global customer selection mechanism.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar-layout.tsx`**
    *   **Timestamp: 11/5/2025, 2:54:04 PM**: Initial setup of the `AppSidebarLayout` component, integrating a `Navbar` and `SideBar` for the overall page structure.
    *   **Timestamps: 11/5/2025, 4:33:08 PM & 4:33:15 PM**: Temporary debug string "kk" was added inside the `NavbarSpacer` component, which was subsequently removed.
    *   **Timestamp: 11/5/2025, 9:14:23 PM**: The temporary "kk" debug string was removed, restoring the `NavbarSpacer` to its intended state.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\dashboard\dashboard-page.tsx`**
    *   **Timestamp: 11/5/2025, 2:58:06 PM**: A significant update introducing extensive dashboard functionality. This includes importing various UI components, icons, and Redux hooks for state management. It defines API queries (`useFetchShipmentDataQuery`, `useFetchDeliveryDataQuery`, `useFetchCartDataQuery`) with parameters for filters, pagination, sorting, and date ranges. `CompanyName` was added to the `useFetchCartDataQuery` parameters, initially pulling from `localUser.companyname`.
    *   **Timestamps: 11/5/2025, 2:58:53 PM, 3:03:35 PM, 3:03:46 PM**: Minor or no functional changes, likely re-saves. The `inspector` console import was removed during this period.
    *   **Timestamp: 11/5/2025, 6:13:35 PM**: Added a `useEffect` hook to dispatch `setCustomer` with `localUser.companyname` for "CUSTOMER" roles, indicating an initial attempt to automatically set customer context for specific users. The `setCustomer` action was imported from `shipment-dashboard-slice`.
    *   **Timestamp: 11/5/2025, 9:01:12 PM**: The `CompanyName` parameter in `useFetchShipmentDataQuery` and `useFetchCartDataQuery` was updated to prioritize `dashboardSelector.selectedCustomer` over `localUser.companyname`, reflecting a shift towards dynamic customer selection. The `useEffect` for automatically setting the customer for "CUSTOMER" roles was commented out.
    *   **Timestamp: 11/5/2025, 9:11:03 PM**: `selectedCustomer` is now explicitly fetched from `state.globalSlice` instead of `state.shipmentDashboardSlice`, indicating a refactoring to centralize customer selection globally. The auto-set customer `useEffect` remained commented.
    *   **Timestamp: 11/5/2025, 10:36:47 PM**: Added debug `console.log` statements for `selectedCustomer` and `dashboardCartData`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\usercard.tsx`**
    *   **Timestamp: 11/5/2025, 3:05:14 PM**: Refactored the `handleLogout` function to ensure local storage items (`token`, `user`) are cleared before redirecting for both Auth0 and standard authentication flows. The logout label was "Sign out1".
    *   **Timestamp: 11/5/2025, 4:23:14 PM**: Introduced Redux and `useAccess` hook. `SelectCustomerPanel` was conditionally rendered based on user permissions, indicating initial exploration of placing customer selection within the user card dropdown area.
    *   **Timestamp: 11/5/2025, 4:30:28 PM**: The `permission` variable and `useAccess` hook were temporarily commented out, making `SelectCustomerPanel` always render (if present).
    *   **Timestamp: 11/5/2025, 4:31:21 PM**: The `permission` variable and `useAccess` hook were uncommented, restoring conditional rendering.
    *   **Timestamp: 11/5/2025, 4:32:38 PM**: `SelectCustomerPanel` component was removed from the `UserCard`'s render, suggesting its relocation. The logout label was changed from "Sign out1" to "Sign out".

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`**
    *   **Timestamp: 11/5/2025, 3:53:07 PM**: This file underwent a major update, introducing comprehensive analytics toolbar features. Key additions include PDF export functionality using `jsPDF` and `html2canvas-pro` (with a `getBase64Image` helper), advanced filtering options, and custom view management. It also integrates Redux for managing analytics state and uses a `useAccess` hook for permissions.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx`**
    *   **Timestamp: 11/5/2025, 4:20:15 PM**: Implemented token expiration checks with automatic logout and redirect. Introduced conditional rendering for "ADMIN" users, displaying a specific message on the dashboard if an admin is logged in, suggesting a need for customer selection.
    *   **Timestamp: 11/5/2025, 4:41:20 PM**: An incomplete `useEffect` was added for "CUSTOMER" roles, attempting to access `localUser?.companyname`.
    *   **Timestamp: 11/5/2025, 4:42:14 PM**: The `useEffect` for "CUSTOMER" roles was completed to dispatch `setCustomer` (from `analytics-slice`) with the user's company name.
    *   **Timestamp: 11/5/2025, 6:10:32 PM**: The admin-specific rendering logic and the `useEffect` for dispatching `setCustomer` for "CUSTOMER" roles were removed, simplifying the `Layout` component.
    *   **Timestamp: 11/5/2025, 9:12:02 PM**: The `useEffect` for "CUSTOMER" roles was re-added, but now dispatches `setSelectedCustomer` from the newly introduced `global-slice`, centralizing customer selection state.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx`**
    *   **Timestamp: 11/5/2025, 4:34:00 PM**: A temporary "kk" debug string was added to `SidebarLabel` elements, which was later removed.
    *   **Timestamp: 11/5/2025, 4:35:17 PM**: `SelectCustomerPanel`, `useAccess`, and Redux hooks were imported. `SelectCustomerPanel` was rendered directly within a `SidebarSection`. The "kk" debug string was removed.
    *   **Timestamp: 11/5/2025, 4:35:58 PM**: `SelectCustomerPanel` rendering became unconditional (removed `permission?.selectCustomer` check).
    *   **Timestamp: 11/5/2025, 4:37:01 PM**: The conditional rendering of `SelectCustomerPanel` based on `permission?.selectCustomer` was reintroduced.
    *   **Timestamp: 11/5/2025, 9:07:53 PM**: Significant refactoring for customer selection. `selectedCustomer` is now accessed from `state.globalSlice`. `SelectCustomerPanel` is rendered only for "ADMIN" users and includes an `onSelect` callback that dispatches `setSelectedCustomer` to the `global-slice`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\shipment-dashboard-slice.ts`**
    *   **Timestamp: 11/5/2025, 6:12:29 PM**: The `initialState` was updated to include `selectedCustomer` and `selectedDashobard`. A `setCustomer` reducer was added to manage these new state properties.
    *   **Timestamp: 11/5/2025, 6:13:25 PM**: The `setCustomer` action was exported, making it dispatchable.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\dashboard-api.ts`**
    *   **Timestamp: 11/5/2025, 8:39:50 PM**: Introduced Redux Toolkit Query API definitions for `fetchDeliveryData` and `fetchCartData`, including parameter handling and authorization headers.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\types\shipment.types.ts`**
    *   **Timestamp: 11/5/2025, 9:00:51 PM**: Updated the `DashboardState` interface to include `selectedCustomer` and `selectedDashobard` properties, reflecting changes in the Redux slice.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts`**
    *   **Timestamp: 11/5/2025, 9:06:15 PM**: A new Redux slice named `globalSlice` was created to manage a centralized `selectedCustomer` state with `setSelectedCustomer` and `clearSelectedCustomer` reducers.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\store.ts`**
    *   **Timestamp: 11/5/2025, 9:07:08 PM**: The `globalSlice` was integrated into the main Redux store's `appReducer`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\SelectCustomerPanel.tsx` (and `src\components\screen\analytics\SelectCustomer.tsx`, `src\SelectCustomer.tsx`)**
    *   **Timestamp: 11/5/2025, 9:08:52 PM**: Initial or major update of the `SelectCustomerPanel` component, providing a UI for searching, selecting, and paginating customer options via an API. It used to dispatch `setCustomer` to `analytics-slice`.
    *   **Timestamp: 11/5/2025, 9:13:54 PM**: The component, now potentially renamed `SelectCustomer.tsx`, was refactored to use the new `global-slice` for `setSelectedCustomer` and introduced an `onSelect` callback prop. It also added a "No results found" message.
    *   **Timestamp: 11/5/2025, 9:16:47 PM**: The `SelectCustomer` component was moved directly to the `src` directory, and its `ApiManager` import path was adjusted accordingly.

### Patterns and Recurring Elements:

1.  **Redux State Centralization**: A strong pattern of refactoring state management to centralize `selectedCustomer`. Initially, `CompanyName` was tied to `localUser`, then to `shipmentDashboardSlice`, then `analytics-slice`, and finally migrated to a dedicated `global-slice` for application-wide access. This indicates a growing need for a consistent customer context across different parts of the application.
2.  **User Role-Based Features**: Consistent implementation of conditional logic (especially in `Layout` and `Sidebar`) to enable or restrict features (like customer selection) based on the user's role ("ADMIN" vs. "CUSTOMER").
3.  **Component Relocation/Refactoring**: The `SelectCustomerPanel` component underwent several structural changes, including being moved between directories and having its Redux integration updated multiple times, signifying ongoing modularization and architectural adjustments.
4.  **API Integration with Redux Toolkit Query**: Widespread use of Redux Toolkit Query for efficient data fetching and caching for various dashboard and analytics data.
5.  **Debugging Practices**: Presence of commented-out code blocks, `console.log` statements, and temporary text insertions ("kk") indicates active development, testing, and debugging efforts.

## 10:43:26 PM
The changes log details updates across authentication routes and two core service files, `AuthRoutes.ts` and `dashboardService.ts`, and `UserService.ts`, all occurring on 11/5/2025. The modifications primarily involve refining data retrieval logic, enhancing user-specific data filtering, and improving logging within the backend system.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\routes\AuthRoutes.ts`**
    *   **Timestamp: 11/5/2025, 12:54:56 PM**
    *   This file introduces an authentication route `AuthRoutes.get(Paths.Auth.GetUserInfo)` to retrieve authenticated user information. It logs the user object and responds with the user details and an `OK` status. This appears to be an initial implementation of a user info endpoint.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\dashboardService.ts`**
    *   **Timestamp: 11/5/2025, 6:04:50 PM**
    *   Initial creation or a significant update. This file defines two main asynchronous functions: `getDashboardCardInfo` and `getDeliveryData`.
        *   `getDashboardCardInfo` is responsible for fetching key dashboard metrics like "Pending Departure," "In Transit," "ETA within 1 week," and "Out for delivery," along with their respective TEU (Twenty-foot Equivalent Unit) counts. It filters data based on `fromDate`, `toDate`, `CompanyName`, shipment completion status, and a departure date cutoff. User-specific conditions are applied using `buildMtrUserCondition`.
        *   `getDeliveryData` retrieves either "delivery" or "vessel" related tracking data based on query parameters. It also applies user-specific filtering and different SQL logic based on delivery `mode` (e.g., "ALL", "Rail Delivery", "Door Delivery", "Port Delivery").
    *   **Timestamps: 11/5/2025, 6:05:15 PM; 11/5/2025, 6:05:28 PM; 11/5/2025, 6:06:01 PM**
        *   These entries show no functional changes from the previous timestamp, indicating minor saves or reformatting without code modification.
    *   **Timestamp: 11/5/2025, 8:41:51 PM**
        *   **Significant Change:** The `getDashboardCardInfo` function was modified. The `CompanyName` is no longer directly pulled from `req.query`. Instead, it is now dynamically fetched using a new utility function `getUserObject` (imported from `@src/utils/analytics`) based on the authenticated `req.user` and additional query parameters (`companyname`, `access`). This suggests an enhancement in how company context is determined for dashboard data.
    *   **Timestamp: 11/5/2025, 8:42:22 PM**
        *   A minor update was introduced to `getDashboardCardInfo`, adding a `console.log("req.CompanyName", CompanyName);` statement to debug or log the dynamically resolved `CompanyName`.
    *   **Timestamp: 11/5/2025, 10:41:50 PM**
        *   The `getDashboardCardInfo` function's query parameter destructuring was refined, removing `CompanyName` from the explicit `req.query` extraction, aligning it with the logic of fetching `CompanyName` via `getUserObject`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\UserService.ts`**
    *   **Timestamp: 11/5/2025, 8:51:00 PM**
    *   This file provides a comprehensive set of user management services. Key functions include:
        *   `login`: Authenticates users, checks account status, and generates a JWT token upon successful login.
        *   `saveUserPreferences`: Manages user preferences (basic info, email/push notifications, event configurations like `t49_cont_emptyout_date`) by inserting or updating records in the `v2_user_details` table.
        *   `fetchUserPreferences`: Retrieves user preferences, using `COALESCE` to provide fallback values from `mtr_user_master` if `v2_user_details` lacks specific data.
        *   `saveInviteLog`: Records user invitations in `v2_user_invite_log` and triggers an email invitation.
        ...
        *   An `all` export aggregates these functions.
    *   **Timestamps: 11/5/2025, 8:51:15 PM; 11/5/2025, 8:51:46 PM**
        *   These entries show no functional changes from the previous timestamp, indicating minor saves or reformatting without code modification.

### Patterns and Recurring Elements:

*   **Timestamp Consistency:** All recorded changes occurred on 11/5/2025, suggesting a focused day of development or deployment activity.
*   **User Context and Authorization:** `req.user` is consistently used across `AuthRoutes.ts`, `dashboardService.ts`, and `UserService.ts` to access authenticated user information (e.g., `usercode`, `companyname`, `role`). This emphasizes a user-centric design where data access and preferences are tailored to the logged-in user.
*   **Database Interaction:** Direct SQL queries are prevalent, often built dynamically based on request parameters and user context. Both parameterized queries (e.g., `$1`, `$2`) for inserts/updates and string interpolation for complex `SELECT` statements are used.
*   **Logging and Error Handling:** Extensive use of `logger.info` for successful operations and `logger.error` within `try-catch` blocks for error logging is a recurring pattern, indicating a robust approach to monitoring and debugging. Error responses consistently include `HttpStatusCodes` and messages.
*   **Dashboard Logic:** The `dashboardService` consistently calculates TEU by dividing container type values by 20 and defines distinct filters for different shipment statuses (pending departure, in transit, etc.).
*   **Preferences Management:** The `UserService` demonstrates a detailed approach to managing user preferences, categorizing them into basic info, email/push configs, and event-specific notifications, and ensuring data consistency with `ON CONFLICT DO UPDATE` clauses.