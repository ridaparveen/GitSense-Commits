...
*   **Initial State (11/26/2025, 12:21:43 PM - 12:30:44 PM):**
    *   The `createHblMasterBooking` function was primarily set up for inserting new HBL (House Bill of Lading) master records and their associated details (vessel, HBL details, routing, containers, POs) into various database tables (`hbl_master`, `agent_booking_vessel`, `agent_booking_hbl`, `agent_booking_routing`, `agent_booking_container`, `agent_booking_po`).
    *   The `saveHblBooking` function, likely intended for updates, included initial queries to fetch current data, but the core update logic was truncated in the provided logs.
    *   A `bookingdate` field in the `bookingInsertQuery` object was initially commented out in both functions.

*   **Early Afternoon Development (11/26/2025, 2:32:34 PM):**
    *   The `bookingdate: payload.booking_date` field was uncommented in the `bookingInsertQuery` for both `createHblMasterBooking` and `saveHblBooking`, making it an active field for data persistence.
    *   A new field, `transport_mode: req.body.transportaion_mode`, was added to the `bookingInsertQuery` object within `createHblMasterBooking`.

*   **Introduction of Conditional Logic (11/26/2025, 2:49:21 PM):**
    *   `createHblMasterBooking` was significantly modified to handle both creation and updates. It introduced an `if (req.query.serial_id)` block. If a `serial_id` was present in the query parameters, it would attempt to `updateQuery` the `hbl_master` table. For all other related tables (vessel, HBL details, routing, containers, POs), it continued to use `insertQuery` or `insertMany`, implying that these related records would be newly inserted even during an update operation, rather than updated. If `serial_id` was absent, the original insertion logic was executed.

*   **Temporary Regression (11/26/2025, 4:55:51 PM):**
    *   A brief, but critical, change occurred where the `if (req.query.serial_id)` block in `createHblMasterBooking` was emptied, effectively disabling the conditional update logic and causing the function to always perform insertions, irrespective of a `serial_id` being provided.

*   **Logic Restoration and Partial Improvement (11/26/2025, 5:05:17 PM):**
    *   The conditional update logic in `createHblMasterBooking` was restored. Crucially, the `agent_booking_vessel` table's data persistence was changed to use `updateQuery` (with a `where` clause targeting `hbl_master_id: serial_id`) when `serial_id` is present, indicating a proper update for vessel details. However, routing, container, and PO lists still continued to use `insertMany`, suggesting a design where these lists are always new entries, or the update logic for them is pending.

*   **Parameter Source Change (11/26/2025, 5:47:24 PM - 6:04:47 PM):**
    *   The source for `serial_id` in `createHblMasterBooking` was moved from `req.query` to `req.body` (`const {serial_id} = req.body;`). This means the endpoint expects the record identifier for updates to be part of the request body rather than the URL query string.

**2. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`**

*   **New Route Added (11/26/2025, 2:49:43 PM):**
    *   A new POST route, `/create`, was added and configured to use the `createHblMasterBooking` controller function. This explicitly created a dedicated endpoint for the HBL master booking creation/conditional update process, separating it from the existing root POST route handled by `saveHblBooking`.

**Timestamps of Significant Changes:**

*   **11/26/2025, 2:32:34 PM**: `bookingdate` field activated and `transport_mode` added to `bookingInsertQuery`.
*   **11/26/2025, 2:49:21 PM**: `createHblMasterBooking` transformed to handle both insert and conditional update logic based on `req.query.serial_id`.
*   **11/26/2025, 2:49:43 PM**: New `/create` route added to separate booking creation/conditional update endpoint.
*   **11/26/2025, 5:05:17 PM**: `agent_booking_vessel` update logic improved in `createHblMasterBooking` to use `updateQuery` when `serial_id` is present.
*   **11/26/2025, 5:47:24 PM**: `serial_id` source for updates in `createHblMasterBooking` changed from `req.query` to `req.body`.

**Patterns and Recurring Elements:**

*   **Transactional Database Operations**: All major database interactions are wrapped within `query("BEGIN")`, `query("COMMIT")`, and `query("ROLLBACK")` to ensure atomicity and data consistency.
*   **Extensive Data Sanitization**: Almost all incoming fields from `req.body` are processed using `sanitizeString()` before being used in database operations, indicating a strong focus on preventing injection vulnerabilities.
*   **Consistent Date Formatting**: Dates from the request body are consistently formatted into `YYYY-MM-DD` using `AppDateFormate()` before being stored in the database.
*   **Modular Database Utilities**: The code relies on utility functions like `insertMany`, `insertQuery`, and `updateQuery` from a `../config/database` module.
*   **Role-Based Location Determination**: The `locationByRole` variable is dynamically set based on the authenticated user's role (`AGENT` vs. other roles) to determine the appropriate location data.
*   **Comprehensive Payload Mapping**: Both `createHblMasterBooking` and `saveHblBooking` map a large number of fields from the request body into a structured `hblEntryPayloadTypes` object.
*   **Evolution of `createHblMasterBooking`**: The function evolved from a pure creation function to a versatile endpoint capable of both creating new records and performing targeted updates to master and vessel data based on an identifier, while still inserting new related list items.

## 10:08:03 PM
The recent code changes primarily focus on the integration and refinement of House Bill of Lading (HBL) functionality within the application.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\store\api\agentBookingApi.js`**
    *   **Timestamp: 11/26/2025, 2:53:21 PM** - This file saw a significant expansion of its API endpoints. New mutations and queries were added for `createBookingXml`, `uploadBookingXml`, `saveHblBooking`, `fetchHblBookingFormDetails`, `updateCarrierBooking`, `updateDo`, `exportSprReport`, and `getHblDashboard`. `FormDetails` was introduced as a new `tagType` and integrated into `saveBooking` and `fetchBookingFormDetails`.
    *   **Timestamp: 11/26/2025, 5:48:11 PM** - A minor adjustment was made to the `saveHblBooking` mutation, changing its URL from `/hblEntry/create` to `/hblEntry`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLListDetails.jsx`**
    *   **Timestamp: 11/26/2025, 2:57:40 PM** - A new `HblListDetails` component was introduced, designed to display HBL information. It imported `useGetHblDashboardQuery`, `HBL_COLUMNS`, and `getHblActions`, preparing the UI for HBL data. At this point, it was still rendering `isfDummyData`.
    *   **Timestamp: 11/26/2025, 2:58:49 PM** - This component was updated to render actual HBL data by changing the `ThemedGrid`'s `data` prop from `isfDummyData` to `hblData?.data` and setting its `uniqueId` to `serial_id`.
    *   **Timestamp: 11/26/2025, 6:03:04 PM** - The `useGetHblDashboardQuery` call was enhanced to pass `agent_booking_id` from the `location.state`, making the HBL list context-aware. The "Add HBL" button's navigation state was also updated to pass both `booking_serial_id` and `booking_id`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\data\columns\agent.booking.js`**
    *   **Timestamp: 11/26/2025, 3:01:38 PM** - New column definitions, `HBL_COLUMNS`, were added. These columns are specifically designed for displaying HBL data, including fields like "Serial ID", "HBL No", "HBL Date", "Shipper", "Consignee", "Notify", and "Status" (with chip rendering).
    *   **Timestamp: 11/26/2025, 3:10:12 PM** - No functional changes were observed in this update; the content appears identical to the previous version.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\actions.js`**
    *   **Timestamp: 11/26/2025, 3:13:47 PM** - The action handlers were significantly refactored. `getBookingActions` was modularized into several arrays (`bookingActions`, `isfActions`, `hblAction`, `createHblAction`, `viewHblAction`, `viewAction`, `updateLineBookingAction`, `updateDOAction`), which are combined based on user access roles. A new `getHblActions` function was introduced, initially containing an "Edit HBL" action that navigated using `agent_booking_id`.
    *   **Timestamp: 11/26/2025, 5:33:49 PM** - The "Edit HBL" action within `getHblActions` was updated to pass `serial_id` instead of `agent_booking_id` in the navigation state for editing an HBL.
    *   **Timestamp: 11/26/2025, 5:35:42 PM** - Further refinement to the "Edit HBL" action in `getHblActions` now passes both `agent_booking_id` and `serial_id` to the navigation state for comprehensive context.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLform.jsx`**
    *   **Timestamp: 11/26/2025, 5:55:57 PM** - A major new component, `HBLForm`, was introduced for creating and editing HBLs. It integrates `useSaveHblBookingMutation`, `Formik` for form management with `HBLvalidationSchema`, and various HBL-specific sub-forms (e.g., `HblContainerDetailsForm`, `HblRoutingForm`). It uses Redux for state management of sub-lists (routing, containers, POs) and features a tabbed interface for different HBL details. Keyboard shortcuts for common actions were also implemented.
    *   **Timestamp: 11/26/2025, 6:11:50 PM** - The `handleSave` function in `HBLForm` was updated to ensure `serial_id` from `location.state` is consistently included in the payload for HBL updates. A `console.warn` statement was also added for debugging tab changes.

**Timestamps of Significant Changes:**

The project underwent rapid development on **November 26, 2025**. Key changes occurred at:
*   **2:53:21 PM:** Major API expansion in `agentBookingApi.js`, laying the foundation for HBL and XML functionalities.
*   **2:57:40 PM** and **2:58:49 PM:** Introduction and immediate activation of `HBLListDetails.jsx` to display live HBL data.
*   **3:01:38 PM:** Definition of `HBL_COLUMNS` for the new HBL data grid.
*   **3:13:47 PM:** Refactoring of action handlers in `actions.js` for improved access control and the initial setup of HBL actions.
*   **5:55:57 PM:** Introduction of the comprehensive `HBLForm.jsx` component, enabling full HBL creation and editing capabilities.

**Patterns and Recurring Elements:**

*   **HBL-Centric Development:** The overwhelming focus across all changes is the implementation and integration of House Bill of Lading (HBL) features. This includes API endpoints, UI components, data structures, and user actions.
*   **Redux Toolkit Query (RTK Query):** This pattern is heavily utilized in `agentBookingApi.js` for API interactions, demonstrating efficient data fetching, caching, and invalidation using `tagTypes`.
*   **Modular UI Components:** Components like `HBLListDetails` and `HBLForm` are built using sub-components (e.g., `HblHeaderForms`, `HblContainerDetailsForm`) and common utilities (`ThemedGrid`, `ThemeTabs`, `GridActions`), promoting reusability and maintainability.
*   **Formik for Complex Forms:** `Formik` is consistently used for handling form state, validation, and submission, especially for multi-section forms like `HBLForm`.
*   **Role-Based Action Management:** The `actions.js` file clearly demonstrates a pattern of defining and combining actions based on user roles (`agentBookingMenu`, `customerBookingMenu`, `adminBookingMenu`), ensuring appropriate access control for different user types.
*   **Data Passing via Navigation State:** The `react-router-dom` `useNavigate` hook is frequently used with the `state` object to pass contextual data (like `agent_booking_id`, `serial_id`, `formAction`) between different screens.
*   **Date Formatting:** Dates are consistently formatted using the `appDateFormat` utility function across various display columns.
*   **Debugging/Logging:** The presence of `console.log` statements throughout the code indicates active development and debugging efforts.

## 10:08:05 PM
**File-specific updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`**:
    *   The file consistently defines two main asynchronous functions: `createHblMasterBooking` and `saveHblBooking`.
    *   Initially, `createHblMasterBooking` was solely for inserting new HBL records into multiple related database tables (`hbl_master`, `agent_booking_vessel`, `agent_booking_hbl`, `agent_booking_routing`, `agent_booking_container`, `agent_booking_po`).
    *   Around **11/26/2025, 2:32:34 PM**, `bookingdate` and `transport_mode` fields were explicitly added to the `bookingInsertQuery` object within `createHblMasterBooking`.
    *   Significant refactoring occurred in `createHblMasterBooking` between **11/26/2025, 4:41:31 PM** and **11/26/2025, 5:58:58 PM**. This function was enhanced to handle both creation and update operations based on the presence of a `serial_id`.
        *   Initially, `serial_id` was expected as a query parameter (`req.query.serial_id`).
        *   The logic was updated to perform an `updateQuery` on `hbl_master` and `agent_booking_vessel` if `serial_id` exists, using `serial_id` in the `where` clause for `hbl_master` and `hbl_master_id` for `agent_booking_vessel`.
        *   Related lists (`routingList`, `containerList`, `basicPoList`) continue to be handled by `insertMany`, without explicit deletion of prior entries during an update, which might lead to duplicate records.
        *   The `else` block retains the original insertion logic for new HBL records.
        *   A key change at **11/26/2025, 5:58:58 PM** shifted the source of `serial_id` from `req.query` to `req.body`, meaning update operations would now expect the `serial_id` in the request body.
    *   The `saveHblBooking` function (partially logged) consistently shows logic for determining `EntryMode` (ADD or UPDATE) and fetching existing booking data (`agent_booking_entry`, `agent_booking_vessel`, etc.) before a potential `updateQuery`. The full update logic for this function is not shown in the logs.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`**:
    *   At **11/26/2025, 2:49:43 PM**, a new POST route `/create` was added, directly mapping to the `createHblMasterBooking` controller function. This makes the newly enhanced `createHblMasterBooking` accessible via a dedicated endpoint.

**Timestamps of significant changes:**

*   **11/26/2025, 12:21:43 PM**: Initial commit outlining the structure for HBL creation and partial update.
*   **11/26/2025, 2:32:34 PM**: Minor functional update to `createHblMasterBooking` by adding more fields to the main booking query.
*   **11/26/2025, 2:49:43 PM**: A new route `/create` was introduced, implying activation of the `createHblMasterBooking` functionality.
*   **11/26/2025, 4:41:31 PM - 5:58:58 PM**: Major logic overhaul in `createHblMasterBooking` to support both creation and update operations, including changes in how the update identifier (`serial_id`) is obtained (from `req.query` to `req.body`).

**Patterns or recurring elements in the content:**

*   **Input Handling and Sanitization:** A consistent pattern of sanitizing all string inputs using `sanitizeString()` and formatting dates using `AppDateFormate()` with a `YYYY-MM-DD` format (`db_format`) is applied to nearly all incoming `req.body` fields.
*   **Transactional Database Operations:** All significant database interactions (`createHblMasterBooking` and `saveHblBooking`) are wrapped in explicit SQL transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) to ensure data integrity.
*   **Multi-table Persistence:** HBL data is stored across several normalized tables, requiring multiple `insertQuery`, `updateQuery`, and `insertMany` calls within a single API request.
*   **Error Handling:** Custom `HttpError` is used for application-specific error messages, and `logger.error` is consistently employed for logging caught exceptions.
*   **Code Duplication:** There's significant code duplication in payload construction and initial database operations between `createHblMasterBooking` and `saveHblBooking`, suggesting opportunities for abstraction. The repeated conditional insertion logic for lists within `createHblMasterBooking`'s `if`/`else` blocks also highlights this.
*   **Debugging Statements:** `console.log(req.body)` appears at the start of both major controller functions, indicating active debugging during development.