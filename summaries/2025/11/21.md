...
## 4:07:02 PM
The provided log details recent changes across two files in a React frontend application, `t360-frontend`.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\App.js`**
    *   **Timestamp:** Two updates occurred on 11/21/2025 at 11:31:13 AM and 11:34:00 AM.
    *   **Changes:** A minor adjustment was made to the `allowedRoles` prop for two specific routes: `"shipment_tracking"` and `"booking"`. In the later update (11:34:00 AM), `UserRoleEnum.ADMIN_V2` was removed from the list of roles permitted to access these routes, suggesting a refinement in role-based access control for these dashboards. The overall structure of imports and routes remains identical, indicating a stable routing configuration with a minor permission tweak. The file contains a console log `console.log("🚀 ~ file: App.js:2 ~ TEST on Flow");` which is present in both versions.
    *   **Content Overview:** This file acts as the main routing configuration for the application, importing a large number of screen components from various `pages` directories. It defines nested routes, applies `ProtectedRoute` for role-based access control using `UserRoleEnum`, and uses `AuthGuard` for initial authentication. Many routes are grouped under an `/app` path, indicating a core application area.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\MetricsBox.jsx`**
    *   **Timestamp:** 11/21/2025, 11:36:28 AM
    *   **Changes:** This entry represents a single state of the `MetricsBox` component, a React functional component designed for displaying various analytics charts.
    *   **Content Overview:**
        *   It imports several chart components (`PieChart`, `BarChart`, `LineChart`, `StackedBarChart`, `GaugeChart`) and utility components (`MetricToolbar`, `MetricFooter`, `ThemedModal`, `NoDataAvailable`).
        *   Manages local state for `chartView` (e.g., 'bar', 'pie', 'line') and an `enlarge` modal.
        *   Uses Redux (`useSelector`) to access analytics data.
        *   Includes a `getChartComponent` function that dynamically renders the appropriate chart based on `item.chartType` and `chartView`, handling both general and specific chart types (e.g., 'StackBar', 'Transit', 'Count'). It also calculates dynamic chart height for enlarged views.
        *   Renders a `MetricToolbar` for interactions, the selected chart component (or `NoDataAvailable`), and a `MetricFooter` which likely handles full-screen functionality via `ThemedModal`.
        *   Styling is applied to create a distinct box for the metric.

**Patterns and Recurring Elements:**

*   **Extensive Routing and Role-Based Access Control:** The `App.js` file demonstrates a complex routing structure with a large number of distinct screens, all secured by `ProtectedRoute` and granular `allowedRoles` using `UserRoleEnum`. This indicates a large-scale application with sophisticated user management and diverse functionalities for different user types (ADMIN, CUSTOMER, AGENT, CSF, FRONTOFFICE, BACKOFFICE, SPR_CUSTOMER, ADMIN_V2).
*   **Modular Component Design:** The `MetricsBox.jsx` file exemplifies a modular approach to building UI, where a parent component orchestrates the display and interaction of several specialized sub-components (like different chart types, toolbars, and footers).
*   **Analytics and Data Visualization:** The presence of `MetricsBox.jsx` and numerous chart imports, alongside an `analytics` route in `App.js`, highlights a strong focus on data visualization and reporting within the application.
*   **Application Feature Expansion:** The sheer number of distinct pages imported and routed in `App.js` (e.g., packing lists, service invoices, PO/DSO orders, booking, tracking, detention, commercial invoices, various "code" masters, reports, ALC, etc.) suggests an actively developed and continuously expanding application with a wide range of business functionalities.

## 4:07:06 PM
The file `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\company_details.controller.ts` underwent two minor updates on 11/21/2025, both at approximately 10:07-10:08 AM. The primary functionality of the `getAll` controller remains to fetch company names, differentiating between 'AGENT' (from `agent_po_account_master`) and 'CUSTOMER' (from `mtr_tracking_data`) roles. The 'CUSTOMER' role specifically supports pagination with `LIMIT` and `OFFSET` clauses. The second change involved removing several `console.log` statements within the 'CUSTOMER' role's pagination logic, indicating a cleanup or switch to a more centralized logging approach.

The file `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\otm_bol.route.ts` was updated on 11/21/2025 at 11:40:37 AM. This update expanded the OTM BOL (Bill of Lading) routing capabilities. New routes added include `/upload_save_edit` (POST), `/fetch/:common_key` (GET), `/generate_xml` (POST), and `/send_to_otm` (POST), alongside existing GET and POST routes for various BOL operations. A previously commented-out file upload route (`/upload`) is also noted.

The file `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts` saw a series of rapid changes on 11/21/2025, between 3:11 PM and 3:19 PM.
Initially, at 3:11:14 PM, two main functions were introduced: `createHblMasterBooking` for inserting new HBL master records and `saveHblBooking` for handling both creation and updating of HBL bookings with extensive detail. `saveHblBooking` includes logic to fetch current booking data before updates for logging history, and then performs updates across multiple related tables (`agent_booking_entry`, `agent_booking_vessel`, `agent_booking_routing`, `agent_booking_container`, `agent_booking_hbl`, `agent_booking_po`) by deleting and re-inserting list-type data (routing, containers, POs).
Subsequent changes focused on refining date handling and error management:
*   At 3:13:11 PM, the error handling in `createHblMasterBooking` was modified to only `logger.error` the error, removing the explicit `next(new HttpError...)` response.
*   At 3:16:23 PM, `bookingdate` in the `bookingInsertQuery` for `saveHblBooking` was made optional, falling back to `null` if not provided.
*   At 3:17:07 PM, `bookingdate` in `createHblMasterBooking`'s payload was also made optional (to `null`), and `hbldate` in `saveHblBooking` was corrected to use actual `null` instead of the string `'null'`. However, `bookingdate` in `bookingInsertQuery` was reverted to its previous state.
*   Finally, at 3:19:06 PM, `bookingdate` was entirely commented out from the payload in `createHblMasterBooking`, effectively removing it from the insertion process for `hbl_master`.

The related file `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts` was also updated on 11/21/2025.
*   At 3:11:42 PM, the routes were initially defined with a conflict: two distinct POST endpoints (`saveHblBooking` and `createHblMasterBooking`) were assigned to the exact same path (`/`).
*   This conflict was resolved at 3:19:52 PM by moving the `createHblMasterBooking` endpoint to a dedicated path, POST `/create`, while `saveHblBooking` retained POST `/`.

**Overall Patterns:**
*   **Database Transactionality:** Consistent use of `await query("BEGIN");`, `await query("COMMIT");`, and `await query("ROLLBACK");` across multiple controller functions (`createHblMasterBooking`, `saveHblBooking`) indicates a strong emphasis on data integrity.
*   **Data Sanitization and Formatting:** The `sanitizeString` utility is widely applied to request body parameters, and `AppDateFormate` is used for date conversion, highlighting concerns for security and consistent data storage.
*   **Detailed Logging:** `logger.info` and `logger.error` are frequently used for debugging and operational monitoring.
*   **Modular Design:** Controllers are separated from routes, and helper functions (like `getAgentBookingCode`) are utilized.
*   **Evolutionary Development:** The sequential changes to `hblEntry.controller.ts` and `hblEntry.route.ts` demonstrate an iterative development process, with initial implementation followed by refinements, bug fixes (like the route conflict), and adjustments to data handling.
*   **Role-Based Access/Logic:** The `company_details.controller.ts` explicitly branches logic based on `req.query.role`, and `hblEntry.controller.ts` uses `req.user.role` to determine `locationByRole`, indicating a role-aware application design.

## 9:25:49 PM
The code changes primarily focus on user authentication, logout functionality, and dynamic content rendering based on user roles and selected customer status.

**c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\usercard.tsx (Timestamp: 11/21/2025, 10:15:47 AM)**
This update refines the `UserCard` component, which handles displaying user details and the logout process. The key change is in the `handleLogout` function:
*   **Auth0 Logout Logic:** For Auth0 authenticated users, the local storage items (`token`, `user`) are now removed and the `auth/logout` Redux action is dispatched *before* redirecting. Previously, this happened after a `setTimeout` or inconsistently. Crucially, the redirection target for Auth0 users has changed from a `/logout` endpoint to the application's `/login` endpoint on the UI V1 base URL.
*   **Normal Logout Logic:** The process for non-Auth0 users remains largely consistent, involving clearing local storage, dispatching the logout action, and navigating to the root path (`/`).
The component continues to display the user's initials, full name (preferring fetched data over local storage), and role within a dropdown menu, providing a "Sign out" option.

**c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx (Timestamp: 11/21/2025, 10:20:01 AM)**
This change enhances the main `Layout` component, focusing on user session management and conditional rendering based on user roles and customer selection.
*   **Session Management:** An `isTokenExpired` function is introduced to check the validity of the user's token stored in `localStorage`. If the token is expired or missing, the `user` and `token` are cleared from `localStorage`, and the user is redirected to the root path (`/`). Otherwise, `initializeSelectedCustomer()` is dispatched.
*   **Customer Selection Logic:** A `useEffect` hook now dynamically manages the `selectedCustomer` state based on the user's role:
    *   **"CUSTOMER" Role:** Automatically sets the `selectedCustomer` to their own company name, updating both Redux state and `localStorage`.
    *   **"ADMIN_V2" Role:** Clears the `selectedCustomer` (sets to `null`), removing any previous selection from `localStorage` and Redux state.
*   **Conditional Content Rendering:** The application's `Outlet` (where nested routes are rendered) is only displayed if a `selectedCustomer` is present. If an "ADMIN_V2" user has not selected a customer, an informational message prompting them to "Please select a customer" is displayed instead.

**Patterns and Recurring Elements:**
*   **Local Storage Reliance:** Both files extensively use `localStorage` to store and retrieve user authentication details (`user`, `token`) and specific application state (`selectedCustomer`, `authtype`).
*   **Redux for State Management:** `useDispatch` and `useSelector` are consistently used for managing global application state, particularly for authentication (`auth/logout`) and customer selection (`initializeSelectedCustomer`, `setSelectedCustomer`).
*   **React Router for Navigation:** `useNavigate` from `react-router-dom` is utilized in both components for programmatic redirection based on authentication status or user actions.
*   **Role-Based Logic:** Both files contain logic that differentiates behavior based on user roles (e.g., "CUSTOMER" vs. "ADMIN_V2"), indicating a robust role-based access control (RBAC) implementation.
*   **Timestamps:** The changes were applied on the same date (11/21/2025) within a short time frame, suggesting a coordinated update or feature implementation related to user experience and authentication flow.

## 9:25:54 PM
The recent code changes, all occurring on 11/21/2025 within a few minutes (11:31 AM to 11:36 AM), primarily focus on two areas of the `t360-frontend` application: expanding routing and refining an analytics component.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\App.js` (11/21/2025, 11:31:13 AM and 11:34:00 AM):**
    This file, which defines the main application routing, received substantial updates. The two entries for this file show identical code, indicating a save operation without functional changes between those timestamps.
    The code adds a significant number of new route imports and corresponding `Route` definitions. These new routes cover diverse functionalities, including:
    *   **Tracking:** New screens for FCL, LCL, Rail, and Air tracking, along with specific components like `AddMtrForm`, `GetTruckingDetails`, and a general `ShipmentTrackingScreen`.
    *   **Booking and Orders:** `AgentBookingFormScreenExtend`, `AgentPoFormExtend`, `BookingDashbaordScreenExtend`, `CngBookingScreen`, `CngBookingScreenExtend`, `CngBookingFormScreen`, `ShipmentsScreen`.
    *   **Master Data:** `CityMaster` and `PortmasterScreen`.
    *   **Dashboard Enhancements:** `Dashboard7501Screen`.
    *   **ALC (Actual Landing Cost) related:** `AlcGenerated`, `SprAlcScreen`, `AlcExceptionDashboard`, `AlcDestinationCostRecord`.
    *   **Reports:** `ReportSchedulerScreen`, `NewReportScreen`, `EditReportScreen`.
    *   **Utility:** `PdfParserScreen` for parsing PDFs.
    *   **Administrative/Back Office:** `BackOfficeAdminTerminal`.
    Many existing routes also had their `ProtectedRoute` `allowedRoles` updated or confirmed, featuring a comprehensive list of user roles (`ADMIN`, `CUSTOMER`, `AGENT`, `CSF`, `FRONTOFFICE`, `BACKOFFICE`, `SPR_CUSTOMER`, `ADMIN_V2`). The application seems to have shifted its default root path (`/`) from `Auth0Login` to `LoginScreen`, with the Auth0 route commented out.
...
*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\MetricsBox.jsx` (11/21/2025, 11:36:28 AM):**
    This update focuses on the `MetricsBox` component within the analytics section. The most notable change is a minor correction in an import path, changing `import PieChart from './charts/PieChart';` to `import PieChart from './Charts/PieChart';`.
    The component itself is responsible for rendering various chart types (Bar, Pie, Line, Stacked Bar, Gauge) dynamically based on `item.chartType` and a user-controlled `chartView`. It includes functionality for filtering chart options, displaying "No Data Available" messages, and providing a full-screen view for charts via `ThemedModal`. It also integrates with Redux for state management and uses Material-UI for styling. Debug `console.log` statements are present.

**Patterns and Recurring Elements:**

*   **Extensive Feature Expansion:** The `App.js` changes reveal a significant expansion of the application's feature set, particularly in tracking, booking, and administrative modules.
*   **Robust Role-Based Access Control (RBAC):** The consistent use of `ProtectedRoute` with specific `UserRoleEnum` values across nearly all routes underscores a strong emphasis on granular access control.
*   **Modular Architecture:** The large number of imported and routed components indicates a highly modular and organized frontend structure.
*   **Focus on Shipping/Logistics Domains:** The types of screens (packing lists, BOLs, service invoices, various tracking, PO/DSO orders, detention, commercial invoices, ALC) strongly suggest an application serving a shipping, logistics, or supply chain management domain.
*   **Minor Code Refinements:** The `MetricsBox.jsx` change, specifically the case correction in the import path, points to ongoing code hygiene and potentially addressing compatibility issues on case-sensitive file systems.

## 9:26:03 PM
In `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\company_details.controller.ts`, significant changes occurred on **11/21/2025, 10:07:54 AM**. The `getAll` function was updated to dynamically fetch company names based on the user's `role` (AGENT or CUSTOMER). For AGENTs, it queries `cname` from `agent_po_account_master` with a search filter and a limit of 10. For CUSTOMERs, the logic was largely refactored to query `account_name` from `mtr_tracking_data`. This CUSTOMER-specific implementation now supports optional pagination, fetching total counts and applying `LIMIT` and `OFFSET` parameters if `pagination` is `true`, otherwise defaulting to a limit of 10. All results are formatted into `value`/`label` pairs. A subsequent change on **11/21/2025, 10:08:08 AM** for this file involved removing temporary `console.log` statements from the CUSTOMER role's pagination logic.

The file `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\otm_bol.route.ts` was updated on **11/21/2025, 11:40:37 AM**. This update defined several new routes for OTM (Oracle Transportation Management) Bill of Lading (BOL) operations, including endpoints for fetching Excel data (`/excel_data`), getting PL/PO by BOL (`/`), fetching payload (`/fetch_payload`), fetching by specific BOL number (`/:bol`), editing BOLs (`POST /`), saving uploaded edits (`/upload_save_edit`), fetching by common key (`/fetch/:common_key`), generating XML (`/generate_xml`), and sending data to OTM (`/send_to_otm`). Notably, a `POST /upload` route for file uploads using `uploadMiddleware` was present but commented out.

In `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`, a series of detailed updates took place.
*   On **11/21/2025, 3:11:14 PM**, two new core functions, `createHblMasterBooking` and `saveHblBooking`, were introduced. `createHblMasterBooking` handles the initial creation of an HBL record in the `hbl_master` table within a database transaction. `saveHblBooking` is a more complex function designed for both adding and updating comprehensive HBL booking details. For updates, it first fetches existing booking data from multiple tables (`agent_booking_entry`, `agent_booking_vessel`, `agent_booking_routing`, `agent_booking_container`, `agent_booking_hbl`, `agent_booking_po`), then proceeds to update the main entry and vessel details, and uses a delete-and-reinsert strategy for routing, container, and purchase order lists. A critical auditing feature was added, logging both the old and new states of the HBL booking to `agent_booking_hbl_history`.
*   A change on **11/21/2025, 3:13:11 PM** updated the error handling in `createHblMasterBooking` to log errors using `logger.error` instead of propagating them via `next(new HttpError(...))`.
*   Further refinements occurred on **11/21/2025, 3:16:23 PM**, where the `booking_date` field in `saveHblBooking`'s `bookingInsertQuery` was made optional by allowing it to be `null`.
*   On **11/21/2025, 3:17:07 PM**, the `bookingdate` in `createHblMasterBooking`'s payload was also made optional, though the `bookingdate` in `saveHblBooking`'s `bookingInsertQuery` was reverted to being non-optional.
*   Finally, on **11/21/2025, 3:19:06 PM**, the `bookingdate` field was completely commented out from the payload construction in `createHblMasterBooking`, indicating it is no longer processed or stored by this specific function.

The corresponding route file, `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`, also saw changes.
*   On **11/21/2025, 3:11:42 PM**, initial routes were defined, but a conflict existed where both `saveHblBooking` and `createHblMasterBooking` were assigned to the `POST /` endpoint.
*   This conflict was resolved on **11/21/2025, 3:19:52 PM** by moving `createHblMasterBooking` to a new dedicated endpoint `POST /create`, while `saveHblBooking` remained mapped to `POST /`. `GET /` continued to map to `fetchHblFormDetails`.

**Recurring Patterns:**
Across these changes, several patterns are evident:
*   **Database Transaction Management:** Consistent use of `BEGIN`, `COMMIT`, and `ROLLBACK` for multi-step database operations to ensure data integrity.
*   **Data Sanitization:** Extensive use of `sanitizeString` for incoming request body parameters, highlighting a focus on preventing common web vulnerabilities.
*   **Date Formatting:** Reliance on a utility function `AppDateFormate` to standardize date inputs for database storage.
*   **Logging:** Frequent use of `logger.info` and `logger.error` for operational insights and debugging.
*   **Robust Error Handling:** Implementation of `HttpError` and consistent 500-level error responses.
*   **Complex Booking Logic:** The HBL entry controllers demonstrate a sophisticated system for managing booking data, including detailed payload construction, multi-table updates, and an auditing mechanism.

## 10:36:17 PM
The recent code changes primarily focus on user session management, authentication, and user interface adjustments within two core frontend components.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\usercard.tsx` (Timestamp: 11/21/2025, 10:15:47 AM)**
This file updates the `UserCard` component, specifically refining its logout functionality. The `handleLogout` function has been restructured to ensure a more robust logout process, particularly for Auth0 users.
*   **Auth0 Logout Flow:** Previously, `localStorage` items (`token`, `user`) were cleared *after* a `setTimeout` and dispatching a Redux `auth/logout` action, followed by a redirect to a logout endpoint. The updated logic now clears `localStorage` and dispatches the Redux `auth/logout` action *before* initiating the redirect. Crucially, the redirect target for Auth0 users has changed from `${link}/logout` to `${link}/login`, ensuring users are directed back to the login page after clearing their session.
*   **Standard Logout Flow:** For non-Auth0 users, the logout process remains largely consistent: `localStorage` items are cleared, the `auth/logout` action is dispatched, and the user is navigated to the root path (`/`).
*   The component continues to display user information (name, role) within a dropdown menu, leveraging `Avatar` and `Dropdown` components for presentation and `useFetchUserPreferenceQuery` for user data.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx` (Timestamp: 11/21/2025, 10:20:01 AM)**
This file introduces significant logic for managing user sessions and customer selection within the main application layout.
*   **Token Expiration Check:** A new `isTokenExpired` function has been added to verify the validity of the user's token stored in `localStorage` by comparing the token's expiration time (`localUser.exp`) against the current time.
*   **Session Initialization and Redirection:** An `useEffect` hook now runs on component mount. If the token is found to be expired or invalid, all user-related data (`user`, `token`) is removed from `localStorage`, and the user is redirected to the root path (`/`). Otherwise, `initializeSelectedCustomer()` is dispatched via Redux.
*   **Role-Based Customer Selection:** Another `useEffect` manages the `selectedCustomer` state based on the user's role:
    *   **"CUSTOMER" Role:** If the logged-in user has the "CUSTOMER" role, their `companyname` is automatically set as the `selectedCustomer` in both Redux and `localStorage`. This implies customers are confined to viewing their own company data.
    *   **"ADMIN_V2" Role:** If the user is an "ADMIN_V2", the `selectedCustomer` is explicitly cleared (set to `null` in Redux and removed from `localStorage`). This suggests that administrators have the flexibility to select a specific customer to view or manage, and are not tied to one by default.
*   The component conditionally renders its `Outlet` (child routes) only when a `selectedCustomer` is present. If an `ADMIN_V2` user has no selected customer, a prompt "Please select a customer" is displayed.

**Patterns and Recurring Elements:**
*   **`localStorage` for Session Management:** Both files heavily rely on `localStorage` to store and retrieve user authentication details (`user`, `token`) and `selectedCustomer` state, indicating a persistent client-side session mechanism.
*   **Redux for State Management:** `useDispatch` and `useSelector` are consistently used for managing global state, particularly for authentication (`auth/logout`) and customer selection (`initializeSelectedCustomer`, `setSelectedCustomer`).
*   **`react-router-dom` for Navigation:** `useNavigate` is employed in both components to programmatically redirect users based on authentication status or logout actions.
*   **Role-Based Logic:** There's a clear pattern of implementing different logic flows and UI elements based on the user's assigned role (e.g., "CUSTOMER" vs. "ADMIN_V2").
*   **Timestamp Proximity:** The timestamps are very close, suggesting these changes were part of a single session or a closely related set of updates to enhance session security and user experience.

## 10:36:22 PM
The changes log shows updates across two files within the `t360-frontend` project, all occurring on November 21, 2025.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\App.js` (Timestamps: 11/21/2025, 11:31:13 AM and 11:34:00 AM)**
    *   Both entries for this file show identical code.
    *   The `App.js` file serves as the main routing configuration for the React application.
    *   It imports a very large number of components, primarily "Screen" components (e.g., `LoginScreen`, `DashboardScreen`, `UserManagementScreen`, `PackingListScreen`, `ServiceInvoiceScreen`, `ExpenseCodeScreen`, `DestinationScreen`, `VendorScreen`, `PoOrderScreen`, `DsoOrderScreen`, `OtmBolScreen`, `CustomerScreen`, `PartyScreen`, `AgentScreen`, `BookingPlanBoardScreen`, `SPRPlanBoardScreen`, `LineWiseScreen`, `CIScreen`, `AnalyticsScreen`, `ISFEntryFormScreen`, `AgentPoScreen`, `CngInvoiceScreen`, `AlcScreen`, `ShippingOrdersScreen`, `ContainerReportScreen`, `ReportSchedulerScreen`, `PdfParserScreen`, `AlcExceptionDashboard`, `AlcDestinationCostRecord`, various `TrackingFclScreen`, `LclScreen`, `RailScreen`, `AirTrackingScreen`, `ShipmentTrackingScreen`, `Dashboard7501Screen`, `ShipmentsScreen`, `CngBookingScreen`, `CityMaster`, `PortmasterScreen`).
    *   The application uses `react-router-dom` to define an extensive set of nested routes, all protected by `ProtectedRoute` components that enforce `allowedRoles` based on `UserRoleEnum` (including `ADMIN`, `CUSTOMER`, `AGENT`, `CSF`, `FRONTOFFICE`, `BACKOFFICE`, `SPR_CUSTOMER`, `ADMIN_V2`).
    *   Key top-level routes include `/`, `/callback`, `/register_user`, and the main application route `/app`.
    *   The `/app` route acts as a layout wrapper and contains numerous sub-routes for administrative tasks, master data management (customer, party, agent, city, port), analytics, various shipping and invoice processes (packing list, service invoice, commercial invoice, OTM BOL, CNG invoice, ALC), tracking features, and planboards.
    *   Several routes, particularly those related to booking and agent POs, show "Extend" or "V2" versions (e.g., `AgentBookingFormScreenExtend`, `BookingDashbaordScreenExtend`, `AgentPoFormExtend`), suggesting feature enhancements or replacements.
    *   A `console.log("🚀 ~ file: App.js:2 ~ TEST on Flow");` statement is present, likely for debugging or build flow testing.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\MetricsBox.jsx` (Timestamp: 11/21/2025, 11:36:28 AM)**
    *   This component is responsible for rendering various analytical charts and managing their display options within an analytics dashboard.
    *   It imports different chart types: `PieChart`, `BarChart`, `LineChart`, `StackedBarChart`, and `GaugeChart` from a `./Charts` directory.
    *   It utilizes `useState` to manage the `chartView` (e.g., 'bar', 'pie', 'line', 'stack', 'gauge') and an `enlarge` state for a full-screen modal view of the charts using `ThemedModal`.
    *   The component integrates `MetricToolbar` for controls (like changing chart view) and `MetricFooter` for additional actions, including the enlarge functionality.
    *   It dynamically selects and renders chart components based on the `item.chartType` property, supporting 'General' (for bar, pie, line), 'StackBar', 'Transit' (bar), and 'Count' (gauge) types.
    *   A `getChartComponent` function handles the conditional rendering of charts, including logic to calculate dynamic heights for enlarged charts.
    *   It displays a `NoDataAvailable` component when chart data is empty.
    *   `console.log` statements (`"==data==jjj", data` and `"===item===1234556", item`) are present for debugging purposes.

### Patterns and Recurring Elements:

*   **Consistent Timestamp:** All recorded changes occurred on the same date, November 21, 2025, suggesting a single development session or a set of closely related commits.
*   **Extensive Feature Set:** The `App.js` file reveals a complex application with a vast array of features related to logistics, shipping, finance (invoicing, expense codes), and administration.
*   **Role-Based Access Control:** The widespread use of `ProtectedRoute` with `UserRoleEnum` demonstrates a robust, granular role-based access control system across the entire application, with a significant number of defined roles including `ADMIN`, `CUSTOMER`, `AGENT`, `CSF`, `FRONTOFFICE`, `BACKOFFICE`, `SPR_CUSTOMER`, and `ADMIN_V2`.
*   **Modular UI Components:** The application is highly modular, with distinct "Screen" components for each major feature and separate chart components for analytics.
*   **Analytics Focus:** The `MetricsBox.jsx` component highlights a strong focus on data visualization and analytics within the application, offering multiple chart types and viewing options.
*   **Development Practices:** The presence of `console.log` statements in both `App.js` and `MetricsBox.jsx` indicates ongoing development or debugging efforts.
*   **Feature Evolution:** The inclusion of "Extend" suffixes (e.g., `AgentBookingFormScreenExtend`) suggests that existing features are being iterated upon or replaced with enhanced versions.

## 10:36:23 PM
The code changes primarily focus on refining data retrieval logic, restructuring API routes, and enhancing HBL (House Bill of Lading) booking functionality with detailed logging and update capabilities.

**File-specific updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\company_details.controller.ts`**
    *   **11/21/2025, 10:07:54 AM**: The `getAll` controller function was updated to support fetching company names based on user `role` (AGENT or CUSTOMER). For `AGENT`, it queries `agent_po_account_master`. For `CUSTOMER`, it implements conditional pagination logic, querying `mtr_tracking_data` for `account_name` with `LIMIT` and `OFFSET` based on `isPagination` flag, or a simple `LIMIT 10` if pagination is not requested. Debugging `console.log` statements were present.
    *   **11/21/2025, 10:08:08 AM**: A minor refinement was made in the `CUSTOMER` pagination logic, specifically removing two `console.log` statements related to `totalResult` and `result`, while retaining one for `total`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\otm_bol.route.ts`**
    *   **11/21/2025, 11:40:37 AM**: This route file defines various API endpoints for OTM BOL operations. It includes GET routes for `excel_data`, `getPLPOByBOL`, `fetch_payload`, `fetchByBOLNo`, and `fetchByCommonKey`. POST routes exist for `editOtmBol`, `upload_save_edit`, `generateXML`, and `sendToOTM`. Notably, a `POST /upload` route that used `uploadMiddleware` for handling `plFile` and `dutyFile` was commented out.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`**
    *   **11/21/2025, 3:11:14 PM**: Introduced two key functions: `createHblMasterBooking` for inserting basic HBL master data into `hbl_master`, and `saveHblBooking` which handles comprehensive HBL booking creation and updates. The `saveHblBooking` function supports transactional operations, extensive data sanitization, and logs old and new booking details to `agent_booking_hbl_history` upon update, fetching current state from multiple `agent_booking_*` tables before applying changes. New bookings receive an `agent_booking_id` via `getAgentBookingCode`. Error handling utilized `HttpError` and `next()` for propagation.
    *   **11/21/2025, 3:13:11 PM**: The error handling in `createHblMasterBooking` was modified to only `logger.error(error)` instead of passing the error via `next(new HttpError(error.message, 500))`, altering its error propagation mechanism.
    *   **11/21/2025, 3:16:23 PM**: In `saveHblBooking`, the `bookingdate` field within the `bookingInsertQuery` object was made explicitly nullable by changing its assignment to `payload.booking_date || null`.
    *   **11/21/2025, 3:17:07 PM**: The `bookingdate` field in `createHblMasterBooking`'s payload was also made explicitly nullable (`AppDateFormate(...) || null`). However, the change to `bookingInsertQuery.bookingdate` in `saveHblBooking` from the previous timestamp (`payload.booking_date || null`) was reverted back to `payload.booking_date`.
    *   **11/21/2025, 3:19:06 PM**: The `bookingdate` field in the `payload` for `createHblMasterBooking` was commented out entirely, indicating it's no longer being processed or saved by this function.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`**
    *   **11/21/2025, 3:11:42 PM**: Initially, `router.post("/")` was incorrectly defined to call both `saveHblBooking` and `createHblMasterBooking`, which would lead to unpredictable behavior.
    *   **11/21/2025, 3:19:52 PM**: This issue was resolved by refactoring the routes: `router.post("/")` now exclusively calls `saveHblBooking`, and a new specific route `router.post("/create")` was added for `createHblMasterBooking`.

**Patterns and Recurring Elements:**

*   **Date Handling:** Consistent use of `AppDateFormate` with a `YYYY-MM-DD` database format (`db_format`). There's a recurring pattern of making date fields nullable (`|| null`) in payload constructions.
*   **Data Sanitization:** `sanitizeString(req.body.<field>)` is heavily used across multiple controller functions (`createHblMasterBooking`, `saveHblBooking`) to clean incoming request data.
*   **Database Transactions:** `await query("BEGIN");`, `await query("COMMIT");`, and `await query("ROLLBACK");` are consistently used to ensure data integrity during multi-step database operations.
*   **Error Management:** `try...catch` blocks are ubiquitous, often utilizing `logger.error` for internal logging and `HttpError` for standardized error responses to the client.
*   **Logging:** Frequent use of `logger.info` for tracking process flow and debugging, alongside `console.log` for immediate output.
*   **Role-Based Logic:** Logic often branches based on `req.user.role` (e.g., 'AGENT', 'CUSTOMER') to tailor data fetching or record creation.
*   **CRUD Operations:** The controllers demonstrate typical Create, Read, and Update patterns, often involving fetching existing data before modifying it (e.g., in `saveHblBooking`'s update path).
*   **Modularity:** Controllers handle business logic, while routes define API endpoints, indicating a structured application design.