...
## 8:55:42 PM
In `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`, updated on 11/28/2025 at 3:24:38 PM, two core functions for managing House Bill of Lading (HBL) master bookings are defined: `createHblMasterBooking` and `updateHblMasterBooking`.
*   **`createHblMasterBooking`**: This function is responsible for creating a new HBL entry. It extracts and sanitizes numerous fields from the request body, formats dates using `AppDateFormate`, and then performs a series of database inserts within a transaction. Data is inserted into `hbl_master`, `agent_booking_vessel`, `agent_booking_hbl`, `agent_booking_routing`, `agent_booking_container`, and `agent_booking_po`. It identifies `locationByRole` based on the user's role and includes error handling with transaction rollback.
*   **`updateHblMasterBooking`**: This function updates an existing HBL entry identified by `serial_id`. It begins a database transaction and updates the `hbl_master`, `agent_booking_vessel`, and `agent_booking_hbl` tables with new data from the payload. For `agent_booking_routing` and `agent_booking_container` lists, it implements a "delete-then-insert" strategy, first removing all existing entries associated with the `hbl_master_id` and then inserting new ones provided in the request payload. The provided code snippet for this function is truncated.

The file `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts` saw multiple updates:
*   **11/28/2025, 3:25:07 PM**: Initial setup of routes. It imports various controller functions and defines basic GET, POST, and DELETE endpoints:
    *   `router.get("/")` for `fetchHblFormDetails`
    *   `router.get("/get-hbl")` for `getHbl`
    *   `router.post("/create")` for `createHblMasterBooking`
    *   `router.post("/update")` for `updateHblMasterBooking`
    *   `router.delete("/delete")` for `deleteHblForm`
*   **11/28/2025, 3:32:50 PM**: A significant change occurred here. `authMiddleware` was imported and added to the `deleteHblForm` route. Additionally, the DELETE route path was modified to include a `serial_id` parameter: `router.delete("/delete/:serial_id", authMiddleware, deleteHblForm);`.
*   **11/28/2025, 3:36:05 PM**: The DELETE route path was partially reverted. While `authMiddleware` remained, the `serial_id` parameter was removed from the path, making it `router.delete("/delete", authMiddleware, deleteHblForm);` again.

**Patterns and Recurring Elements:**
*   **Database Interactions:** A strong recurring pattern is the extensive interaction with a PostgreSQL database (`pool`, `query`, `insertQuery`, `insertMany`, `updateQuery`). Transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) are consistently used for complex operations involving multiple table modifications to ensure data integrity.
*   **Data Sanitization and Formatting:** The `sanitizeString` utility and `AppDateFormate` function are frequently used to process incoming request data, indicating a focus on data quality and security.
*   **Modularity:** The separation of concerns is evident, with controller functions handling business logic and route files managing API endpoints.
*   **Authentication/Authorization:** The introduction of `authMiddleware` in the route file indicates an implementation of security measures for specific endpoints, particularly for sensitive operations like deletion.
*   **API Endpoints for CRUD:** The routes clearly define standard Create, Read, Update, Delete (CRUD) operations for HBL entries (`/create`, `/get-hbl`, `/update`, `/delete`).
*   **Complex Data Structures:** HBL data involves multiple related entities (vessel, routing, containers, POs), which are handled as nested lists in the payload and managed through separate database tables.
*   **Temporal proximity of changes:** The updates to the routes file occurred in quick succession, within minutes of each other, suggesting rapid iteration or fine-tuning of the API design, particularly concerning the DELETE endpoint and middleware application.