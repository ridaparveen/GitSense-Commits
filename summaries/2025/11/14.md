...
## 3:20:37 PM
The changes log details updates across two primary files related to report generation and scheduling. The timestamps indicate a concentrated period of development and debugging on November 14, 2025.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\reports\instantReports.ts`**
This file handles the immediate generation of reports (`getInstantReport`).

*   **Initial State (11/14/2025, 1:03:09 PM):** The `getInstantReport` function retrieves report parameters (columns, date range, company name) from the request body. It dynamically constructs a SQL query to fetch data from `mtr_tracking_data`, including conditional joins for booking-related data and a subquery for `pl_no`. The `companyName` is initially derived from `req.user?.companyname`.
*   **Company Name Logic Refinement (11/14/2025, 1:06:38 PM & 1:07:25 PM & 1:08:13 PM):** A significant change involves modifying how `companyName` is resolved. It's updated to first check `req.body.CompanyName` (decoding, lowercasing, and trimming it) and fall back to `req.user?.companyname` if not provided. There was a brief deviation (1:07:25 PM) where `resolvedCompanyName` was introduced and used, and `req?.user?.companyname || ""` was used as a fallback, but this was quickly reverted to the earlier form (1:08:13 PM).
*   **Logging Correction/Addition (11/14/2025, 1:08:36 PM, 1:09:18 PM, 1:12:09 PM):** A minor correction was made to log `reportData.rows.length` instead of `reportData.rows` (1:08:36 PM, corrected at 1:09:18 PM). A debugging `console.log` for the entire `reportData` object was also added (1:12:09 PM).

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\reports\scheduleReport.ts`**
This file manages the creation, fetching, and editing of scheduled reports, including timezone conversion utilities.

*   **Initial State (11/14/2025, 2:33:08 PM):** The file defines `setReport` (to insert new scheduled reports), `fetchScheduledReports` (to retrieve reports), and `editReport` (to update reports). All functions build SQL queries using string concatenation. A `timeZoneConverter` object is included to adjust dates and times based on user-defined timezones. `companyName` resolution in `setReport` mirrors the logic from `instantReports.ts`.
*   **SQL Injection Mitigation (11/14/2025, 2:45:38 PM):** A critical change was made in `fetchScheduledReports` to escape single quotes in the `companyName` parameter (`.replace(/'/g, "''")`) when used in the SQL `LIKE` clause. This directly addresses a potential SQL injection vulnerability for this specific parameter, although other parameters in the file remain susceptible.
*   **Type Safety and Debugging (11/14/2025, 2:46:28 PM & 2:47:38 PM):** A type assertion `as string | undefined` was added to `req.query.CompanyName` for improved type inference. A `console.log` was also added to display the `companyName` parameter for debugging.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\reports\triggers.ts`**
This file contains the background logic for triggering and processing scheduled reports (e.g., daily, weekly, monthly).

*   **Initial State (11/14/2025, 3:06:10 PM):** It includes functions (`activeScheduledForSDT`, `activeScheduledForDaily`, `activeScheduledForWeekly`, `activeScheduledForMonthly`) that periodically check for active reports to trigger. When a schedule's time and conditions match, it calls `schedulerFunc.scheduleReport`, which then calls `schedulerFunc.triggerReport`. `triggerReport` fetches company name from `mtr_user_master`, constructs the report query (similar logic to `instantReports.ts` for `pl_no` and booking joins), applies date filters, sends the report via email, saves report history, and deactivates non-repetitive schedules. Notably, the SQL query construction here also uses string concatenation for `CompanyName` and report columns, posing a SQL injection risk.
*   **Scheduled Time Validation (11/14/2025, 3:07:29 PM, 3:18:11 PM, 3:19:16 PM):** Initially, a null/invalid time check in `activeScheduledForDaily` was removed (3:07:29 PM). Later, a `if (!scheduledTime) return;` check was explicitly added to `activeScheduledForSDT` (3:18:11 PM and duplicated at 3:19:16 PM) to prevent errors if `schedule.time` is missing.

**Patterns and Recurring Elements:**

*   **Report Query Construction:** A consistent pattern across `instantReports.ts` and `triggers.ts` is the dynamic construction of SQL queries for reports. This involves filtering by `account_name`, conditional inclusion of a `pl_no` subquery, and conditional `LEFT JOIN` for `agent_booking_entry` based on requested columns.
*   **SQL Injection Vulnerability:** A recurring concern is the reliance on direct string concatenation to build SQL queries in `scheduleReport.ts` and `triggers.ts`. While `instantReports.ts` uses parameterized queries for core filters, the dynamic column and join parts (and `CompanyName` in `triggers.ts`) are still vulnerable. The single mitigation in `scheduleReport.ts` is a localized fix rather than a systemic solution.
*   **Logging:** All files make extensive use of `logger.info` and `logger.error` for operational tracking and error reporting, indicating a strong emphasis on observability. `console.log` is also used for debugging.
*   **Company Context:** Report generation and scheduling are tied to a `companyName` derived from either the user's session or the request body, highlighting a multi-tenant or company-specific data access model.
*   **Timezone Handling:** There's a clear pattern of needing to convert and interpret dates/times based on specific timezones, with `timeZoneConverter` in `scheduleReport.ts` and `internalFunc` in `triggers.ts` serving this purpose.

## 3:38:52 PM
**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx`**
    *   **Styling and Icons (11/13/2025, 4:35 PM - 4:38 PM)**: Multiple small changes were made to the "Please select a customer" message. This included adjusting vertical margins (`my-10` to `my-15` and back), adding an info icon (`BsInfo` initially, then `BsInfoCircle`), and refining Flexbox utilities (`flex`, `items-center`, `justify-center`, `gap-2`, `mt-1`) to improve visual alignment.
    *   **Cleanup (11/13/2025, 5:52 PM)**: The unused `BsInfo` import was removed.
    *   **Core Logic**: The fundamental logic for token expiration and setting the selected customer based on local storage and user role remained consistent throughout the changes.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\booking\booking-dashboard.tsx`**
    *   **API Parameter Handling (11/13/2025, 4:39 PM - 5:53 PM)**: The `CompanyName` parameter in the `useFetchBookingDashboardDataQuery` call was refined. It initially used `selectedCustomer || localUser?.companyname`, then changed to `selectedCustomer || ""`, and finally settled on `selectedCustomer || undefined`. This shows an evolution in how the customer's company name is passed to the API.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx`**
    *   This file experienced frequent and often cyclical changes related to conditional rendering, data fetching, and loading states.
    *   **Conditional Rendering (11/13/2025, 5:01 PM - 5:20 PM)**: The logic for displaying `AnalyticsContainer` versus a "Please Select Any Customer" message for `ADMIN_V2` users with or without a `selectedCustomer` was repeatedly modified, inverted, and corrected, indicating a period of iteration or debugging. Eventually, an `AnalyticsContainer` with a `Loader` was included.
    *   **Loading State Management (11/13/2025, 5:03 PM - 5:26 PM)**: There was a shift in how the loading state was managed. Initially, local `isLoading` state was briefly removed, then `setLoading` was moved from `analytics-slice` to `global-slice`, and `Loader` or `Spinner` components were introduced and toggled for display based on loading status.
    *   **API Query Parameters and Guards (11/13/2025, 5:13 PM - 11/14/2025, 11:07 AM)**: The guard clause `!selectedCustomer` in the `useEffect` hook responsible for fetching reports was added and removed multiple times. The `companyname` parameter in API calls (`useFetchDashboardsQuery`, `fetchReport`, `fetchEndpoints`) also varied between `selectedCustomer`, `mainState.selectedCustomer`, and a combination of both.
    *   **`skipToken` Introduction (11/14/2025, 10:55 AM)**: `useFetchDashboardsQuery` was updated to use `skipToken` if `selectedCustomer` is not available, preventing unnecessary API calls.
    *   **Cleanup (11/14/2025, 11:14 AM)**: Unused imports like `Alert` and `BsInfoCircle` were removed.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts`**
    *   **Global Loading State (11/13/2025, 5:05 PM)**: A `loading` boolean state and a `setLoading` reducer were introduced, enabling a global loading indicator for the application.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\notifications\notification-page.tsx`**
    *   **Prop Type Refinement (11/13/2025, 5:38 PM)**: `selectedNotification` and `setSelectedNotification` prop types were temporarily broadened to `any` and then reverted to more specific types (`NotificationItemValues | null` and `NotificationItem | null | any`).
    *   **Loading Placeholder (11/13/2025, 5:38 PM)**: The number of placeholder items shown during loading was briefly changed from `limit` to `3` and then reverted.
    *   **API Parameter Handling (11/13/2025, 5:38 PM)**: The `CompanyName` parameter in `useFetchNotificationDataQuery` was explicitly set to `selectedCustomer || ""`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\reports\report-page.tsx`**
    *   **API Parameter (11/14/2025, 2:50 PM)**: The `CompanyName` parameter was added to the `useGetSavedReportsQuery` call, using `selectedCustomer ?? null`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\dashboard\dashboard-page.tsx`**
    *   **API Parameter Handling (11/13/2025, 5:54 PM)**: `CompanyName` in `useFetchShipmentDataQuery` and `useFetchDeliveryDataQuery` was updated to `selectedCustomer || undefined` from `selectedCustomer || localUser?.companyname`. `useFetchCartDataQuery` retained the `localUser?.companyname` fallback.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-container-page.tsx`**
    *   **API Parameter & Conditional Rendering (11/13/2025, 5:48 PM)**: The `CompanyName` parameter for `useFetchShipmentDataQuery` and `useFetchShipmentCardDataQuery` changed from `selectedCustomer || ""` to `selectedCustomer || undefined`. A top-level conditional rendering based on `selectedCustomer` was removed, with a message displayed internally if no customer is selected.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`**
    *   **Custom Date Range (11/14/2025, 11:23 AM)**: Introduced `PopoverPanel` and `DateRange` components to allow users to select custom date ranges for global filters.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\reports\report-form.tsx`**
    *   **API Parameter Integration (11/14/2025, 1:01 PM - 1:15 PM)**: The `CompanyName` parameter, derived from `selectedCustomer`, was added to the payload for both `handleDownload` and `handleNewReport` API calls.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\reports.api.ts`**
    *   **Query Refactoring (11/14/2025, 2:53 PM)**: The `getSavedReports` endpoint was refactored to use `URLSearchParams` for constructing query parameters, making the handling of the optional `CompanyName` parameter more explicit and robust.

**Timestamps of Significant Changes:**
*   **11/13/2025, 5:05:40 PM**: Introduction of a global `loading` state in `global-slice.ts`.
*   **11/13/2025, 5:10:16 PM**: First clear implementation of a loading spinner (`<Spinner />`) for the Analytics page. This was later replaced by `<Loader />` at **11/13/2025, 5:11:13 PM**.
*   **11/13/2025, 5:13:31 PM**: Introduction of `!selectedCustomer` guard for fetching analytics reports and attempts to use `mainState.selectedCustomer` in API queries, indicating a focus on customer context.
*   **11/14/2025, 10:55:49 AM**: Introduction of `skipToken` in `useFetchDashboardsQuery` to prevent API calls when `selectedCustomer` is absent, improving efficiency.
*   **11/14/2025, 11:23:34 AM**: Enhancement of the Analytics Toolbar with custom date range selection functionality.
*   **11/14/2025, 1:01:10 PM / 1:15:25 PM**: Integration of `selectedCustomer` into `report-form.tsx` API payloads, confirming customer-specific report handling.
*   **11/14/2025, 2:53:40 PM**: Refactoring of `reports.api.ts` to explicitly handle `CompanyName` as a query parameter.

**Patterns and Recurring Elements:**

*   **Global Customer Context (`selectedCustomer`)**: The most dominant pattern is the pervasive integration of the `selectedCustomer` state (from Redux `globalSlice`) across almost all data-fetching operations and conditional UI rendering. This confirms that the application is heavily reliant on a global customer selection to filter and display relevant data for different user roles (especially `ADMIN_V2`).
*   **Refinement of API Query Parameters**: There's a recurring effort to standardize how the `CompanyName` parameter is passed to API endpoints, evolving from `selectedCustomer || localUser?.companyname` to `selectedCustomer || ""` and then `selectedCustomer || undefined`, and finally using `URLSearchParams` for robust parameter construction.
*   **Loading State Management**: The development log shows an iterative process of centralizing and refining loading state feedback, moving towards a global `loading` indicator and using common `Loader` or `Spinner` components.
*   **UI/UX Iterations**: Many entries, particularly for `layout.tsx` and `analytics-page.tsx`, involve minor but frequent adjustments to UI elements, styling (CSS classes), and component imports, indicating active design and user experience refinement.
*   **Rapid, Iterative Development**: The very close timestamps (often within seconds or minutes) suggest a fast-paced development or debugging workflow, where small changes are made, tested, and immediately followed by further modifications or reverts. This is evident in the back-and-forth nature of some conditional logic and parameter choices.

## 4:50:20 PM
The provided log details a series of changes primarily focused on the analytics section of the `t360-frontend-v2` application, specifically concerning dashboard management, report generation, and UI components. The timestamps indicate a rapid iteration of development within a short period on **November 14, 2025**.

Here's a breakdown of the key information:

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx` (Multiple timestamps from 3:50:22 PM to 3:51:00 PM)**:
    *   This file represents the main Analytics screen.
    *   The changes are minor and seem to be related to `console.log` statements, indicating debugging or logging adjustments. One `console.log("==selectedCustomer", selectedCustomer);` statement was removed.
    *   The core logic involves fetching dashboards and endpoints based on `selectedCustomer` and user role, then fetching reports and processing them with `metricsPackageHandler`.
    *   It uses Redux for state management (e.g., `setDashboard`, `setAllMetric`, `setEndpoints`, `setLoading`, `setSelectedCustomer`).
    *   `useEffect` hooks are crucial for data fetching and state initialization. For instance, one `useEffect` initializes `selectedCustomer` for users with the "CUSTOMER" role.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx` (Multiple timestamps from 3:52:06 PM to 3:56:52 PM)**:
    *   Manages the toolbar for the analytics dashboard, including filtering, export, and chart view controls.
    *   A significant change was observed at **3:56:08 PM** where the initial `action` state for `showDrawer` was changed from `"new"` to `"add"`, and then reverted back to `"new"` at **3:56:52 PM**. This suggests a quick correction or experimentation with naming conventions.
    *   Includes functionality for downloading the dashboard as a PDF using `jspdf` and `html2canvas-pro`. The PDF generation includes adding a logo.
    *   Handles date filtering (global vs. local, custom date range) and operation type (move/resize for charts).
    *   Integrates `ChipsTabView` for dashboard selection/management and `AppDrawer` for custom view creation/editing.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\chip-tabs-view.tsx` (3:54:16 PM)**:
    *   Introduces a `ChipsTabView` component for displaying a scrollable list of "chips" or tabs.
    *   Includes functionality for horizontal scrolling using `ChevronLeft` and `ChevronRight` icons.
    *   Supports a "New" button chip for creating new items.
    *   The component is styled with Tailwind CSS (`scrollbar-hide`).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx` (Multiple timestamps from 3:57:46 PM to 4:01:28 PM)**:
    *   This component facilitates creating, editing, and deleting custom analytics views.
    *   The initial `action` prop was likely `add` or `edit`, and at **4:00:12 PM**, the type for `metricsId` in `CheckBoxManager` was broadened from `string` to `string | number`.
    *   Uses `useFetchDashboardOptionsQuery` and `useSaveDasboardMutation` from `analytics-api`.
    *   Manages a `checkBoxManager` state to track selected metrics (with a limit of 9).
    *   Includes `RadioNav` for switching between "Container Wise" and "MBL Wise" views.
    *   Features audit functionality via `AuditViewDrawer` and a `PopupAlert` for confirmation dialogs (e.g., for deletion).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\radio-nav.tsx` (Multiple timestamps from 3:58:39 PM to 3:58:51 PM)**:
    *   A generic `RadioNav` component (button group) for navigation or selection.
    *   The `onChange` prop type was updated at **3:58:51 PM** to return `void` instead of `void | undefined`, a minor type refinement.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\TreeViewContianer.tsx` (4:00:47 PM)**:
    *   Displays hierarchical data with expandable groups for metric selection.
    *   Uses `Checkbox` components and `ChevronDownIcon`/`ChevronRight` for toggling group visibility.
    *   The `id` for `InnerItem` was updated to `string | number`, reflecting the change in `ViewContainer.tsx`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\metricBox.tsx` (Multiple timestamps from 4:05:00 PM to 4:36:59 PM)**:
    *   Responsible for rendering individual metric boxes, which contain charts and their controls.
    *   Includes various chart types (`BarChart`, `PieChart`, `LineChart`, `StackedBarChart`, `GaugeChart`).
    *   Handles full-screen view (`ThemedModal`) for charts.
    *   Minor type casting adjustments were made at **4:07:48 PM**, **4:32:58 PM**, **4:34:37 PM**, **4:35:14 PM**, **4:35:23 PM**, and **4:36:59 PM**, changing `ReactNode` to `any` or explicit casting. These suggest attempts to resolve TypeScript issues or simplify typing.
    *   The `getChartComponent` utility dynamically renders the correct chart based on `chartType` and `chartView`.
    *   The `filteredChartOptions` were explicitly cast to `any` in some instances.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\analyticsContainer.tsx` (4:05:53 PM)**:
    *   The main container for analytics metrics, leveraging `react-dnd` for drag-and-drop reordering and resizing of metric boxes.
    *   Uses `ResizableGridItem` to manage individual metric box sizes and positions.
    *   Integrates `useSaveSequenceMutation` and `useSaveMetricSizeMutation` for persisting layout changes.
    *   Includes `fetchSingleReport` to update individual metric data based on filter changes.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\resizableGridItem.tsx` (Multiple timestamps from 4:10:50 PM to 4:40:22 PM)**:
    *   A reusable component for creating resizable and draggable grid items.
    *   Uses `react-resizable` for size adjustments and `react-dnd` for drag-and-drop functionality.
    *   Adjusts `columnSpan` and `rowSpan` based on resize events.
    *   The `ReactNode` type import was changed from default to named import at **4:40:22 PM**.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\ColumnsSelector.tsx` (Multiple timestamps from 4:13:30 PM to 4:15:07 PM)**:
    *   Allows users to select columns for reports and download the data.
    *   Includes search functionality, "Show All/Hide All" toggle for columns, and a list of checkboxes for column selection.
    *   At **4:15:07 PM**, the `color` prop for `CircularProgress` inside the "Download" button was changed from `"error"` to `"inherit"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\DateRange.tsx` (Multiple timestamps from 4:14:09 PM to 4:14:42 PM)**:
    *   A component for selecting a date range using MUI's `DatePicker` and `moment.js`.
    *   The `disableFuture` prop is used to prevent selecting future dates.
    *   No functional changes between the two timestamps; likely a reformat or minor save.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\methods.ts` (Multiple timestamps from 4:16:18 PM to 4:34:21 PM)**:
    *   Contains utility functions for analytics, including:
        *   `TruncateText`: Truncates text to a specified length.
        *   `metricsPackageHandler`: Processes raw metric data into a structured format for display, determining chart type, calculation type (avg, total), and formatting.
        *   `allChartOptions` and `chartOptionsByType`: Defines available chart types and their corresponding options.
        *   `reportFormula`: Generates formulas for specific reports (e.g., "Scheduled vs Actual Transit Time").
        *   `weekDateFormater`: Formats dates to "D MMM".
        *   `downloadReportSpreadsheet`: Handles Excel spreadsheet generation and download using `exceljs` and `file-saver`. This function has received multiple type definitions and refined logic for different chart types and period filters (`month`, `week`, `quarter`). Explicit types were added for `ReportItem`, `PeriodFilter`, and `ViewByFilter` at **4:27:25 PM**, and `user` parameter type was changed from generic to `string`. Type casting for `chartOptionsByType` was added at **4:34:21 PM**.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricToolbar.tsx` (Multiple timestamps from 4:37:37 PM to 4:38:13 PM)**:
    *   The toolbar specific to individual metric boxes.
    *   Manages date, period, and view-by filters for each metric.
    *   `useEffect` ensures that `fetchSingleReport` is called whenever filters change.
    *   Minor style adjustments (`mb:2` for margin-bottom) and type casting (`chartOptions as any`) were made.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\RenderValuePopup.tsx` (Multiple timestamps from 4:39:24 PM to 4:40:06 PM)**:
    *   A component for displaying a popover with details, likely for values in a table.
    *   The `MouseEvent` type import was changed from default to named import at **4:40:06 PM**.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx` (4:41:09 PM)**:
    *   The main sidebar navigation component.
    *   Includes a logo, user card, and a list of menu items.
    *   A "Select Customer Panel" is rendered conditionally for `ADMIN_V2` roles.
    *   Uses `useLocation` to determine the current active menu item.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts` (4:46:16 PM)**:
    *   Redux slice for global state management, specifically for `selectedCustomer` and `loading`.
    *   `setSelectedCustomer` action now stores the selected customer in `localStorage`.
    *   `initializeSelectedCustomer` action retrieves the customer from `localStorage` on initialization.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\SelectCustomer.tsx` (4:46:40 PM)**:
    *   Component for selecting a customer, typically displayed in the sidebar.
    *   Fetches paginated customer options from an API using `ApiManager.getPaginatedCompanyOptions`.
    *   Includes search, pagination, and a clear all functionality.
    *   Uses `debounce` for search input to optimize API calls.
    *   Updates the global `selectedCustomer` state via Redux and `localStorage`.

**Patterns and Recurring Elements:**

1.  **Redux Integration:** Many components extensively use `useDispatch` and `useSelector` to interact with the Redux store, specifically the `analyticSlice` and `globalSlice`, indicating a centralized state management approach for analytics data and user/customer selections.
2.  **API Interaction:** The `useLazyFetchReportQuery`, `useFetchDashboardsQuery`, `useSaveDasboardMutation`, `useSaveSequenceMutation`, `useSaveMetricSizeMutation`, and `ApiManager` show a consistent pattern of fetching and mutating data related to analytics dashboards and reports.
3.  **UI Component Reusability:** There's a strong emphasis on reusable UI components (e.g., `SelectBox`, `RadioNav`, `ChipsTabView`, `InputBox`, `Button`, `AppDrawer`, `ThemedModal`, `PopoverPanel`), often imported from `@components/common` or `@components/kit`.
4.  **Date and Time Handling:** `moment.js` is consistently used across multiple files for date formatting and manipulation, especially for filtering analytics data.
5.  **TypeScript Usage:** The code heavily utilizes TypeScript interfaces for defining data structures (e.g., `EndpointType`, `MetricData`, `User`, `ChartOption`, `MetricItem`, `AlertConfig`, `ColumnItem`), demonstrating a commitment to type safety. Minor adjustments to type declarations are frequent, suggesting ongoing refinement.
6.  **Analytics-Specific Logic:** Common patterns in the analytics files include:
    *   Filtering data by date range (`dateFilterHandle`).
    *   Dynamically rendering chart types based on data properties (`getChartComponent`).
    *   Exporting data to PDF/Excel.
    *   Handling user-defined custom views and their metrics.
    *   Managing layout (drag-and-drop, resize) of dashboard widgets.
7.  **Debugging/Logging:** Frequent `console.log` statements are present, suggesting active development and debugging. These are sometimes added and removed, or their content adjusted.
8.  **Styling:** A mix of Material-UI (`@mui/material`) and Tailwind CSS (implied by classes like `flex items-center space-x-4`, `rounded-md`, `shadow-[...]`, `scrollbar-hide`) is used for styling.

## 5:51:22 PM
The provided log details several code changes primarily focused on the Analytics section of the T360-frontend-v2 application. The updates span from November 14, 2025, 3:50:22 PM to 4:55:53 PM.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx`**
    *   (Timestamp: 11/14/2025, 3:50:22 PM, 3:50:29 PM, 3:51:00 PM)
    *   This file handles the main Analytics page. The changes across these timestamps are minor, appearing to be whitespace or negligible formatting adjustments. The core logic involves fetching dashboard views and endpoint data based on the `selectedCustomer` and `user.role` from Redux state and local storage. It dispatches actions to set dashboard data, endpoints, and metrics, and handles loading states. A key `useEffect` block orchestrates fetching all reports for the selected dashboard, including date range handling and data processing via `metricsPackageHandler`. It also ensures that a `CUSTOMER` role automatically sets their `companyname` as the `selectedCustomer`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`**
    *   (Timestamp: 11/14/2025, 3:52:06 PM, 3:56:08 PM, 3:56:52 PM)
    *   This component manages the analytics toolbar functionalities, including date filtering, dashboard selection (via `ChipsTabView`), and PDF export.
    *   A minor change occurred where the initial `action` state for `showDrawer` was changed from `"add"` to `"new"`, then back to `"new"` in subsequent commits. This indicates a minor refactoring or correction in how new custom views are initiated.
    *   The `downloadDashboardPDF` function remains consistent, using `jspdf` and `html2canvas-pro` to generate a PDF of the `#metric-board` element, including the company logo.
    *   It uses `RadioNav` for filter types (Local/Global) and chart view operations (move/resize).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\chip-tabs-view.tsx`**
    *   (Timestamp: 11/14/2025, 3:54:16 PM)
    *   This component provides a horizontally scrollable tab view using "chips" and a "New" button. It handles scrolling functionality and dynamically renders tabs based on options. No significant functional changes were observed.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx`**
    *   (Timestamp: 11/14/2025, 3:57:46 PM, 4:00:12 PM, 4:01:28 PM)
    *   This component allows users to create, edit, or delete custom analytics views.
    *   The `action` prop type was updated from `"add" | "edit"` to specifically use `"new" | "edit"`. This is likely a type refinement to match the logic used in `AnalyticsToolbar.tsx` more precisely.
    *   The `metricsId` type in `CheckBoxManager` interface was updated from `string` to `string | number`, allowing for more flexible ID types.
    *   The `handleCheckBoxClicked` function ensures a maximum of 9 metrics can be selected.
    *   It integrates `TreeViewContainer` for displaying `containerWise` and `mblWise` metric options.
    *   It uses `useFetchDashboardOptionsQuery` and `useSaveDasboardMutation` for API interactions, along with Redux actions to update the dashboard and trigger refetches.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\radio-nav.tsx`**
    *   (Timestamp: 11/14/2025, 3:58:39 PM, 3:58:51 PM)
    *   This component renders a radio navigation bar. A minor change in the `onChange` prop type, from `(value: string) => void | undefined` to `(value: string) => void`, indicating a slight stricter type definition.
...
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\TreeViewContianer.tsx`**
    *   (Timestamp: 11/14/2025, 4:00:47 PM)
    *   Displays metric groups in a collapsible tree-like structure, allowing users to select metrics using checkboxes. The `id` type for `InnerItem` was made more flexible (`string | number`).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\metricBox.tsx`**
    *   (Timestamp: 11/14/2025, 4:05:00 PM, 4:05:06 PM, 4:07:48 PM, 4:07:56 PM, 4:32:58 PM, 4:34:37 PM, 4:35:14 PM, 4:35:23 PM, 4:36:59 PM)
    *   This component renders individual metric charts within the analytics dashboard.
    *   Multiple changes primarily involve refining TypeScript types, explicitly defining `ReactNode` as a return type for `getChartComponent` and `any` for `data`, `enlargeFlag`, `length`, `sizeInfo`, `chartTree` parameters in `getChartComponent`. This suggests an ongoing effort to improve type safety or resolve type-related warnings during development.
    *   It dynamically renders different chart types (Bar, Pie, Line, Stacked Bar, Gauge) based on `item.chartType` and `chartView`.
    *   It includes a fullscreen modal for enlarged chart viewing and a `MetricToolbar` for per-metric filtering.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\analyticsContainer.tsx`**
    *   (Timestamp: 11/14/2025, 4:05:53 PM)
    *   This component manages the grid layout of analytics metrics, enabling drag-and-drop reordering and resizing.
    *   It uses `DndProvider` with `HTML5Backend` for drag-and-drop.
    *   It includes API calls (`useSaveSequenceMutation`, `useSaveMetricSizeMutation`) to persist changes to metric order and size, and `useLazyFetchReportQuery` for fetching single report data.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\resizableGridItem.tsx`**
    *   (Timestamp: 11/14/2025, 4:10:50 PM, 4:40:22 PM)
    *   This component implements resizable and draggable grid items for the analytics dashboard.
    *   Type for `ReactNode` was explicitly imported.
    *   It manages `columnSpan` and `rowSpan` for layout adjustments and calls `onChange` and `moveItem` callbacks to propagate changes.
    *   The component adapts its behavior (resizable vs. draggable) based on the `operationType` prop.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\ColumnsSelector.tsx`**
    *   (Timestamp: 11/14/2025, 4:13:30 PM, 4:15:07 PM)
    *   A component for selecting columns to include in a report, with search and "Show All/Hide All" functionality.
    *   The `color` prop for `CircularProgress` in the "Download" button was changed from `"white"` to `"inherit"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\DateRange.tsx`**
    *   (Timestamp: 11/14/2025, 4:14:09 PM, 4:14:42 PM)
    *   A date range picker component using `@mui/x-date-pickers` and `moment`. No functional changes were observed, just minor formatting or save operations.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\methods.ts`**
    *   (Timestamp: 11/14/2025, 4:16:18 PM, 4:19:44 PM, 4:21:04 PM, 4:27:25 PM, 4:28:54 PM, 4:29:23 PM, 4:30:50 PM, 4:34:03 PM, 4:34:21 PM)
    *   This file contains utility functions for analytics, including `metricsPackageHandler` to process metric data, `TruncateText`, `reportFormula`, and `weekDateFormater`.
    *   Extensive additions and refinements of TypeScript types were made across multiple commits for interfaces like `AnalyticsData`, `Customizer`, `Metric`, `Endpoint`, `FormatSettings`, `MetricsPackageReturn`, `ReportItem`, `PeriodFilter`, and `ViewByFilter`.
    *   The `downloadReportSpreadsheet` function was significantly updated to include specific handling for different report names and `StackedBar` chart types, particularly for "Cargo-Ready Date to ETD" reports (both standard and "+/- 10 Days") and "Containers Shipped (Mbl Wise)", dynamically adjusting headers and data mapping based on `periodFilter` (month, week, quarter). The `headerCopy` variable type was also explicitly set to `string[]`.
    *   The `chartOptionsByType` object was explicitly typed as `any` in the final commit, indicating a possible relaxation of strict typing for this specific object.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricToolbar.tsx`**
    *   (Timestamp: 11/14/2025, 4:37:37 PM, 4:38:13 PM)
    *   The `chartOptions` prop in `SwitchBox` was explicitly cast to `any` in the second commit.
    *   This toolbar handles per-metric filtering (date, period, view by) and chart view selection, triggering `fetchSingleReport` on filter changes.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\RenderValuePopup.tsx`**
    *   (Timestamp: 11/14/2025, 4:39:24 PM, 4:39:54 PM, 4:40:06 PM)
    *   This component renders a Popover for displaying detailed values, primarily handling TypeScript type imports for `MouseEvent`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx`**
    *   (Timestamp: 11/14/2025, 4:41:09 PM)
    *   The sidebar component was updated to include a `SelectCustomerPanel` only for `ADMIN_V2` roles, allowing them to select a customer. For other roles, or if no customer is selected, it handles the display accordingly. It also incorporates user authentication checks based on local storage.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts`**
    *   (Timestamp: 11/14/2025, 4:46:16 PM)
    *   This Redux slice manages global state, specifically `selectedCustomer` and `loading`.
    *   The `setSelectedCustomer` reducer was enhanced to persist `selectedCustomer` to local storage if a valid value is provided, and remove it otherwise. An `initializeSelectedCustomer` reducer was added to retrieve the saved customer from local storage on app startup.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\SelectCustomer.tsx`**
    *   (Timestamp: 11/14/2025, 4:46:40 PM)
    *   This component provides a dropdown for selecting customers, with search and pagination features. It uses `lodash.debounce` for search input and interacts with `ApiManager` to fetch paginated customer options. It dispatches `setSelectedCustomer` to update the global state and local storage.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx`**
    *   (Timestamp: 11/14/2025, 4:50:29 PM, 4:51:08 PM, 4:53:59 PM)
    *   The main layout component.
    *   A significant change was introduced in how `selectedCustomer` is handled for different user roles. If the user `role` is `CUSTOMER`, their `companyname` is automatically set as the `selectedCustomer` and stored in local storage. If the role is `ADMIN_V2` (or non-customer), `selectedCustomer` is explicitly cleared from Redux state and local storage. This ensures proper customer context for different user types.
    *   It also checks for token expiration and redirects to the login page if expired.
    *   The `Outlet` is only rendered if a `selectedCustomer` is present, otherwise, a "Please select a customer" message is displayed.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\reports.api.ts`**
    *   (Timestamp: 11/14/2025, 4:55:53 PM)
    *   This Redux Toolkit Query API slice defines endpoints for interacting with reports.
    *   It includes queries for `getSavedReports`, `getReportHistory`, and mutations for `getReport`, `createReport`, and `editReport`.
    *   `getSavedReports` now accepts `id` and `CompanyName` as optional parameters.
    *   `getReportHistory` now supports pagination (`page`, `limit`) and dynamic filtering, appending all provided filters as query parameters.

### Patterns and Recurring Elements:

1.  **TypeScript Refinement:** A prominent pattern is the continuous refinement and addition of TypeScript types (`any` to specific types like `ReactNode`, `string | number`, `string[]`, or explicit casts to `any` where strict typing might be problematic initially). This indicates an ongoing effort to improve type safety and code clarity.
2.  **Redux Integration:** The application heavily relies on Redux for state management, with `useDispatch` and `useSelector` being ubiquitous in functional components. Global and analytics-specific slices (`global-slice`, `analytics-slice`) are used to manage shared state, customer selection, dashboard configurations, and report data.
3.  **Local Storage Usage:** User information (`user`, `token`) and `selectedCustomer` are frequently stored and retrieved from `localStorage`, often with `JSON.parse` and `JSON.stringify` operations. This is used for persisting user sessions and selected customer context.
4.  **API Interaction via RTK Query:** The application leverages Redux Toolkit Query for efficient data fetching and caching, evident in files like `analytics-api`, `reports.api.ts`, and components using `useFetch...Query` and `use...Mutation` hooks.
5.  **Analytics-Specific Functionality:** Many changes revolve around the analytics dashboard:
    *   **Dynamic Chart Rendering:** `MetricsBox.tsx` dynamically renders various chart types based on configuration.
    *   **Interactive Dashboard Layout:** `AnalyticsContainer.tsx` and `resizableGridItem.tsx` implement drag-and-drop and resizing for dashboard widgets.
    *   **Custom View Management:** `AnalyticsToolbar.tsx` and `ViewContainer.tsx` provide functionality to create, manage, and delete custom analytics views.
    *   **Report Export:** Functionality to export analytics data to PDF (`AnalyticsToolbar.tsx`) and Excel spreadsheets (`methods.ts`).
6.  **Component Reusability:** Common UI components like `SelectBox`, `RadioNav`, `InputBox`, `Button`, `AppDrawer`, `PopoverPanel`, and `ThemedModal` are used across different parts of the analytics module, promoting consistency and reducing code duplication.
7.  **Date and Time Handling:** `moment.js` is consistently used for date formatting and calculations across various components and utility functions (`analytics-page.tsx`, `methods.ts`, `DateRange.tsx`).
8.  **User Role-Based Logic:** There's clear logic to differentiate behavior based on `user.role` (e.g., `CUSTOMER` vs. `ADMIN_V2`) especially concerning customer selection and dashboard access.
9.  **Error Handling and Toasts:** `toast.error` (from `react-hot-toast`) and `console.error` are used to provide user feedback and log errors during API calls and data processing.

### Timestamps of Significant Changes:

*   **11/14/2025, 3:57:46 PM - 4:01:28 PM (`ViewContainer.tsx`):** Introduction of `string | number` type for `metricsId` and refinement of `action` prop type for creating/editing views.
*   **11/14/2025, 4:05:00 PM - 4:36:59 PM (`metricBox.tsx`):** A series of continuous type refinement and minor adjustments within the `MetricsBox` component, specifically around dynamic chart rendering and prop typing.
*   **11/14/2025, 4:16:18 PM - 4:34:21 PM (`methods.ts`):** A significant block of updates for `methods.ts`, focusing on extensive TypeScript typing for various interfaces and detailed logic expansion within the `downloadReportSpreadsheet` function for different report types and time filters.
*   **11/14/2025, 4:46:16 PM (`global-slice.ts`):** Introduction of `initializeSelectedCustomer` and logic to persist `selectedCustomer` in local storage, indicating improved state management for customer context.
*   **11/14/2025, 4:46:40 PM (`SelectCustomer.tsx`):** Implementation of customer selection with search and pagination, reinforcing the global customer context feature.
*   **11/14/2025, 4:50:29 PM - 4:53:59 PM (`layout.tsx`):** Critical logic added to `Layout` to manage `selectedCustomer` based on user role and to gate content rendering if no customer is selected.
*   **11/14/2025, 4:55:53 PM (`reports.api.ts`):** Expansion of report API endpoints, particularly for `getSavedReports` and `getReportHistory`, introducing dynamic query parameters and pagination.

## 6:38:15 PM
The log details changes to a single file, `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\AnalyticsToolbar.jsx`, with two timestamps: 11/14/2025, 3:47:26 PM and 11/14/2025, 3:47:47 PM.

The changes between these two timestamps are primarily cosmetic and related to code formatting and cleanup, with no observable functional modifications to the active code:

*   **Code Formatting/Linting:** Numerous small syntactical changes were made, likely due to an auto-formatter or linter. These include:
    *   Removal of trailing commas in import statements, object literals (e.g., `buttonConfig`, `editOptions`, `filterTypeOptions`), and array elements.
    *   Adjustments in the use of single vs. double quotes for string literals (e.g., `disabled={toolState?.filterType === "local"}` became `'local'`, `action: "edit"` became `'edit'`).
    *   Slight reformatting of JSX conditional rendering, removing some redundant parentheses.
    *   Removal of a leading space character from the "PDF" button's text (`{" "} PDF` to `PDF`).
*   **Code Cleanup:** An older, commented-out version of the `downloadDashboardPDF` asynchronous function was removed from the file, indicating cleanup of deprecated or alternative implementations. The active `downloadDashboardPDF` function, which incorporates logo addition to the PDF, remained unchanged in its logic.

The proximity of the timestamps (21 seconds apart) strongly suggests these were part of a single commit or save action, focusing on code style and removal of commented-out sections rather than new feature development or bug fixes.

## 6:50:55 PM
The changes, all occurring on November 14, 2025, between 3:50 PM and 4:55 PM, primarily focus on the development and refinement of an analytics dashboard and its supporting infrastructure.

**File-specific updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx`** (3:50:22 PM - 3:51:00 PM): This file handles the main analytics screen, orchestrating data fetching for dashboards, endpoints, and reports. It utilizes Redux for state management, retrieves user information from `localStorage`, and conditionally fetches data based on the selected customer and user role. A hardcoded API key (`"nRKniUrJd6rjNCyvT8iRRHmG"`) is present in `fetchReport` calls. The customer selection logic is also updated to automatically set a customer for users with the "CUSTOMER" role.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`** (3:52:06 PM - 3:56:52 PM): This component provides interactive controls for the analytics dashboard. Key additions include date filtering (global/local, custom date ranges), dashboard tab management via `ChipsTabView`, PDF export functionality using `jsPDF` and `html2canvas-pro` (including a logo), and radio navigation for chart manipulation (move/resize). It integrates a drawer (`AppDrawer`) for creating ("new") and managing custom views.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\chip-tabs-view.tsx`** (3:54:16 PM): A new reusable component, `ChipsTabView`, was introduced. It displays a horizontal, scrollable list of "chips" (tabs) with an optional "New" button, designed for selecting different dashboard views.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx`** (3:57:46 PM - 4:01:28 PM): This component manages custom analytics views, allowing users to create, edit, or delete them. It provides an interface to select metrics (checkboxes) categorized by "Container Wise" or "MBL Wise," enforcing a maximum of 9 selected metrics. It fetches dashboard options and handles saving/deleting via mutations. A minor type refinement was made for `metricsId` from `string` to `string | number`.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\radio-nav.tsx`** (3:58:39 PM - 3:58:51 PM): A new `RadioNav` component was added for creating custom radio button groups, supporting icons and dynamic styling. A minor type change for the `onChange` prop was observed.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\TreeViewContianer.tsx`** (4:00:47 PM): This component provides a hierarchical, collapsible list (tree view) for metric selection within custom views, utilizing checkboxes.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\metricBox.tsx`** (4:05:00 PM - 4:36:59 PM): This component displays individual metric charts, handling various chart types (Bar, Pie, Line, Stacked Bar, Gauge). It dynamically renders charts, provides a fullscreen modal for detailed viewing, and integrates a `MetricToolbar` for local filtering. This file shows a recurring pattern of adding and subsequently loosening TypeScript type definitions (e.g., casting to `any`).
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\analyticsContainer.tsx`** (4:05:53 PM): This is the main container for displaying multiple metrics in a grid. It uses `react-dnd` and `react-resizable` to allow users to drag, drop, and resize individual metric boxes, saving their layout changes via API mutations. It also supports fetching individual report data.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\resizableGridItem.tsx`** (4:10:50 PM - 4:40:22 PM): This component makes child elements resizable and draggable within a grid. It uses `react-resizable` and `react-dnd` to manage `columnSpan` and `rowSpan`, allowing changes to be saved.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\ColumnsSelector.tsx`** (4:13:30 PM - 4:15:07 PM): A component for selecting columns to include in a report export, featuring search, "Show All/Hide All" toggle, and mandatory column handling. A minor styling change for a loading spinner was made.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\DateRange.tsx`** (4:14:09 PM - 4:14:42 PM): A dedicated component for selecting a "From" and "To" date range, utilizing MUI date pickers and preventing future date selections.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\methods.ts`** (4:16:18 PM - 4:34:21 PM): This utility file centralizes functions for analytics, including text truncation, processing raw metric data (`metricsPackageHandler`), defining chart options, generating report formulas, date formatting, and a complex `downloadReportSpreadsheet` function for Excel exports based on report type and filters. Explicit typings were added to `downloadReportSpreadsheet` parameters, followed by a type-loosening change (casting `chartOptionsByType` to `any`).
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricToolbar.tsx`** (4:37:37 PM - 4:38:13 PM): Provides local filtering options for individual metric boxes, including chart view, period, and date range. Changes to these filters trigger a re-fetch of the single report. A minor type-loosening change was made for the `SwitchBox` `chartOptions` prop.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\RenderValuePopup.tsx`** (4:39:24 PM - 4:40:06 PM): A UI component to display detailed values in a popover when a "View Details" button is clicked. Minor import type adjustments were made.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx`** (4:41:09 PM): The sidebar was updated to include a `SelectCustomerPanel` specifically for `ADMIN_V2` users, allowing them to switch between customers.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts`** (4:46:16 PM): A new Redux slice (`globalSlice`) was created to manage global application state, specifically `selectedCustomer` and a `loading` flag, with persistence to `localStorage`.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\SelectCustomer.tsx`** (4:46:40 PM): This new component provides a searchable and paginated dropdown interface for `ADMIN_V2` users to select customers. It integrates with the `globalSlice` to update the selected customer.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx`** (4:50:29 PM - 4:53:59 PM): The main layout component was updated to enforce customer selection. The application's content (`Outlet`) is only rendered if a `selectedCustomer` is active; otherwise, a prompt to select a customer is displayed. It handles token expiration and customer initialization based on user roles and `localStorage`. A comment was updated to clarify behavior for `ADMIN_V2` users.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\reports.api.ts`** (4:55:53 PM): A new Redux Toolkit Query API (`reportsApi`) was established for interacting with report-related backend endpoints, including fetching saved reports and history, and mutations for creating, getting, and editing reports.

**Patterns and Recurring Elements:**

*   **Analytics Module Focus:** The bulk of the changes revolve around enhancing the analytics dashboard, introducing new features like custom views, interactive metrics, and robust report generation/export.
*   **Redux Toolkit (RTK) and RTK Query:** These are consistently used for state management and API interactions, centralizing data flow and fetching logic throughout the application.
*   **User Role-Based Features:** Functionality, particularly customer selection, is often conditional on the user's role (e.g., `ADMIN_V2` vs. `CUSTOMER`), indicating a multi-role application structure.
*   **`localStorage` for Persistence:** User and selected customer information are frequently stored and retrieved from `localStorage` for persistence across sessions.
*   **Type Management Iteration:** There's a noticeable pattern in several files (e.g., `metricBox.tsx`, `methods.ts`) where explicit TypeScript typings are introduced, but sometimes later relaxed with `any` casts, suggesting an ongoing process of balancing strictness with development flexibility.
*   **Rapid Iteration:** The close timestamps and several identical entries indicate a period of rapid development, saving, and minor adjustments.
*   **Hardcoded API Key:** The API key "nRKniUrJd6rjNCyvT8iRRHmG" is hardcoded in several API requests.