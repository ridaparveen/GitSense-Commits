...
*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\company_details.controller.ts`**
    *   **Initial Update (11/21/2025, 10:07:54 AM)**: The `getAll` function was implemented to retrieve company names based on user roles (`AGENT` or `CUSTOMER`). For `AGENT`s, it fetches `cname` from `agent_po_account_master`. For `CUSTOMER`s, it fetches `account_name` from `mtr_tracking_data` and includes pagination logic (page, perPage, offset) with distinct values and total counts, alongside a non-paginated search. Debugging `console.log` statements were present for pagination results.
    *   **Subsequent Update (11/21/2025, 10:08:08 AM)**: Debugging `console.log` statements within the `getAll` function for the `CUSTOMER` role's pagination logic were removed.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\otm_bol.route.ts`**
    *   **Update (11/21/2025, 11:40:37 AM)**: This file defines API routes for OTM Bill of Lading (BOL) operations. It includes GET routes for fetching excel data, BOL details, and payload, and POST routes for editing BOLs, uploading/saving edits, generating XML, and sending to OTM. A file upload route (`/upload`) using `uploadMiddleware` was present but commented out, suggesting it was disabled or under development.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`**
    *   **Initial Major Update (11/21/2025, 3:11:14 PM)**: Introduced `createHblMasterBooking` to insert single HBL master records and `saveHblBooking` for comprehensive HBL booking management. `saveHblBooking` handles both new bookings (ADD) and updates (UPDATE) across multiple related tables (`agent_booking_entry`, `agent_booking_vessel`, `agent_booking_routing`, `agent_booking_container`, `agent_booking_hbl`, `agent_booking_po`) within a database transaction, logging changes to `agent_booking_hbl_history`. Error handling initially used `HttpError` via `next`.
    *   **Minor Adjustments (11/21/2025, 3:13:11 PM - 3:19:06 PM)**:
        *   `createHblMasterBooking` changed error handling from propagating `HttpError` to just logging.
        *   Variations in handling nullable `bookingdate` and `hbldate` fields occurred, with some changes introducing string `'null'` for dates before reverting to actual `null`.
        *   The `bookingdate` field in `createHblMasterBooking`'s payload was temporarily re-enabled then commented out again.
    *   **Refactoring for Bulk Inserts (11/24/2025, 1:23:51 PM)**: `createHblMasterBooking` was significantly refactored to support bulk insertion of HBL entries. It began expecting an `hbl_entries` array, validated its presence, and used `insertMany` for a single-transaction bulk insert. Error handling was also updated to return a 500 status directly.
    *   **Brief Regression (11/24/2025, 1:26:39 PM)**: A temporary bug was introduced where `insertQuery` was incorrectly used instead of `insertMany` for the bulk payload.
    *   **Correction (11/24/2025, 1:27:31 PM)**: The bug was fixed, reverting to `insertMany`.
    *   **Reversion of Bulk Insert (11/24/2025, 3:22:19 PM)**: The bulk insertion functionality in `createHblMasterBooking` was completely removed, reverting the function to handle a single HBL object from `req.body` using `insertQuery`, similar to its earlier state. A `console.log` was added for debugging `req.body`.
    *   **Payload Extension (11/24/2025, 3:36:33 PM)**: The payload for `createHblMasterBooking` was extended to include `transportaion_mode`, `customer`, and `description`.
    *   **Commenting Out (11/24/2025, 5:09:51 PM)**: The entire `createHblMasterBooking` function was commented out, indicating its temporary deactivation or a major re-evaluation. Additionally, the `bookingdate` field in `saveHblBooking`'s `bookingInsertQuery` was commented out.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`**
    *   **Route Conflict & Resolution (11/21/2025, 3:11:42 PM - 3:19:52 PM)**: A route `router.post("/",createHblMasterBooking);` was added, creating a conflict with `router.post("/",saveHblBooking);`. This was resolved by changing `createHblMasterBooking`'s route to `router.post("/create",createHblMasterBooking);`.
    *   **Route Deactivation (11/24/2025, 5:10:21 PM - 5:10:39 PM)**: The `createHblMasterBooking` route was commented out, and its import removed, reflecting the corresponding deactivation in the controller.

**Patterns and Recurring Elements:**

*   **Database Transaction Management**: Extensive use of `await query("BEGIN")`, `await query("COMMIT")`, and `await query("ROLLBACK")` demonstrates a commitment to data integrity through explicit transaction handling for database operations.
*   **Data Sanitization and Formatting**: The `sanitizeString` utility is frequently applied to request body fields, highlighting a consistent effort to clean input data. `AppDateFormate` is also consistently used for date handling.
*   **Role-Based Logic**: The `company_details.controller.ts` explicitly separates logic based on `req.query.role` (`AGENT` vs. `CUSTOMER`), indicating a role-based access or display system.
*   **Complex Booking Logic**: The `saveHblBooking` function in `hblEntry.controller.ts` demonstrates a highly interconnected data model, updating multiple tables (`agent_booking_entry`, `agent_booking_vessel`, `agent_booking_routing`, `agent_booking_container`, `agent_booking_hbl`, `agent_booking_po`) simultaneously, with logging of old and new states for auditing.
*   **Iterative Development & Debugging**: The frequent addition and removal of `console.log` and `logger.info` statements, alongside several functional changes and reversions (especially around bulk HBL inserts), indicate an active and iterative development process with ongoing debugging and refactoring.
*   **API Route Evolution**: Route definitions show a typical lifecycle of adding new endpoints, resolving conflicts (e.g., duplicate POST routes), and temporarily commenting out features (like `createHblMasterBooking`).

## 10:53:21 PM
The log primarily details changes within a React frontend application, `t360-frontend`, focusing on routing, UI components, API integration, and form validation for booking-related features.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\App.js`**
    *   **11/21/2025, 11:31:13 AM:** This timestamp shows a comprehensive update to the main application routing. Numerous new page components were imported and integrated into the `react-router-dom` `Routes` structure. Key additions include routes for `ShippingOrdersScreen`, `ContainerReportScreen`, `ReportSchedulerScreen`, `NewReportScreen`, `EditReportScreen`, `PdfParserScreen`, various tracking screens (`FCLScreen`, `LCLScreen`, `AddMtrForm`, `GetTruckingDetails`, `RailScreen`, `AirTrackingScreen`), extended booking forms (`AgentBookingFormScreenExtend`, `BookingDashbaordScreenExtend`), `Dashboard7501Screen`, `ShipmentsScreen`, `CngBookingScreen` and its related forms, `CityMaster`, `PortmasterScreen`, and `UploadOTMBOL`. The routing system consistently uses `AuthGuard` and `ProtectedRoute` for role-based access control (e.g., `ADMIN`, `CUSTOMER`, `AGENT`, `SPR_CUSTOMER`, `ADMIN_V2`).
    *   **11/21/2025, 11:34:00 AM:** No discernible code changes were made in this entry; the content is identical to the previous timestamp, likely a non-substantive save operation.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\MetricsBox.jsx`**
    *   **11/21/2025, 11:36:28 AM:** This entry defines the `MetricsBox` component, responsible for rendering various analytical charts (Bar, Pie, Line, Stacked Bar, Gauge). It includes functionality for filtering chart options via `MetricToolbar`, displaying "No Data Available" messages, and an "enlarge" feature using `ThemedModal`. The component dynamically selects the chart based on `item.chartType` and manages its view state.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\actions.js`**
    *   This file centrally manages actions available for booking entries based on user roles.
    *   **11/24/2025, 2:52:57 PM:** Initial detailed setup of `getBookingActions`, which defines an array of actions (Edit Booking, Copy Booking, Edit ISF, Copy ISF, Edit HBL, Create HBL, View Details, Update Line Booking, Update DO) and dynamically combines them based on `agentBookingMenu`, `customerBookingMenu`, and `adminBookingMenu` access flags. A large block of older, commented-out logic for `getBookingActions` is present. Notably, the `createHblAction` at this point uses `<Edit />` icon and `formAction: "edit"`.
    *   **11/24/2025, 2:53:07 PM:** The `formAction` for `createHblAction` was logically updated from `"edit"` to `"add"`, correctly reflecting its purpose of creating a new HBL.
    *   **11/24/2025, 2:53:44 PM:** A minor, likely temporary, change where the label "Edit HBL" was altered to "Edit HBL123".
    *   **11/24/2025, 2:54:37 PM:** The `createHblAction` was added to the list of actions available for `agentBookingMenu` and `adminBookingMenu` roles, enabling these users to initiate HBL creation.
    *   **11/24/2025, 2:56:17 PM:** The "Edit HBL123" label was reverted back to "Edit HBL".
    *   **11/24/2025, 3:43:34 PM:** The icon for `createHblAction` was changed from `<Edit />` to the more appropriate `<Create />`.
    *   **11/24/2025, 3:43:58 PM:** No discernible code changes were made in this entry; the content is identical to the previous timestamp.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\store\api\agentBookingApi.js`**
    *   **11/24/2025, 3:02:03 PM:** This entry introduces the `agentBookingApi` using Redux Toolkit Query, defining various API endpoints and their invalidation/provision tags. It includes mutations for `saveBooking`, `saveISF`, `saveBookingStatus`, XML generation/upload (`isfDescXmlCreate`, `createBookingXml`), and specifically for HBL, `saveHblBooking` and `fetchHblBookingFormDetails`. New mutations `updateCarrierBooking` and `updateDo` were added, along with a `exportSprReport` query. This indicates a significant expansion of booking-related API functionalities.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLform.jsx`**
    *   This file manages the House Bill of Lading (HBL) form, including multiple tabs for parties, routing, containers, and POs.
    *   **11/24/2025, 3:14:32 PM:** The initial entry shows a complex form component integrating `Formik` and `Yup` for validation. It manages form state, handles saving HBL data (including associated routing, container, and PO lists) via `useSaveHblBookingMutation`, and provides UI for editing, viewing, and adding parties. Keyboard shortcuts for common actions (edit, view, add party, save, focus) are implemented. Many `initialValues` properties are commented out, indicating potential refactoring of how form data is initialized.
    *   **11/24/2025, 3:15:08 PM, 3:16:19 PM:** These timestamps show no discernible code changes, suggesting non-substantive save operations.
    *   **11/24/2025, 3:17:08 PM:** The initialization logic for `agent_booking_id` was simplified, removing the conditional check for `formAction === "copy"`, which might imply a change in how copy operations handle this ID or a potential oversight if "copy" should explicitly reset the ID.
    *   **11/24/2025, 3:17:51 PM, 3:17:57 PM:** No discernible code changes were made in these entries.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\validation.js`**
    *   **11/24/2025, 3:20:42 PM:** This entry defines the `HBLvalidationSchema` using `Yup`, enforcing validation rules for various HBL fields like `pol`, `pod`, `vessel`, `voyage`, dates, and client names. Crucially, it includes specific regex-based format validation for `mblno` and `blno` (HBL No.), requiring them to be 4 uppercase letters followed by 1-12 uppercase letters or numbers. A `generateAddressValidation` helper is used to apply consistent address validation across different party types, with a conditional rule for `state` validation.

**Patterns and Recurring Elements:**

*   **Focused Development Sprints:** The changes are grouped by dates, indicating distinct development sprints. `11/21/2025` focused on broad routing and UI components, while `11/24/2025` was heavily dedicated to the booking and HBL features, encompassing UI actions, API endpoints, form logic, and validation.
*   **React Ecosystem Best Practices:** The project utilizes modern React best practices including `react-router-dom` for routing, `@mui/material` for UI components, `react-redux` and `@reduxjs/toolkit/query/react` for state and API management, and `Formik` with `Yup` for robust form handling and validation.
*   **Role-Based Access Control:** Authentication and authorization are a core concern, with `AuthGuard` and `ProtectedRoute` explicitly used alongside `UserRoleEnum` across numerous routes in `App.js`.
*   **Modular Design:** Actions related to booking are encapsulated in `actions.js`, API interactions in `agentBookingApi.js`, and form logic in `HBLform.jsx` and `validation.js`, reflecting a modular approach to feature development.
*   **Debugging and Iteration:** The presence of `console.log` statements, temporary label changes, and multiple identical save timestamps suggest an active development environment with frequent testing and iterative refinement.
*   **Enhanced User Experience:** The implementation of keyboard shortcuts in `HBLform.jsx` demonstrates an attention to improving user efficiency for form interactions.