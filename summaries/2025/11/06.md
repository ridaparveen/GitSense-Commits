...
## 7:18:33 PM
The provided log details recent code changes across several backend service and route files, primarily focusing on dashboard features, user management, shipment tracking, and booking functionalities.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\routes\AuthRoutes.ts`**
    *   **Timestamp: 11/5/2025, 12:54:56 PM**
    *   This file defines an authentication route (`/auth/verify-token`) that verifies a user's token and returns user information. This appears to be an initial or established route for user verification.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\dashboardService.ts`**
    *   **Initial Implementation (11/5/2025, 6:04:50 PM):** Introduced `getDashboardCardInfo` (for "pending departure", "in transit", "ETA within 1 week", "out for delivery" statistics) and `getDeliveryData` (for delivery/vessel-related shipments). Both functions used `buildMtrUserCondition` for user-specific data filtering and accepted `fromDate`, `toDate`, and `CompanyName` as query parameters.
    *   **Company Name Derivation (11/5/2025, 8:41:51 PM):** Modified `getDashboardCardInfo` to dynamically fetch `CompanyName` using `getUserObject` from `req.user` and query parameters, rather than directly from `req.query`. This introduced more robust company-specific data retrieval.
    *   **Query Condition Adjustments (11/5/2025, 10:43:59 PM):** The `useWhere` condition generated by `buildMtrUserCondition` was commented out for `getDashboardCardInfo`, removing a layer of user-specific filtering from the card statistics.
    *   **`getDeliveryData` Company Filter Refinements (11/5/2025, 10:46:26 PM - 11/5/2025, 11:13:53 PM):**
        *   Initially, `getDeliveryData` also started using `getUserObject` to derive `CompanyName`.
        *   A significant update introduced a `companyFilter` variable in `getDeliveryData` which handles `CompanyName` from `req.query` and explicitly sanitizes it using `.replace(/'/g, "''")` to prevent SQL injection, applying it to both `sqlQueryDelivery` and `SqlQueryVesselArrival`.
        *   The logic for dynamically fetching `CompanyName` using `getUserObject` was commented out, reverting to taking `CompanyName` directly from `req.query`, but now with proper sanitization applied via `companyFilter`.
    *   **Debugging Additions (Multiple Timestamps):** Several `console.log` statements were added, particularly to inspect `req.query`, `CompanyName`, `result[0]`, and `companyFilter`, indicating active debugging during development.
    *   **Minor Syntax Changes (Multiple Timestamps):** Several commits show identical code or minor formatting changes, suggesting iterative development and frequent saves.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\UserService.ts`**
    *   **Initial Implementation (11/5/2025, 8:51:00 PM):** This file was heavily updated or introduced with multiple user-related functions:
        *   `login`: Handles user authentication, validates credentials, checks account status, and generates JWT tokens.
        *   `saveUserPreferences`, `fetchUserPreferences`: Manages user profile preferences, including basic info, notification settings, and shipment event configurations. It uses an `ON CONFLICT DO UPDATE` strategy for saving.
        *   `saveInviteLog`, `fetchAllCompanyUser`: Manages user invitations and fetches a combined list of active and pending invited users for a company.
        *   `userSettings`: Retrieves specific user settings like date format.
    *   **Minor Changes (Multiple Timestamps):** Subsequent timestamps for this file (11/5/2025, 8:51:15 PM and 11/5/2025, 8:51:46 PM) show no functional code changes, likely reflecting formatting or metadata updates.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\shipmentService.ts`**
    *   **Initial Implementation (11/6/2025, 1:27:09 PM):** Introduced the `getAll` function for fetching shipment data. This function leverages an `SqlQueryBuilder` class to construct dynamic SQL queries.
    *   **`SqlQueryBuilder` Functionality:** The builder supports filtering by user role, various shipment parameters (`pol`, `pod`, `status`, `dates`, `cardFilter`, `contno`, `viewType`), pagination, and sorting. It also applies specific conditions for dashboard "card filters" like 'pending\_departure', 'in\_transit', etc.
    *   **Shipment Status Calculation:** The `getAll` function includes logic to determine and assign a human-readable `status` (e.g., "Pending", "Loading", "In Transit", "Arrived", "Delivered") and a corresponding `value` based on various date and status fields from `mtr_tracking_data`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\bookingService.ts`**
    *   **Initial Implementation (11/6/2025, 2:02:45 PM):** Introduced `BookingQueryBuilder` and `getAll` and `saveBooking` functions.
    *   **`BookingQueryBuilder`:** Dynamically builds queries for booking data, supporting user-role-based access, pagination, sorting, search, filtering by various booking attributes (POL, POD, shipper, status, type), and date ranges. It also provides counts for different booking statuses and options for filter dropdowns.
    *   **`getAll` Function:** Uses `BookingQueryBuilder` to retrieve booking data, status counts, and filter options.
    *   **Cargo Ready Date Filtering (11/6/2025, 2:05:16 PM - 11/6/2025, 2:16:39 PM):** Added `est_cargo_ready_date` to selected columns and introduced `CargoReadyFromDate` and `CargoReadyToDate` parameters for date range filtering on estimated cargo ready dates.
    *   **`whereClauses` Refactoring and Reversion (11/6/2025, 2:26:13 PM - 11/6/2025, 2:27:15 PM):** An attempt was made to change `whereClauses` from a single string to a string array for better query construction, but this change was quickly reverted, possibly due to issues or incomplete implementation.
    *   **`COUNT` Query Correction (11/6/2025, 5:57:33 PM - 11/6/2025, 5:57:41 PM):** A typo in the `buildCountQuery` was introduced (`COUNTabe`) and then corrected, along with a change in whether `DISTINCT` was used in the count.
    *   **New Booking Status: CONFIRMED (11/6/2025, 6:03:28 PM):** Added `CONFIRMED` as a new `BookingStatus` and updated the query builder and `getAll` function to handle and display counts for this new status.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\models\common\types\booking.ts`**
    *   **Initial Implementation (11/6/2025, 6:02:57 PM):** Defined interfaces and enums for booking-related data, including `BookingPayloadTypes`, `BookingRequestBody`, `AuthReqUser`, `EntryMode`, `BookingStatus`, and `BookingQueryType`. The `CONFIRMED` status was added to `BookingStatus` in this update.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\constants\Paths.ts`**
    *   **Booking Route Additions (11/6/2025, 5:06:22 PM - 11/6/2025, 5:13:57 PM):** Added new paths under the `Booking` category:
        *   `getBookingApproveDetails: "/booking_approve/:agent_booking_id"`
        *   `createBookingApproveDetails: "/booking_approve/:agent_booking_id"` (a duplicate path with a different implied action, likely for POST vs GET).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\routes\BookingRoutes.ts`**
    *   **New Booking Approval Routes (11/6/2025, 5:06:41 PM - 11/6/2025, 5:15:38 PM):**
        *   Registered a GET route for `Paths.Booking.getBookingApproveDetails` to `getBookingApproveDetails` service function.
        *   Added a POST route for `Paths.Booking.createBookingApproveDetails` to `updateBookingApproveDetails` service function, indicating the implementation of a booking approval/update mechanism.

### Patterns and Recurring Elements:

*   **Modular Architecture:** The backend uses a clear structure with separate files for routes, services, and common types/constants, promoting maintainability.
*   **Dynamic SQL Query Building:** A consistent pattern across `dashboardService.ts` and `shipmentService.ts` (and `bookingService.ts`) is the use of dedicated "QueryBuilder" classes (e.g., `SqlQueryBuilder`, `BookingQueryBuilder`) to construct dynamic SQL queries based on user roles, filters, pagination, and sorting parameters. This approach centralizes query logic and aims to reduce redundancy.
*   **User-Centric Filtering:** User roles (`ADMIN`, `CUSTOMER`, `FRONTOFFICE`, `AGENT`) and associated company/location information (`req.user`) are consistently used to apply data filtering, ensuring users only access relevant information.
*   **Robust Logging and Error Handling:** `logger.info` and `logger.error` are extensively used for tracking execution flow and debugging. `try...catch` blocks with `HttpError` and `HttpStatusCodes` are standard for API responses.
*   **Iterative Development:** Numerous closely timed commits for `dashboardService.ts` and `bookingService.ts` with small, focused changes (e.g., adding a column, commenting out a line, adding `console.log` for debugging) suggest an active and incremental development process, possibly involving rapid testing and refinement.
*   **Data Sanitization:** The addition of `.replace(/'/g, "''")` for `CompanyName` in `dashboardService.ts` highlights an awareness of and effort to address potential SQL injection vulnerabilities.
*   **Expansion of Features:** The changes indicate ongoing development to expand dashboard analytics, improve shipment tracking details, and enhance the booking management system (including a new "CONFIRMED" status and approval routes).