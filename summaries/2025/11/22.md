...
## 2:13:29 PM
The code changes log primarily reflects updates to two core areas: company details retrieval and HBL (House Bill of Lading) entry and management, along with minor routing adjustments for OTM BOL (Bill of Lading). All modifications occurred on **11/21/2025**.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\company_details.controller.ts`**
    *   **(11/21/2025, 10:07:54 AM & 10:08:08 AM)**: A minor cleanup was performed in the `getAll` function. Specifically, three `console.log` debugging statements within the `CUSTOMER` role's pagination logic were removed, indicating a commit to clean up development logs. The function retrieves distinct company names (`cname` for 'AGENT' role from `agent_po_account_master` and `account_name` for 'CUSTOMER' role from `mtr_tracking_data`), supporting both paginated and non-paginated searches, with search queries case-insensitive (`ILIKE`).

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\otm_bol.route.ts`**
    *   **(11/21/2025, 11:40:37 AM)**: This file saw a significant expansion of OTM BOL related functionalities.
        *   New GET routes were added for `"/fetch_payload"` and `"/fetch/:common_key"`.
        *   New POST routes were introduced for `"/upload_save_edit"`, `"/generate_xml"`, and `"/send_to_otm"`.
        *   An existing `"/upload"` POST route, likely for file uploads, was commented out. This suggests a shift from a generic upload to more specific, granular operations for OTM integration, including payload fetching, editing, XML generation, and sending data.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`**
    *   **(11/21/2025, 3:11:14 PM)**: This timestamp introduces two major functions: `createHblMasterBooking` and `saveHblBooking`.
        *   `createHblMasterBooking` handles the initial creation of an HBL in the `hbl_master` table, sanitizing various shipper, consignee, notify, and booking details. It uses database transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) and `HttpError` for error handling.
        *   `saveHblBooking` is a more comprehensive function supporting both `ADD` and `UPDATE` operations for HBL bookings. It handles extensive payload types including routing, container, and basic PO lists. For updates, it first fetches current booking, vessel, routing, container, HBL, and PO details for auditing, then updates the main `agent_booking_entry` and `agent_booking_vessel` tables. It performs a `DELETE` followed by `INSERT` for related `agent_booking_routing`, `agent_booking_container`, and `agent_booking_po` records, implying that these are managed as separate lists that can be fully replaced. Crucially, it logs both the `old_hbl_logs` (data before update) and `new_hbl_logs` (new payload) into `agent_booking_hbl_history` for detailed change tracking. New bookings generate a unique `agent_booking_id`.
    *   **(11/21/2025, 3:13:11 PM)**: The error handling in `createHblMasterBooking` was modified to use `logger.error(error);` instead of `next(new HttpError(error.message, 500));`, indicating a change in how errors from this specific function are managed (from middleware propagation to internal logging).
    *   **(11/21/2025, 3:16:23 PM)**: In `saveHblBooking`, the `bookingdate` field in the `bookingInsertQuery` for `agent_booking_entry` was made nullable (`payload.booking_date || null`).
    *   **(11/21/2025, 3:17:07 PM)**: Similarly, in `createHblMasterBooking`, the `bookingdate` field in its payload was also made nullable (`AppDateFormate(req.body.bookingdate, db_format) || null`).
    *   **(11/21/2025, 3:19:06 PM)**: The `bookingdate` field within the `payload` for `createHblMasterBooking` was commented out entirely, suggesting that this specific controller might no longer be responsible for setting or using this field during the initial HBL master booking creation.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`**
    *   **(11/21/2025, 3:11:42 PM)**: Initial creation of HBL entry routes, with two `router.post("/")` definitions for `saveHblBooking` and `createHblMasterBooking`. This creates a routing conflict where `createHblMasterBooking` would be unreachable.
    *   **(11/21/2025, 3:19:52 PM)**: The routing conflict was resolved by changing the route for `createHblMasterBooking` from `router.post("/")` to `router.post("/create")`, allowing both post endpoints to be accessible.

### Patterns and Recurring Elements:

*   **Consistent Timestamp:** All changes are concentrated on a single day (11/21/2025), indicating a focused development sprint or a significant feature implementation.
*   **Database Transaction Management:** Frequent use of `await query("BEGIN")`, `await query("COMMIT")`, and `await query("ROLLBACK")` for ensuring data integrity during complex multi-step database operations, particularly in `hblEntry.controller.ts`.
*   **Data Sanitization:** Widespread application of `sanitizeString` for request body parameters to prevent potential injection attacks and ensure clean data.
*   **Date Formatting:** Consistent use of `AppDateFormate` with a `YYYY-MM-DD` format for handling date inputs before database interaction.
*   **Structured Logging:** Extensive use of `logger.info` and `logger.error` for tracking application flow, debugging, and operational insights, especially evident in the `hblEntry.controller.ts` with detailed logging for HBL updates.
*   **Modular Architecture:** The changes demonstrate a clear separation of concerns with controllers handling business logic and routes defining API endpoints, using imported utilities (`getAgentBookingCode`, `AppDateFormate`, `sanitizeString`) and custom types (`hblEntryPayloadTypes`, `EntryMode`).
*   **Auditing and History Tracking:** The `saveHblBooking` function explicitly implements a robust auditing mechanism by fetching "current" data before updates and logging both old and new states to an `agent_booking_hbl_history` table, showcasing a strong emphasis on traceability for critical booking changes.
*   **Update Strategy for Lists:** For nested data like `routingList`, `containerList`, and `basicPoList` in `saveHblBooking`, the update strategy involves deleting existing records associated with an `agent_booking_id` and then re-inserting the new list items, rather than item-by-item updates.