...
## 7:58:55 PM
In `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\LoginScreen.jsx`, a series of changes occurred related to user login and redirection logic:

*   **Timestamp: 11/20/2025, 6:06:35 PM**: The initial implementation handles user login. If the login is successful, it checks if the user's role is `CUSTOMER` and `v2_flag` is true. If so, it redirects the user to a V2 UI URL (`REACT_APP_V2_UI_BASE_URL`) with their token and `authtype=normal`. Otherwise, it dispatches the login action to Redux and navigates to the `/app` route or a specified `redirectUrl`. Debugging `console.log` statements for `redirectUrl` and user role were present.
*   **Timestamp: 11/20/2025, 6:07:02 PM**: A significant update was made to the V2 UI redirection condition. The condition was expanded to include `ADMIN_V2` role in addition to `CUSTOMER`. Now, if the user's role is either `CUSTOMER` or `ADMIN_V2`, and `v2_flag` is true, the user will be redirected to the V2 UI.
*   **Timestamp: 11/20/2025, 6:25:53 PM**: This update primarily involved cleanup. The `console.log` statements that were logging `redirectUrl` and user role were removed, while the core login and redirection logic (including the updated V2 UI condition) remained intact.

In `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\HandleAuthCallback.js`:

*   **Timestamp: 11/20/2025, 6:07:47 PM**: This file, responsible for processing Auth0 authentication callbacks, was introduced. It retrieves user details and ID token claims post-authentication. It then calls an `ApiManager` to `CheckUser`. Similar to the `LoginScreen.jsx` logic, it includes a conditional redirection to the V2 UI: if the user's role is `CUSTOMER` or `ADMIN_V2` and `v2_flag` is true, they are redirected to `REACT_APP_V2_UI_BASE_URL` with an Auth0 token and `authtype=auth0`. Otherwise, it dispatches the login action with the Auth0 token to Redux and navigates based on a `redirect_url` stored in `localStorage` or defaults to `/app`. If the user is not registered, they are redirected to a `/register_user` route.

**Patterns and Recurring Elements:**

*   **V2 UI Redirection Logic:** A prominent pattern across both `LoginScreen.jsx` and `HandleAuthCallback.js` is the conditional redirection to an external "V2 UI" base URL (`process.env.REACT_APP_V2_UI_BASE_URL`). This redirection is triggered for users with `CUSTOMER` or `ADMIN_V2` roles (after the initial update) if their `v2_flag` is set.
*   **User Role-Based Access:** The `UserRoleEnum` (`CUSTOMER`, `ADMIN_V2`) plays a critical role in determining user flow, particularly for accessing the V2 UI.
*   **Redux for Authentication State:** Both components utilize `react-redux` to dispatch a `login` action, centralizing the storage of user token, user object, and authentication type (`normal` or `auth0`).
*   **`redirect_url` Management:** There's a consistent approach to handling post-login navigation, using a `redirect_url` from either URL query parameters or `localStorage`.
*   **API Manager Interaction:** Both files integrate with `ApiManager` for core authentication operations (login or user checking).
*   **Debugging Console Logs:** Console logs were used for development insight, particularly in `LoginScreen.jsx` which later saw their removal, suggesting a common development workflow of adding and then cleaning up debug statements. `HandleAuthCallback.js` still contains several `console.log`s in the provided state.

## 7:59:46 PM
The code changes primarily involve two service files and one utility file related to dashboard information display and analytics data export.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\dashboardService.ts`**

*   **Timestamp: 11/20/2025, 6:10:56 PM**
    *   This file provides two main API endpoints: `getDashboardCardInfo` and `getDeliveryData`.
    *   `getDashboardCardInfo` fetches aggregated statistics for a dashboard, including counts and TEU (Twenty-foot Equivalent Unit) values for "Pending Departure," "In Transit," "ETA Within 1 Week," and "Out for Delivery" shipments. It applies filters based on `fromDate`, `toDate`, `CompanyName`, and user-specific conditions (`buildMtrUserCondition`). The TEU calculation involves extracting numeric values from `t49_cont_type` or `cont_type` and dividing by 20.
    *   `getDeliveryData` retrieves either delivery or vessel arrival data, determined by a `type` parameter. It supports different delivery `mode` filters ("ALL," "Rail Delivery," "Door Delivery," "Port Delivery") and `CompanyName` filters. Both functions extensively use dynamic SQL queries on `mtr_tracking_data`.

*   **Timestamp: 11/20/2025, 6:12:53 PM**
    *   A minor update was made to `getDashboardCardInfo`, adding a debugging `console.log(useWhere,"===useWhere",req.user);` statement to inspect the `useWhere` variable and the `req.user` object. No functional changes to the query logic were introduced.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\export.query.ts`**

*   **Timestamp: 11/20/2025, 7:21:07 PM**
    *   This file exports a `getMonthValue` utility function to convert month names (e.g., "Jan") or numbers into numeric month values.
    *   It also exports a `bookingQuery` function which acts as a factory for generating various SQL query fragments specifically for "booking" related analytics reports. This function takes parameters like `reportName`, `CompanyName`, date filters (`stackedMonth`, `startWeek`, `endWeek`, `quarter`), `stackedGroup`, `stackedExport`, and `displayBy`.
    *   The `bookingQuery` function dynamically constructs SQL clauses for reports such as "Booking Date to Actual Departure," "Booking Date to Estimated Departure," and "Booking Date to POD Arrival," including "Stacked Bar" and "MBL Wise" variations.
    *   Queries typically join `mtr_tracking_data` with `agent_booking_entry` on `mblno` and calculate `day_diff` between dates.
    *   It consistently filters out invalid date values like `'0001-01-01 00:00:00 BC'`.
    *   Stacked bar report logic includes complex `CASE` statements to categorize `day_diff` into time ranges (e.g., 'Less than 1 week', '1 - 2 weeks').

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\analyticsExportService.ts`**

*   **Timestamp: 11/20/2025, 7:23:44 PM**
    *   This file defines `getExportData`, a primary function responsible for handling requests to export analytics data.
    *   It processes various query parameters for report generation, including `fromDate`, `toDate`, `reportName`, `CompanyName`, and specific filters for stacked exports.
    *   It leverages `bookingQuery` (from `export.query.ts`) and an unshown `nonBookingQuery` to construct the main SQL query for data retrieval.
    *   It handles user and company context using `getUserObject` and saves column preferences using `saveColumns`.
    *   Default date ranges are applied if not provided in the request.
    *   A `replaceColumnHandler` utility is used to standardize location names, specifically changing "Long Beach" to "Los Angeles" for `pod` and `place_of_delivery` columns.
    *   For MBL-wise TEU reports, it includes specific logic to de-duplicate entries based on a combination of `voyage`, `contno`, and `cont_type`, marking duplicates with `teu: "-"`.
    *   The service performs database queries and sends the processed data as an HTTP response.

**Patterns and Recurring Elements:**

*   **Dynamic SQL Generation:** A pervasive pattern across all files is the construction of complex SQL queries dynamically based on user input, report types, and various filtering conditions.
*   **`mtr_tracking_data` Table:** The `mtr_tracking_data` table is central to almost all queries, indicating it's the primary source for shipment and tracking information. The `agent_booking_entry` table is also frequently used in conjunction with it.
*   **Date and Company Filtering:** Consistent application of `fromDate`, `toDate`, and `CompanyName` as crucial filtering parameters across different data retrieval functions. Invalid date values (`'0001-01-01 00:00:00 BC'`) are explicitly handled in SQL queries.
*   **User-Specific Data Access:** The `buildMtrUserCondition` and `getUserObject` functions are used to ensure data access is restricted or filtered based on the authenticated user's profile and associated company.
*   **Logging:** Extensive use of `logger.info` and `logger.error` statements for tracking application flow, debugging, and error handling.
*   **TEU Calculation Logic:** Specific logic for calculating TEU from container types appears in both dashboard and analytics export contexts.
*   **Location Normalization:** A recurring business rule to normalize "Long Beach" to "Los Angeles" in location fields (`pod`, `place_of_delivery`) is present in `analyticsExportService.ts`.
*   **Error Handling:** All service functions include `try...catch` blocks to handle errors and return `HttpStatusCodes.INTERNAL_SERVER_ERROR`.

## 8:01:08 PM
The code changes primarily occurred on 11/20/2025, indicating focused development within a short period, particularly on user authentication/logout flow and analytics reporting features.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\usercard.tsx`**

*   **Timestamp: 11/20/2025, 6:35:28 PM**
    *   The `handleLogout` function was refactored. For Auth0 authentication, `localStorage` items (`token`, `user`) and Redux state (`auth/logout`) are now cleared *before* redirecting the user to the `VITE_UI_V1_BASE_URL/logout` endpoint. This ensures local state is cleaned up synchronously prior to navigation.

*   **Timestamp: 11/20/2025, 7:56:52 PM**
    *   A significant update was made to the `handleLogout` function again. The Auth0 logout redirect URL was changed from `${link}/logout` to `${link}/login`. This implies that after logging out via Auth0, users are now directed to the login page of the V1 UI instead of a general logout endpoint.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricFooter.tsx`**

*   **Timestamp: 11/20/2025, 7:27:55 PM**
    *   Introduced new functionalities for analytics reporting:
        *   **PDF Export:** A new `handlePdf` function was added, leveraging `jsPDF` and `html2canvas-pro` to convert chart components into a PDF document. It uses a hidden DOM container to render the `PdfRenderer` component, captures it with `html2canvas`, and saves it as a PDF.
        *   **Excel Export Column Selection:** A new `selectColumnsForExportExcel` function was implemented to allow users to choose specific columns for Excel export using a `ThemedModal` and `ColumnsSelector` component.
        *   **Loading Indicators:** `pdfLoading` and `loading` states were added with `CircularProgress` to provide feedback during PDF and Excel export operations.
        *   **UI Enhancements:** Added a PDF icon (`MdPictureAsPdf`), `data-html2canvas-ignore` attributes to exclude buttons from PDF captures, and minor layout adjustments for the footer.

**File: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\methods.ts`**

*   **Timestamp: 11/20/2025, 7:32:19 PM**
    *   This timestamp captures the initial implementation or a significant update to analytics utility methods. It includes:
        *   Type definitions for `AnalyticsData`, `Customizer`, `Metric`, `Endpoint`, `FormatSettings`, and `MetricsPackageReturn`.
        *   `metricsPackageHandler`: A function to process raw metric data into a structured format for display, including determining chart types and calculation types (Avg, Total, NA).
        *   `allChartOptions` and `chartOptionsByType`: Constants defining available chart types and their corresponding icons.
        *   `reportFormula`: A function to generate formula strings for reports based on date columns.
        *   `weekDateFormater`: A utility for formatting week dates.
        *   `downloadReportSpreadsheet`: A complex asynchronous function for generating and downloading Excel spreadsheets (`ExcelJS`) with detailed report data, including logic for different chart types (StackedBar, Transit), period filters (month, week, quarter), and calculations (Avg, Total).

*   **Timestamp: 11/20/2025, 7:39:29 PM**
    *   A minor, targeted change was made within the `downloadReportSpreadsheet` function. The client name displayed in the Excel report now converts `selectedCustomer` to `toLowerCase()` before being included in the report array:
        *   Old: `["Client", selectedCustomer || user?.companyname],`
        *   New: `["Client", selectedCustomer?.toLowerCase() || user?.companyname],`

**Recurring Elements and Patterns:**

*   **Authentication and Session Management:** Consistent use of `localStorage` to store user information (`user`, `token`, `authtype`) and `react-redux` for dispatching logout actions.
*   **Analytics Feature Development:** A strong focus on enhancing the analytics section, adding robust export functionalities (Excel with column selection, PDF generation) and refining how report data is processed and presented.
*   **Date and Time Handling:** Usage of `moment.js` for formatting dates in reports.
*   **UI Component Library:** Reliance on custom `kit` components (e.g., `Dropdown`, `Avatar`, `SidebarItem`, `Badge`, `IconButton`) and Material-UI icons for consistent UI.

## 8:58:53 PM
**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\LoginScreen.jsx`**
    *   **11/20/2025, 6:06:35 PM:** The `LoginScreen` component was updated to introduce a conditional redirection logic. If a user successfully logs in and their role is `CUSTOMER` with `v2_flag` enabled, they are redirected to a V2 UI base URL, passing the authentication token and `authtype=normal` as query parameters. Otherwise, their login details are dispatched to the Redux store, and they are navigated to `/app` or a specified `redirectUrl`. Debug `console.log` statements were present for `redirectUrl` and `res.user.role`.
    *   **11/20/2025, 6:07:02 PM:** A quick update to the `LoginScreen` component's redirection logic expanded the V2 UI access. The condition now checks if the user's role is either `CUSTOMER` or `ADMIN_V2` (in addition to `v2_flag` being true) for redirection to the V2 UI.
    *   **11/20/2025, 6:25:53 PM:** The `LoginScreen` component was refined by removing the previously added `console.log` debug statements related to `redirectUrl` and `res.user.role`, indicating a clean-up after initial development or testing. The core logic for V2 UI redirection for `CUSTOMER` or `ADMIN_V2` roles remained unchanged from the 6:07:02 PM version.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\HandleAuthCallback.js`**
    *   **11/20/2025, 6:07:47 PM:** A new component, `HandleAuthCallback`, was introduced or significantly updated to manage Auth0 authentication callbacks. This component uses `useAuth0` to get user information and claims. It attempts to verify the user with `ApiManager.CheckUser` and, similar to `LoginScreen.jsx`, implements conditional redirection to the V2 UI for `CUSTOMER` or `ADMIN_V2` roles with `v2_flag` enabled, using the Auth0 token (`token.__raw`) and `authtype=auth0`. For non-V2 users, it dispatches login details to the Redux store. It also handles cases where a user might not be registered, redirecting them to `/register_user`. During the callback processing, it displays a "Please wait" message with a `CircularProgress` indicator. The component also manages a `redirect_url` stored in `localStorage` for post-authentication navigation.

**Patterns and Recurring Elements:**

*   **V2 UI Redirection Logic:** A consistent pattern across both `LoginScreen.jsx` and `HandleAuthCallback.js` is the conditional redirection to a "V2 UI". This redirection is triggered if the authenticated user's role is either `CUSTOMER` or `ADMIN_V2` and a `v2_flag` is set. The redirection uses `window.location.href` and passes the authentication token along with an `authtype` parameter (`normal` for standard login, `auth0` for Auth0 login).
*   **Redux for Auth State:** For users not redirected to the V2 UI, the application consistently dispatches a `login` action to a Redux `authSlice` to store the user's token, user object, and authentication type.
*   **Redirect URL Handling:** Both login methods (standard and Auth0 callback) incorporate logic to handle a `redirectUrl` to navigate the user to their intended destination after successful authentication.
*   **Role-Based Access:** The `UserRoleEnum` is used to define and check user roles, centralizing role management.
*   **UI/UX Feedback:** `react-hot-toast` is used for displaying user feedback (success/error messages), and `CircularProgress` indicates loading states during authentication.
*   **Debugging Practices:** Initial versions included `console.log` statements for debugging, which were later removed, indicating a development and clean-up cycle.

## 8:59:47 PM
In `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\dashboardService.ts`, there were two updates on **11/20/2025**.
The first update at **6:10:56 PM** introduced two asynchronous functions: `getDashboardCardInfo` and `getDeliveryData`.
- `getDashboardCardInfo` is responsible for fetching key dashboard metrics (Pending Departure, In Transit, ETA Within 1 Week, Out for Delivery). It dynamically constructs a complex SQL query to count and sum TEU for various shipment statuses, incorporating filters for `fromDate`, `toDate`, `CompanyName`, and user-specific conditions using `buildMtrUserCondition`. It also includes checks for `shipment_completed` and a `MTR.DepartureDateCutoff`.
- `getDeliveryData` retrieves either delivery or vessel arrival information based on `type` and `mode` parameters. It also applies `CompanyName` and user-specific filters. It uses two main SQL queries: `sqlQueryDelivery` for distinct delivery data based on ETA or delivery date, and `SqlQueryVesselArrival` for vessel arrival data, which includes detailed logic for different `mode` values (ALL, Rail, Door, Port Delivery) and employs `ROW_NUMBER()` for deduplication.

A subsequent minor change to the same file at **6:12:53 PM** involved updating a `console.log` statement within `getDashboardCardInfo` from `console.log(useWhere,"===useWhere");` to `console.log(useWhere,"===useWhere",req.user);`, adding `req.user` for additional debugging context.

At **11/20/2025, 7:21:07 PM**, the file `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\export.query.ts` was updated. This file introduced a `getMonthValue` helper function to convert month names or numbers to integer month values, and `bookingQuery`. The `bookingQuery` is a function that returns various SQL query fragments based on a `reportName` (e.g., "Booking Date to Actual Departure", "Booking Date to POD Arrival") and filters like `CompanyName`, date ranges (month, week, quarter), and specific stacked groups (e.g., 'Less than 1 week'). These queries typically join `mtr_tracking_data` with `agent_booking_entry` and calculate date differences. A common pattern in these queries is filtering out invalid historical dates like `'0001-01-01 00:00:00 BC'`.

Shortly after, at **11/20/2025, 7:23:44 PM**, changes were made to `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\analyticsExportService.ts`. This update included the `getExportData` function, which serves as a central point for exporting analytics data.
- It parses numerous query parameters for filtering and report generation.
- It leverages `getUserObject` for user and company context and calls helper functions `columnsQuery` and `saveColumns`.
- The core logic conditionally constructs a `mainQuery` by calling either `bookingQuery` (from `export.query.ts`) or `nonBookingQuery` based on a `booking` parameter.
- It handles default date ranges, applies dynamic `DISTINCT ON` clauses for booking-related reports, and includes special deduplication logic for "TEU MBL" reports.
- Data is processed through `dateFilterOnExport` and `replaceColumnHandler` before being sent as a response.
- The `replaceColumnHandler` utility function normalizes geographic data by replacing "LONG BEACH" with "LOS ANGELES" (case-insensitive) in `pod` and `place_of_delivery` fields.

**Patterns and Recurring Elements:**
- **Dynamic SQL Query Generation:** A consistent theme across these changes is the robust generation of SQL queries based on user inputs and specific report requirements, demonstrating a flexible reporting architecture.
- **User and Company-Specific Filtering:** Both dashboard and analytics services implement filtering based on `CompanyName` and user access conditions (`buildMtrUserCondition`), indicating a multi-tenant or role-based access control design.
- **Date and Time Manipulation:** Extensive use of date-related SQL functions (`EXTRACT`, `BETWEEN`, `CURRENT_DATE`, `INTERVAL`) and application-level date handling for various filtering and reporting periods.
- **TEU Calculation and Handling:** TEU (Twenty-foot Equivalent Unit) is a key metric, with specific SQL logic for its calculation and special handling in analytics reports.
- **Error Handling and Logging:** All services consistently use `try...catch` blocks for error management and comprehensive `logger.info` and `console.log` statements for debugging and operational visibility.
- **Data Normalization:** The `replaceColumnHandler` for geographic data suggests an effort to standardize data presentation.
- **Common Data Sources:** The `mtr_tracking_data` and `agent_booking_entry` tables are fundamental, being joined and queried across different services.
- **Invalid Date Exclusion:** The frequent use of `!= '0001-01-01 00:00:00 BC'` in SQL queries highlights a recurring need to filter out placeholder or invalid date entries from the database.
- **Timestamp Proximity:** All changes occurred on the same date (11/20/2025) within a relatively short period, suggesting a concentrated development effort on reporting and dashboard functionalities.

## 10:32:55 PM
The provided log details changes across two frontend files: `LoginScreen.jsx` and `HandleAuthCallback.js`, primarily focusing on authentication flow and a new V2 UI redirection strategy.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\LoginScreen.jsx`**

*   **11/20/2025, 6:06:35 PM:** The `handleSubmit` function was updated to introduce a conditional redirection to a "V2 UI". If a user successfully logs in and has the `CUSTOMER` role with `v2_flag` set to true, they are redirected to a V2 UI base URL (retrieved from `process.env.REACT_APP_V2_UI_BASE_URL`) with their token and `authtype=normal`. Otherwise, the standard login flow (dispatching login action to Redux and navigating to `/app` or a `redirectUrl`) proceeds.
*   **11/20/2025, 6:07:02 PM:** The V2 UI redirection logic within `handleSubmit` was refined. The condition for redirection was expanded to include users with either `CUSTOMER` **or** `ADMIN_V2` roles, provided their `v2_flag` is true. This means more user roles are now eligible for the V2 UI redirect.
*   **11/20/2025, 6:25:53 PM:** Two debug `console.log` statements (one for `redirectUrl` and another for `res.user.role`) were removed from the component, indicating a cleanup of development logs. The core V2 UI redirection logic remains as per the 6:07:02 PM update.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\HandleAuthCallback.js`**

*   **11/20/2025, 6:07:47 PM:** A new file, `HandleAuthCallback.js`, was introduced. This component is responsible for processing authentication callbacks from Auth0. Upon successful Auth0 authentication, it retrieves user claims and calls an internal API (`ApiManager.CheckUser`) to verify the user's status within the application.
    *   Crucially, this file also implements the V2 UI redirection logic. If the authenticated user (via Auth0) has `CUSTOMER` **or** `ADMIN_V2` roles and `v2_flag` is true, they are redirected to the V2 UI with their Auth0 token (`token.__raw`) and `authtype=auth0`.
    *   For users not redirected to V2 UI, it dispatches the login action to Redux, similar to the `LoginScreen`, and handles navigation, including redirecting based on a `redirect_url` stored in `localStorage`.
    *   It provides visual feedback with a "Please wait..." message and a `CircularProgress` component during the verification process.
    *   Error handling is included, guiding unregistered users to a `/register_user` path.

**Key Patterns and Recurring Elements:**

*   **V2 UI Redirection:** A prominent pattern is the introduction and refinement of a mechanism to redirect specific users (initially `CUSTOMER`, later also `ADMIN_V2`, provided a `v2_flag` is true) to a new "V2 UI" application. This logic is consistently applied in both the standard login screen and the Auth0 callback handler.
*   **Role-Based Access/Redirection:** The redirection decision is based on user roles (`CUSTOMER` and `ADMIN_V2`) and a `v2_flag`, indicating a move towards differentiating user experiences based on their access level or a feature flag.
*   **Authentication Type Parameter:** When redirecting to the V2 UI, an `authtype` parameter is included in the URL (e.g., `authtype=normal` for direct login, `authtype=auth0` for Auth0 callback), which likely helps the V2 UI understand the origin of the authentication.
*   **Redux Integration:** Both files use `react-redux` to dispatch a `login` action, centralizing user session management after successful authentication.
*   **External vs. Internal Navigation:** Both components utilize `window.location.href` for external redirects to the V2 UI and `useNavigate` (from `react-router-dom`) for internal application navigation.
*   **Environment Variables:** The V2 UI base URL is retrieved from `process.env.REACT_APP_V2_UI_BASE_URL`, indicating reliance on environment configurations for external service URLs.
*   **Debugging Cleanup:** The removal of `console.log` statements in `LoginScreen.jsx` points to a general practice of cleaning up debug code as development progresses.

## 10:33:03 PM
The provided code change log focuses on three TypeScript files related to dashboard and analytics functionality, all updated on 11/20/2025.

### File-Specific Updates:

1.  **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\dashboardService.ts`**
    *   **Timestamp: 11/20/2025, 6:10:56 PM**
        *   This file contains two primary asynchronous functions: `getDashboardCardInfo` and `getDeliveryData`.
        *   `getDashboardCardInfo` is responsible for fetching key dashboard metrics like "Pending Departure," "In Transit," "ETA Within 1 Week," and "Out for Delivery," along with their corresponding Twenty-foot Equivalent Unit (TEU) counts. It uses complex PostgreSQL queries with `FILTER (WHERE ...)` clauses to categorize shipments based on various date and status fields. Filters include `fromDate`, `toDate`, `CompanyName`, and a user-specific condition (`buildMtrUserCondition`).
        *   `getDeliveryData` retrieves either "delivery" or "vessel" related data. It uses `DISTINCT ON (mblno)` for delivery data and a subquery with `ROW_NUMBER()` for vessel arrivals to get the latest ETA per MBL. It supports filtering by `CompanyName` and dynamically adjusts the query based on `mode` (e.g., "ALL," "Rail Delivery," "Door Delivery," "Port Delivery").
    *   **Timestamp: 11/20/2025, 6:12:53 PM**
        *   A minor, non-functional change was introduced within the `getDashboardCardInfo` function. A `console.log` statement was updated to include `req.user` (`console.log(useWhere,"===useWhere",req.user);`), likely for debugging purposes, without altering the core logic or SQL queries.

2.  **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\export.query.ts`**
    *   **Timestamp: 11/20/2025, 7:21:07 PM**
        *   This file exports `getMonthValue` (a utility to convert month names to numbers) and `bookingQuery`.
        *   `bookingQuery` is a significant function that dynamically generates SQL query fragments for various "Booking Date to X" reports, where X can be "Actual Departure," "Estimated Departure," or "POD Arrival." It also supports "Stacked Bar" and "MBL Wise" variations.
        *   The generated queries typically join `mtr_tracking_data` with `agent_booking_entry`, filter by `CompanyName`, check for valid dates (not null or '0001-01-01 00:00:00 BC'), and calculate date differences.
        *   Conditional logic is present to include additional `WHERE` clauses based on `stackedExport`, `displayBy` (month, week, quarter), and `stackedGroup` (e.g., categorizing date differences into "Less than 1 week," "1 - 2 weeks"). The provided log snippet for this file is truncated.

3.  **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\analyticsExportService.ts`**
    *   **Timestamp: 11/20/2025, 7:23:44 PM**
        *   This file exports `getExportData`, the main function for handling analytics data exports.
        *   It processes various query parameters for filtering and display, including `fromDate`, `toDate`, `reportName`, `booking`, `displayBy`, `stackedMonth`, `startWeek`, `endWeek`, `quarter`, `stackedGroup`, and `columns`.
        *   It incorporates a `defaultDateRange` for missing date parameters and uses `getUserObject` to determine company-specific data access.
        *   Based on whether the report is `booking`-related (`isBooking`), it calls either `bookingQuery` (from `export.query.ts`) or `nonBookingQuery` (not detailed in the log).
        *   It dynamically constructs the final SQL query using `DISTINCT ON` clauses for MBL-wise or container-wise unique entries.
        *   Post-query processing includes `dateFilterOnExport` and `replaceColumnHandler`.
        *   A specific logic is implemented for "mblwise teu" reports to de-duplicate rows based on voyage, container number, and container type to ensure correct TEU aggregation.
        *   The `replaceColumnHandler` function explicitly maps "LONG BEACH" to "LOS ANGELES" for `pod` and `place_of_delivery` fields in the exported data.

### Patterns and Recurring Elements:

*   **Timestamp Proximity:** All changes occurred within a relatively short period (approximately an hour and a quarter) on the same day, 11/20/2025, suggesting a concentrated development effort or deployment.
*   **Data Source:** All three files interact extensively with `mtr_tracking_data` and `agent_booking_entry` tables, indicating a common underlying data model for shipment tracking and booking information.
*   **Filtering by Company and User:** A consistent pattern of applying filters based on `CompanyName` and user-specific conditions (`buildMtrUserCondition` in `dashboardService.ts`, `getUserObject` in `analyticsExportService.ts`) is observed across the services. This suggests a multi-tenant application with role-based access to data.
*   **Date-Based Queries:** Date filtering and calculations are central to all functionalities. `fromDate`, `toDate`, `departure_date`, `bookingdate`, `eta_pod`, and date difference calculations (`EXTRACT(DAY FROM ...)`) are frequently used. There's also a recurring check for invalid date values like `'0001-01-01 00:00:00 BC'`.
*   **TEU Calculation Logic:** TEU calculations based on `cont_type` or `t49_cont_type` appear in both dashboard metrics and analytics reports, highlighting the importance of container unit tracking.
*   **PostgreSQL Features:** The code leverages advanced PostgreSQL features such as `FILTER (WHERE ...)` for conditional aggregation, `DISTINCT ON (...)`, `ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ...)` for unique record selection, `COALESCE` for handling nulls, and `REGEXP_REPLACE` for data cleaning.
*   **Extensive Logging and Error Handling:** All service functions incorporate detailed logging (`logger.info`, `console.log`) for tracking execution and include robust `try...catch` blocks for error management, returning HTTP 500 status codes on failure.
*   **Modular Query Generation:** The `export.query.ts` file acts as a centralized module for generating complex SQL snippets, promoting reusability and maintainability of analytics queries.

## 10:33:12 PM
The code changes, all occurring on 11/20/2025, primarily focus on enhancing user experience, particularly around authentication/logout, and expanding reporting capabilities within the analytics section.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\usercard.tsx`**
    *   **11/20/2025, 6:35:28 PM**: The `handleLogout` function was activated, standardizing the logout process. It now consistently clears local storage items (`token`, `user`) and dispatches a Redux `auth/logout` action for both "auth0" and "normal" authentication types. For "auth0" users, these clearing actions now happen *before* redirecting to the external logout URL (`${link}/logout`).
    *   **11/20/2025, 7:56:52 PM**: A change was introduced to the Auth0 logout flow, modifying the redirection target from `/${link}/logout` to `/${link}/login`. This would redirect Auth0 users back to the login page after clearing their session.
    *   **11/20/2025, 8:10:05 PM**: The previous change was reverted, and the Auth0 logout redirection target was restored to `/${link}/logout`, indicating a reconsideration or correction of the post-logout user flow.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricFooter.tsx`**
    *   **11/20/2025, 7:27:55 PM**: Significant new functionality was added for PDF report generation.
        *   New imports like `PdfRenderer`, `html2canvas-pro`, and `createRoot` suggest a robust client-side PDF export mechanism.
        *   A `handlePdf` function was implemented to render a component (`PdfRenderer`) into a hidden DOM container, capture it as an image using `html2canvas-pro`, and then convert it into a PDF using `jspdf`.
        *   A `pdfLoading` state and `CircularProgress` indicator were added to provide visual feedback during the PDF generation process.
        *   An `IconButton` for PDF export was integrated into the metric footer alongside the existing Excel export and fullscreen options.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\methods.ts`**
    *   **11/20/2025, 7:32:19 PM**: The `downloadReportSpreadsheet` function, responsible for Excel exports, received substantial updates.
        *   It now dynamically structures the Excel report header and data based on the `reportName`, `chartType`, and `periodFilter` (month, week, quarter).
        *   Specific logic was added to handle "StackedBar" charts, particularly for "Cargo-Ready Date to ETD" reports, arranging data into appropriate columns (e.g., "Less than 1 week", "1 - 2 weeks").
        *   Calculations for "Average" and "Total" values are included conditionally.
    *   **11/20/2025, 7:39:29 PM**: A minor refinement was made in the `downloadReportSpreadsheet` function. The client name in the Excel report now converts `selectedCustomer` to lowercase if available, otherwise falling back to `user?.companyname`.

**Patterns and Recurring Elements:**

*   **Focused Development Session:** All changes occurred on the same date (11/20/2025), indicating a concentrated effort on these features.
*   **Reporting Enhancements:** A dominant theme is the expansion and refinement of reporting capabilities, with new PDF export functionality and detailed improvements to Excel spreadsheet generation.
*   **Logout Flow Management:** The `usercard.tsx` file shows recurring adjustments to the logout logic, specifically fine-tuning the Auth0 authentication path and post-logout redirection.
*   **State and Local Storage Usage:** Both user management and analytics components heavily rely on Redux for state management and `localStorage` for persistent user and authentication details.