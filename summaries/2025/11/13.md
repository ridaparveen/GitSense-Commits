...
## 9:10:25 PM
The changes primarily revolve around enhancing the user experience and data access control within a frontend application, specifically focusing on customer selection and content display.

**File-Specific Updates:**

*   **`src\components\core\layout.tsx`** (multiple updates between 11/13/2025, 3:15:38 PM and 4:38:13 PM):
    *   Initially set up basic layout with token expiration and Redux `selectedCustomer` initialization.
    *   A significant update at 11/13/2025, 4:31:01 PM introduced conditional rendering for the main `Outlet` based on `selectedCustomer`. If no customer is selected, a "Please select a customer" message is displayed instead of the application content.
    *   Subsequent minor updates refined the styling and placement of this "Please select a customer" message, including adding a `BsInfoCircle` icon.

*   **`src\components\core\sidebar.tsx`** (updated at 11/13/2025, 3:16:14 PM and 4:26:07 PM, then 4:33:08 PM):
    *   The sidebar initially included a `SelectCustomerPanel` for `ADMIN_V2` users and displayed the menu.
    *   At 11/13/2025, 4:26:07 PM, conditional rendering was added to display a "Please select a customer to view the menu" message for `ADMIN_V2` users if no customer was selected, effectively hiding the entire menu.
    *   This conditional menu hiding was then reverted at 11/13/2025, 4:33:08 PM, meaning the menu is always rendered, but its functionality is tied to `selectedCustomer` where relevant.

*   **`src\components\screen\analytics\AnalyticsToolbar.tsx`** (updated at 11/13/2025, 3:16:47 PM):
    *   This component manages analytics dashboard filters and includes a robust PDF export functionality using `jspdf` and `html2canvas-pro`, with provisions for custom headers and logo embedding. It demonstrates refinement in the PDF generation process through multiple commented-out attempts.

*   **`src\components\screen\booking\booking-dashboard.tsx`** (multiple updates between 11/13/2025, 3:17:47 PM and 5:53:52 PM):
    *   The dashboard displays booking summaries and a data grid with filtering, sorting, and pagination.
    *   Initially, the entire page content was conditional on `selectedCustomer`. This was removed at 11/13/2025, 4:27:00 PM, implying the `selectedCustomer` check moved to a higher-level component (e.g., `Layout`).
    *   The `CompanyName` parameter in API queries (`useFetchBookingDashboardDataQuery`) was adjusted from `selectedCustomer || localUser?.companyname` to `selectedCustomer || ""` (11/13/2025, 4:50:11 PM) and then to `selectedCustomer || undefined` (11/13/2025, 5:53:52 PM), indicating refinement in how the company name is passed when no customer is explicitly selected.

*   **`src\components\screen\planboard\PlanboardCardView.tsx`** (minor updates at 11/13/2025, 3:18:16 PM and 3:18:22 PM):
    *   Renders planboard data in year or month view, fetching data based on selected year and customer. Includes a loading indicator and "No Data Available" message.

*   **`src\config\menu.ts`** (updated at 11/13/2025, 3:18:43 PM):
    *   Defines the application's sidebar menu. Notably, the "Preferences" item is dynamically disabled for `ADMIN_V2` roles based on user data from local storage. A "Dev" section is conditionally included based on the `VITE_MODE` environment variable.

*   **`src\pages\dashboard\dashboard-page.tsx`** (multiple updates between 11/13/2025, 3:19:32 PM and 5:54:28 PM):
    *   Displays a comprehensive dashboard with shipment data, a calendar for deliveries/vessels, and recent updates.
    *   At 11/13/2025, 5:46:38 PM, the `CompanyName` parameter for `useFetchDeliveryDataQuery` was changed from `selectedCustomer || ""` to `selectedCustomer || undefined`, and the `localUser?.companyname` fallback was removed for `useFetchShipmentDataQuery` and `useFetchCartDataQuery`.

*   **`src\pages\planboard\planboard-page.tsx`** (updated at 11/13/2025, 3:20:12 PM and 5:42:25 PM):
    *   Provides UI for selecting planboard view (year/month) and year. Includes a tooltip explaining color coding.
    *   The top-level conditional rendering based on `selectedCustomer` was removed at 11/13/2025, 5:42:25 PM, similar to `booking-dashboard`.

*   **`src\pages\reports\report-page.tsx`** (updated at 11/13/2025, 3:20:39 PM and 5:41:46 PM):
    *   Manages scheduled and recent reports, allowing download of Excel files and filtering.
    *   Similar to other pages, the top-level conditional rendering based on `selectedCustomer` was removed at 11/13/2025, 5:41:46 PM.

*   **`src\pages\shipment\shipment-container-page.tsx`** (multiple updates between 11/13/2025, 3:20:56 PM and 5:49:14 PM):
    *   Displays container-specific shipment data with filters and pagination.
    *   The `CompanyName` parameter in `useFetchShipmentDataQuery` was updated from `selectedCustomer || ""` to `selectedCustomer || undefined` at 11/13/2025, 5:47:04 PM.

*   **`src\pages\shipment\shipment-page.tsx`** (updated at 11/13/2025, 3:21:09 PM and 5:48:09 PM):
    *   Shows overall shipment data with cards and a data grid. Defines status filters for air and ocean shipments.
    *   At 11/13/2025, 5:48:09 PM, the `CompanyName` parameter in `useFetchShipmentDataQuery` and `useFetchShipmentCardDataQuery` was changed from `selectedCustomer || localUser?.companyname` to `selectedCustomer || undefined`.

*   **`src\data\options\global.ts`** (minor updates at 11/13/2025, 3:23:14 PM and 3:23:24 PM):
    *   Consolidates a wide range of application-wide status, role, and filter options (e.g., `SHIPMENT_STATUS`, `BOOKING_SORT_OPTIONS`, `DATE_FILTER_OPTIONS`).

*   **`src\store\fretures\global-slice.ts`** (new file, created 11/13/2025, 5:05:40 PM):
    *   Introduced a new Redux slice to manage global state, specifically `selectedCustomer` and a general `loading` flag, with reducers to set/initialize these values and persist `selectedCustomer` in local storage.

*   **`src\pages\analytics\analytics-page.tsx`** (extensive updates between 11/13/2025, 5:01:14 PM and 5:35:37 PM):
    *   Manages analytics dashboards, fetching data for various metrics.
    *   Underwent significant refactoring regarding conditional rendering for `AnalyticsContainer` based on `user.role` and `selectedCustomer`, including displaying a "Please Select Any Customer" message for admins when no customer is chosen.
    *   Refined loading state management, shifting from an internal `useState` to using the global Redux `loading` state from `global-slice` (between 11/13/2025, 5:06:27 PM and 5:08:12 PM), and then back and forth between global and local `setLoading` in subsequent commits.
    *   Added `!selectedCustomer` to the `useEffect` guard clause at 11/13/2025, 5:33:03 PM, ensuring analytics reports are only fetched when a customer is selected.

*   **`src\pages\notifications\notification-page.tsx`** (multiple updates between 11/13/2025, 5:37:21 PM and 5:40:54 PM):
    *   Displays user notifications, including messages and updates, with a "View" button to navigate to shipment details.
    *   Fetches notification data based on `selectedCustomer || ""`.
    *   Introduced loading placeholders at 11/13/2025, 5:38:36 PM, and refactored notification list rendering into a separate function.

**Timestamps of Significant Changes:**

*   **11/13/2025, 4:31:01 PM:** Conditional content rendering based on `selectedCustomer` introduced in `layout.tsx`.
*   **11/13/2025, 4:26:07 PM:** Conditional sidebar menu rendering based on `selectedCustomer` for admins introduced in `sidebar.tsx`.
*   **11/13/2025, 4:50:11 PM - 5:53:52 PM:** Refinement of `CompanyName` parameter fallback logic in API calls across `booking-dashboard.tsx`, `dashboard-page.tsx`, `shipment-container-page.tsx`, and `shipment-page.tsx`.
*   **11/13/2025, 5:05:40 PM:** Creation of `global-slice.ts` for managing global `selectedCustomer` and `loading` states.
*   **11/13/2025, 5:06:27 PM - 5:19:33 PM:** Iterative changes in `analytics-page.tsx` to handle loading states and conditional rendering related to customer selection for admin users.
*   **11/13/2025, 5:33:03 PM:** Guard clause in `analytics-page.tsx` `useEffect` to prevent report fetching without a `selectedCustomer`.

**Patterns and Recurring Elements:**

1.  **Centralized Customer Selection:** The most prominent pattern is the application-wide adoption of a `selectedCustomer` state managed by Redux's `global-slice`. This state is used in almost every major page and component to filter data, enable/disable features, and display prompts. This suggests a multi-tenant or administrative view where an overarching customer context is required.
2.  **Conditional Content Rendering:** Components frequently check for the presence of `selectedCustomer` (and often `user.role` for `ADMIN_V2`) to determine what content to display. If no customer is selected, placeholder messages like "Please select a customer" are common.
3.  **API Query Parameter Consistency:** There is a recurring pattern of including `CompanyName: selectedCustomer || fallback` in API calls, with `fallback` values (`localUser?.companyname`, `undefined`, `""`) being experimented with and refined across several components.
4.  **Redux for State Management:** Redux is extensively used for managing filters, pagination, sort models, and global states like `selectedCustomer` and `loading`.
5.  **Reusable UI Components:** The application relies heavily on a set of common UI components (`PageWrapper`, `Card`, `AppGrid`, `Button`, `FilterSidebar`, `AutocompleteBox`, `DatePeriodSelector`, etc.) and a rich icon library.
6.  **Loading Indicators:** The use of `Loader` or `Spinner` components is common during data fetching, often managed by Redux `isLoading` or `loading` states.
7.  **Dynamic Menu/Feature Access:** `sidebar.tsx` and `config/menu.ts` demonstrate dynamic modification of the application's menu based on user roles and potentially other conditions (like `VITE_MODE`).
8.  **Local Storage Usage:** `localStorage` is consistently used for persisting user session information (`user`, `token`) and the `selectedCustomer` state.

## 10:39:17 PM
The code changes primarily focus on implementing and refining a global customer selection mechanism, impacting data fetching, UI rendering, and user experience across various parts of the application. Several files also show iterative development of loading states and PDF export functionality.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx`**
    *   **11/13/2025, 3:15:38 PM:** Established the core layout with authentication checks and initial customer selection logic.
    *   **11/13/2025, 4:31:01 PM - 4:38:13 PM:** Introduced significant conditional rendering: the main application content (`<Outlet />`) is now displayed only if a `selectedCustomer` is present in the Redux store. Otherwise, an informative "Please select a customer" message, progressively styled and enhanced with a `BsInfoCircle` icon, is rendered. This change saw multiple minor styling adjustments.
    *   **11/13/2025, 5:43:05 PM - 5:52:44 PM:** Minor refactorings, such as removing unused imports and re-applying the final styling for the "Please select a customer" message.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx`**
    *   **11/13/2025, 3:16:14 PM:** Initialized sidebar with dynamic menu and customer selection for `ADMIN_V2` users.
    *   **11/13/2025, 4:26:07 PM:** Added conditional display for the sidebar menu, showing a "Please select a customer..." message if the user is `ADMIN_V2` and no customer is selected.
    *   **11/13/2025, 4:33:08 PM:** Reverted the conditional rendering, causing the sidebar menu to be always visible irrespective of customer selection for `ADMIN_V2`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`**
    *   **11/13/2025, 3:16:47 PM:** Provides filtering and dashboard selection for analytics. It features a PDF export capability using `jspdf` and `html2canvas-pro`, with recent updates to robustly handle image conversion to base64 and error logging for PDF generation. (Note: The hardcoded API key in this file is not included in this summary.)

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\booking\booking-dashboard.tsx`**
    *   **11/13/2025, 3:17:47 PM:** Initially displayed a "Please select a customer..." message if `selectedCustomer` was not set, and API calls fell back to `localUser?.companyname`.
    *   **11/13/2025, 4:27:00 PM:** The top-level conditional rendering for customer selection was removed, making the booking dashboard content always visible.
    *   **11/13/2025, 4:50:11 PM:** The `CompanyName` parameter in `useFetchBookingDashboardDataQuery` was changed to `selectedCustomer || ""`, removing the fallback to `localUser?.companyname`.
    *   **11/13/2025, 5:53:52 PM:** The `CompanyName` parameter was reverted to `selectedCustomer || undefined`, re-introducing an optional `CompanyName` parameter if `selectedCustomer` is null.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\planboard\PlanboardCardView.tsx`**
    *   **11/13/2025, 3:18:16 PM:** Component for displaying planboard data in various views (year/month). No functional changes were observed after its initial entry.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\config\menu.ts`**
    *   **11/13/2025, 3:18:43 PM:** Defines the application's sidebar menu structure, including conditional inclusion of a "Dev" section based on an environment variable and disabling certain menu items for specific user roles.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\dashboard\dashboard-page.tsx`**
    *   **11/13/2025, 3:19:32 PM:** Main dashboard, fetching shipment and delivery data, using `selectedCustomer` with `localUser?.companyname` as a fallback in most API calls.
    *   **11/13/2025, 5:46:38 PM:** Removed `localUser` variable declaration. Modified the `CompanyName` parameter for `useFetchDeliveryDataQuery` to `selectedCustomer || ""`.
    *   **11/13/2025, 5:54:28 PM:** Reverted the `CompanyName` parameter for `useFetchDeliveryDataQuery` back to `selectedCustomer || undefined`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-container-page.tsx`**
    *   **11/13/2025, 3:20:56 PM:** Page for container-view shipments, initially used `selectedCustomer || localUser?.companyname` for `CompanyName` in API calls and had conditional rendering for customer selection.
    *   **11/13/2025, 5:47:04 PM:** Removed `localUser` declaration. Changed `CompanyName` in `useFetchShipmentDataQuery` to `selectedCustomer || ""`.
    *   **11/13/2025, 5:48:40 PM:** Reverted `CompanyName` in `useFetchShipmentDataQuery` back to `selectedCustomer || undefined`.
    *   **11/13/2025, 5:49:14 PM:** Finalized the removal of `localUser` fallback by changing `CompanyName` in `useFetchShipmentCardDataQuery` to `selectedCustomer || undefined`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-page.tsx`**
    *   **11/13/2025, 3:21:09 PM:** Displays shipments with various filters and status options. API calls used `selectedCustomer || localUser?.companyname`.
    *   **11/13/2025, 5:48:09 PM:** Removed `localUser` declaration. Updated both `useFetchShipmentDataQuery` and `useFetchShipmentCardDataQuery` to use `selectedCustomer || undefined` for `CompanyName`, eliminating the `localUser` fallback.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts`**
    *   **11/13/2025, 3:23:14 PM:** Defines a wide array of global constants, including various shipment and booking statuses, team member roles, and filter options for dates, periods, and inventory.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\planboard\planboard-page.tsx`**
    *   **11/13/2025, 3:20:12 PM:** Planboard page with initial conditional rendering for customer selection.
    *   **11/13/2025, 5:42:25 PM:** Removed the top-level conditional rendering for customer selection, making the planboard content always visible.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\reports\report-page.tsx`**
    *   **11/13/2025, 3:20:39 PM:** Reports page with functionalities for generating, downloading, and managing reports. `CompanyName` for queries used `selectedCustomer ?? undefined`. Initially included conditional rendering for customer selection.
    *   **11/13/2025, 5:41:46 PM:** Removed the top-level conditional rendering for customer selection, making report content always visible. Adjusted filter sidebar integration.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx`**
    *   **11/13/2025, 5:01:14 PM - 5:35:37 PM (series of changes):** This page underwent frequent updates, especially concerning conditional rendering for `ADMIN_V2` users. Multiple iterations were made to either display an `AnalyticsContainer` or a "Please Select Any Customer" message, based on the `selectedCustomer` state. The loading state management was shifted from a local state to a global Redux `loading` state (`global-slice.ts`). The `useEffect` hook for data fetching was enhanced with a guard condition to prevent API calls if a customer is not selected or other key parameters are missing. API calls consistently use `companyname: selectedCustomer`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts`**
    *   **11/13/2025, 5:05:40 PM:** A new Redux slice was introduced to manage global application state, including `selectedCustomer` (with persistence to `localStorage`) and a `loading` flag, along with corresponding actions (`setSelectedCustomer`, `initializeSelectedCustomer`, `setLoading`).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\notifications\notification-page.tsx`**
    *   **11/13/2025, 5:37:21 PM - 5:40:54 PM (series of changes):** Integrated the `selectedCustomer` into API calls for fetching notifications. Introduced a skeleton loading UI for better user feedback during data fetching. Improved the display for empty notification states and refined the logic to disable "View" buttons for notifications without a valid `mblno`.

**Patterns and Recurring Elements:**

1.  **Global Customer Selection (`selectedCustomer`):** A dominant pattern is the strong integration of a global `selectedCustomer` state, managed via Redux, across almost all data-intensive pages (`layout`, `sidebar`, `analytics`, `booking`, `planboard`, `reports`, `shipments`, `notifications`). This ensures that data displayed and fetched is always scoped to the active customer.
2.  **Conditional UI for Customer Selection:** Core layout and dashboard components (`layout.tsx`, `sidebar.tsx`, `analytics-page.tsx`) frequently implement conditional rendering to enforce customer selection, displaying "Please select a customer" messages when `selectedCustomer` is missing, especially for `ADMIN_V2` roles. This logic was subject to several iterations and refinements.
3.  **API Parameter Standardization:** There's a recurring pattern of passing `CompanyName: selectedCustomer || undefined` (or `selectedCustomer || ""`) to API queries. This indicates a move away from relying on `localUser?.companyname` as a fallback and centralizing customer context.
4.  **Loading State Management:** Consistent use of `isLoading`, `isFetching` from RTK Query hooks, complemented by a global `loading` state from `global-slice.ts`, is observed. This results in the display of various loading indicators (spinners, loaders, animated skeletons) throughout the application.
5.  **Iterative UI/UX Improvements:** Multiple timestamp entries for the same file, especially `layout.tsx` and `analytics-page.tsx`, show minor stylistic adjustments and refinements to conditional rendering logic and messages. This highlights an iterative development process focused on improving the application's visual presentation and user experience, particularly for empty data states or when a customer selection is required.

## 11:39:01 PM
The log indicates a series of frontend code changes focused on data fetching, state management, and user interface responsiveness across various application sections. A central theme is the handling of `selectedCustomer` state, likely for multi-tenancy or customer-specific data views.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\layout.tsx` (Latest Timestamp: 11/13/2025, 5:52:44 PM)**
    The `Layout` component, responsible for the overall app structure and authentication, has been significantly updated to conditionally render its child routes (`<Outlet />`). If no `selectedCustomer` is present in the Redux global state (fetched from `localStorage` on initialization or set by user interaction), a message "Please select a customer" is displayed, often with a `BsInfoCircle` icon, instead of the main content. Minor styling adjustments were iteratively applied to this placeholder message. The token expiration check and customer initialization logic remain consistent.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx` (Latest Timestamp: 11/13/2025, 4:33:08 PM)**
    The sidebar component initially included conditional rendering for an `SelectCustomerPanel` if the user role is "ADMIN_V2". A temporary change at `4:26:07 PM` introduced conditional rendering to hide the entire sidebar menu (except the logo and user card) if the user is `ADMIN_V2` and no `selectedCustomer` is chosen, displaying a placeholder message. This specific conditional rendering for the whole menu was then reverted by `4:33:08 PM`, restoring the full menu visibility, while the `SelectCustomerPanel` remains conditional for `ADMIN_V2` users.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx` (Timestamp: 11/13/2025, 3:16:47 PM)**
    This file underwent substantial changes related to PDF export functionality. It imports `jspdf` and `html2canvas-pro` to generate PDF reports of the dashboard. Multiple commented-out sections indicate development iterations, with the final version refining the image rendering (converting a logo to base64) and centering logic for the PDF output.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\booking\booking-dashboard.tsx` (Latest Timestamp: 11/13/2025, 5:53:52 PM)**
    The booking dashboard consistently fetches data based on filters and pagination, using `selectedCustomer` (or `undefined` if none is selected) for API calls. Unlike some other pages, the UI structure for the dashboard is always rendered, even if `selectedCustomer` is not set; the data displayed will simply be empty or default in such cases. Minor adjustments were made to the `CompanyName` parameter in `useFetchBookingDashboardDataQuery`, alternating between `selectedCustomer || ""`, `selectedCustomer || localUser?.companyname`, and `selectedCustomer || undefined`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\planboard\PlanboardCardView.tsx` (Latest Timestamp: 11/13/2025, 3:18:22 PM)**
    This component is responsible for displaying planboard data in year or month view. It fetches data using `selectedYear` and `CompanyName` derived from `selectedCustomer`. It includes a fallback `NoDataAvailable` component.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\config\menu.ts` (Timestamp: 11/13/2025, 3:18:43 PM)**
    The sidebar menu configuration was updated to introduce a `disabled` property for `SidebarItemType`, which is applied to the "Preferences" item for "ADMIN_V2" users. Additionally, a "Dev" section with "Components" link is conditionally included based on the `VITE_MODE` environment variable.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\dashboard\dashboard-page.tsx` (Latest Timestamp: 11/13/2025, 5:54:28 PM)**
    The main dashboard page fetches shipment, delivery, and cart data. The `CompanyName` parameter for `useFetchDeliveryDataQuery` was changed from `selectedCustomer || localUser?.companyname` to `selectedCustomer || undefined`. Other data fetching queries (`useFetchShipmentDataQuery`, `useFetchCartDataQuery`) continue to use `selectedCustomer || localUser?.companyname`. The component includes complex logic for calendar data preparation and various filter/pagination handlers.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\planboard\planboard-page.tsx` (Latest Timestamp: 11/13/2025, 5:42:25 PM)**
    Similar to the booking dashboard, the planboard page now unconditionally renders its `PageWrapper`. It uses `selectedCustomer` for data queries. An informative tooltip with color-coded legends for 'Past Week', 'Current Week', and 'Future Week' was styled and included.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\reports\report-page.tsx` (Latest Timestamp: 11/13/2025, 5:41:46 PM)**
    The reports page also removed the top-level conditional rendering based on `selectedCustomer`. Data queries for report history (`useGetReportHistoryQuery`) and saved reports (`useGetSavedReportsQuery`) explicitly use `selectedCustomer ?? undefined` for the `CompanyName` parameter. Functionality includes downloading individual or multiple reports as Excel files, and displays scheduled report statuses.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-container-page.tsx` (Latest Timestamp: 11/13/2025, 5:49:14 PM)**
    This page displays shipments in a container-specific view. The `CompanyName` parameter for all data fetching (`useFetchShipmentDataQuery`, `useFetchShipmentCardDataQuery`) has been standardized to `selectedCustomer || undefined`. The main content remains conditionally rendered based on `selectedCustomer`, displaying a "Please select a customer to view container data." message if not set.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-page.tsx` (Latest Timestamp: 11/13/2025, 5:48:09 PM)**
    The general shipments page now consistently uses `selectedCustomer || undefined` for the `CompanyName` parameter in `useFetchShipmentDataQuery` and `useFetchShipmentCardDataQuery`. It defines `AIR_SHIPMENT_STATUS_FILTERS` and `TRANSPORT_MODE_OPTIONS` and includes a `DatePeriodSelector` for filtering data.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts` (Latest Timestamp: 11/13/2025, 3:23:24 PM)**
    This file defines numerous static data options for shipment statuses (ocean and air), report statuses, team member roles and statuses, booking statuses, shipment scopes, and various filter options (date, period, view by, inventory). No functional code changes, only data declarations.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\global-slice.ts` (Timestamp: 11/13/2025, 5:05:40 PM)**
    A new `loading` state and `setLoading` reducer were added to the global Redux slice, allowing global control over loading indicators.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx` (Latest Timestamp: 11/13/2025, 5:35:37 PM)**
    This page underwent significant refactoring related to loading state management and customer selection.
    *   The local `isLoading` state was replaced with the global `loading` state from `global-slice`, with `dispatch(setLoading(true))` and `dispatch(setLoading(false))` calls wrapping API fetches.
    *   The `useEffect` hook for fetching reports and endpoints was updated with a guard clause `if (!user?.userid || !mainState.selectedDashobard || !selectedCustomer) return;` at `5:33:03 PM`, ensuring API calls only fire when a customer is selected. This guard was temporarily removed and re-added.
    *   Conditional rendering for the `AnalyticsContainer` was refined: for "ADMIN_V2" users, it now explicitly checks if a `selectedCustomer` is present; if not, it displays a "Please select any customer" message. If a customer is selected, it then checks the global `loading` state to display a `Loader` or the `AnalyticsContainer`.
    *   The `companyname` parameter in analytics API calls (`fetchReport`, `fetchEndpoints`) consistently uses `selectedCustomer`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\notifications\notification-page.tsx` (Latest Timestamp: 11/13/2025, 5:54:48 PM)**
    The notification page fetches data using `CompanyName: selectedCustomer || ""`. It displays a list of notifications or a message indicating no new notifications, with loading placeholders shown during data fetching. Minor type adjustments and console logs were observed throughout its change history. The ability to navigate to shipment details based on a notification's `mblno` is a core feature.

**Patterns and Recurring Elements:**

1.  **Centralized Customer Selection:** The `selectedCustomer` state from a Redux `global-slice` is a critical dependency across almost all data-fetching components (`CompanyName` parameter in RTK Query hooks).
2.  **Conditional Content Display:** A prominent pattern is the introduction or refinement of conditional rendering logic to display a "Please select a customer" message in various dashboards (Layout, Analytics, Shipments, Containers, Reports, Planboard) when the `selectedCustomer` is not set, particularly for `ADMIN_V2` roles. For some pages (Booking, Planboard, Reports), this conditional wrapper around the entire `PageWrapper` was later removed, implying that the page structure is always rendered, but data loading still depends on the customer selection.
3.  **Loading State Integration:** There's a consistent effort to integrate loading feedback (`isLoading`, `isFetching` from RTK Query, and the new global `loading` state) into the UI, often through placeholders (`animate-pulse`) or dedicated `Loader`/`Spinner` components.
4.  **API Parameter Standardization:** There's a subtle but recurring adjustment in how the `CompanyName` parameter is passed to API calls when `selectedCustomer` might be null or undefined. It evolves from `selectedCustomer || localUser?.companyname` to `selectedCustomer || ""` or `selectedCustomer || undefined`, which impacts how API requests are formed when no customer is explicitly chosen.
5.  **UI Component Usage:** Frequent imports and usage of common UI components like `PageWrapper`, `Card`, `AppGrid`, `FilterSidebar`, `Button`, and various icons from `react-icons` and `@heroicons/react`.
6.  **Redux Integration:** All summarized files heavily rely on Redux (`useSelector`, `useDispatch`) for state management and RTK Query (`useFetch...Query`, `useLazyFetch...Query`) for data fetching.