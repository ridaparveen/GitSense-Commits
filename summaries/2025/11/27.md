...
## 3:39:16 PM
The recent code changes primarily focus on developing and refining the House Bill of Lading (HBL) functionality within the T360 application's frontend.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\store\api\agentBookingApi.js` (Multiple timestamps from 11/26/2025, 2:53:21 PM to 11/27/2025, 2:41:28 PM):**
    *   **Initial Setup:** Introduces `FormDetails` tag type and new query/mutation endpoints for `fetchBookingFormDetails`, `updateCarrierBooking`, `updateDo`, `exportSprReport`, and `getHblDashboard`.
    *   **HBL API Integration:** Adds `saveHblBooking` and `fetchHblBookingFormDetails` endpoints. The `saveHblBooking` endpoint's URL was initially `/hblEntry` then reverted to `/hblEntry/create`, indicating a split between create and update operations.
    *   **HBL Update Capability:** A new `updateHblBooking` mutation (using `/hblEntry/update`) was added and subsequently exported, enabling modification of existing HBL records.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLListDetails.jsx` (Multiple timestamps from 11/26/2025, 2:57:40 PM to 11/27/2025, 2:56:44 PM):**
    *   **HBL Dashboard Integration:** The HBL list component (`HBLListDetails`) was updated to fetch HBL data dynamically using `useGetHblDashboardQuery`, moving from dummy data to actual API-driven content.
    *   **Dynamic Filtering:** The `useGetHblDashboardQuery` now accepts `agent_booking_id` from the location state, allowing the dashboard to display HBLs related to a specific booking.
    *   **Navigation for HBL Creation:** The "Add HBL" button's navigation logic was updated to pass `booking_serial_id` and `agent_booking_id` to the HBL form.
    *   **Grid Configuration:** The `ThemedGrid` component was configured with `HBL_COLUMNS` and `GridActions` using `getHblActions` for interactive rows. The `uniqueId` was set to `serial_id`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\data\columns\agent.booking.js` (Multiple timestamps on 11/26/2025 and 11/27/2025):**
    *   **Extensive Column Definitions:** This file defines various column configurations (`BOOKING_VESSEL_COLUMN`, `BOOKING_CARGO_COLUMN`, `BOOKING_DASHBOARD_COLUMNS_GRID`, `HBL_COLUMNS` implicitly used by `HBLListDetails.jsx`) with consistent styling, date formatting, and conditional rendering (e.g., status chips). Minor save operations without functional changes were observed.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\actions.js` (Multiple timestamps from 11/26/2025, 3:13:47 PM to 11/26/2025, 5:35:42 PM):**
    *   **Role-Based Actions:** The `getBookingActions` function was refactored to dynamically provide booking-related actions (edit, copy, view, update line booking, update DO, create HBL) based on user roles (`agentBookingMenu`, `customerBookingMenu`, `adminBookingMenu`).
    *   **HBL-Specific Actions:** A new `getHblActions` function was introduced, initially offering an "Edit HBL" option. The parameters for editing HBLs evolved to consistently use `serial_id` and `agent_booking_id` in the navigation state.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLform.jsx` (Multiple timestamps from 11/26/2025, 5:55:57 PM to 11/27/2025, 3:35:15 PM):**
    *   **HBL Form Development:** This new component provides the user interface for creating and editing HBLs, using `Formik` for form management and a `HBLvalidationSchema`.
    *   **Data Handling:** The `handleSave` function processes form data, extracts values from concatenated strings, incorporates Redux state (for routing, containers, POs), and conditionally calls `saveHblBooking` (for new HBLs) or `updateHblBooking` (for existing ones based on `formAction` and `serial_id`).
    *   **Navigation after Save:** Upon successful save, the form navigates to `/app/booking/hbl_form_screen` (previously `/app/booking`), passing the relevant `agent_booking_id` and setting `formAction` to "edit".
    *   **Error Handling:** Includes client-side validation and server-side error display via `AlertConfig`.
    *   **UI Structure:** Organizes the form into `HblHeaderForms` and `ThemeTabs` for various HBL details (Parties, Containers, Routing, Basic POs).
    *   **Keyboard Shortcuts:** Implemented shortcuts for common party management actions and saving the form.
    *   **Payload Refinements:** Iterative changes were made to correctly include `serial_id`, `booking_serial_id`, and `agent_booking_id` in the payload for `saveBooking` and `updateHblBooking`, and to ensure `saveBooking` receives the payload correctly for new entries. Default date field values (`hbldate`, `bookingdate`) fluctuated between `null` and `""`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\booking\HblEntry.jsx` (Multiple timestamps from 11/27/2025, 12:05:38 PM to 11/27/2025, 12:07:36 PM):**
    *   **HBL Page Integration:** This component serves as the entry point for HBL forms, using `useFetchHblBookingFormDetailsQuery` to populate initial values.
    *   **Redux List Initialization:** On successful data fetch, it dispatches actions (`addList`, `removeList`) to initialize Redux store slices (`containerList`, `routingList`) with transformed data from the API response.
    *   **Form Action Logic:** Determines if the form is in "copy" or "edit" mode based on location state.
    *   **Cleanup:** Includes a `useEffect` hook to reset Redux state on component unmount.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HblEntryFormScreen.jsx` (Multiple timestamps from 11/27/2025, 12:07:51 PM to 11/27/2025, 3:20:00 PM):**
    *   **Wrapper Component:** This component wraps `HBLForm` and manages higher-level state such as modals for party management (`HblPartyScreen`, `PartyFormScreen`) and error displays.
    *   **Dynamic Data Fetching:** Refined the `useFetchHblBookingFormDetailsQuery` call to fetch HBL details using `serial_id` from the location state, and only when in "edit" mode, preventing unnecessary data fetches.
    *   **List Initialization:** Similar to `HblEntry.jsx`, it initializes Redux lists for containers, routing, and POs.
    *   **Party Management:** Provides UI for exporting/importing parties and toggling between viewing party lists and adding/editing individual parties within a modal.

**Patterns and Recurring Elements:**

*   **HBL-Centric Development:** A strong and consistent focus on building out the HBL feature across API, UI, and state management.
*   **Redux Toolkit Query (RTK Query):** Widespread adoption for data fetching and mutation, indicated by `createApi`, `fetchBaseQuery`, `builder.query`, `builder.mutation`, and exported `use...Query`/`use...Mutation` hooks. `tagTypes` are used for cache invalidation.
*   **Formik for Forms:** All detailed forms, particularly `HBLform.jsx`, rely on `Formik` for state management, validation, and submission.
*   **MUI Components:** Extensive use of Material-UI (`@mui/material`) components for consistent UI design (Box, Card, Stack, Typography, Buttons, Tabs).
*   **Modular Architecture:** Code is organized into API slices (`agentBookingApi.js`), reusable UI components, and separate screen components for better maintainability.
*   **`useLocation` and `useNavigate`:** Frequently used for navigating between screens and passing state, especially `formAction` (add, edit, copy) and record identifiers (`agent_booking_id`, `booking_serial_id`, `serial_id`).
*   **Data Transformation:** Consistent patterns for parsing concatenated strings (e.g., `value.split("~")[i]`) and mapping API data structures to local component/Redux state.
*   **Iterative Refinement and Debugging:** Several commits demonstrate small, successive changes to fix bugs, improve logic (e.g., `agent_booking_id` fallback, `saveBooking` payload), or adjust UI behavior (e.g., date defaults, console logs), suggesting active and agile development.