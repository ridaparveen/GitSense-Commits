...
## 6:58:53 PM
The provided log details several code changes across different services, routes, and utility files, primarily focusing on data retrieval, document handling, and API endpoint definitions. The timestamps indicate a rapid development phase on November 7, 2025.

### Key File-Specific Updates:

1.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\docsParseService.ts` (Timestamp: 11/7/2025, 5:03:25 PM)**
    *   This file contains an `getAllPdfParse` function that is, in fact, responsible for retrieving **vendor details**.
    *   It implements comprehensive vendor listing with pagination, sorting (`created_at` by default), and filtering by `vendor_name`, `email`, and `status`.
    *   Crucially, it fetches associated `document_upload` records (where `ref_type = 'vendor_doc'`) for each vendor and groups them, then attaches them to the respective vendor objects.
    *   It also dynamically generates distinct `status` and `email` options from the vendor data, cleaning and formatting them (e.g., "Active" / "Inactive").
    *   The naming `docsParseService` for a vendor-centric function suggests either a misnomer, an early stage of development, or a file that was later refactored/moved.

2.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\pdfParseService.ts` (Significant changes between 5:06:48 PM and 6:40:55 PM)**
    *   This file, correctly named for PDF parsing, handles the `getAllPdfParse` function to retrieve data from the `AI_Parse_Document` table.
    *   **Initial implementation (5:06:48 PM - 5:07:04 PM):** Used `QueryBuilder` for pagination and sorting (`process_date_time` by default) and returned all columns. A minor update at 5:06:59 PM added `count` to the response.
    *   **Attempt at URL generation (6:15:41 PM):** Modified the `QueryBuilder` to use `CONCAT` with `/uploads/` and `process.env.UPLOAD_DOC_PATH` to generate full URL paths for `pdf_pa` and `json_pa` columns.
    *   **Reversion (6:25:14 PM):** The URL generation logic was reverted, returning to `setColumns(["*"])`.
    *   **Shift to direct SQL (6:29:11 PM):** Abandoned `QueryBuilder` for this specific function in favor of a direct SQL query. This query explicitly `CONCAT`s `/envosys_uploads/` with `pdf_pa` and `json_pa` for URL construction, suggesting a change in how static assets are served or referenced.
    *   **Further URL/Query adjustments (6:40:33 PM - 6:40:55 PM):** The direct SQL query was updated to hardcode `http://localhost:8080/uploads/` for generating file URLs. The `ORDER BY` clause was fixed to `process_date_time DESC`, and `LIMIT`/`OFFSET` were changed to string interpolation. An initial change from `ser_no` to `id` was quickly reverted back to `ser_no` in the final change.
    *   Error logging consistently uses `logger.error` in later versions.

3.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\constants\Paths.ts` (Changes between 5:07:36 PM and 5:15:03 PM)**
    *   This file defines all API endpoints.
    *   **Key changes:**
        *   Added a new base path for `Notes` (`/notes`) with `GetAll` and `Add` endpoints.
        *   Renamed the `Document.GetPdf` path from `/get-pdf` to `/get-pdf-parse`, indicating an API endpoint change related to PDF parsing data.

4.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\documentUploadRoutes.ts` (Timestamp: 11/7/2025, 5:08:07 PM)**
    *   Defines routes for document management.
    *   It uses `uploadMiddleware` for file handling.
    *   A significant point is that it maps `Paths.Document.GetPdf` (which was `/get-pdf` at this timestamp) to the `getAllPdfParse` service function. This means that after the `Paths.ts` file changed the name to `/get-pdf-parse`, this route file would become inconsistent if not updated, potentially leaving an old `/get-pdf` endpoint active or breaking the intended `/get-pdf-parse` route.

5.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\linebookingService.ts` (Changes between 5:14:34 PM and 5:14:48 PM)**
    *   The `create` function for line bookings implements a transaction-based approach to add new line bookings.
    *   It inserts data into `line_booking_master`, `line_booking_equipments`, and handles `other_services_master` (deleting old entries before inserting new ones).
    *   The `status` of a line booking is determined by the presence of `do_number` and party information.
    *   A large `getAll` function is commented out, suggesting it's either under active development, paused, or a temporary placeholder. This commented code reveals advanced filtering capabilities based on status, dates, ports, line names, and equipment details.

6.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\util\optionQuery.ts` (Timestamp: 11/7/2025, 5:14:43 PM)**
    *   This utility file provides helper functions for building SQL queries, specifically for autocomplete and option selection.
    *   It includes `buildOptionQuery` and `buildQueryOptions` for generic query construction.
    *   The `OptionQuery` class offers a range of specific autocomplete queries for various entities like `Port`, `GstLocation`, `MentionUsers`, `VesselVoyage`, `CustomerName`, `LineName`, `Vendor`, `Vessel`, and `Party` using case-insensitive `ILIKE` searches.

7.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\vendorServices.ts` (Timestamp: 11/7/2025, 5:59:15 PM)**
    *   This file provides the `getAll` and `addVendor` functions, with `updateVendor` partially defined.
    *   The `getAll` function is functionally very similar to the `getAllPdfParse` in `docsParseService.ts` but correctly placed within `vendorServices.ts`, implying a potential consolidation or refactor. It includes logic to fetch and attach `document_upload` records. A commented-out section indicates future work on attaching `notes` to vendors.
    *   The `addVendor` function handles vendor creation, including:
        *   Input validation for `vendor_name` and `status`.
        *   Inserting the new vendor into the `vendor` table.
        *   Triggering an audit log entry via `simpleAudit`.
        *   Processing and saving uploaded documents using `saveDoc`.
        *   Processing and saving `notes` associated with the vendor, with a redundant nested `if (notes)` block. It ensures new notes are filtered and inserted with `new Date()` for timestamps.

8.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\index.ts` (Timestamp: 11/7/2025, 6:34:36 PM)**
    *   The main API router, `apiRouter`, aggregates all sub-routers.
    *   It now includes `NotesRoutes`.
    *   A notable entry: `apiRouter.use(Paths.Audit.Base, getAllAudit);` directly maps the `/audit` base path to a service function (`getAllAudit`), which is an unusual pattern for Express router setup and might be an error or specific design choice.

9.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\server.ts` (Timestamp: 11/7/2025, 6:34:51 PM)**
    *   Configures the Express application.
    *   Sets up a static file server for `/uploads` pointing to `/root/node/envosys/envosys_bucket/ai/upload`, which directly supports the file URL construction seen in `pdfParseService.ts`.
    *   Configures `express.json` and `express.urlencoded` with a 10MB limit.
    *   Uses CORS with `origin: "*"` and `morgan("combined")` for request logging.
    *   Establishes PostgreSQL connection and logs its status.
    *   Applies `authenticate` middleware to most `/api` routes.

### Patterns and Recurring Elements:

*   **Database Interaction:** Extensive use of `query` and `insertQuery` helper functions for all database operations. Transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) are used for multi-step operations (e.g., `linebookingService.create`).
*   **Query Building:** The `QueryBuilder` utility class is frequently used across services (e.g., `docsParseService`, `pdfParseService` initially, `vendorServices`) for constructing dynamic SQL queries with pagination, sorting, and filtering. However, `pdfParseService.ts` later switched to direct SQL for more control over `CONCAT` operations.
*   **Pagination and Sorting:** A consistent pattern of extracting `page`, `perPage`, `orderBy` from `req.query`, calculating `offset`, and applying sorting logic is present in many `getAll` functions.
*   **Error Handling:** All service functions consistently wrap their logic in `try...catch` blocks, using `logger.info` or `logger.error` for logging and returning `HttpStatusCodes.INTERNAL_SERVER_ERROR` in case of failure.
*   **API Path Management:** The `Paths.ts` constant file is the central place for defining API endpoints, facilitating consistency and future modifications, though a discrepancy was noted between `Paths.ts` and `documentUploadRoutes.ts` regarding the `/get-pdf` endpoint.
*   **Document and Notes Integration:** There's a recurring need to associate `document_upload` records and `notes` with primary entities (like vendors, line bookings). This is evidenced by fetching/attaching documents in `vendorServices.ts` and explicit handling of `notes` insertion in `addVendor`. The presence of commented-out notes fetching in `vendorServices.ts` suggests this is an ongoing feature.
*   **URL Construction for Files:** There is an evident struggle or iteration in `pdfParseService.ts` to correctly construct full URLs for static files (`pdf_pa`, `json_pa`), moving from dynamic `process.env` usage to hardcoded server paths, indicating refinement in how these assets are served and accessed by the frontend.

## 7:16:12 PM
The changes log details a series of updates primarily focused on enhancing document upload, download, and display functionalities within the `envosys-frontend` application, all occurring on November 7, 2025.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\store\api\uploadDocumentApi.ts` (Timestamp: 11/7/2025, 5:20:16 PM)**
    *   This file establishes the core API interaction layer using `@reduxjs/toolkit/query/react`.
    *   It defines three main endpoints:
        *   `uploadDocument`: A mutation for `POST` requests to `/api/documents/upload`, handling `FormData` with authorization headers, and invalidating the `doc` tag upon success.
        *   `downloadDocument`: A mutation for `GET` requests to `/api/documents/download/{refType}/{documentName}`, designed to return a `Blob` and transform the response to include the file's blob and name. It uses `cache: 'no-cache'`.
        *   `getPdfParseData`: A query for `GET` requests to `/api/documents/get-pdf-parse` with URL parameters, providing the `doc` tag.
    *   This initial update lays the groundwork for all document-related API calls.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\components\screen\upload-docs\uploadDocs.tsx`**
    *   **Timestamp: 11/7/2025, 5:21:15 PM (and subsequent minor changes at 5:23:29 PM, 5:24:08 PM, 6:00:45 PM)**
        *   The component `ConfirmLineBookingForm` is introduced, providing a user interface for uploading documents.
        *   It features drag-and-drop functionality, a file input, and displays a list of currently uploaded files with a remove option.
        *   Uses `formik` for form management and integrates the `useUploadDocumentMutation` and `useGetPdfParseDataQuery` from the `uploadDocumentApi`.
        *   The upload logic iterates through selected files, constructs `FormData` with fields like `document_type`, `ref_type`, `remark`, `file`, `ref_id`, and `uploaded_by`, then calls the `uploadDocument` mutation.
        *   Initially, the "Uploaded Documents Table Data" section uses `tableDummyData`.
        *   Minor changes like `console.log` additions or temporary text changes (`{file.name}kk` at 6:00:45 PM, immediately reverted) occurred without significant functional impact in early commits.
    *   **Timestamp: 11/7/2025, 6:01:08 PM**
        *   A significant update was made to the `AppGrid` component within this file. The `data` prop was switched from `tableDummyData` to `getPdfParseData?.data || []`, and the `count` prop from `tableDummyData.length` to `getPdfParseData?.total || 0`. Additionally, the `uniqueId` prop was changed from `"id"` to `"ser_no"`.
        *   This change indicates the transition from displaying static, dummy data to dynamically fetching and rendering actual parsed PDF data from the backend.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\data\coulms\upload-doc-coulmn.tsx`**
    *   **Timestamp: 11/7/2025, 5:23:08 PM (Initial state)**
        *   Defines `UPLOAD_TAB_COLUMNS` for the `AppGrid`.
        *   The `pdf_pa` column renders a direct download link for files.
        *   The `json_pa` column displays a summarized string of JSON data and allows downloading the *in-cell* JSON content as a file.
        *   A `process_date_time` column displays date and time.
    *   **Timestamp: 11/7/2025, 5:28:58 PM**
        *   Added a `console.log("JSON Data:", data);` within the `json_pa` column's `renderCell` for debugging.
    *   **Timestamp: 11/7/2025, 5:32:50 PM**
        *   The `json_pa` column was heavily refactored. Instead of displaying and downloading in-cell JSON, it was modified to fetch JSON content from a `fileUrl` (derived from `params.row.json_pa`, with logic to handle both full URLs and relative paths) and then download it. The displayed content changed from a data summary to just the `fileName`.
    *   **Timestamp: 11/7/2025, 5:33:56 PM**
        *   The refactor to fetch JSON from a URL in the `json_pa` column was reverted, returning it to its previous state (displaying summarized JSON data and downloading it directly from the cell's `data` prop). The debugging `console.log` remained.
    *   **Timestamp: 11/7/2025, 6:05:35 PM**
        *   The `pdf_pa` column's `renderCell` was refactored to include more sophisticated download logic. It now checks the `fileName` extension: if it's a `.json` file, it fetches the JSON content and downloads it; otherwise, it opens the file URL in a new tab for download. The `fileName` is converted to lowercase for comparison.
        *   The `json_pa` column remained in its reverted state (displaying summarized data).
    *   **Timestamp: 11/7/2025, 6:30:30 PM**
        *   The `json_pa` column was again significantly updated. It now directly fetches JSON data from the `fileUrl` (provided by `params.row.json_pa`) and downloads it, similar to the logic implemented at 5:32:50 PM, but without the intelligent URL path construction. The displayed text is the `fileName`.

**Patterns and Recurring Elements:**

*   **Focus on Document Management:** The entire set of changes is dedicated to improving the functionality for uploading, parsing, and presenting document data.
*   **RTK Query for Data Handling:** The consistent use of `@reduxjs/toolkit/query/react` in `uploadDocumentApi.ts` shows a standardized approach to API interactions.
*   **Dynamic Data Presentation:** The `uploadDocs.tsx` component evolved from using static dummy data to dynamically fetching and displaying actual parsed document data via `getPdfParseDataQuery` in its data grid.
*   **Client-Side File Downloading:** Both `pdf_pa` and `json_pa` columns in `upload-doc-coulmn.tsx` implement client-side file download mechanisms using `Blob`, `URL.createObjectURL`, and dynamically created `<a>` tags.
*   **Evolving JSON Handling:** There's a clear pattern of refining how JSON data is handled. Initially, it was embedded and summarized; then, attempts were made to fetch it from a URL. This eventually settled on both `pdf_pa` and `json_pa` potentially pointing to external file URLs (with `json_pa` specifically for fetching and downloading JSON content).
*   **UI Components:** `Card`, `Button`, `AppGrid` are consistently used Tailwind CSS-based UI components.
*   **Form Management:** `Formik` is used for form handling in the upload component.
*   **Unified Development Session:** All timestamps fall within a concentrated period on November 7, 2025, indicating a focused development effort on this specific feature.

## 7:17:24 PM
The provided log details changes across several frontend files within a `T360-V2` project, primarily focusing on booking-related features, particularly the management of vessel and purchase order details, and booking approval flows. The timestamps cluster around **November 7, 2025**, indicating a concentrated development effort on this date.

**File-specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\coulmns\booking-coulmns.tsx`**
    *   **(11/7/2025, 11:11:33 AM - 11:14:31 AM):** Initial changes focused on `BOOKING_VESSEL_COLUMN_CUSTOMER`. The `renderCell` logic for editable fields (`mother_feeder`, `vessel`, `voyage`, `line`, `etd`, `eta`, `cy_cutoff`, `docs_cutoff`) was updated to conditionally render `InputBox` components or standard input elements when a row is being edited (`editingRowId === params.id`). The `action` column's `headerName` was initially dynamic with an "Add Vessel" icon, and its `renderCell` toggled between a "Save" button and "Edit" / "Delete" buttons.
    *   **(11/7/2025, 11:16:09 AM):** The text for the "Save" button in the `action` column of `BOOKING_VESSEL_COLUMN_CUSTOMER` was changed from "Save" to "Update".
    *   **(11/7/2025, 11:21:16 AM):** A significant refactor introduced a `bookingStatus` parameter to `BOOKING_VESSEL_COLUMN_CUSTOMER`. This parameter is used to determine if the table should be `isLocked` (if `bookingStatus` is "Approved" or "Cancelled"). All editable input fields and action buttons within the vessel table now respect this `isLocked` state, being disabled or hidden accordingly. The "Add Vessel" icon's appearance and clickability also depend on `isLocked`.
    *   **(11/7/2025, 11:25:54 AM):** The "Add Vessel" icon's conditional rendering in the `action` column header was inverted; it is now `hidden` when `isLocked` is true and `block` when false, ensuring it disappears when the booking is locked.
    *   **(11/7/2025, 11:32:04 AM - 1:03:08 PM):** Debugging changes were observed, with `console.log` statements added to the `renderCell` function of the `action` column in `BOOKING_VESSEL_COLUMN_CUSTOMER`, specifically logging `editingRowId` and `params`. These were likely for understanding the current editing state.
    *   **(11/7/2025, 1:19:06 PM):** The `action` column's `renderCell` logic was further refined to differentiate between "Save" and "Update" button labels based on whether the row is a `isNewRow` (lacking `serial_id`). An `onRowEdit` callback was also implicitly added or clarified for the edit button functionality.
    *   **(11/7/2025, 1:28:37 PM - 1:39:29 PM):** Refinements were made to the `BOOKING_VESSEL_COLUMN_CUSTOMER` function signature, renaming `onEdit` to `onFieldEdit` (for inline field editing) and introducing a new `onRowEdit` parameter (for activating edit mode on a row via an icon). The conditional rendering for the "Add Vessel" button in the `action` column's header was adjusted to hide it when `isLocked`. CSS class `my-2` was added to input boxes within the render cells, suggesting styling adjustments.
    *   **(11/7/2025, 1:40:38 PM - 1:44:13 PM):** The button color for "Add Vessel" / "Update Vessel" within the `action` column's `renderCell` was changed to `color="dark"`.
    *   **(11/7/2025, 1:54:01 PM - 1:55:33 PM):** The `headerName` for the main `GET_BOOKING_COULMN`'s "Action" field was changed from "Action" to "Approved/Rejected". Its `minWidth` was also increased from 100px to 160px.
    *   **(11/7/2025, 1:58:47 PM - 2:11:31 PM):** Minor cosmetic and behavioral changes within the `action` column's `renderCell` for `BOOKING_VESSEL_COLUMN_CUSTOMER`, such as adjusting the hover text color for the "Add Vessel" icon. The `onRowEdit` parameter for the "Edit" button was clarified to set `editingRowId`.
    *   **(11/7/2025, 4:27:51 PM):** The `renderCell` for the `action` button of the main booking table was also changed to display "Approved/Rejected" and its `minWidth` was adjusted to 160.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\booking\booking-view.tsx`**
    *   **(11/7/2025, 11:23:01 AM):** This file introduces the `BookingApproveContainer` component, which handles the display and interaction for booking approval. It uses `Formik` for form management, `useState` for `reasonFlag`, `vesselData`, `editingRowId`, and `useRef` for `reasonRef`. It fetches booking details using `useFetchCustomerApproveDetailsQuery` and saves status with `useSaveBookingStatusMutation`. Key functionalities include:
        *   `PO_COLUMNS` and `INVENTORY_COLUMNS` are defined for `DynamicEditableTable`.
        *   `vesselData` is initialized from `bookingData.vessel_details` on mount.
        *   `isReadOnly` state is derived from `bookingData?.booking_status`.
        *   `handleAddVessel`, `handleEditField`, `handleSaveRow`, `handleDeleteRow` functions are defined for managing vessel data.
        *   `handleApproval` handles the logic for approving or rejecting a booking, including validation for a reason on rejection and API calls to `saveBookingState`.
        *   A `LocationPath` component is introduced to display POL/POD.
        *   The component renders several `Card` sections for PO Details, Inventory Required, Vessel Details, and Routing Details, utilizing `DynamicEditableTable` and `RadioSelectTable`.
    *   **(11/7/2025, 12:09:06 PM):** A minor change fixed how `id` is generated for existing `vessel_details` by ensuring `v.serial_id` is used, and only falling back to `Date.now() + Math.random()` for new rows.
    *   **(11/7/2025, 12:09:45 PM):** A bug was introduced in `handleAddVessel` where `newRow.id` was not explicitly assigned for setting `editingRowId`, potentially causing issues with tracking the newly added row. The `serial_id` for `newRow` was also removed, causing `updatedRow.serial_id` to be always `undefined` in `handleSaveRow`.
    *   **(11/7/2025, 12:10:44 PM):** The `id` generation logic in `handleAddVessel` for new rows was corrected to use `serial_id: Date.now()`.
    *   **(11/7/2025, 12:11:08 PM):** The `id` generation logic was reverted for new rows, using `id: Date.now()`, ensuring consistent client-side ID generation. The previous change to `serial_id` in `handleAddVessel` was likely incorrect for client-side editing.
    *   **(11/7/2025, 12:20:34 PM):** `useAddVesselMutation` was imported and integrated. `handleSaveRow` was updated to conditionally call `addNewVessel` API for new rows (those without `serial_id`) and then update the local state with the backend-returned vessel data, assigning its `serial_id` to the `id` field. For existing rows, it simply updates local state.
    *   **(11/7/2025, 12:30:33 PM):** Console logs were added in `handleSaveRow` to inspect `updatedRow` and `data?.agent_booking_id`.
    *   **(11/7/2025, 12:34:21 PM):** An unused `import { console } from "inspector";` was added and subsequently removed. `console.log` statements were also added to `handleSaveRow`.
    *   **(11/7/2025, 12:44:14 PM):** The `PO_COLUMNS` definition was simplified, removing the `type: "text"` from `pono` and removing the `type: "text"` from `qty`. The `handleSaveRow` function was significantly refactored. Instead of directly using `updatedRow` as payload, `currentRow` from the local state is used, ensuring consistency. It now explicitly handles both new (no `serial_id`) and existing vessels, updating the `vesselData` and Formik state after a successful API call. A `res.isNew` property in the API response is anticipated to determine the toast message.
    *   **(11/7/2025, 1:07:05 PM):** `handleSaveRow` was updated to send the `currentRow` data in the payload for `addNewVessel`, explicitly setting `null` for `etd`, `eta`, `cy_cutoff`, `docs_cutoff` if they are empty. This ensures consistent data format for the API. The `setFieldValue` call for `vessel_details` now uses a functional update to avoid stale `vesselData`.
    *   **(11/7/2025, 1:10:16 PM):** `payload` for `addNewVessel` was adjusted to explicitly pass `null` for empty date fields (`etd`, `eta`, `cy_cutoff`, `docs_cutoff`).
    *   **(11/7/2025, 1:16:53 PM):** `handleSaveRow` was refined to consolidate `addNewVessel` API call for both new and existing rows by including `serial_id` (or `null`) in the payload, making the API endpoint handle the upsert logic. The success message for `toast` now dynamically indicates "New vessel added" or "Vessel updated" based on a `res.isNew` property. `setFieldValue` for `vessel_details` was also updated to use the latest `vesselData` state. A new local `onEdit` function was added, which sets `editingRowId` and updates `vesselData`, possibly replacing `handleEditField`.
    *   **(11/7/2025, 1:22:25 PM - 1:22:40 PM):** The `RadioSelectTable` component in the JSX now uses `(id) => setEditingRowId(id)` for its `onRowEdit` prop, directly calling the `setEditingRowId` state updater.
    *   **(11/7/2025, 1:27:33 PM):** The `RadioSelectTable` component in the JSX was updated to pass `(id) => setEditingRowId(id)` as the `onRowEdit` prop. The last argument (previously `isReadOnly`) was changed to `bookingData?.booking_status` to match the updated column definition.
    *   **(11/7/2025, 4:23:18 PM - 4:23:53 PM):** The `PO_COLUMNS` definition was updated. The `pono` column now uses a `valueGetter` to parse PostgreSQL array strings (`{"a","b","c"}`) and clean them up, ensuring correct display. The `qty` field was removed from `PO_COLUMNS`.
    *   **(11/7/2025, 4:25:53 PM - 4:26:36 PM):** The `DynamicEditableTable` for "PO Details" and "Inventory Required" was set to `editable={false}`, preventing direct editing on these tables in this view.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\booking-api.ts`**
    *   **(11/7/2025, 12:18:36 PM):** A new `addVessel` mutation was introduced with the endpoint `/booking/vessels/${params.agent_booking_id}` (POST method). The `saveBookingStatus` mutation's `invalidatesTags` now includes "update".
    *   **(11/7/2025, 12:19:28 PM):** `useAddVesselMutation` was exported.
    *   **(11/7/2025, 12:25:21 PM):** The `addVessel` mutation's URL parameter was changed from `/booking/vessels/${params.agent_booking_id}` to `/booking/vessels/${params}`, suggesting a potential change in how the API expects the `agent_booking_id`.
    *   **(11/7/2025, 12:26:13 PM):** The `addVessel` mutation's URL was simplified to `/booking/vessels`, with the `agent_booking_id` presumably being part of the `body`.
    *   **(11/7/2025, 12:27:45 PM):** The `addVessel` mutation's URL was updated to `/booking/vessel` (singular).
    *   **(11/7/2025, 4:17:56 PM):** The `saveBookingStatus` mutation now invalidates "update" and "bookingDashboardData" tags in addition to "CUSTOMER_APPROVE" and "add".

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\types\delivery.type.ts`**
    *   **(11/7/2025, 4:21:04 PM):** No significant changes, the file defines `DeliveryType`, `FilterType`, and `BookingStatusType` interfaces.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts`**
    *   **(11/7/2025, 4:21:18 PM - 4:22:14 PM):** The color for the "Confirmed" `BOOKING_STATUS` was changed from "teal" to "pink". The `INVENTORY_OPTIONS` list contains a duplicate "40HC" entry which was already present.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\dynamicEditable.tsx`**
    *   **(11/7/2025, 4:25:29 PM):** A new `editable` prop was added to the `DynamicEditableTable` component, defaulting to `true`. This prop controls whether table rows can be edited, added, or deleted. The `handleChange`, `handleDelete`, and `addNewRow` functions, as well as the rendering of input fields and action buttons, are now conditional on this `editable` prop.
    *   **(11/7/2025, 4:30:02 PM):** A `valueGetter` prop was added to the `EditableTableColumn` interface. The table's cell rendering now checks for this `valueGetter` when `editable` is false, allowing custom formatting for displayed (non-editable) cell values.
    *   **(11/7/2025, 4:34:37 PM):** The logic for displaying cell content when `editable` is `false` was updated to correctly utilize `column.valueGetter` if available, otherwise falling back to `row[column.key]`.

**Patterns and Recurring Elements:**

*   **Dates:** All changes occurred on **November 7, 2025**.
*   **Booking Management Focus:** The core of the changes revolves around a "Booking Approve" feature, specifically `BOOKING_VESSEL_COLUMN_CUSTOMER` and `BookingApproveContainer`, indicating active development on a booking approval workflow.
*   **Table Editing Logic:** There's a consistent pattern of managing editable tables:
    *   **Conditional Rendering:** Input fields and action buttons are rendered conditionally based on an `editingRowId` state, allowing specific rows to be in edit mode.
    *   **State Management:** `useState` hooks are extensively used for `vesselData`, `editingRowId`, and `reasonFlag`.
    *   **API Integration:** RTK Query (`useAddVesselMutation`, `useSaveBookingStatusMutation`, `useFetchCustomerApproveDetailsQuery`) is central for interacting with a backend booking API.
    *   **Read-Only Mode:** A prominent feature is the `isLocked` or `isReadOnly` state, which disables or hides interactive elements when a booking is in "Approved" or "Cancelled" status, preventing further modifications.
    *   **Console Logging:** `console.log` statements frequently appear and disappear, indicating active debugging during development.
    *   **Component Reuse:** Custom components like `InputBox`, `Button`, `Badge`, `DynamicEditableTable`, and `RadioSelectTable` are consistently used.
*   **Refinement and Bug Fixes:** The log shows an iterative process of:
    1.  Implementing core features (editable tables, API calls).
    2.  Adding conditional logic (e.g., `isLocked` state).
    3.  Refining UI/UX details (button labels, icon visibility, input styling).
    4.  Addressing bugs or inconsistencies (e.g., `id` generation for new rows, `valueGetter` for display).
*   **Code Comments:** Comments marked with `✅` and `🔹` are used to highlight new features or clarify changes, suggesting a structured approach to development.

## 7:58:50 PM
The code changes, all dated 11/7/2025, reflect a concentrated development effort across several core functionalities of the `envosys-backend`.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\docsParseService.ts` (5:03:25 PM):** A `getAllPdfParse` function was added (despite the file name, it primarily functions as a vendor service). This service retrieves vendor details with pagination, sorting, and filtering capabilities (by vendor name, status, email). It also fetches and attaches associated `document_upload` records for each vendor, and processes vendor `status` and `email` for filter options, converting status values to "Active"/"Inactive".

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\pdfParseService.ts` (5:06:48 PM - 6:40:55 PM):** This file saw multiple rapid iterations:
    *   **5:06:48 PM:** Initial implementation of `getAllPdfParse` for the `AI_Parse_Document` table, featuring pagination and sorting by `process_date_time` using `QueryBuilder`.
    *   **5:06:59 PM:** Modified the `getAllPdfParse` response to include a `count` of total records.
    *   **6:15:41 PM:** Updated `getAllPdfParse` to construct full URL paths for `pdf_pa` and `json_pa` columns using `CONCAT` and dynamic environment variables, and changed error logging from `logger.info` to `logger.error`.
    *   **6:25:14 PM:** Reverted the explicit URL construction and error logging changes, returning to querying all columns (`"*"`) and `logger.info`.
    *   **6:29:11 PM:** Underwent a significant refactor in `getAllPdfParse`, replacing the `QueryBuilder` with a direct SQL query. This direct query once again included explicit `CONCAT` to form full URLs for `pdf_pa` and `json_pa` (using `/envosys_uploads/`), and dynamically applied `ORDER BY` clauses. It also started selecting `ser_no` as an identifier.
    *   **6:40:33 PM:** Further adjusted `getAllPdfParse` to use hardcoded `http://localhost:8080/uploads/` for constructing file URLs, fixed the selected identifier to `id`, and reverted to a static `ORDER BY process_date_time DESC`.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\constants\Paths.ts` (5:07:36 PM - 5:15:03 PM):**
    *   **5:07:36 PM:** Defined a comprehensive set of API paths for various modules (Users, Auth, AutoComplete, Audit, UploadDocuments, Customer, Linebooking, CustomerBooking, Vessel, Voyage, Vendor, Job, Document).
    *   **5:15:03 PM:** Renamed the `Document.GetPdf` path to `Document.GetPdfParse` and introduced new base paths for a "Notes" module (`/notes`).

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\documentUploadRoutes.ts` (5:08:07 PM):** Integrated the `getAllPdfParse` service by adding a new GET route at `Paths.Document.GetPdf`.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\linebookingService.ts` (5:14:34 PM - 5:14:48 PM):** The `create` function for line bookings was implemented. It handles database transactions, inserts line booking details, associated equipment, and other services. It also determines the booking status based on specific conditions. A substantial `getAll` function, implementing extensive filtering and sorting for line bookings, was found fully commented out, suggesting it's either under development or temporarily disabled.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\util\optionQuery.ts` (5:14:43 PM):** A new utility file `optionQuery.ts` was introduced. It provides a `OptionQuery` class with methods to generate specialized SQL queries for various autocomplete and dropdown options (e.g., ports, GST locations, users, vessels, vendors, parties), primarily using `ILIKE` for case-insensitive search and limiting results.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\vendorServices.ts` (5:59:15 PM):** This file received significant updates:
    *   The `getAll` function for vendors was enhanced to fetch and attach `uploadDocuments` to each vendor. A commented-out section for fetching `notes` indicates future integration.
    *   A `addVendor` function was introduced to create new vendors. It includes validation, inserts data into the `vendor` table, calls an `simpleAudit` function, processes `uploadDocuments` (Multer files), and importantly, implements logic to parse and save new `notes` associated with the vendor.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\index.ts` (6:34:36 PM):** Integrated the new `NotesRoutes` and linked `Paths.Audit.Base` directly to the `getAllAudit` service.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\server.ts` (6:34:51 PM):** Configured the Express server to serve static files from `/root/node/envosys/envosys_bucket/ai/upload` via the `/uploads` URL path, and increased the request body size limit for JSON and URL-encoded payloads to 10MB.

**Patterns and Recurring Elements:**

*   **Timestamp Cohesion:** All changes occurred on the same day, indicating a focused and rapid development or debugging session.
*   **Query Management:** The project utilizes a `QueryBuilder` utility extensively for constructing database queries, though there's a trend, especially in `pdfParseService.ts`, to revert to direct SQL for more fine-grained control or specific data formatting (like URL concatenation).
*   **Pagination & Sorting:** Most `getAll` endpoints consistently implement pagination and dynamic sorting parameters.
*   **Robust Error Handling:** Services consistently wrap database operations in `try-catch` blocks, returning `HttpStatusCodes.INTERNAL_SERVER_ERROR` and logging errors with `logger`.
*   **Integrated Document & Note Management:** There is a clear pattern of attaching and managing `document_upload` records and `notes` with core entities like vendors.
*   **Centralized Path Definitions:** API routes are managed centrally in `Paths.ts` for better organization.
*   **Autocomplete Utilities:** A dedicated `optionQuery.ts` class simplifies the generation of SQL queries for autocomplete functionality.
*   **PDF Parsing Focus:** The `pdfParseService.ts` file shows significant and iterative changes, particularly in how it constructs and exposes file URLs for parsed PDF and JSON data, suggesting active development or refinement of this feature.

## 8:16:03 PM
The changes primarily affect the `AuthRoutes.ts`, `dashboardService.ts`, `UserService.ts`, `Paths.ts`, and `bookingService.ts` files.

**File-Specific Updates:**

*   **`AuthRoutes.ts` (11/5/2025, 12:54:56 PM):**
    *   No significant functional changes. The code remains focused on handling user information requests (`/auth/verify-token`) by logging the user and returning their data with an `OK` status.

*   **`dashboardService.ts` (Multiple timestamps from 11/5/2025, 6:04:50 PM to 11/5/2025, 11:13:53 PM):**
    *   **Initial Functionality (6:04:50 PM):** Introduced `getDashboardCardInfo` and `getDeliveryData` functions.
        *   `getDashboardCardInfo`: Retrieves shipment card information (pending departure, in transit, ETA within 1 week, out for delivery) with filters for `fromDate`, `toDate`, and `CompanyName`. It uses `buildMtrUserCondition` for user-specific filtering.
        *   `getDeliveryData`: Fetches delivery or vessel arrival data based on `type` and `mode` query parameters, also applying user-specific conditions.
    *   **Minor Edits (6:05:15 PM, 6:05:28 PM, 6:06:01 PM):** These changes appear to be very minor or re-saves, with no discernible functional differences in the provided code snippets for `dashboardService.ts`. The code content is identical across these three timestamps.
    *   **Introduction of `getUserObject` (8:41:51 PM, 8:42:22 PM):** The `getDashboardCardInfo` function was modified to dynamically determine `CompanyName` using a new `getUserObject` utility function. This function takes `req.user`, `req.query.companyname`, and `req.query.access` to retrieve the relevant company name. A `console.log` for `req.CompanyName` was added (8:42:22 PM).
    *   **Refinement of `CompanyName` Handling and `useWhere` (10:41:50 PM, 10:43:59 PM, 10:44:51 PM, 10:46:26 PM):**
        *   The `CompanyName` query parameter type was narrowed.
        *   A `console.log` for `req.CompanyName` was added.
        *   A significant change was made: the `if(useWhere) conditions.push(useWhere);` line in `getDashboardCardInfo` was commented out (10:43:59 PM), indicating a shift in how user-specific conditions might be applied for card info, potentially relying solely on the derived `CompanyName`.
        *   `console.log("result[0]", result[0]);` was added for debugging the dashboard card info (10:44:51 PM).
    *   **Enhanced `getDeliveryData` Filtering and Security (10:51:27 PM to 11:13:53 PM):**
        *   The `getDeliveryData` function received updates to how `CompanyName` is handled. It now explicitly retrieves `CompanyName` via `getUserObject` (similar to `getDashboardCardInfo`).
        *   A `companyFilter` variable was introduced in `getDeliveryData` to safely construct the `account_name` filter, including a `replace(/'/g, "''")` to escape single quotes, mitigating potential SQL injection risks.
        *   `console.log("companyFilter", companyFilter);` was added (11:11:09 PM).
        *   The `buildMtrUserCondition(req.user!)` was seemingly commented out for `CompanyName` derivation but then re-added as `useWhere` and the `CompanyName` derivation logic using `getUserObject` and `companyFilter` was made more explicit and safer by passing `CompanyName` as a query parameter.
        *   The `getDashboardCardInfo`'s `CompanyName` logic was also made explicitly safe, and the `useWhere` condition was also commented out (11:13:53 PM). The query string for `CompanyName` was also updated for safer string interpolation.

*   **`UserService.ts` (Multiple timestamps from 11/5/2025, 8:51:00 PM to 11/5/2025, 8:51:46 PM):**
    *   **Comprehensive User Management (8:51:00 PM):** This file provides various user-related services:
        *   `login`: Authenticates users, creates a JWT, and handles inactive accounts.
        *   `saveUserPreferences`: Stores user preferences (basic info, email/push configs, event configs) into `v2_user_details` with `ON CONFLICT` for updates.
        *   `fetchUserPreferences`: Retrieves user preferences, falling back to `mtr_user_master` for basic info if `v2_user_details` is empty.
        *   `saveInviteLog`: Logs user invitations and sends an invite email.
        *   `fetchAllCompanyUser`: Fetches both active registered users and pending invited users for a given company.
        *   `userSettings`: Retrieves specific user settings like `date_format`.
    *   **Minor Edits (8:51:15 PM, 8:51:46 PM):** These appear to be minor re-saves or formatting changes, as the provided code content for `UserService.ts` is identical across these three timestamps.

*   **`bookingService.ts` (Multiple timestamps from 11/6/2025, 2:02:45 PM to 11/7/2025, 8:07:31 PM):**
    *   **Initial Booking Management (11/6/2025, 2:02:45 PM):** Introduced `BookingQueryBuilder` class for constructing dynamic SQL queries for booking data.
        *   `getAll`: Fetches booking records with pagination, filtering by `type` (PENDING, APPROVED, CANCELLED, NEW, ALL), `agent_booking_id`, `pol`, `pod`, `shipper`, `status`, `booking_type`, and date ranges (`fromDate`, `toDate`). It also provides filter options dynamically.
        *   `saveBooking`: Handles saving new or updating existing booking records.
    *   **Added `est_cargo_ready_date` (11/6/2025, 2:05:16 PM):** The `selectedColumns` in `BookingQueryBuilder` was updated to include `abe.est_cargo_ready_date`.
    *   **Added Cargo Ready Date Range Filtering (11/6/2025, 2:16:39 PM):**
        *   New properties `CargoReadyFromDate` and `CargoReadyToDate` were added to `BookingQueryBuilder`.
        *   The `setDateRange` method was updated to accept and apply filters for `abe.est_cargo_ready_date`.
    *   **Where Clause Refinement in `BookingQueryBuilder` (11/6/2025, 2:26:13 PM):** The `whereClauses` property was changed from a string to a string array `string[]`, and a new private `addDateRangeClause` method was introduced, which pushes conditions into this array. The logic for building the final `WHERE` clause was adapted to join conditions from the array. This change was reverted shortly after.
    *   **Reverted `whereClauses` and minor updates (11/6/2025, 2:27:15 PM to 11/6/2025, 5:05:30 PM):** The `whereClauses` in `BookingQueryBuilder` was reverted to a single string (from array). This indicates some refactoring or testing of query building logic. A new import `NextFunction, Response` was added (11/6/2025, 5:05:22 PM).
    *   **`countData` function parameter change (11/6/2025, 5:57:33 PM, 5:57:41 PM):** The `buildCountQuery` parameter was simplified in its usage from `COUNT(DISTINCT abe.agent_booking_id)` to `COUNT(abe.agent_booking_id)` in some instances, implying a potential optimization or correction. However, this change was also quickly reverted, as `COUNT(DISTINCT abe.agent_booking_id)` appears in later versions.
    *   **New `BookingStatus.CONFIRMED` (11/6/2025, 6:03:28 PM):** The `BookingStatus` enum was updated to include a `CONFIRMED` status, and the `setWhereClause` method in `BookingQueryBuilder` was updated to support this new status. The `counts` object in `getAll` was also updated to include `confirmed` bookings.
    *   **BaseFrom Joins Update (11/7/2025, 6:59:14 PM):** The `BookingQueryBuilder`'s `baseFrom` was simplified, removing the `LEFT JOIN agent_booking_po AS abp` and consequently removing `abp.pono` from `selectedColumns`. This suggests that PO information might no longer be directly joined for the main booking list view or that the join moved elsewhere. This was quickly reverted.
    *   **Reverted BaseFrom Join and Minor Changes (11/7/2025, 7:02:52 PM to 11/7/2025, 8:07:31 PM):** The `baseFrom` modification was reverted, re-including `agent_booking_po`. Minor, seemingly non-functional code changes or re-saves are observed across several timestamps. New interfaces `InventoryI` and `VesselI` were introduced (11/7/2025, 8:09:03 PM).
    *   **Consistently `COUNT(DISTINCT abe.agent_booking_id)` (8:05:59 PM):** The `buildCountQuery` function now consistently uses `COUNT(DISTINCT abe.agent_booking_id)`, indicating a final decision on counting distinct booking IDs.

*   **`Paths.ts` (Multiple timestamps from 11/6/2025, 5:06:22 PM to 11/7/2025, 12:24:55 PM):**
    *   **Added `getBookingApproveDetails` (11/6/2025, 5:06:22 PM):** A new path `/booking_approve/:agent_booking_id` was added under `Booking` for `getBookingApproveDetails`.
    *   **Added `createBookingApproveDetails` (11/6/2025, 5:13:57 PM):** A path `/booking_approve/:agent_booking_id` was added for `createBookingApproveDetails` under `Booking`, indicating POST action for booking approval.
    *   **Added `vesselAdd` (11/7/2025, 12:17:18 PM, 12:24:55 PM):** A new path `/vessel/:agent_booking_id` was added under `Booking` for `vesselAdd`, later changed to just `/vessel` (12:24:55 PM), implying a general vessel addition endpoint for bookings.

*   **`BookingRoutes.ts` (Multiple timestamps from 11/6/2025, 5:06:41 PM to 11/7/2025, 1:14:50 PM):**
    *   **New Routes for Booking Approval (11/6/2025, 5:06:41 PM):** The `getBookingApproveDetails` endpoint was added.
    *   **Added `updateBookingApproveDetails` (11/6/2025, 5:15:38 PM):** A POST route for `createBookingApproveDetails` (which maps to `updateBookingApproveDetails` service function) was added.
    *   **Added `addBookingVessel` (11/7/2025, 12:17:36 PM, 1:14:41 PM, 1:14:50 PM):** A POST route for `vesselAdd` (mapped to `addBookingVessel`, later renamed to `addOrUpdateBookingVessel`) was added.

**Timestamps of Significant Changes:**

*   **11/5/2025, 8:41:51 PM:** `dashboardService.ts` introduced `getUserObject` for dynamic `CompanyName` resolution, indicating a security/access control improvement or refactor.
*   **11/5/2025, 10:51:27 PM:** `dashboardService.ts` significantly improved `getDeliveryData` by safely handling `CompanyName` and implementing SQL injection prevention (`replace(/'/g, "''")`).
*   **11/6/2025, 2:16:39 PM:** `bookingService.ts` enhanced filtering by adding support for "cargo ready" date ranges, improving booking data query capabilities.
*   **11/6/2025, 6:03:28 PM:** `bookingService.ts` added `CONFIRMED` status, expanding the workflow states for bookings.
*   **11/7/2025, 12:17:18 PM:** `Paths.ts` introduced a new route for vessel management within bookings, indicating new functionality for adding vessels.
*   **11/7/2025, 8:09:03 PM:** `bookingService.ts` introduced `InventoryI` and `VesselI` interfaces, signaling upcoming or recent changes related to inventory and vessel data handling within the booking service.

**Patterns and Recurring Elements:**

*   **Query Builder Pattern:** Both `dashboardService.ts` (`SqlQueryBuilder`) and `bookingService.ts` (`BookingQueryBuilder`) extensively use a query builder class pattern to construct dynamic SQL queries based on user roles and various filters. This promotes cleaner code and potentially reduces SQL injection risks, especially with the `replace(/'/g, "''")` addition.
*   **Role-Based Access Control (RBAC):** Filtering conditions consistently check `req.user.role` (ADMIN, CUSTOMER, FRONTOFFICE, AGENT) to apply specific data visibility rules, particularly in `buildUserCondition` for `dashboardService` and `setWhereClause` for `bookingService`.
*   **Error Handling and Logging:** All service functions include `try...catch` blocks for robust error handling, returning `HttpStatusCodes.INTERNAL_SERVER_ERROR` on failure and using a `logger` for informational and error messages.
*   **Dynamic Filtering:** There's a strong emphasis on dynamic filtering for lists (shipments, bookings) based on various query parameters (dates, IDs, statuses, locations, etc.), indicating a flexible UI that can request data with different criteria.
*   **Status Management:** Booking statuses (`PENDING`, `APPROVED`, `CANCELLED`, `NEW`, `CONFIRMED`, `ALL`) are clearly defined and used to categorize and filter booking data. Shipment statuses are also dynamically determined based on MTR tracking data.
*   **Code Duplication/Reverts:** There are multiple instances of nearly identical code commits for `dashboardService.ts` and `UserService.ts`, especially on 11/5/2025, and `bookingService.ts` on 11/6/2025 and 11/7/2025. This might suggest frequent small saves, formatting changes, or iterative development/testing where changes were made and then reverted or slightly adjusted.
*   **SQL Query Complexity:** The SQL queries, especially in `getDashboardCardInfo`, are complex, using `FILTER (WHERE ...)` and `REGEXP_REPLACE` for data aggregation and manipulation, suggesting a rich dataset and advanced reporting needs.

## 8:16:16 PM
The changes primarily focus on implementing and refining a document upload and display feature within a React frontend application, utilizing Redux Toolkit Query for API interaction and `@mui/x-data-grid` for data presentation.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\store\api\uploadDocumentApi.ts` (Timestamp: 11/7/2025, 5:20:16 PM):**
    *   This file was initialized to define the API endpoints for document management.
    *   It introduces `uploadDocument` as a mutation for `POST` requests to `/api/documents/upload`, handling `FormData` and `Authorization` headers.
    *   `downloadDocument` is defined as a `GET` mutation for downloading files as blobs from `/api/documents/download/{refType}/{documentName}`.
    *   `getPdfParseData` is a `GET` query for fetching parsed PDF data from `/api/documents/get-pdf-parse`.
    *   This sets up the core communication layer for the document functionality.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\components\screen\upload-docs\uploadDocs.tsx` (Multiple timestamps):**
    *   **Initial (11/7/2025, 5:21:15 PM):** The `ConfirmLineBookingForm` component was created. It provides a UI for uploading documents via drag-and-drop or file input. It uses `formik` for form handling, `react-hot-toast` for user feedback, and `useState` hooks for managing file uploads and UI state. The `AppGrid` initially displayed static `tableDummyData`.
    *   **Dynamic Data Integration (11/7/2025, 5:24:08 PM):** The `AppGrid` was updated to consume real data fetched from the `useGetPdfParseDataQuery` hook (`getPdfParseData?.data`) and reflect the total count (`getPdfParseData?.total`). The `uniqueId` prop for the grid was also changed from `"id"` to `"ser_no"`.
    *   **Minor Edits (11/7/2025, 6:00:45 PM & 6:01:08 PM):** A temporary debugging string (`kk`) was added to the file name display in the uploaded files list, then immediately reverted, indicating minor UI adjustments or testing.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\data\coulms\upload-doc-coulmn.tsx` (Multiple timestamps):**
    *   **Initial Column Definitions (11/7/2025, 5:23:08 PM):** Defined columns for "File" (`pdf_pa`), "JSON" (`json_pa`), and "Date And Time" (`process_date_time`). The "File" column allowed direct download/opening via an `<a>` tag. The "JSON" column displayed a summary of in-memory JSON data and provided a mechanism to download that stringified data locally.
    *   **JSON Download Refactor (11/7/2025, 5:32:50 PM):** A significant change was introduced in the "JSON" column to fetch JSON content from a URL (`params.row.json_pa`) before downloading it, aiming for more dynamic and robust data retrieval.
    *   **Reversion (11/7/2025, 5:33:56 PM):** The server-fetch JSON download logic introduced at 5:32:50 PM was reverted, returning to the simpler in-memory data stringification and download.
    *   **Intelligent File Handling (11/7/2025, 6:05:35 PM):** The "File" column (`pdf_pa`) was enhanced with `handleClick` logic to intelligently handle downloads: if the file name indicated a `.json` extension, it would fetch and download the JSON content; otherwise, it would open/download the file normally.
    *   **Re-refactor of JSON Column (11/7/2025, 6:30:30 PM):** The "JSON" column (`json_pa`) was again updated to fetch its content from a URL before downloading, re-implementing the server-side data fetching approach.
    *   **Final Major Reversion and Renaming (11/7/2025, 8:15:39 PM):** All complex download logic in both the "File" and "JSON" columns was reverted. The "File" column (`pdf_pa`) returned to a basic `<a>` tag for direct download. The "JSON" column reverted to displaying summary text from in-memory data and downloading that stringified content. Additionally, column field names were updated: `pdf_pa` was renamed to `file`, and `process_date_time` was renamed to `date_time`.

**Timestamps of Significant Changes:**

*   **11/7/2025, 5:20:16 PM:** Initial setup of API services for document management.
*   **11/7/2025, 5:24:08 PM:** Integration of dynamic API data into the document display grid.
*   **11/7/2025, 5:32:50 PM & 6:05:35 PM & 6:30:30 PM:** A series of changes and reversions in `upload-doc-coulmn.tsx` indicating ongoing development and refinement of download logic, particularly for JSON files.
*   **11/7/2025, 8:15:39 PM:** Final adjustment to column definitions, including field name changes and a reversion to simpler download logic for the table columns.

**Patterns or Recurring Elements:**

*   **Focus on Document Management:** The entire log revolves around features for uploading, downloading, and displaying information related to documents.
*   **RTK Query for API:** Redux Toolkit Query is consistently used for defining and interacting with API endpoints, centralizing data fetching and caching logic.
*   **Dynamic Data Integration:** There's a clear trend of moving from static dummy data to fetching and rendering dynamic data from the backend.
*   **Iterative Development and Reversions:** The `upload-doc-coulmn.tsx` file demonstrates a common development pattern of implementing a solution, testing it, and then potentially reverting or refining it based on further requirements or issues. This is especially evident in the changes to the JSON download mechanism.
*   **Authorization Headers:** API calls consistently include `Authorization` headers derived from `localStorage`, indicating a secure application context.
*   **Debugging Statements:** The presence of `console.log` statements in multiple files at various points (`console.log("getPdfParseData")`, `console.log("JSON Data:")`) suggests active debugging during development.

## 9:50:15 PM
The provided logs indicate significant development work on a React frontend application, primarily focusing on booking management and editable table functionalities, along with API integration.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\booking\booking-view.tsx`**
    *   **Initial Structure and Core Functionality (11/7/2025, 12:10:44 PM - 11/7/2025, 12:11:08 PM):** This file defines the `BookingApproveContainer` component, a React functional component for managing booking approvals. It utilizes `Formik` for form handling, `react-hot-toast` for notifications, and several custom components like `InputBox`, `Card`, `Button`, `AppTable`, `DynamicEditableTable`, and `RadioSelectTable`.
        *   It imports `useFetchCustomerApproveDetailsQuery` and `useSaveBookingStatusMutation` from `booking-api.ts`.
        *   `PO_COLUMNS` and `INVENTORY_COLUMNS` are defined as `EditableTableColumn` arrays for displaying PO and inventory details.
        *   Initial state variables `reasonFlag`, `vesselData`, `editingRowId` are set up.
        *   `useEffect` populates `vesselData` from `bookingData`.
        *   Key functions `handleAddVessel`, `handleEditField`, `handleSaveRow`, `handleDeleteRow`, and `handleApproval` are defined for managing vessel data and booking status.
        *   A `LocationPath` FC is included for displaying POL/POD.
        *   The main UI uses `Formik` to render cards for PO Details, Inventory, Vessel Details, and Routing Details, all using dynamic tables.
        *   A "Reason" input box is conditionally displayed.
        *   "Not Approved" and "Approved" buttons trigger `handleApproval`.
        *   A minor change around 12:11:08 PM involves changing `v.serial_id` to `v.serial_id || Date.now() + Math.random()` when mapping `bookingData.vessel_details` to `vesselData`, likely to ensure unique IDs for new local rows before they have a `serial_id` from the backend.
    *   **Integration of `useAddVesselMutation` and `handleSaveRow` Logic Refinement (11/7/2025, 12:20:34 PM - 11/7/2025, 12:20:42 PM):** The `useAddVesselMutation` hook is introduced, allowing the frontend to add new vessel entries to the backend. The `handleSaveRow` function is updated to distinguish between new vessel rows (without `serial_id`) and existing ones. New rows trigger the `addNewVessel` mutation and, upon success, update the local state with the backend-provided `serial_id`.
    *   **Console Logging and Minor Adjustments (11/7/2025, 12:30:33 PM - 11/7/2025, 12:34:21 PM):** Console logs are added to `handleSaveRow` for debugging the `updatedRow` and `agent_booking_id`. An unnecessary `import { console } from "inspector";` is also added, which is a common developer mistake.
    *   **Refinements in `handleSaveRow` and `onEdit` parameters (11/7/2025, 1:07:05 PM - 11/7/2025, 1:27:33 PM):** The `handleSaveRow` logic is further refined to correctly retrieve `currentRow` from `vesselData` before forming the payload for `addNewVessel`. It explicitly handles null dates (`etd`, `eta`, `cy_cutoff`, `docs_cutoff`). The update for Formik's `vessel_details` is also made more robust by mapping the `vesselData` state after local updates. A new `onEdit` function is introduced and passed to `RadioSelectTable` for better handling of edit state.
    *   **PO_COLUMNS `valueGetter` and `renderCell` (11/7/2025, 4:23:18 PM - 11/7/2025, 7:48:07 PM):** The `PO_COLUMNS` definition is significantly updated.
        *   Initially, the `type` property for 'pono' is removed, and a `valueGetter` function is added to process `pono` values (handling arrays and PostgreSQL-style array strings by cleaning and joining them).
        *   Later, a `renderCell` function is added to the 'podate' column to format the date to "YYYY-MM-DD" for display.
        *   The `PO_COLUMNS` and `INVENTORY_COLUMNS` are explicitly set to `editable={false}` in `DynamicEditableTable` at various points, indicating that these sections are for display only within this view.
        *   The `PO_COLUMNS` definition becomes a function that takes `formatDate` as an argument around 7:33:12 PM, requiring it to be called when passed to `DynamicEditableTable`.
    *   **Removal of `LocationPath` (11/7/2025, 4:44:14 PM):** The `LocationPath` component and its usage are removed from the `BookingApproveContainer` rendering.
    *   **Routing Columns Function Call (11/7/2025, 2:05:26 PM - 11/7/2025, 2:05:35 PM):** `ROUTING_COLUMNS` is changed from a direct import to a function call (`ROUTING_COLUMNS()`), implying it now returns the columns.
    *   **Minor CSS Class Additions (11/7/2025, 1:39:29 PM - 11/7/2025, 1:43:05 PM):** `className="my-2"` is added to various `InputBox` components within `BOOKING_VESSEL_COLUMN_CUSTOMER`'s `renderCell` functions.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\booking-api.ts`**
    *   **API Endpoint and Hook Updates (11/7/2025, 12:18:36 PM - 11/7/2025, 12:20:00 PM):**
        *   A new mutation endpoint `addVessel` is introduced for adding vessel details.
        *   Initially, `addVessel` expects `params.agent_booking_id` in its URL.
        *   The exported hooks are updated to include `useAddVesselMutation`.
    *   **`addVessel` Endpoint URL and Payload Changes (11/7/2025, 12:25:21 PM - 11/7/2025, 12:27:45 PM):** The `addVessel` mutation's URL parameter is modified from `/booking/vessels/${params.agent_booking_id}` to `/booking/vessels/${params}` and then simplified to just `/booking/vessel`. The payload for this mutation also changes to pass the `params` object directly as the body.
    *   **`saveBookingStatus` Invalidation Tags (11/7/2025, 4:17:56 PM):** `saveBookingStatus` mutation now invalidates `update` and `bookingDashboardData` tags in addition to `CUSTOMER_APPROVE` and `add`, ensuring broader UI refresh after a booking status change.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\coulmns\booking-coulmns.tsx`**
    *   **Vessel Column Customer Configuration (11/7/2025, 12:32:47 PM):** The `BOOKING_VESSEL_COLUMN_CUSTOMER` function is introduced. It takes various callbacks (`onAdd`, `onEdit`, `onSave`, `onDelete`) and a `bookingStatus` parameter.
        *   It defines columns for `mother_feeder`, `vessel`, `voyage`, `line`, `etd`, `eta`, `cy_cutoff`, and `docs_cutoff`.
        *   Cells are rendered as `InputBox` when `editingRowId` matches, otherwise as plain text or formatted dates.
        *   An "Action" column contains an "Add Vessel" icon (disabled if `bookingStatus` is "Approved" or "Cancelled"), and "Update" or "Edit" buttons depending on the `editingRowId`.
        *   The `isLocked` variable based on `bookingStatus` ensures read-only behavior for approved/cancelled bookings.
    *   **Refinement of `BOOKING_VESSEL_COLUMN_CUSTOMER` and Action Buttons (11/7/2025, 1:19:06 PM - 11/7/2025, 1:44:13 PM):**
        *   The parameters for `BOOKING_VESSEL_COLUMN_CUSTOMER` are clarified, renaming `onEdit` to `onFieldEdit` (for inline editing) and adding `onRowEdit` (for clicking an explicit edit button, which is commented out in this snippet but implied by the function signature).
        *   The text for the save button changes dynamically between "Add Vessel" and "Update Vessel" based on `isNewRow`.
        *   The color of the Save/Update button is set to "dark" in the final changes.
        *   The "Add Vessel" icon in the header is conditionally hidden (`hidden`) if `isLocked` (booking is Approved or Cancelled).
        *   The `onEdit` logic passed to the InputBox's `onChange` is changed to `onFieldEdit`.
    *   **Action Column Width and Header Change (11/7/2025, 1:54:01 PM - 11/7/2025, 1:55:33 PM):** The "Action" column in `GET_BOOKING_COULMN` has its `minWidth` increased and header text changed from "Action" to "Approved/Rejected".

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\types\delivery.type.ts`**
    *   **`BookingStatusType` Update (11/7/2025, 4:21:04 PM):** The `BookingStatusType` definition is updated to explicitly list possible string literal values: `"New" | "Approved" | "Pending" | "Cancelled" | "Confirmed"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts`**
    *   **`BOOKING_STATUS` Color Update (11/7/2025, 4:21:18 PM - 11/7/2025, 4:22:14 PM):** The color for the "Confirmed" status in `BOOKING_STATUS` is changed from "teal" to "pink".

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\dynamicEditable.tsx`**
    *   **`editable` Prop and `valueGetter`/`renderCell` in Columns (11/7/2025, 4:25:29 PM - 11/7/2025, 7:38:08 PM):**
        *   A new `editable` prop is added to the `EditableTableProps` interface and component, defaulting to `true`. This prop controls whether the table allows editing, deleting rows, or adding new rows.
        *   The column interface `EditableTableColumn` gains `valueGetter` (a function to transform cell data for display) and `renderCell` (a function to render custom React nodes in a cell).
        *   The rendering logic within `DynamicEditableTable` is updated to respect the `editable` prop. When `editable` is false, it uses `valueGetter` or `renderCell` for display, falling back to raw data if those are not provided. Otherwise, it renders input fields or selects.

### Timestamps of Significant Changes:

*   **11/7/2025, 12:10:44 PM - 12:11:08 PM:** Initial implementation and minor ID generation fix for `BookingApproveContainer`.
*   **11/7/2025, 12:18:36 PM - 12:27:45 PM:** Introduction and iterative refinement of `addVessel` API endpoint.
*   **11/7/2025, 12:32:47 PM - 1:44:13 PM:** Development of `BOOKING_VESSEL_COLUMN_CUSTOMER` for editable vessel details, including add, edit, save, and read-only logic.
*   **11/7/2025, 4:17:56 PM:** Enhanced API tag invalidation for `saveBookingStatus`.
*   **11/7/2025, 4:21:04 PM - 4:22:14 PM:** Type definition and constant updates for booking statuses.
*   **11/7/2025, 4:25:29 PM - 7:38:08 PM:** Major refactoring in `DynamicEditableTable` to support a general `editable` prop and custom `valueGetter`/`renderCell` for columns, making it more flexible.
*   **11/7/2025, 4:23:18 PM - 7:48:07 PM:** Refinements to `PO_COLUMNS` and `INVENTORY_COLUMNS` in `booking-view.tsx`, including making them non-editable and introducing `valueGetter`/`renderCell` for date formatting.

### Patterns and Recurring Elements:

*   **Component-Based Development:** The changes consistently use React functional components and hooks (`useState`, `useRef`, `useEffect`).
*   **API Integration with RTK Query:** The `src\store\api\booking-api.ts` file clearly shows the use of `@reduxjs/toolkit/query/react` for defining API endpoints (`createApi`, `fetchBaseQuery`, `builder.mutation`, `builder.query`) and generating hooks (`useSaveBookingMutation`, `useFetchCustomerApproveDetailsQuery`, `useAddVesselMutation`). This pattern simplifies data fetching and caching.
*   **Form Management with Formik:** `Formik` is consistently used in `booking-view.tsx` for managing form state, validation, and submission, indicating a robust form handling strategy.
*   **Toast Notifications:** `react-hot-toast` is frequently used for user feedback (success/error messages), suggesting a consistent UX approach for asynchronous operations.
*   **Dynamic and Editable Tables:** A recurring theme is the implementation and enhancement of dynamic and editable table components (`DynamicEditableTable`, `RadioSelectTable`). This suggests a need for flexible data display and inline editing capabilities across the application.
*   **Read-Only/Locked States:** Logic to handle read-only states (e.g., when `booking_status` is "Approved" or "Cancelled") is repeatedly implemented in various UI components and event handlers (`isReadOnly` variable). This is crucial for preventing modifications to finalized data.
*   **Date Formatting:** Custom utility functions like `appDateFormat` and `useDateFormate` are used for consistent date display, indicating a standard approach to date handling.
*   **Conditional Rendering:** Many UI elements, like the "Reason" input box and action buttons within tables, are conditionally rendered based on component state or data properties.
*   **Iterative Refinement:** The log shows an iterative process of adding new features (like `addVessel` endpoint), integrating them into components, and then refining the implementation details (e.g., `handleSaveRow` logic, column definitions, `editable` prop).
*   **File Naming Conventions:** Files are generally well-named following a feature-sliced or layered architecture (e.g., `components/screen/booking`, `store/api`, `data/coulmns`, `types`).
*   **Console Logging for Debugging:** Frequent additions and removals of `console.log` statements throughout the changes indicate active debugging during development.

## 9:50:23 PM
The code changes primarily focus on implementing and refining a document upload and display feature within a React frontend application, using RTK Query for API interactions and Material-UI for data grids.

### File-Specific Updates:

**1. `c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\store\api\uploadDocumentApi.ts` (Timestamp: 11/7/2025, 5:20:16 PM)**
*   This file introduces a Redux Toolkit Query API slice named `uploadDocumentApi`.
*   It defines three API endpoints:
    *   `uploadDocument`: A mutation for `POST` requests to `/api/documents/upload`, expecting `FormData`. It includes `Authorization` and `authtype` headers from `localStorage`.
    *   `downloadDocument`: A mutation for `GET` requests to `/api/documents/download/{refType}/{documentName}`, returning a `Blob`. It also includes `Authorization` and `authtype` headers.
    *   `getPdfParseData`: A query for `GET` requests to `/api/documents/get-pdf-parse` with URL parameters, using `getAppHeaders()`.
*   Exports corresponding hooks (`useUploadDocumentMutation`, `useDownloadDocumentMutation`, `useGetPdfParseDataQuery`).

**2. `c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\components\screen\upload-docs\uploadDocs.tsx`**
*   **Initial State (11/7/2025, 5:21:15 PM):** Implements a `ConfirmLineBookingForm` component for document uploading. It features a drag-and-drop area, file input, and displays uploaded files. It uses `formik` for submission and `useUploadDocumentMutation` to send files as `FormData` to the backend. Initially, it uses hardcoded `tableDummyData` for an `AppGrid`.
*   **Integration with API Data (11/7/2025, 5:24:08 PM):** The `AppGrid` component was updated to display actual data fetched from `useGetPdfParseDataQuery` (`getPdfParseData?.data`) instead of the dummy data. The `uniqueId` prop for the grid was also changed from `"id"` to `"ser_no"`, and the `count` prop was linked to `getPdfParseData?.total`.
*   **Minor UI Changes (11/7/2025, 6:00:45 PM - 6:01:08 PM):** A temporary `kk` string was accidentally appended and then removed from the displayed file name in the uploaded files list, indicating minor UI adjustments or debug attempts.

**3. `c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-frontend\src\data\coulms\upload-doc-coulmn.tsx`**
*   **Initial State (11/7/2025, 5:23:08 PM):** Defines `UPLOAD_TAB_COLUMNS` for a data grid.
    *   "File" column (`pdf_pa`): Renders a direct download link for the file.
    *   "JSON" column (`json_pa`): Renders a clickable element that, upon click, stringifies the JSON data present in the row (`params.row.json_pa`) and downloads it as a `.json` file.
    *   "Date And Time" column (`process_date_time`).
*   **JSON Column Evolution (11/7/2025, 5:32:50 PM, 5:33:56 PM, 6:30:30 PM, 8:15:39 PM, 8:22:38 PM, 8:28:57 PM, 8:29:24 PM, 8:30:05 PM, 8:30:18 PM):** This column underwent several changes:
    *   **5:32:50 PM:** Changed to fetch JSON data from a URL specified in `params.row.json_pa` (handling both full and relative paths) and then download it.
    *   **5:33:56 PM:** Reverted back to stringifying and downloading the `data` directly from `params.row.json_pa`, similar to the initial state.
    *   **6:30:30 PM:** Re-implemented fetching JSON data from a URL specified in `params.row.json_pa` and downloading it. The `console.log` added earlier was removed.
    *   **8:15:39 PM:** The "File" column's field name changed from `pdf_pa` to `file`, and "Date And Time" from `process_date_time` to `date_time`. The JSON column reverted *again* to stringifying and downloading `data` directly, reintroducing a `console.log`.
    *   **8:22:38 PM - 8:30:18 PM:** Subsequent changes consistently refined the JSON column to fetch data from `jsonUrl` (from `params.row.json_pa`) and download it. The field name for the "File" column (`pdf_pa` vs `file`) and the "Date And Time" column (`date_time` vs `process_date_time`) were juggled multiple times, eventually settling on `pdf_pa` for the file URL and `process_date_time` for the date field by the last entry.
*   **File Column Update (11/7/2025, 6:05:35 PM):** The "File" column's `renderCell` logic was modified to handle different file types. If the file is a `.json`, it fetches and downloads the JSON content; otherwise, it opens/downloads the file in a new tab. This change was later simplified in subsequent entries, reverting to a direct download link without special JSON handling in the "File" column, implying the JSON handling moved back to the "JSON" column.

### Timestamps of Significant Changes:

*   **11/7/2025, 5:20:16 PM:** Definition of the core API endpoints for document upload, download, and PDF parse data retrieval.
*   **11/7/2025, 5:24:08 PM:** Integration of the `AppGrid` in `uploadDocs.tsx` to display live data from the `getPdfParseDataQuery` instead of static dummy data.
*   **11/7/2025, 5:32:50 PM - 8:30:18 PM:** Frequent and significant iterations on the "JSON" column logic in `upload-doc-coulmn.tsx`, repeatedly switching between directly processing in-row data and fetching JSON from a URL before downloading. The final state at 8:30:18 PM uses the URL-fetching approach for JSON download.

### Patterns and Recurring Elements:

*   **RTK Query:** Consistent use of Redux Toolkit Query for defining and consuming API services, emphasizing mutations for upload/download and queries for data retrieval.
*   **Authentication:** Both upload and download operations include `Authorization` and `authtype` headers retrieved from `localStorage`, indicating a standard authentication mechanism across document-related API calls.
*   **File Handling:** The application consistently handles file uploads via drag-and-drop or file input, appending necessary metadata (`document_type`, `ref_type`, `remark`, `ref_id`, `uploaded_by`) to `FormData` before submission.
*   **Dynamic Data Grids:** The `AppGrid` component is used to display dynamic data, with a clear pattern of shifting from dummy data to real API-fetched data (`getPdfParseData`).
*   **JSON Download Logic:** The "JSON" column's implementation repeatedly attempts to provide a download functionality. It shows a back-and-forth between embedding JSON data directly in the table row versus storing a URL to the JSON file and fetching it dynamically for download. The latter approach appears to be the final stable implementation.
*   **Column Name Fluctuations:** The field names for the "File" and "Date And Time" columns in `upload-doc-coulmn.tsx` (`pdf_pa` vs `file`, `process_date_time` vs `date_time`) changed multiple times, suggesting refinement or correction in data mapping.
*   **Future Timestamps:** All timestamps are set to 11/7/2025, which is a future date. This is an unusual but consistent pattern across the log, possibly indicating a planned release date or placeholder values.

## 9:50:40 PM
The provided log details changes across several backend service and utility files, primarily focusing on data retrieval, management, and API route definitions.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\docsParseService.ts`** (Timestamp: 11/7/2025, 5:03:25 PM):
    *   The `getAllPdfParse` function (which appears to be a misnomer, as it handles vendor data) was updated to retrieve vendor details with pagination, sorting, and filtering by `vendor_name`, `status`, and `email`.
    *   It now efficiently fetches and attaches `document_upload` records for all retrieved vendors in a single batch query.
    *   Added post-processing for filter options, specifically cleaning up empty/null emails and converting status values to "Active" / "Inactive" for a more user-friendly frontend display.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\pdfParseService.ts`** (Multiple timestamps, with significant changes at 11/7/2025, 6:15:41 PM, 6:29:11 PM, 6:40:33 PM, and 8:16:08 PM):
    *   **Initial Implementation (5:06:48 PM, 5:06:59 PM, 5:07:04 PM):** The `getAllPdfParse` function was introduced to query the `AI_Parse_Document` table using a `QueryBuilder` for pagination and sorting by `process_date_time`. It was refined to include the total count in the response.
    *   **Attempt to Include Full Paths (6:15:41 PM):** Modified `setColumns` to `CONCAT` relative `pdf_pa` and `json_pa` fields with base URLs (from `process.env.UPLOAD_DOC_PATH` and `/uploads/`) to generate full file paths in the API response. Error logging was switched from `logger.info` to `logger.error`.
    *   **Partial Revert (6:25:14 PM):** Reverted the column concatenation and error logging back to the earlier state, using `setColumns(["*"])` and `logger.info`.
    *   **Switch to Direct SQL (6:29:11 PM):** Abandoned `QueryBuilder` for `getAllPdfParse` and rewrote the function to use direct SQL queries with explicit `LIMIT`, `OFFSET`, `ORDER BY`, and `CONCAT` for file paths (using `/envosys_uploads/`). `logger.error` was reintroduced for the catch block.
    *   **Refinement of Direct SQL Paths (6:40:33 PM, 6:40:55 PM):** The hardcoded `CONCAT` paths were updated to `http://localhost:8080/uploads/`. There was a brief change to select `id` instead of `ser_no` at 6:40:33 PM, which was immediately reverted to `ser_no` at 6:40:55 PM. The `ORDER BY` clause was hardcoded to `process_date_time DESC`.
    *   **Final Revert to QueryBuilder (8:16:08 PM, 8:32:03 PM):** The function was reverted back to using the `QueryBuilder` approach, similar to the initial implementation, including `count` in the response and `logger.info` for errors. This indicates an iterative process of finding the best implementation strategy.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\constants\Paths.ts`** (Significant changes at 11/7/2025, 5:15:03 PM):
    *   Introduced a new `Notes` base path with `/getAll` and `/add` routes.
    *   The `Document.GetPdf` path was renamed from `/get-pdf` to `/get-pdf-parse`.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\documentUploadRoutes.ts`** (Timestamp: 11/7/2025, 5:08:07 PM):
    *   Added a new GET route `Paths.Document.GetPdf` (later renamed to `Paths.Document.GetPdfParse` in `Paths.ts`) to serve `getAllPdfParse` from `pdfParseService`.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\linebookingService.ts`** (Timestamp: 11/7/2025, 5:14:34 PM, 5:14:48 PM):
    *   The `create` function for line bookings was implemented, enabling the creation of multiple line bookings within a single transaction.
    *   It handles insertions into `line_booking_master`, `line_booking_equipments`, and `other_services_master`.
    *   The booking status is dynamically set based on the presence of `do_number` and associated party information.
    *   A significant portion of the `getAll` function remains commented out, suggesting it's either under development or temporarily disabled.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\util\optionQuery.ts`** (Timestamp: 11/7/2025, 5:14:43 PM):
    *   A new `OptionQuery` class was introduced to centralize methods for generating autocomplete/dropdown options for various entities like Port, GST Location, Users, Vessel Voyage, Customer Name, Line Name, Vendor, and Party.
    *   Utility functions `buildOptionQuery` and `buildQueryOptions` were added for generic query building and option handling.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\vendorServices.ts`** (Timestamp: 11/7/2025, 5:59:15 PM):
    *   The `getAll` function for vendors was implemented with advanced filtering, pagination, and sorting capabilities, including fetching related `document_upload` records. It also had commented-out logic for fetching notes.
    *   The `addVendor` function was enhanced to handle the creation of new vendors, including the processing and saving of associated `uploadDocuments` and `notes` within a transaction, alongside audit logging. A duplicated code block for notes processing was present.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\index.ts`** (Significant change at 11/7/2025, 6:34:36 PM):
    *   Updated the main API router to include routes for `Audit` and the newly added `Notes` functionality.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\server.ts`** (Timestamp: 11/7/2025, 6:34:51 PM):
    *   The Express server setup was configured to serve static files from `/root/node/envosys/envosys_bucket/ai/upload` under the `/uploads` endpoint.
    *   Increased the JSON and URL-encoded body size limits to 10MB.
    *   Enabled comprehensive CORS settings.
    *   Integrated `morgan` for HTTP request logging.
    *   Set up specific routes (`/temp-auth/login`, `/api/users/add`) to be accessible before the main authentication middleware.

### Patterns and Recurring Elements:

*   **QueryBuilder Utility:** A `QueryBuilder` utility is consistently used across multiple services (`docsParseService.ts`, `pdfParseService.ts`, `vendorServices.ts`) for constructing dynamic SQL queries, demonstrating a structured approach to database interaction, especially for pagination, sorting, and filtering.
*   **Pagination and Sorting:** `page`, `perPage`, `offset`, `orderBy`, `orderField`, and `orderDirection` are standard query parameters processed in `getAll` type functions across various services.
*   **Robust Error Handling:** All service functions employ `try...catch` blocks, logging errors using a `logger` utility, and returning `HttpStatusCodes.INTERNAL_SERVER_ERROR`. Complex write operations use database transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) for data integrity.
*   **Associated Data Management:** Services handling primary entities (like vendors, line bookings) are designed to also manage associated child data, such as `uploadDocuments` and `notes`, indicating a holistic approach to entity management.
*   **API Route Consistency:** `Paths.ts` acts as a central configuration for API endpoints, which are then systematically registered in `index.ts`, ensuring uniform routing.
*   **Iterative Development:** The `pdfParseService.ts` file shows a clear pattern of iterative development, including attempts, reverts, and re-implementations of logic (e.g., direct SQL vs. QueryBuilder, dynamic URL construction), reflecting an active development and refinement process.
*   **Autocomplete/Search Options:** The introduction of `optionQuery.ts` signifies a dedicated effort to provide robust and reusable autocomplete/search functionality for various data types across the application.