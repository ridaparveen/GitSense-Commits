...
## 11:42:57 PM
The recent code changes primarily focus on enhancing the analytics and reporting capabilities of the `t360-backend` application, specifically within the `analytics` module.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.view.controller.ts` (Timestamp: 10/28/2025, 1:58:56 PM)**
    *   Introduced a new `logAudit` helper function to centralize audit logging for dashboard creation, updates, and deletions.
    *   The `create`, `update`, and `deleteView` functions now utilize `logAudit`, which serializes `previous_value` and `new_value` to JSON before storing them in the `analytics_view_log` table.
    *   `update` now explicitly fetches `oldData` to log the previous state of `dashboard_metrics`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\export.query.ts` (Significant changes between 10/29/2025, 1:52:59 PM and 10/29/2025, 9:30:56 PM)**
    *   A new report type, `"Container Shipped (MBL Wise)"` (and `Containers Shipped (Mbl Wise)`), was added to the `bookingQuery` function, enabling export of MBL-wise container shipment data.
    *   The queries for `"Container Shipped (MBL Wise)"` reports evolved significantly over multiple commits. Initially, it was a direct `SELECT` of columns. By the end of the log, the plural `"Containers Shipped (Mbl Wise)"` report was restructured into a Common Table Expression (CTE) to first derive `final_delivery_date` and `quarter` (based on dynamic 4-month and 8-month intervals relative to `NOW()`) and then perform an aggregation (`COUNT`, `SUM`) grouped by month, specifically calculating "This Quarter" and "Last Quarter" container counts. The singular version was updated to provide detailed raw data for export, including the derived `final_delivery_date` and `quarter` columns.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.metric.controller.ts` (Significant change at 10/28/2025, 3:49:50 PM and 3:50:57 PM)**
    *   A new `saveSize` function was introduced to persist layout dimensions (`columnSpan`, `rowSpan`) for individual metrics within a user's dashboard view. This information is stored in a `size` JSONB column in the `dashboards` table.
    *   The `dashboardEndPointList` function was updated to retrieve and return these saved `rowSpan` and `columnSpan` values alongside report names and URLs.
    *   Minor debugging `console.log` added to `getMetricList`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\mbls.query.ts` (Significant changes between 10/28/2025, 5:05:54 PM and 10/29/2025, 12:17:47 PM)**
    *   The `mblQueryBuilder` was refactored: an initial `ContainerShippedMblWise` report type was renamed to `TotalShipments`.
    *   A new `ContainerShippedMblWise` report type was introduced (10/28/2025, 5:05:54 PM) which calculates container counts per quarter, dynamically determining "Current Quarter" and "Last Quarter" based on `NOW()`. This query uses a CTE (`completed_shipments`).
    *   Further refinements (10/29/2025, 10:22:22 AM onwards) modified the quarter calculation to define "This Quarter" and "Last Quarter" based on 4-month intervals, returning monthly aggregated container counts within these periods for stacked bar charts.
    *   The `customizer` object is updated with `calType` and `chartType` metadata (e.g., `"avg"`, `"StackedBar"`) and `dateColumns` (e.g., `['Month', 'TEU']`) to reflect the report's characteristics.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\others.controller.ts` (Significant changes between 10/28/2025, 5:09:46 PM and 10/29/2025, 1:17:31 PM)**
    *   The dispatch logic within `getTableDataReports` was adjusted to incorporate the new `ContainerShippedMblWise` report, initially directing it to `mblQueryBuilder`.
    *   Subsequently, references to `ContainerShippedMblWise` within this controller were commented out or moved (10/29/2025, 1:14:39 PM and 1:17:31 PM), suggesting a re-evaluation of its handling or a temporary disabling. For example, it was briefly moved into the large `bookingQueryBuilder` block before being commented out.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.export.controller.ts` (Significant changes between 10/29/2025, 1:36:20 PM and 10/29/2025, 9:33:01 PM)**
    *   The `getExportData` function received a crucial fix (10/29/2025, 6:26:45 PM) in its `mainQuery` construction for booking reports: the date filter was correctly positioned within a `WHERE` clause, dynamically checking if the `customQuery` already contains a `WHERE` keyword to avoid SQL syntax errors.
    *   Specific export logic was implemented for the `"Containers Shipped (MBL Wise)"` report (10/29/2025, 9:33:01 PM). This involves building a base CTE and then aggregating data by month and quarter for export, mirroring the reporting logic found in `mbls.query.ts`.
    *   Debugging `console.log` statements were extensively added and some minor query formatting adjustments were made and reverted.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts` (Multiple minor updates from 10/29/2025, 11:23:09 AM onwards)**
    *   No significant functional changes, but multiple timestamps suggest frequent saving or minor edits to this file without altering core logic. The content remains consistent across these entries, mostly defining various booking-related queries, often involving `EXTRACT(DAY FROM date1 - date2)` for calculating day differences and grouping by time buckets (e.g., "Less than 1 week").

**Patterns and Recurring Elements:**

*   **Analytics Focus:** All changes are concentrated on developing and refining analytics reports and dashboard management.
*   **Dynamic SQL Generation:** SQL queries are dynamically constructed using string interpolation, incorporating parameters like `CompanyName`, date ranges (`fromDate`, `toDate`), and report-specific filters.
*   **Date and Time Manipulation:** Frequent use of SQL functions like `EXTRACT`, `DATE_TRUNC`, `TO_CHAR`, and `INTERVAL` for date calculations, filtering, and formatting.
*   **Data Validation and Filtering:** Repetitive checks for `IS NOT NULL` and `!= '0001-01-01 00:00:00 BC'` are common in SQL `WHERE` clauses for date fields to ensure data quality.
*   **User Context and Authorization:** `AuthenticatedRequest` and `getUserObject` are consistently used to retrieve user and company details for securing and personalizing report data.
*   **Error Handling:** `try...catch` blocks are consistently used to wrap asynchronous operations, logging errors with `logger.error` and returning `500 Internal Server Error` responses.
*   **Customizer Object:** A `customizer` object is frequently passed through functions to convey metadata about the current report (e.g., `dateRange`, `calType`, `chartType`, `dateColumns`, `mblwise`, `selectedColumns`).
*   **MBL/Container Distinction:** Many reports (`MblWise` suffixes) differentiate between master bill of lading (MBL) and container-level data, often influencing `DISTINCT ON` clauses in SQL queries.
*   **"LONG BEACH" to "LOS ANGELES" Normalization:** The `replaceColumnHandler` utility function is consistently applied in export functions to standardize port names, converting "LONG BEACH" to "LOS ANGELES" for both `pod` and `place_of_delivery` fields.
*   **Debugging:** Extensive use of `console.log` and `logger.info` throughout the code to trace query generation, request parameters, and intermediate data.