...
## 6:50:31 PM
The `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-thirdparty\src\routes\external.routes.ts` file, last updated on October 30, 2025, at 3:40:26 PM, defines a set of external API routes using Express.js.

Key updates in this file include:
*   **Controller Imports**: It imports five distinct controllers for `shipments`, `bookings`, `7501_duties`, `invoice`, and `isf` from the `../controller/external/v1/` directory, indicating a versioned external API structure.
*   **Route Definitions**:
    *   A `/test` GET route is set up to return a message confirming protected data access, including `req.companyname`.
    *   Specific GET routes are defined to retrieve all data for various resources:
        *   `/tracking` maps to `shipments.getAll`.
        *   `/booking` maps to `bookings.getAll`.
        *   `/7501_duties` maps to `duties.getAll`.
        *   `/invoice` maps to `invoice.getAll`.
        *   `/isf` maps to `isf.getAll`.

A recurring pattern observed is the use of `getAll` methods from dedicated controllers for each resource, consistently handling GET requests to fetch data for different external entities.