...
## 7:16:52 PM
The code changes primarily focus on the analytics and reporting functionality within a frontend application, specifically related to Excel export capabilities and report configuration.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\methods.js`**

*   **10/31/2025, 10:23:35 AM:** The `downloadReportSpreadsheet` function, responsible for generating Excel reports, was present with initial logic for handling various `StackedBar` chart types, including "Cargo-Ready Date to ETD (Stacked Bar)", "Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", "Average Transit Time per Month (Days)", and "Cargo-Ready Date to ETD (Stacked Bar +/- 10 Days)". The implementation for "Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", and "Average Transit Time per Month (Days)" initially included a temporary `updatedData` mapping that was partially commented out for "Average Transit Time per Month (Days)".
*   **10/31/2025, 10:26:57 AM:** The `downloadReportSpreadsheet` function was updated. The temporary `updatedData` mapping for "Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", and "Average Transit Time per Month (Days)" was refactored. The logic for these reports was standardized to directly map data based on `periodFilter` (month, week, quarter), making it consistent with how other stacked bar reports handle time-series data.
*   **10/31/2025, 12:30:38 PM & 12:31:21 PM:** Minor debugging `console.log` statements (`"====hhjjjk", headerCopy` and `"jjjj", item`) were added within the `downloadReportSpreadsheet` function. These changes were likely for temporary inspection of report data during development.
*   **10/31/2025, 1:28:39 PM:** A functional change occurred within `downloadReportSpreadsheet`, where the report name check for "Containers Shipped (Mbl Wise)" was briefly modified to "Containers Shipped". This suggests an attempt to rename or generalize the report handling.
*   **10/31/2025, 1:36:18 PM:** The previous change was reverted, and the report name check in `downloadReportSpreadsheet` was restored to "Containers Shipped (Mbl Wise)". This indicates the change was either a test or incorrect and was rolled back.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\config\analytics.js`**

*   **10/31/2025, 12:33:31 PM:** This file defines constants crucial for analytics, including `DEFAULT_ANALYTICS_ENDPOINTS`, `EXCEL_EXPORT_COLUMNS` (listing fields for Excel export), `CargoReadyToETD10Days` (specific columns for a report), and `ANALYTICS_REPORT_HEADER` (defining column headers for various reports). Many report headers are defined for "Container Wise", "MBL Wise" (Master Bill of Lading), and different metrics like CBM, TEU, KGS, and Transit times.
*   **10/31/2025, 1:36:25 PM:** No discernible changes were made to the provided content of this file at this timestamp.
*   **10/31/2025, 2:45:55 PM:** A temporary debugging change was introduced in `EXCEL_EXPORT_COLUMNS`, where the `name` for the `completed_date` key was altered from "Completed Date" to "Completed Date123".
*   **10/31/2025, 2:46:58 PM:** The debugging change to `completed_date` was reverted, restoring its `name` to "Completed Date".

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\data\report_options.js`**

*   **10/31/2025, 2:47:32 PM:** This entry shows the file defining various options for scheduling and filtering reports, such as `scheduleOptions` (Specific Date and Time, Daily, Weekly, Monthly), `filterByColumnOption` (a comprehensive list of date columns for filtering), `timeOptions` (for hours, minutes, AM/PM), `timeOptionsDaily`, `weekOptions`, and `monthOptions`. There were no subsequent changes for this file in the provided log.

**Key Patterns and Recurring Elements:**

*   **Analytics Report Generation:** The primary focus across all changes is the robust generation and export of analytics reports, particularly in Excel format.
*   **Debugging and Iteration:** There's a clear pattern of adding temporary debugging statements (`console.log`) and making and then reverting small, specific changes (like report names or column labels), indicating an active development and testing cycle.
*   **Stacked Bar Chart Specificity:** A recurring theme is the detailed and specific handling of data mapping for "StackedBar" chart types within the Excel export logic, especially when dealing with different time granularities (month, week, quarter).
*   **Centralized Configuration:** The `analytics.js` file serves as a central configuration hub for report endpoints, exportable columns, and display headers, demonstrating an organized approach to managing report metadata.
*   **Date and Time Sensitivity:** The presence of a `weekDateFormater` function and extensive date/time-related configuration options underscores the importance of time-series analysis in the application's analytics capabilities.

## 7:17:10 PM
The code changes primarily revolve around the analytics export functionality, specifically in how SQL queries are constructed and executed, and how data is processed.

**File-specific updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.export.controller.ts`**:
    *   **Dynamic Query Construction**: There's an iterative development process for handling complex SQL queries. Initially, the controller introduced logic (at 10/31/2025, 1:45:46 PM) to determine if a `customQuery` was a full SQL statement (e.g., starting with `WITH`), in which case it was used directly. This logic was briefly removed (1:48:05 PM) and later re-introduced to handle `WITH` queries differently from partial queries requiring wrapping in a `SELECT * FROM (...)` statement (3:02:56 PM onwards).
    *   **Date Filtering**: A consistent date filtering mechanism (`AND ${filterBy} >= '${fromDate}'::timestamp AND ${filterBy} < '${toDate}'::timestamp + INTERVAL '1 day'`) is applied to most generated SQL queries. An important refinement (2:53:46 PM) prefixed the `filterBy` column with `m.` (`m.${filterBy}`) to ensure correct table alias resolution.
    *   **Port Name Normalization**: The `replaceColumnHandler` function, which maps 'LONG BEACH' to 'LOS ANGELES' for `pod` and `place_of_delivery` fields, was introduced and remained consistently active throughout the changes.
    *   **Debugging**: Numerous `console.log` statements were added and subsequently commented out or removed between 2:34:52 PM and 4:17:38 PM, indicating active debugging of the query generation and data transformation stages.
    *   **Minor Logic Fluctuations**: Brief, quickly reverted changes occurred, such as toggling `mblwise` check condition (`mblwise === ''` vs `mblwise === 'true'`) and a temporary inversion of the `isBooking` flag (`booking === 'false'`).

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\export.query.ts`**:
    *   This file acts as a central repository for SQL query templates (`bookingQuery`) for various analytics reports.
    *   **"Containers Shipped (MBL Wise)" Report Instability**: The SQL query for the "Containers Shipped (MBL Wise)" report underwent significant and frequent rewrites. It changed multiple times between:
        *   A concise `WITH shipment_data AS (...) SELECT ... FROM shipment_data` structure (1:49:49 PM).
        *   A direct, verbose `SELECT DISTINCT ON (...)` listing many columns (1:55:32 PM, 3:18:14 PM).
        *   A complex multi-CTE aggregation for monthly counts across quarters (2:22:48 PM, 3:02:24 PM, 6:02:35 PM).
        *   Simplified `EXTRACT(DAY FROM ...)` calculations (3:39:12 PM, 3:47:48 PM, 6:13:31 PM, 6:34:50 PM).
    *   **Stacked Export Logic for "Containers Shipped"**: The conditional `stackedExport` logic within this report's query was also frequently modified. It sometimes used `m.departure_date` for filters and `m.t49_vessel_actualdeparted_date - a.bookingdate` for grouping (potential inconsistency), and at other times correctly used `m.delivery_customer_date` for filtering (4:03:28 PM) or was entirely removed (5:46:56 PM, 6:02:35 PM, 6:37:05 PM).
    *   **New Report Types**: Several new reports were added over time (starting 2:23:26 PM and 6:28:44 PM), including "Booking Date to Estimated Departure", "Booking Date to POD Arrival", and their "MBL Wise" and "Stacked Bar" variants, all focusing on calculating date differences.
    *   **"Average Transit Time" Simplification**: The "Average Transit Time per Month (Days)" query was changed from a complex CTE-based aggregation to a simpler `EXTRACT(DAY FROM ...)` calculation (6:16:02 PM).
    *   **Hardcoded Values**: Instances of hardcoded company names (`'sp richards company'`) and specific date ranges were temporarily introduced and then corrected to use dynamic variables or removed (3:02:24 PM, 3:14:37 PM, 4:51:16 PM, 4:52:14 PM, 6:13:31 PM).

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\utils\analytics.ts`**:
    *   This file provides utility functions used by the analytics controllers.
    *   **Date Utility Enhancements**: The `dateFilterOnExport` function was finalized (2:47:46 PM) to ensure date validity, format valid dates using `moment.js` to "MM/DD/YYYY", and set invalid dates to `null`. The list of `dateKeys` to be processed by this function was expanded to include `inv_date` (6:19:14 PM).
    *   **Centralized Query Helpers**: Functions like `replaceValueInQuery` (for conditional column value replacement), `getFilterQueryByDisplayValue` (for dynamic SQL `GROUP BY`, `ORDER BY`, `dateTrunc`, and `SELECT` clauses based on display type), and `getExcelSelectedColumns` (to define report-specific columns) were defined and maintained.
    *   **User Object Handling**: The `getUserObject` function was updated to correctly retrieve user and company information, including specific logic for 'ADMIN' access (2:47:46 PM).

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts`**:
    *   **New File for Modular Queries**: This file was introduced (4:29:05 PM) to modularize booking-related query generation into a `bookingQueryBuilder` function. This indicates a move towards more structured, programmatic SQL query generation for charts.
    *   **Detailed Chart Queries**: It contains complex SQL queries, often using multiple CTEs and `CASE` statements to categorize data into time ranges (e.g., 'Less than 1 week') for stacked bar charts, and calculating percentages.
    *   It relies on utility functions from `../../../utils/analytics` for common tasks.

**Timestamps of significant changes:**

*   **10/31/2025, 1:45:46 PM**: Initial detailed SQL handling and `replaceColumnHandler` definition in `core.export.controller.ts`.
*   **10/31/2025, 1:55:32 PM - 2:22:48 PM**: Major rewrites and structural changes to "Containers Shipped (MBL Wise)" query in `export.query.ts`.
*   **10/31/2025, 2:23:26 PM**: Significant expansion of report types in `export.query.ts`.
*   **10/31/2025, 2:47:46 PM**: Centralization of analytics utility functions (date handling, column selection) into `utils\analytics.ts`.
*   **10/31/2025, 2:53:46 PM**: Added `m.` prefix to `filterBy` in `core.export.controller.ts` for clearer SQL aliasing.
*   **10/31/2025, 3:02:56 PM**: Re-establishment of conditional `WITH` query handling in `core.export.controller.ts`.
*   **10/31/2025, 4:29:05 PM**: Introduction of `booking.query.ts` for programmatic query building.
*   **10/31/2025, 6:19:14 PM**: Addition of `inv_date` to `dateFilterOnExport` in `utils\analytics.ts`.
*   **10/31/2025, 6:34:50 PM - 6:37:05 PM**: Final simplifications and removal of complex `stackedExport` logic for "Containers Shipped (Mbl Wise)" in `export.query.ts`.

**Patterns and recurring elements:**

*   **Modularization of Query Logic**: There's a clear evolution from embedding complex query logic directly in controllers to separating it into dedicated query files (`export.query.ts`, `booking.query.ts`) and further into dynamic query builders.
*   **Iterative Refinement**: Specific reports, notably "Containers Shipped (MBL Wise)", undergo frequent refactoring, suggesting ongoing development, optimization, or resolution of complex requirements. This includes changes to SQL structure (CTEs vs. direct SELECT), aggregation methods, and conditional filtering.
*   **Consistent Data Cleaning/Standardization**: Date validation and formatting, along with port name replacement, are consistently applied across the analytics module.
*   **Conditional Logic for Stacked Reports**: Many report queries include conditional blocks (`${stackedExport === "true" ? ... : ...}`) to apply date-based filtering and grouping specifically for stacked bar visualizations, typically categorizing date differences into ranges (e.g., "Less than 1 week", "1-2 weeks").
*   **User and Company Context**: Queries consistently use `${CompanyName}` (derived from user context) to filter data, ensuring multi-tenancy.
*   **Debugging Practices**: Frequent use and eventual cleanup of `console.log` statements during development cycles is evident.

## 8:16:49 PM
The log details a series of iterative changes across several TypeScript files related to analytics data export functionality within a backend application (`t360-backend`). The updates primarily occurred on **October 31, 2025**.

**File-Specific Updates:**

1.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.export.controller.ts`**
    *   **Initial State (1:45:46 PM)**: The `getExportData` function was responsible for retrieving analytics data, distinguishing between "booking" and "non-booking" reports. It included logic for `mblwise` queries and conditionally wrapping custom SQL queries. It also featured a `replaceColumnHandler` to standardize "LONG BEACH" to "LOS ANGELES" for port-related columns.
    *   **Query Structure Refinements (1:48:05 PM, 3:02:56 PM, 3:15:01 PM, 3:21:35 PM, 3:23:45 PM)**: There were several cycles of modifying how `mainQuery` is constructed, specifically regarding whether CTE-based (`WITH`) or `SELECT`-starting custom queries are wrapped in an outer `SELECT * FROM (...)` statement. The logic repeatedly shifted between special handling for `WITH` clauses and a unified wrapping approach.
    *   **Filtering Logic Adjustments (2:53:46 PM, 3:02:56 PM)**: An attempt was made to prefix the `filterBy` parameter with `m.` (e.g., `m.${filterBy}`) in the generated SQL, indicating the `filterBy` column was expected from the `mtr_tracking_data` table. This change was later reverted, suggesting the prefixing might be handled elsewhere or was unnecessary.
    *   **Logical Bug Introduction and Reversion (2:58:56 PM - 2:59:33 PM)**: A brief but critical bug was introduced where `isBooking` was set to `booking === 'false'`, inadvertently flipping the entire report logic. This was quickly corrected back to `booking === 'true'`.
    *   **Debugging Statements (2:34:52 PM, 2:37:06 PM, 3:04:53 PM, 4:17:05 PM, 4:17:38 PM)**: Numerous `console.log` statements were added and subsequently commented out, indicating active debugging sessions during development.

2.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\export.query.ts`**
    *   **Initial Report Definitions (1:49:49 PM)**: This file was created to house specific SQL query snippets for various analytics reports, including time-based calculations and conditional logic for stacked exports. It introduced a `getMonthValue` helper.
    *   **"Containers Shipped (MBL Wise)" Evolution (1:55:32 PM, 2:22:48 PM, 3:14:37 PM, 3:18:14 PM, 3:39:12 PM, 3:45:39 PM, 3:47:48 PM, 3:50:18 PM, 3:51:32 PM, 3:58:05 PM, 4:03:28 PM)**: This specific report's query underwent significant and frequent rewrites. It fluctuated between being a complex, self-contained CTE-based aggregated query and a simpler, partial query snippet expecting further composition by the controller. It also saw changes in what was being counted (containers vs. MBLs), adjustments to date filtering columns (e.g., `m.delivery_customer_date` vs. `m.departure_date`), and temporary hardcoding of company names and date ranges for testing purposes.
    *   **"Average Transit Time per Month (Days)" Evolution (3:14:37 PM, 3:18:14 PM, 6:34:50 PM, 6:35:51 PM)**: Similar to the "Containers Shipped" query, this report's SQL also transitioned between being a partial snippet and a full, aggregated CTE query.
    *   **New Report Additions (2:23:26 PM, 6:28:44 PM)**: New report types, "Booking Date to Actual Departure (MBL Wise)", "Booking Date to POD Arrival", "Booking Date to POD Arrival (MBL Wise)", and "Booking Date to POD Arrival (Stacked Bar)", were incrementally added.

3.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\utils\analytics.ts`**
    *   **Initial Utilities (2:47:46 PM)**: This file was introduced to centralize utility functions such as `defaultDateRange` (for default date filters), `replaceValueInQuery` (for data normalization like "LONG BEACH" to "LOS ANGELES"), `getUserObject` (for user and company context), `getFilterQueryByDisplayValue` (for dynamic SQL `GROUP BY`/`ORDER BY` clauses), `getExcelSelectedColumns` (for selecting columns based on report type), and `dateFilterOnExport` (for date validation and formatting).
    *   **Expanded Date Handling (6:19:14 PM)**: The `dateFilterOnExport` function was updated to include `inv_date` in the list of keys whose values are checked for valid dates and formatted, indicating a new date field requiring standard processing.

4.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts`** (New File)
    *   **New Modular Query Builder (4:29:05 PM)**: This file was newly created, introducing `bookingQueryBuilder`. Its purpose is to modularize the generation of complex SQL queries specifically for booking-related analytics reports. It defines an interface `QueryBuilderI` for query parameters and uses `getColumnsByCategory`, `getFilterQueryByDisplayValue` from `analytics.ts`. This indicates a strategic shift towards separating query logic from the core controller.

**Patterns and Recurring Elements:**

*   **Dynamic SQL Generation**: A core pattern is the dynamic construction of SQL queries based on user-provided parameters (`reportName`, `CompanyName`, `stackedMonth`, `startWeek`, `endWeek`, `quarter`, `stackedGroup`, `displayBy`, `columns`, `filterBy`).
*   **Date Filtering and Formatting**: Date-related operations are prominent, including default date ranges, `EXTRACT` for time units, `DATE_PART` for differences, and a dedicated `dateFilterOnExport` utility to validate and format dates consistently across exported data.
*   **Port Data Normalization**: The consistent replacement of 'LONG BEACH' with 'LOS ANGELES' in `pod` and `place_of_delivery` columns highlights a specific data quality or reporting requirement.
*   **Conditional Logic for Report Types**: Extensive `if/else if` statements are used to tailor SQL queries and report configurations based on the `reportName` and other parameters (e.g., `isBooking`, `stackedExport`).
*   **Refactoring and Modularity**: The repeated restructuring of SQL queries within `export.query.ts` and the introduction of `booking.query.ts` suggest an ongoing effort to improve the modularity, maintainability, and clarity of the analytics query generation system.
*   **Debugging Focus**: The frequent addition and commenting out of `console.log` statements throughout the development flow indicates a highly iterative and debug-driven development process.

## 8:17:03 PM
The provided log details changes across three JavaScript files within an analytics frontend component, all occurring on October 31, 2025.

### File-Specific Updates:

1.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\methods.js`**
    *   **10/31/2025, 10:23:35 AM**: This file, responsible for analytics-related methods, included functions for text truncation (`TruncateText`), metric package handling (`metricsPackageHandler`), chart options, report formula calculation (`reportFormula`), date formatting (`weekDateFormater`), and a comprehensive `downloadReportSpreadsheet` function. The `downloadReportSpreadsheet` function already contained extensive logic to generate Excel reports for various chart types, including `StackedBar` reports like "Cargo-Ready Date to ETD", "Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", and "Average Transit Time per Month (Days)".
    *   **10/31/2025, 10:26:57 AM**: The `downloadReportSpreadsheet` function was updated. Specifically, the data mapping logic for `StackedBar` reports ("Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", "Average Transit Time per Month (Days)") was refined. It moved from a general `updatedData` mapping to a more structured conditional approach based on `periodFilter` (month, week, quarter), mirroring the logic used for "Cargo-Ready Date to ETD (Stacked Bar)".
    *   **10/31/2025, 12:30:38 PM & 12:31:21 PM**: Minor changes involved adding `console.log` statements within the `downloadReportSpreadsheet` function (`"====hhjjjk"`, `"jjjj"`) for debugging purposes.
    *   **10/31/2025, 1:28:39 PM**: A functional change was introduced in `downloadReportSpreadsheet` where the report name condition `item.reportName === "Containers Shipped (Mbl Wise)"` was altered to `item.reportName === "Containers Shipped"`.
    *   **10/31/2025, 1:36:18 PM**: The change made at 1:28:39 PM was reverted, restoring the report name condition back to `item.reportName === "Containers Shipped (Mbl Wise)"`.

2.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\config\analytics.js`**
    *   **10/31/2025, 12:33:31 PM**: This configuration file defined default analytics endpoints, columns for Excel exports (`EXCEL_EXPORT_COLUMNS`), specific columns for "CargoReadyToETD10Days", and headers for various analytics reports (`ANALYTICS_REPORT_HEADER`). The `EXCEL_EXPORT_COLUMNS` included an entry for `completed_date` with the name `"Completed Date"`.
    *   **10/31/2025, 1:36:25 PM**: No noticeable functional changes were present in this entry; the code remained identical to the previous timestamp.
    *   **10/31/2025, 2:45:55 PM**: A temporary change was observed in `EXCEL_EXPORT_COLUMNS`, where the display name for the `completed_date` key was modified from `"Completed Date"` to `"Completed Date123"`.
    *   **10/31/2025, 2:46:58 PM**: The temporary change was reverted, and the `completed_date` display name in `EXCEL_EXPORT_COLUMNS` was changed back to `"Completed Date"`.

3.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\data\report_options.js`**
    *   **10/31/2025, 2:47:32 PM**: This file, appearing only once in the log, contains static data for UI options, including report scheduling options (`scheduleOptions`), column filtering options (`filterByColumnOption`), and detailed time, week, and partial month options.

### Timestamps of Significant Changes:

*   **10/31/2025, 10:26:57 AM**: Refinement of Excel export logic for stacked bar charts in `methods.js`.
*   **10/31/2025, 1:28:39 PM**: A brief, quickly reverted change to a report name condition in `methods.js`.
*   **10/31/2025, 2:45:55 PM**: A temporary, quickly reverted modification to a display name in `analytics.js`.

### Patterns and Recurring Elements:

*   **Focused Development**: All changes occurred on a single date, indicating a focused development session.
*   **Excel Export Logic Refinement**: A significant portion of the changes in `methods.js` revolved around enhancing and debugging the `downloadReportSpreadsheet` function, particularly for handling `StackedBar` chart data across different time period filters. This suggests ongoing work to improve the robustness and flexibility of analytics report generation.
*   **Debugging Activities**: The temporary insertion of `console.log` statements in `methods.js` and the `Completed Date123` placeholder in `analytics.js` followed by their quick removal/reversion points to active debugging and testing during the development process.
*   **Configuration Management**: The `analytics.js` and `report_options.js` files serve as centralized configurations, storing report headers, export columns, API endpoints, and various dropdown options, highlighting a structured approach to managing application data and UI elements.
*   **Minor Adjustments and Reversions**: Several changes were quickly introduced and then reverted (e.g., the report name condition in `methods.js`, the `Completed Date123` in `analytics.js`), indicating iterative development and possibly experimentation or error correction.

## 8:18:04 PM
The provided log details changes to a single file: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`.

**Key Information from Changes:**

*   **File-Specific Updates**: The `AnalyticsToolbar.tsx` component, which manages analytics dashboard functionalities like filtering, dashboard selection, and PDF export, underwent several refactorings related to component imports and minor error handling.

*   **Timestamps of Significant Changes**:
    *   **10/31/2025, 7:26:44 PM**: The initial state of the component as recorded in this log, including imports from `@mui/material` for UI components (`Button`, `IconButton`), Redux integration, PDF generation logic using `jspdf` and `html2canvas`, and various custom components. The PDF export functionality already incorporates a logo and aspect ratio handling.
    *   **10/31/2025, 7:37:39 PM**: The `Button` component import was changed. The component stopped importing `Button` from `@mui/material` and instead began importing it from `@components/kit/button`. This signifies a shift towards using a custom or internal UI kit's Button component.
    *   **10/31/2025, 7:40:02 PM**: A new import `MdOutlineFileDownload` from `react-icons/md` was added, likely intended for a download icon, possibly for the PDF export button.
    *   **10/31/2025, 7:54:52 PM**: An import for `toast` from `react-hot-toast` was added, and a `toast.error` notification was introduced to the `catch` block of the `downloadDashboardPDF` function for improved user feedback on PDF generation failures.
    *   **10/31/2025, 7:55:07 PM**: The `toast` import and its usage within the `downloadDashboardPDF`'s `catch` block were reverted, removing the error notification. This indicates a temporary experimental change that was quickly undone.
    *   **10/31/2025, 7:58:57 PM**: Similar to the `Button` change, the `IconButton` component import was refactored. The component stopped importing `IconButton` from `@mui/material` and switched to importing it from `@components/common/icon-button`.

*   **Patterns or Recurring Elements**:
    *   A recurring pattern is the **migration from Material-UI components to custom/internal component library equivalents**. This is evident with the `Button` and `IconButton` components being switched from `@mui/material` to `@components/kit/button` and `@components/common/icon-button`, respectively.
    *   The `downloadDashboardPDF` function remains largely stable throughout the log, with only a brief, reverted attempt to add a `react-hot-toast` notification for error feedback. Its core logic involving `jsPDF` and `html2canvas` for capturing and exporting the dashboard content (identified by `#metric-board` ID) with a logo remains consistent.
    *   Many of the logged timestamps (especially those between major changes) show identical code, suggesting frequent saves without substantive modifications, or automated saves during development.

## 9:17:03 PM
The code changes primarily affect the analytics export functionality of the T360_Api backend.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.export.controller.ts`**

This file, responsible for the `getExportData` endpoint, underwent frequent modifications between 10/31/2025, 1:45:46 PM and 10/31/2025, 4:17:38 PM.
*   **Initial Implementation (1:45:46 PM)**: Included logic to dynamically construct SQL queries (`mainQuery`) based on request parameters (e.g., `fromDate`, `toDate`, `reportName`). It had a conditional block to handle Complex SQL (CTE-based queries starting with "WITH") by directly using their results, otherwise applying `dateFilterOnExport` and `replaceColumnHandler`. The `replaceColumnHandler` function was present to standardize "LONG BEACH" port values to "LOS ANGELES".
*   **Query Handling Refinements and Reversions (1:48:05 PM - 3:02:56 PM)**:
    *   The conditional logic for CTE-based queries was repeatedly commented out and re-enabled, or modified to check for "SELECT" statements, indicating an iterative process to determine the best way to handle different query structures.
    *   A brief logical error (`isBooking` incorrectly assigned `booking === 'false'`) was introduced and quickly corrected.
    *   The `mblWiseQuery` condition was temporarily changed from `mblwise === 'true'` to `mblwise === ''`.
    *   The `mainQuery`'s `ORDER BY` clause was briefly changed to reference `m.${filterBy}` before reverting.
    *   Crucially, at **3:02:56 PM**, the conditional logic to explicitly handle CTE-based queries (`customQuery.trim().toUpperCase().startsWith('WITH')`) was re-enabled and uncommented, allowing such queries to be used directly without additional wrapping.
*   **Debugging (2:34:52 PM - 4:17:38 PM)**: Multiple `console.log` statements were added to debug various query parameters (`mblWiseQuery`, `customQuery`, `filterBy`, `reportName`) and the processed data (`filteredData`, `mergedFilterData`), most of which were later commented out.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\export.query.ts`**

This file, containing SQL query definitions for various analytics reports, saw significant and fluctuating changes between 10/31/2025, 1:49:49 PM and 10/31/2025, 6:37:37 PM.
*   **Dynamic Query Generation**: The `bookingQuery` function builds SQL query strings based on `reportName` and other parameters, often including filters for `CompanyName` and date ranges.
*   **"Containers Shipped (MBL Wise)" Report Instability**: This specific report query underwent several major rewrites:
    *   Initially a CTE-based aggregation (1:49:49 PM).
    *   Changed to a direct `SELECT DISTINCT ON` with all columns (1:55:32 PM).
    *   Switched to a complex three-CTE aggregation focused on quarterly sums (2:22:48 PM).
    *   Reverted to a simpler one-CTE aggregation with hardcoded company names and dates (3:02:24 PM).
    *   Made dynamic again with parameter placeholders for company name and dates, and added `stackedExport` logic (3:14:37 PM).
    *   Changed to a simple `EXTRACT(DAY FROM ...)` without aggregation (3:39:12 PM), seemingly a regression.
    *   Modified to include `COUNT(DISTINCT m.mblno)` and an extensive `GROUP BY` clause (3:48:05 PM - 3:50:18 PM).
    *   Updated `stackedExport` conditions to use `m.delivery_customer_date` for filtering (4:03:28 PM).
    *   Finally, reverted back to the complex CTE structure for "This Quarter"/"Last Quarter" aggregation with hardcoded values for a specific company name and date range (6:02:35 PM), which suggests a non-generic solution was reintroduced for a specific context.
*   **"Average Transit Time per Month (Days)" Regression**: At **6:30:44 PM**, this query was significantly simplified, removing its CTE structure and the average calculation, changing it from a monthly average report to one that would output raw day differences. This indicates a loss of original functionality.
*   **New Report: "Booking Date to POD Arrival (Stacked Bar)" (6:28:44 PM)**: A new query was added to calculate the day difference between booking and POD arrival dates, categorizing them into time-based buckets for a stacked bar chart.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts`**

This is a new file introduced at **10/31/2025, 4:29:05 PM**, providing a more structured way to build specific booking-related queries.
*   **Modular Query Builder**: The `bookingQueryBuilder` function dynamically constructs various booking-related analytics queries.
*   **Consistent Query Structure**: Queries generally use CTEs (`WITH t AS (...)`) for intermediate calculations and aggregate results, often including percentage calculations based on a `max_value` CTE.
*   **Stacked Bar Logic**: Includes detailed logic for reports like `BookingDatetoActualDepartureStackedBar`, categorizing results into time difference buckets (e.g., 'Less than 1 week', '1 - 2 weeks').
*   **Parameterization**: Queries are designed to use dynamic parameters for `CompanyName`, `fromDate`, `toDate`, making them reusable.
*   No significant functional changes after its initial creation were observed in the provided log.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\utils\analytics.ts`**

This utility file provides common functions for analytics, showing minor, stable changes.
*   **Initial State (2:47:46 PM)**: Included utilities like `defaultDateRange`, `replaceValueInQuery` (to map 'LONG BEACH' to 'LOS ANGELES'), `getUserObject`, `getFilterQueryByDisplayValue` (for SQL grouping/ordering), `getExcelSelectedColumns` (for dynamic column selection), `isValidDate`, `dateFilterOnExport` (for validating and formatting dates), and `EXCEL_EXPORT_COLUMNS` (for defining export columns).
*   **Minor Enhancement (6:19:14 PM)**: The only functional change was adding `'inv_date'` to the `dateKeys` array in `dateFilterOnExport`, ensuring that invoice dates are also validated and formatted upon export.

**Patterns and Recurring Elements:**

*   **Iterative Query Refinement**: There's a clear pattern of frequent, sometimes conflicting, modifications to SQL queries, especially in `export.query.ts`. This suggests ongoing development, debugging, and potentially refactoring of complex analytics queries.
*   **Dynamic SQL Generation**: All files demonstrate a strong reliance on building SQL queries dynamically based on user input and report types, using string interpolation with variables like `CompanyName`, `fromDate`, and `toDate`.
*   **Date Handling**: Consistent date validation (`isValidDate`) and formatting (`dateFilterOnExport` to 'MM/DD/YYYY') are applied across various date columns for export data.
*   **CTE Usage**: CTEs (Common Table Expressions) are frequently used in the SQL queries, often for calculating intermediate values or structuring aggregated reports. The handling of these CTEs in the main controller (`core.export.controller.ts`) was a point of recurring modification.
*   **Stacked Bar Logic**: A specific pattern for "Stacked Bar" reports involves calculating time differences and then grouping them into predefined duration buckets.
*   **Debugging Practices**: Extensive use of `console.log` statements for debugging query components and data flow is evident, indicating active development and troubleshooting.
*   **Data Standardization**: The `replaceColumnHandler` is a notable recurring element for data standardization (e.g., port name mapping).

## 9:17:09 PM
The provided log details changes across three files within the `t360-frontend` project, primarily focusing on analytics and reporting functionalities. All activity occurred on 10/31/2025.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\analytics\methods.js`**

*   **10/31/2025, 10:23:35 AM:** This timestamp marks an initial, detailed implementation of the `downloadReportSpreadsheet` function. It includes extensive logic for formatting data into a `reportArray` for Excel export, particularly for various "StackedBar" chart types. Reports like "Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", and "Average Transit Time per Month (Days)" initially included an `updatedData` mapping for "This Quarter" and "Last Quarter" fields, though the "Average Transit Time" report had this mapping commented out.
*   **10/31/2025, 10:26:57 AM:** A significant refinement was made to the `downloadReportSpreadsheet` function. The explicit `updatedData` transformation block for "Containers Shipped (Mbl Wise)", "Total Spend per Month ($)", and "Average Transit Time per Month (Days)" reports was removed. Instead, these stacked bar reports now directly map data (`month`, `This Quarter`, `Last Quarter`) based on the `periodFilter` (month, week, quarter), streamlining the data preparation logic to be consistent with other stacked bar reports.
*   **10/31/2025, 12:30:38 PM & 12:31:21 PM:** Minor changes were introduced, adding `console.log` statements (`"====hhjjjk"`, `"jjjj"`) within the `downloadReportSpreadsheet` function, indicating active debugging or inspection of `headerCopy` and `item` variables during development.
*   **10/31/2025, 1:28:39 PM:** The `reportName` for the stacked bar chart "Containers Shipped (Mbl Wise)" was briefly modified to "Containers Shipped".
*   **10/31/2025, 1:36:18 PM:** The `reportName` change from the previous entry was reverted, restoring "Containers Shipped (Mbl Wise)". This suggests the prior change was temporary or an error in report labeling.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\config\analytics.js`**

*   **10/31/2025, 12:33:31 PM & 1:36:25 PM:** These entries show the configuration for analytics endpoints, Excel export columns, and report headers. No functional changes are evident between these two timestamps in the provided snippets. The `ANALYTICS_REPORT_HEADER` object is comprehensive, defining headers for various CBM, TEU, KGS, and Transit Time reports.
*   **10/31/2025, 2:45:55 PM:** A temporary alteration was made in the `EXCEL_EXPORT_COLUMNS` array. The `name` for the `completed_date` key was changed from "Completed Date" to "Completed Date123".
*   **10/31/2025, 2:46:58 PM:** This change reverted the `completed_date` `name` in `EXCEL_EXPORT_COLUMNS` back to its original "Completed Date".

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\data\report_options.js`**

*   **10/31/2025, 2:47:32 PM:** This entry introduces or confirms the definitions for various options used in report scheduling and filtering. This includes `scheduleOptions`, `filterByColumnOption` (listing numerous date-related fields for report filtering, e.g., 'File Open Date', 'ETA POD', 'Live ETA'), `timeOptions`, `timeOptionsDaily`, `weekOptions`, and `monthOptions`.

**Patterns and Recurring Elements:**

*   **Excel Export Focus:** A dominant pattern is the continuous refinement of Excel export logic, especially within `methods.js`. This involves detailed handling of different chart types and report names, with a strong emphasis on "StackedBar" charts and their data aggregation based on time periods (month, week, quarter).
*   **Debugging and Iteration:** The frequent addition/removal of `console.log` statements and the temporary modification/reversion of a column name (`"Completed Date123"`) indicate an active development and debugging cycle, focusing on correctness and consistency of reporting data.
*   **Analytics Configuration:** The `analytics.js` file serves as a central configuration point for defining analytics endpoints, export columns, and report-specific headers, showing a structured approach to managing reporting metadata.
*   **Date and Time Handling:** The `methods.js` file includes utility functions like `weekDateFormater` and logic for date-based calculations, while `report_options.js` defines various date and time related filtering and scheduling options, highlighting the importance of temporal data in these analytics reports.