...
## 5:49:20 PM
**File: `src\common\constants\Paths.ts`**
*   **10/27/2025, 10:56:34 AM**: The `Analytics` section was initially defined with `/create` for dashboard creation.
*   **10/27/2025, 1:50:28 PM - 1:51:04 PM**: Multiple rapid updates consolidated the `Analytics` dashboard-related endpoints. `Analytics.Create`, `Analytics.Update`, and `Analytics.DeleteView` were all changed to point to the unified path `"/dashboards"`. This indicates a refactoring to manage dashboard CRUD operations under a single base endpoint.
*   **10/27/2025, 2:53:56 PM**: A new `Audit` route was added with a `Get` endpoint at `"/audit"`, introducing audit logging capabilities.

**File: `src\routes\SettingRoutes.ts`**
*   **10/27/2025, 10:58:29 AM**: A new router `SettingRoutes` was created. It defined several GET endpoints (`/format`, `/grid-config`) which were all mapped to `getAll` imported from `bookingService`. This appears to be a placeholder or an incorrect initial implementation, as a `bookingService` function is unlikely to be suitable for settings routes.

**File: `src\services\setting-service.ts`**
*   **10/27/2025, 11:00:09 AM**: The `setting-service.ts` file was created, implementing functions for managing format settings (`getFormat`, `addFormat`, `updateFormat`) and grid settings (`updateGridSettings`, `getGridSettings`). These functions interact with `format_settings` and `grid_settings` tables in the database.
*   **10/27/2025, 11:01:18 AM**: The `addFormat` function was refactored to use the `IAuthReq` type instead of `AuthenticatedRequest`, and unused imports (`AuthenticatedRequest`, `HttpError`) were removed, improving code consistency and cleanliness.

**File: `src\utils\analytics.ts`**
*   **10/27/2025, 11:47:35 AM**: This new utility file was introduced, containing several helper functions for analytics and reporting:
    *   `defaultDateRange`: Provides a standard date range (current date to one year prior).
    *   `replaceValueInQuery`: A utility for conditionally replacing column values in SQL queries (e.g., changing 'pod' values).
    *   `getUserObject`: Retrieves user details, potentially adjusting `usercode` and `companyname` based on admin access.
    *   `getFilterQueryByDisplayValue`: Dynamically generates SQL `GROUP BY`, `ORDER BY`, `date_trunc`, and `SELECT` clauses for reports based on display units (month, week, quarter).
    *   `getExcelSelectedColumns`: Logic to select specific database columns for Excel exports based on the report name and whether it's a booking report, including special handling for "Cargo-Ready Date to ETD" and "BL Release KPI".
    *   `dateFilterOnExport`: Filters and formats date strings within data, converting invalid dates to `null` and formatting valid ones to 'MM/DD/YYYY'.
    *   `EXCEL_EXPORT_COLUMNS`: A constant array defining display names and keys for Excel export columns.

**File: `src\services\analyticsExportService.ts`**
*   **10/27/2025, 11:48:28 AM**: The `analyticsExportService.ts` file was created with a primary function `getExportData`. This function handles complex logic for generating analytical export data based on various query parameters (date ranges, report names, booking status, etc.). It distinguishes between booking and non-booking reports, applies `DISTINCT ON` logic for various scenarios, includes specific handling for "TEU MBLwise" reports to deduplicate containers, and performs post-query data processing like date filtering and column value replacement (e.g., 'LONG BEACH' to 'LOS ANGELES').
*   **10/27/2025, 11:50:55 AM - 11:57:07 AM**: Several minor updates involved adding `console.log` statements throughout the `getExportData` function to assist with debugging request parameters, user codes, generated SQL queries, and raw query results.
*   **10/27/2025, 12:38:23 PM**: The `try...catch` block was moved to encompass the main logic of `getExportData`, and minor stylistic changes were made to variable declarations and string concatenation.
*   **10/27/2025, 12:39:31 PM**: The structural changes from the previous timestamp (moving the `try...catch` and variable declarations) were reverted, returning the function structure to its earlier state, indicating a quick refinement or a rollback of an experimental change.

**File: `src\routes\index.ts`**
*   **10/27/2025, 2:54:23 PM**: A new route `Paths.Audit.Get` (`/audit`) was added but incorrectly mapped to `getAllCompanyDetails`.
*   **10/27/2025, 2:57:08 PM**: The audit route was corrected, importing `getAllAudit` from `auditService.ts` and correctly mapping `Paths.Audit.Get` to this function.

**File: `src\routes\common\types\audit.type.ts`**
*   **10/27/2025, 2:56:11 PM**: This new file was created to define an `AuditQueryType` enum, centralizing the types of audit logs supported in the system (e.g., USER, EXPANSE_CODE, ANALYTICS_VIEW).

**File: `src\services\auditService.ts`**
*   **10/27/2025, 2:55:41 PM**: The `auditService.ts` file was created, containing a `getAll` function responsible for fetching audit logs based on a `type` and `serial_id`. It dynamically selects the audit table and reference column based on the provided `type`. A specialized logic path exists for `ANALYTICS_VIEW` audit, which retrieves metric names from another table based on stored IDs.
*   **10/27/2025, 2:56:27 PM**: The `getAll` function was renamed to `getAllAudit`, and `AuditQueryType` was explicitly imported. The export statement was updated to reflect the new function name.
*   **10/27/2025, 2:57:00 PM**: The `getAllAudit` function was changed to be a named export (`export const getAllAudit = ...`), removing the default export object.

**File: `src\services\coreViewService.ts`**
*   **10/27/2025, 5:13:11 PM**: This new service was added to manage user-defined analytics dashboards/views. It includes:
    *   `getAll`: Fetches existing dashboard titles for a user.
    *   `create`: Allows users to create new dashboards, including checking for existing views and storing associated metrics IDs. It also calls a `logAudit` function.
    *   `update`: Modifies existing dashboards, updating metrics and title, and logs the change via `logAudit`.
    *   `deleteView`: Deletes a dashboard and logs the deletion.
    *   `logAudit`: A dedicated helper function to record audit events in the `analytics_view_log` table, including user ID, action, menu title, previous and new values (as JSON strings), and the user who made the change.
*   **10/27/2025, 5:23:52 PM**: In the `create` function, the `logAudit` call was commented out, and a `console.log` was added, suggesting temporary disabling of audit logging for dashboard creation, possibly for debugging or an ongoing development iteration.

**Summary of Patterns and Recurring Elements:**
*   **Consistent Timestamp (10/27/2025)**: All changes occurred on the same day, indicating a focused development effort or a single deployment snapshot.
*   **Active Debugging**: Multiple `console.log` statements were added and removed in `analyticsExportService.ts` and `coreViewService.ts`, pointing to active debugging sessions.
*   **Enhanced Analytics Functionality**: A significant portion of changes focused on building out a robust analytics feature set, including flexible data export, dynamic query generation, user-defined dashboards/views, and detailed audit logging for analytics actions.
*   **Database Interaction and ORM/Utility Usage**: Extensive use of `query`, `insertQuery`, and `updateQuery` from a common `database` utility, highlighting a direct SQL-based interaction pattern.
*   **Centralized Route Management**: The `Paths.ts` file serves as a central registry for API endpoints, with changes reflecting new feature introductions and endpoint consolidations.
*   **Refactoring and Code Organization**: Evident through import cleanups, type usage standardization (`IAuthReq`), function renaming, and changes in export styles, suggesting ongoing efforts to improve code quality.
*   **Comprehensive Audit Logging**: Introduction of a dedicated audit service and `AuditQueryType` enum, supporting detailed logging across various system entities and specifically for analytics view management, indicating a strong focus on data traceability and integrity.

## 5:49:48 PM
The changes log details a series of updates primarily focused on enhancing the analytics and reporting features of the T360-frontend-v2 application, with a strong emphasis on integrating Redux Toolkit for state management and API interactions, as well as improving type safety with TypeScript.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\settings-api.ts` (Latest Timestamp: 10/27/2025, 10:50:50 AM)**
    *   A new Redux Toolkit Query API slice, `settingsApi`, was introduced. It defines endpoints to `fetchFormat` and `updateFormat` settings, utilizing `Format` as a tag type for caching. This indicates the introduction of format-related data management.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\setting-slice.ts` (Latest Timestamp: 10/27/2025, 11:02:34 AM)**
    *   A new Redux slice, `settingsSlice`, was added to manage the application's formatting state. It includes a `setFormat` reducer and an `extraReducer` that listens for `settingsApi.endpoints.fetchFormat.matchFulfilled` to update the `format` state with fetched data, mapping API response keys to format values.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\store.ts` (Latest Timestamp: 10/27/2025, 2:52:39 PM)**
    *   The central Redux store configuration was updated multiple times. Initially, `settingsApi` and `settingSlice` were integrated. Later, an `auditDataApi` was correctly added to both `combineReducers` and `middleware` configuration, while also correcting a previous duplicate `settingsApi` entry.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\analytics\analytics-page.tsx` (Latest Timestamp: 10/27/2025, 11:28:42 AM)**
    *   This analytics screen component was modified to integrate `settingsState.format` into the `metricsPackageHandler` function, allowing analytics data to be displayed with user-defined formatting. Type definitions for `EndpointType`, `MetricData`, and `User` were added or refined. `useEffect` dependencies were improved for better data fetching control.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\metricBox.tsx` (Latest Timestamp: 10/27/2025, 4:11:50 PM)**
    *   This component, responsible for rendering individual metric charts, saw extensive typing additions (`ChartType`, `ChartView`, `MetricItem`, `MetricsBoxProps`, `EnlargeState`). It dynamically renders various chart types (Bar, Pie, Line, Stacked Bar, Gauge) and integrates `MetricToolbar` and `MetricFooter` for controls and actions. The `chartType` prop passed to `MetricToolbar` was strongly typed.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricToolbar.tsx` (Latest Timestamp: 10/27/2025, 3:49:16 PM)**
    *   The toolbar for individual metrics was significantly updated. It now imports and utilizes `DATE_FILTER_OPTIONS`, `PERIOD_FILTER_OPTIONS`, `VIEW_BY_FILTER_OPTIONS`, `DateRange`, `PopoverPanel`, and `SelectBoxLight`. The component's props and internal state handlers (`handleDateFilterChange`, `handleCustomFilterChange`) were explicitly typed (`MetricToolbarProps`), and the `useEffect` logic was refined for triggering `fetchSingleReport` when filters change.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricFooter.tsx` (Latest Timestamp: 10/27/2025, 4:23:30 PM)**
    *   This component manages Excel and PDF export for individual metrics, as well as full-screen view. It introduced `MetricFooterProps` and `ModalState` interfaces for type safety. Export functions (`handleExcel`, `handlePdf`) were refactored with `try...finally` blocks. The display of calculated values in a `Chip` now uses `applyFormatting` and checks if `calValue` is greater than 0 before rendering, improving data representation.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\analyticsContainer.tsx` (Latest Timestamp: 10/27/2025, 4:02:30 PM)**
    *   The main container for analytics metrics was updated to support drag-and-drop (`DndProvider`, `HTML5Backend`) and persistence for metric ordering (`useSaveSequenceMutation`) and resizing (`useSaveMetricSizeMutation`). It uses `metricsPackageHandler` and `settingsState.format` for metric data processing. Debugging `console.log` statements were added and removed.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\services\ApiManager.ts` (Latest Timestamp: 10/27/2025, 11:32:07 AM)**
    *   A new `analyticsExport` method was added to handle spreadsheet export, while comments around user settings API were observed.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\services\Endpoints.ts` (Latest Timestamp: 10/27/2025, 11:33:07 AM)**
    *   The `ANALYTICS_EXPORT` endpoint was added, providing a detailed query string for analytics data export.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\methods.ts` (Latest Timestamp: 10/27/2025, 4:14:38 PM)**
    *   This utility file received significant updates for type safety, introducing numerous interfaces for analytics data processing (e.g., `AnalyticsData`, `Customizer`, `MetricsPackageReturn`). The `metricsPackageHandler` function was typed, and `allChartOptions` and `chartOptionsByType` were defined. The `reportFormula` function received improved typing, and `downloadReportSpreadsheet` contains detailed logic for constructing Excel reports based on various filters and chart types.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx` (Latest Timestamp: 10/27/2025, 4:56:35 PM)**
    *   The global analytics toolbar was extensively modified. It gained PDF export functionality for the entire dashboard (including a logo), integrated `SelectCustomerPanel` for customer selection, and `ChipsTabView` for dashboard selection. A custom view management `AppDrawer` with `ViewContainer` was introduced. Type definitions for handler functions were added. Debugging `console.log`s were present, and a null check was added for the dashboard element during PDF export. The logo import path was adjusted multiple times.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx` (Latest Timestamp: 10/27/2025, 5:20:48 PM)**
    *   This component manages custom analytics views. It utilizes `useFetchDashboardOptionsQuery` and `useSaveDasboardMutation` to fetch, save, and delete user-defined dashboards. It features a checkbox manager for selecting up to 9 metrics per view and integrates `ThemeTabs` and `TreeViewContianer`. Importantly, audit functionality for views was re-enabled by importing and integrating `AuditViewDrawer` and `PopupAlert`. Button components were also standardized.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\TreeViewContianer.tsx` (Latest Timestamp: 10/27/2025, 1:40:50 PM)**
    *   A new component `TreeViewContainer` was created to display metrics in a hierarchical, selectable tree view, used within `ViewContainer` for custom dashboard creation.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\coulmns\audit-view-coulmns.tsx` (Latest Timestamp: 10/27/2025, 2:48:02 PM)**
    *   A new file defining `ANALYTICS_VIEW_AUDIT_COLUMNS` for displaying audit logs related to analytics views, including fields like "Previous Value" and "New Value" rendered via `RenderValuePopover`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\popupAlert.tsx` (Latest Timestamp: 10/27/2025, 2:48:57 PM)**
    *   A new generic `PopupAlert` component was created to display configurable alert dialogs, including title, message, severity, and confirmation/cancel actions.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\api\audit-api.ts` (Latest Timestamp: 10/27/2025, 2:50:49 PM)**
    *   A new Redux Toolkit Query API slice `auditDataApi` was created specifically for fetching audit logs, including cache invalidation and refetching logic.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\audit-view-drawer.tsx` (Latest Timestamp: 10/27/2025, 2:51:04 PM)**
    *   A new `AuditViewDrawer` component was implemented. It presents audit data in a Material-UI drawer with a searchable `DataGrid` interface, fetching data via `useFetchAuditQuery`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts` (Latest Timestamp: 10/27/2025, 3:26:14 PM)**
    *   New options arrays `PERIOD_FILTER_OPTIONS` and `VIEW_BY_FILTER_OPTIONS` were added to support diverse filtering capabilities in analytics reports.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\select-box.tsx` (Latest Timestamp: 10/27/2025, 3:31:52 PM)**
    *   The `SelectBox` components were significantly refactored for improved type safety (`SelectBoxLightProps`, `Option` interfaces) and to utilize Material-UI's `Select` and `MenuItem` components more effectively, ensuring better styling and functionality, including an optional helper text.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\SelectCustomerPanel.tsx` (Latest Timestamp: 10/27/2025, 3:53:02 PM)**
    *   A new `SelectCustomerPanel` component was introduced. It provides a searchable, paginated popover for selecting customers, debouncing API calls to `ApiManager.getPaginatedCompanyOptions` and dispatching `setCustomer` upon selection.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\settings\format\methods.ts` (Latest Timestamp: 10/27/2025, 4:23:38 PM)**
    *   New utility functions, `applyFormatting` and `revertToNumeric`, were added for handling number formatting (currency, decimals, commas). `applyFormatting` now includes a fallback for undefined format settings to ensure consistent formatting.

**Patterns and Recurring Elements:**

*   **Analytics Feature Focus:** A dominant pattern is the comprehensive development of the analytics dashboard, covering data fetching, display, user interaction (drag/resize, custom views), filtering, and export functionalities.
*   **Redux Toolkit Integration:** There's widespread adoption and refinement of Redux Toolkit for both API interactions (RTK Query) and managing local component state (slices).
*   **TypeScript for Type Safety:** A strong recurring theme is the introduction and refinement of TypeScript interfaces and types across almost all modified files. This indicates a deliberate effort to improve code quality, maintainability, and reduce potential errors.
*   **Export Functionality:** Both individual metric boxes and the overall analytics toolbar gained robust PDF and Excel export capabilities, leveraging `jspdf`, `html2canvas`, and `exceljs`.
*   **Modular Component Design:** Components like `MetricToolbar`, `MetricFooter`, `SelectCustomerPanel`, `ViewContainer`, `TreeViewContianer`, `PopupAlert`, and `AuditViewDrawer` demonstrate a modular approach to building the UI, enhancing reusability and separation of concerns.
*   **API Key Usage:** An API key (`apiKey: "nRKniUrJd6rjNCyvT8iRRHmG"`) is consistently used across various API calls, particularly for analytics and dashboard-related fetches.
*   **Debugging Practices:** Several `console.log` statements appear and are later removed or refined, indicating active debugging during development.
*   **UI/UX Enhancements:** Minor adjustments to button labels ("PDF1" to "PDF"), component sizing (`Chip` size), and visual styling (`SelectBoxLight`) reflect continuous iteration on the user interface and experience.

## 9:20:14 PM
The `core.export.controller.ts` file, last modified on October 27, 2025, at 12:35:46 PM, defines the `getExportData` Express controller responsible for generating and exporting various analytics reports.

**Key Updates and Functionality:**

*   **Unified Export Endpoint:** The `getExportData` function serves as a central point for both "booking" and "non-booking" related analytics reports, dynamically adjusting its logic based on the `booking` query parameter.
*   **Dynamic Query Generation:** The controller constructs SQL queries dynamically using helper functions like `bookingQuery` and `nonBookingQuery`, incorporating parameters such as `reportName`, company name, date ranges (`fromDate`, `toDate`), and specific filters (`mblwise`, `stackedExport`, `filterBy`).
*   **Column Management:** It utilizes `columnsQuery` to process requested columns and `saveColumns` to persist user-selected column configurations, associating them with the user and report name.
*   **Date Range Defaults:** If `fromDate` or `toDate` are missing from the request, the system automatically assigns a default date range.
*   **Distinct MBL/Container Handling (Booking Reports):** For booking reports, the `mainQuery` includes a `DISTINCT ON` clause that intelligently handles uniqueness either by MBL number (`LOWER(m.mblno)`) or a combination of `voyage`, `contno`, and container type, preventing redundant data.
*   **Special Logic for TEU MBLwise Reports (Non-Booking):** A specific filtering logic is implemented for "TEU MBLwise" non-booking reports. It iterates through the results, and for duplicate entries based on `voyage`, `contno`, and container type, it sets the `teu` field to `'-'`, effectively de-duplicating TEU counts.
*   **Location Standardization (`replaceColumnHandler`):** A utility function, `replaceColumnHandler`, is applied to the final dataset. It standardizes geographical data by replacing occurrences of 'LONG BEACH' (case-insensitive) with 'LOS ANGELES' in both the `pod` (Port of Discharge) and `place_of_delivery` fields. This change was an evolution from a previous version that only handled the `pod` field.
*   **Robust Error Handling and Logging:** The function is wrapped in a `try...catch` block to handle potential errors gracefully, logging issues and returning a 500 status. Extensive `logger.info` statements are present throughout the function to provide detailed insights into query construction, column processing, and execution flow.

**Patterns and Recurring Elements:**

*   **User Context Retrieval:** The `getUserObject` utility is consistently used to fetch user and company details necessary for report generation and column saving.
*   **Sequential Data Processing:** After executing the main SQL query, there's a recurring pattern of post-processing steps: `dateFilterOnExport` is applied for additional filtering, followed by `replaceColumnHandler` for data standardization.
*   **Dynamic SQL Generation:** The core logic frequently involves constructing SQL query strings based on a multitude of dynamic parameters, reflecting a flexible reporting system.