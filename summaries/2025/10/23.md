...
## 4:15:24 PM
The changes log details a significant development effort focused on enhancing an analytics module within the T360-V2 backend application, primarily occurring on **October 22nd, 2025**.

**File-Specific Updates:**

*   **`src\common\constants\Paths.ts` (10/22/2025, 12:19:46 PM):** This file received a substantial update, adding numerous new endpoints under a new `Analytics` object. These paths cover operations like getting analytics data, creating/updating/deleting custom views, retrieving dashboard endpoints and metrics lists, exporting data to Excel, and saving user-defined metric sequences and sizes. This indicates a major expansion of the analytics functionality.

*   **`src\routes\analyticsRoutes.ts` (Progressive changes from 12:20:38 PM to 4:40:33 PM):**
    *   Initially, the `AnalyticsRoutes` were configured with dashboard-related services (`getDashboardCardInfo`, `getDeliveryData`), suggesting a preliminary or incorrect setup.
    *   By 12:33:16 PM, it shifted to importing `getAll` from `coreViewService` and mapping `Paths.Analytics.Get` to it, removing the dashboard service dependency.
    *   Subsequent changes by 12:34:42 PM completed the integration of CRUD operations for analytics views (`create`, `update`, `deleteView`) from `coreViewService`.
    *   Later updates (around 4:13:43 PM to 4:40:33 PM) introduced imports from `coreMetricService` and `analyticsExportService`, mapping endpoints like `Paths.Analytics.DashboardEndPoints` to `dashboardEndPointList`, `Paths.Analytics.MetricsList` to `getMetricList`, `Paths.Analytics.SaveSequence` to `saveSequence`, `Paths.Analytics.SaveSize` to `saveSize`, and `Paths.Analytics.GetExcelReport` to `getExportData`. Duplicate `Paths.Analytics.Get` routes persist, possibly as placeholders or an oversight.

*   **`src\routes\index.ts` (10/22/2025, 12:21:23 PM):** This file was updated to integrate the new `AnalyticsRoutes` into the main API router, enabling access to the new analytics features.

*   **`src\models\common\types\analytics.ts` (Progressive changes from 12:27:34 PM to 4:36:01 PM):** This file saw continuous expansion, with new TypeScript interfaces being defined or augmented to support the growing analytics features:
    *   `CheckBoxDetails`: Defines the structure for checkbox selections (metrics).
    *   `MenuRow`: Expanded to include `menu_title`, `metricsId`, `dashboard_metrics`, and `size`.
    *   `DashboardItem`: Added for dashboard-related item details, including `id`, `name`, `end_point`, and `dashboard_name`.
    *   `MetricListDataI`: Defines the structure for metric list data, including `id`, `name`, `group`, `filter_by`, and an optional `checked` status.
    *   `MetricRow`: Introduced for simpler metric row structures with `id` and `name`. These types collectively improve code clarity and safety across the analytics services.

*   **`src\services\coreViewService.ts` (Progressive changes from 12:26:53 PM to 12:46:12 PM):** This service manages the creation, retrieval, update, and deletion of custom analytics views.
    *   Initial versions (12:26:53 PM) defined basic CRUD logic and `logAudit` for tracking changes, using direct string interpolation for SQL queries, which is a potential security risk.
    *   Updates (12:31:11 PM, 12:32:31 PM) introduced more specific request/response types and optional chaining for user properties.
    *   A significant improvement occurred at 12:46:12 PM, where SQL queries for `create` and `update` operations were refactored to use **parameterized queries** (`$1, $2, $3::int[]`), enhancing security against SQL injection. Named exports were also introduced.

*   **`src\services\coreMetricService.ts` (Progressive changes from 4:11:45 PM to 4:38:20 PM):** This new service handles operations related to analytics metrics.
    *   It includes `dashboardEndPointList` (to retrieve dashboard reports and their layout information), `getMetricList` (to fetch and categorize available metrics, with a "preload" feature to mark selected metrics for a dashboard).
    *   New functions `saveSequence` (to store the order of metrics in a dashboard) and `saveSize` (to store the display size/span of individual metrics) were added.
    *   `saveSequence` was notably refactored at 4:32:09 PM to use parameterized queries, similar to `coreViewService`, improving security.
    *   `saveSize` also received type refinements and error handling (4:38:20 PM) but some SQL queries still use string interpolation.

*   **`src\services\analyticsExportService.ts` (Progressive changes from 4:40:11 PM to 11:04:38 AM on 10/23/2025):** This new service provides functionality to export analytics data to Excel.
    *   The initial commit at 4:40:11 PM defined `getExportData` to handle dynamic SQL query generation based on report parameters, date ranges, and booking status. It also introduced a `replaceColumnHandler` for data transformations (e.g., 'LONG BEACH' to 'LOS ANGELES'). This version had inconsistent imports and used direct string concatenation for SQL queries.
    *   An important update at 4:47:46 PM corrected import paths for `query` and `bookingQuery`/`nonBookingQuery` to use consistent `@src` aliases, and improved type safety for `userCode`.
    *   Later timestamps (10/23/2025, 11:00:20 AM and 11:04:38 AM) show no functional changes, likely representing minor reformatting or saves. **This service continues to rely heavily on string interpolation for constructing dynamic SQL queries, leaving it vulnerable to SQL injection.**

*   **`src\utils\analytics.ts` (Progressive changes from 4:43:56 PM to 5:11:25 PM):** This new utility file centralizes helper functions for analytics.
    *   It provides `defaultDateRange`, `replaceValueInQuery` (for SQL string manipulation), `getUserObject` (for user context), `getFilterQueryByDisplayValue` (for dynamic SQL `GROUP BY`/`ORDER BY` clauses), `getExcelSelectedColumns` (for selecting columns for export), `isValidDate`, and `dateFilterOnExport` (for date formatting and filtering).
    *   It also contains a large `EXCEL_EXPORT_COLUMNS` array.
    *   The file shows progressive improvements in type safety (interfaces `User`, `UserRow`, `DataItem`) and clearer parameter types, especially around 5:09:52 PM.

*   **`src\common\util\export.query.ts` (Progressive changes from 4:46:51 PM to 6:05:51 PM):** This new utility file defines complex SQL query fragments.
    *   It includes `getMonthValue` for date conversion and a large `bookingQuery` function that generates highly dynamic SQL `SELECT` and `WHERE` clauses for various booking-related reports. The logic incorporates conditional filtering based on month, week, quarter, and stacked bar groupings.
    *   A recurring pattern in this file is the explicit checking for `IS NOT NULL` and `!= '0001-01-01 00:00:00 BC'` for date fields, indicating a common need to handle potentially invalid default date values from the database. **This file relies exclusively on string interpolation for building SQL queries.**

**Patterns and Recurring Elements:**

1.  **Analytics Module Focus:** The core theme is the development and refinement of a comprehensive analytics feature, encompassing custom view management, metric visualization, and data export.
2.  **Dynamic SQL Generation:** A prevalent pattern is the dynamic construction of SQL queries based on user inputs and report parameters. This is evident across `coreViewService`, `coreMetricService`, `analyticsExportService`, and `export.query.ts`.
3.  **SQL Injection Vulnerability Mitigation:** A significant improvement can be seen in `coreViewService` and `coreMetricService` (`saveSequence` function), where SQL queries were refactored to use parameterized queries, addressing initial SQL injection risks. However, parts of `coreMetricService` (`saveSize`), `analyticsExportService`, and `export.query.ts` continue to use string interpolation, indicating a mixed approach or pending refactoring.
4.  **Type Safety (TypeScript):** There's a strong recurring effort to enhance type safety through the introduction and refinement of interfaces (`CheckBoxDetails`, `MenuRow`, `DashboardItem`, `MetricListDataI`, `MetricRow`, `User`) and explicit type assertions in service functions.
5.  **Error Handling:** All service functions consistently implement `try...catch` blocks to log errors and return `500 Internal Server Error` responses.
6.  **Date and Data Transformation:** Specific utilities and logic are dedicated to handling date ranges, formatting dates (`moment.js` is introduced), and transforming data (e.g., replacing 'LONG BEACH' to 'LOS ANGELES'). The need to handle "bad" date values (`'0001-01-01 00:00:00 BC'`) in SQL queries is a notable recurring pattern.
7.  **Modularization:** The introduction of `coreViewService`, `coreMetricService`, `analyticsExportService`, and `analytics.ts` utilities demonstrates an attempt to modularize different aspects of the analytics functionality.