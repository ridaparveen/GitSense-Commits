...
## 5:47:35 PM
The provided code changes focus on enhancing analytics reporting and customer master data management within the `t360-backend` API, all occurring on December 30, 2025.

**Key Changes by File:**

1.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\transit.controller.ts` (Timestamp: 12/30/2025, 9:54:51 AM)**
    *   This file, specifically the `getTransitReports` function, has been updated to provide dynamic transit analytics.
    *   It now handles various query parameters like `columnName`, `fromDate`, `toDate`, `mblwise`, `viewBy`, `type`, and `reportName`.
    *   Default date ranges are applied if not explicitly provided in the request.
    *   It retrieves user-specific details (`companyname`, `usercode`) using `getUserObject`.
    *   A crucial update is the dynamic construction of `mblwiseCondition` which dictates whether reports are grouped by master bill of lading (`m.mblno`) or voyage (`m.voyage`).
    *   The `transitQueryBuilder` is used to construct the main SQL query, incorporating customizers, date ranges, company, and user information.
    *   The results are queried from the database and returned to the client. Error handling and logging are included.

2.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\code.controller.ts` (Timestamp: 12/30/2025, 10:23:43 AM)**
    *   This file contains comprehensive updates for customer master data management, including `getAllCustomer` and `createCustomer` functions.
    *   **`getAllCustomer`:**
        *   Implements pagination (`page`, `perPage`) and dynamic sorting (`orderBy`).
        *   It dynamically builds search conditions by checking `req.query` parameters against actual table columns in `PO_AccountMaster_Customer_Master`.
        *   The function enriches customer data by fetching related information from `account_master_shipper_MAP` (shipper mappings), `document_upload_details` (associated documents), and `Account_Master_Customer_MAP` (customer mappings), returning a combined JSON object.
        *   Utility function `chkStr` is used for data sanitization.
    *   **`createCustomer`:**
        *   Supports file uploads integrated into the customer creation process.
        *   It extracts a wide range of customer details from the request body.
        *   Includes logic to handle both new customer creation and modification of existing customers (`modetype`). If a customer `acode` exists, relevant old records (customer master, agent type, shipper map) are deleted before re-insertion.
        *   New account codes (`uacode`) are generated using `getAccountCode` if creating a new customer.
        *   Database transactions (`BEGIN`) are used to ensure data integrity during complex insert/update operations.
        *   It handles customer types (specifically 'CUSTOMER') by generating a subcode and inserting into `agent_po_account_master_type`.
        *   The function processes `mapping` data (parsed JSON) to insert multiple shipper mappings into `Account_Master_Shipper_MAP`, dynamically fetching `polcode`, `shippercode`, and `agentcode` from other master tables.
        *   The provided log for `createCustomer` is truncated before completion, specifically within the shipper mapping loop.

**Patterns and Recurring Elements:**

*   **Database Interactions:** Both files heavily rely on the `query` function for executing SQL operations (`SELECT`, `INSERT`, `DELETE`). The `createCustomer` function also demonstrates the use of database transactions (`BEGIN`) for multi-step operations.
*   **Dynamic Query Generation:** There's a clear pattern of constructing SQL queries dynamically based on user input or request parameters (e.g., `transitQueryBuilder`, `searchCondition`, `orderClause`, `mblwiseCondition`).
*   **Request Handling & Utilities:** Both controllers utilize `AuthenticatedRequest` and `Response` objects. They frequently access `req.query` for filtering, sorting, and reporting parameters, and `req.body` for data submission. Common utility functions like `getUserObject`, `chkStr`, `getAccountCode`, `getAccountSubCode` are leveraged across the backend.
*   **Error Management:** Standard `try...catch` blocks are consistently used to handle potential errors during processing, specifically database errors, with `logger.error` for logging and returning `500 Internal Server Error` responses.
*   **Data Enrichment:** The `getAllCustomer` function showcases a pattern of fetching core data and then enriching it by querying related tables, which is a common backend practice for comprehensive data presentation.
*   **Timestamps:** All changes are very close in time on 12/30/2025, suggesting a focused development effort or a single deployment cycle for these related functionalities.

## 5:48:07 PM
The file `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\tracking-stepper.tsx` underwent a series of iterative refinements between 3:04 PM and 3:23 PM on 12/30/2025. Key changes focused on adjusting the completion logic and percentage calculation within the shipment tracking timeline, particularly for "Port-Rail-Door" shipments. Specific updates included:
    *   Modifications to how `podCompleted` and `isRailVerified` booleans were determined, sometimes incorporating temporary hardcoded dates (`"2025-09-27"`) which were subsequently removed or redefined (e.g., `isRailVerified` changing to `Boolean(finalATA) || Boolean(trackingInfo.delivery_customer_date)`).
    *   Adjustments to the `percentage` calculation for Port of Loading (POL) and Port of Discharge (POD) steps, initially setting a 50% completion if not fully departed/verified, and later making this conditional on `trackingInfo.inland_rail_move === "Yes"`.
    *   Minor tweaks to icon rendering and `pinIcon` types, with one instance temporarily removing a "terminal" fallback, then reintroducing it.
    *   The `completed` state for POD in "Port-Rail-Door" scope was briefly tied to `isRailVerified`, then reverted, then re-tied to `isRailVerified`, and then reverted again. This indicates an experimental phase with the completion criteria.
    *   The overall pattern for this file is a series of small, back-and-forth adjustments to conditional logic influencing the visual representation of shipment progress, suggesting ongoing fine-tuning of the tracking stepper's accuracy.

The file `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\SelectCustomer.tsx` experienced substantial refactoring throughout the log, evolving between 3:49 PM and 5:29 PM on 12/30/2025.
*   Initially (`3:49 PM`), it was named `CustomerSelector` and implemented using MUI components (`Autocomplete`, `TextField`). It handled local storage for the selected customer and fetched customer options via an API with basic pagination logic.
*   By `3:53 PM`, the component was renamed to `SelectCustomerPanel`.
*   A significant refactor occurred around `3:59 PM`, moving away from MUI `Autocomplete` to a custom-built component using standard HTML elements and Tailwind CSS classes, introducing manual pagination (`page`, `total`, `itemsPerPage`). The `onSelect` callback was updated to pass only the `label` string. Robust error handling for `localStorage` was also added.
*   This custom implementation was briefly reverted to an MUI `Autocomplete` version around `4:08 PM`, removing explicit pagination from the component's UI but setting `pagination: false` for the API call and using `ITEMS_PER_PAGE = 50`. The `onSelect` callback's expected value for customer changed from `label` to `value`.
*   Another large revert at `4:10 PM` brought back the custom, Tailwind-styled component, reintroducing pagination state (`page`, `total`) and handlers, although the UI pagination component itself was still used from MUI (`Pagination`).
*   Subsequent changes were primarily cosmetic and functional refinements within the custom component structure:
    *   Minor padding adjustments (`px`, `py`) for options and loading indicators.
    *   Correcting typos in rendering `option.label`.
    *   Adding a `name` attribute to an `InputBox`.
    *   Around `4:25 PM`, the component was again refactored to use MUI components, explicitly setting `pagination: false` in the API call and using `ITEMS_PER_PAGE = 50`. Button styling was refined with MUI `sx` props.
    *   `4:48 PM` saw a switch from MUI `Button` to a custom `@components/kit/button`, introducing an `outline` prop for the cancel button.
    *   Between `4:55 PM` and `4:59 PM`, the UI underwent several visual changes, transforming the component from a simple editable pill to a "Customer" label alongside an editable pill input, with detailed inline styling using `sx` properties. Various `gap` and `margin` values were experimented with for alignment.
    *   A final major refactor around `5:01 PM` fully embraced a custom, Tailwind CSS-based implementation, removing all MUI components and replacing MUI icons with `react-icons/fi` (later `PencilIcon` from `@heroicons/react/16/solid`). This also introduced a `dropdownRef` for more precise click-outside detection.
    *   Further refinements included adjusting dropdown width, changing the dropdown header title, and implementing a `handleClearSearch` function for the search input, styled with specific Tailwind classes (`text-pink-700`, `hover:bg-pink-100`).
    *   The `ITEMS_PER_PAGE` constant was changed for debugging from 50 to 1, then back to 5.
    *   The `PencilIcon` received `title` attributes for accessibility/tooltip display and underwent multiple size class adjustments.
    *   Finally (`5:29 PM`), the main "Customer" label styling was standardized using a `paragraph-title` class.
The overall pattern for `SelectCustomer.tsx` demonstrates a clear development cycle involving multiple architectural shifts (MUI vs. custom Tailwind), continuous UI/UX enhancements, and functional tweaks related to customer selection, search, and local storage persistence.

The file `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\core\sidebar.tsx` had two entries around `3:53 PM` and `4:11 PM`. These changes were minor, primarily adapting to the renaming of the `CustomerSelector` component to `SelectCustomerPanel` and ensuring its correct import and conditional rendering based on the `ADMIN_V2` user role. This indicates that the `SelectCustomerPanel` is integrated into the application's main navigation sidebar.