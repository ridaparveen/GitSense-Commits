...
## 4:27:17 PM
The changes across these files primarily focus on enhancing and refining the analytics and data export functionalities of the T360_Api backend. The updates are concentrated in query generation, data processing, and column selection for various reports.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts`**
*   **Timestamp: 12/15/2025, 10:20:53 AM**
    *   This file defines the `bookingQueryBuilder` function, responsible for generating PostgreSQL queries related to booking analytics. It includes logic for calculating the average day difference between booking date and actual departure, for both container-level and MBL (Master Bill of Lading)-level data.
    *   It also generates queries for stacked bar charts, categorizing the day differences into predefined week ranges (e.g., 'Less than 1 week', '1 - 2 weeks').
    *   Queries are dynamically constructed based on `type`, `CompanyName`, `fromDate`, `toDate`, and `displayBy` parameters, with conditions to exclude invalid date entries.
*   **Subsequent timestamps between 10:27:45 AM and 3:46:50 PM on 12/15/2025**
    *   No functional code changes were introduced in this file across these numerous saves. The content remained identical to the first logged version.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\export.query.ts`**
*   **Timestamp: 12/15/2025, 12:01:30 PM**
    *   Introduced the `bookingQuery` function, which serves as a central repository for SQL query fragments used in various booking-related reports for export.
    *   Initializes queries for reports such as "Booking Date to Actual Departure" (both standard and stacked bar versions), "Carrier On-time delivery By carrier", "Average Transit time By lane", "Containers Shipped (Mbl Wise)", "Custom Booking", and "Booking Date to Estimated Departure".
*   **Timestamp: 12/15/2025, 12:19:23 PM**
    *   The SQL query for "Average Transit time By lane" was substantially refactored. The `WHERE` clause was expanded to include more explicit conditions for sea, rail, and truck legs of a shipment, utilizing `COALESCE` for date fields and `OR` for conditional inclusion of rail/truck leg data based on `inland_rail_move`.
*   **Timestamp: 12/15/2025, 2:05:36 PM**
    *   Further refinement to the "Average Transit time By lane" query. A `rail_ata IS NOT NULL` condition was added, and existing `COALESCE` conditions for rail departure dates were slightly adjusted.
*   **Timestamp: 12/15/2025, 2:14:07 PM**
    *   The "Average Transit time By lane" query was overhauled again, introducing clear comments for "SEA (mandatory)", "RAIL (only if inland)", and "TRUCK (optional but safe)" segments. The logical structure shifted to a top-level `OR` condition, allowing a record to be included if any of these legs met its criteria.
*   **Timestamp: 12/15/2025, 2:23:42 PM**
    *   The "Average Transit time By lane" query's `WHERE` clause was simplified, consolidating the conditions for sea, rail, and truck legs under a single `OR` clause to ensure data is included if relevant fields for any transport mode exist.
*   **Timestamp: 12/15/2025, 3:16:55 PM**
    *   The "Custom Booking" query was modified to filter by `LOWER(account_name) = '${CompanyName}'` instead of `LOWER(a.customer)`.
    *   An additional condition `AND m.mblno <> '' AND m.mblno IS NOT NULL` was added to the "Custom Booking" query.
    *   A new report type, "Booking Date to Actual Departure (MBL Wise)", was introduced with its corresponding SQL query fragment.
*   **Timestamp: 12/15/2025, 3:19:08 PM**
    *   The "Custom Booking" query's date check was changed from `a.bookingdate IS NOT NULL` to `a.created_at IS NOT NULL`.
*   **Subsequent timestamps at 3:40:23 PM and 3:40:32 PM on 12/15/2025**
    *   No functional code changes were introduced.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\utils\analytics.ts`**
*   **Timestamp: 12/15/2025, 12:10:06 PM**
    *   This file contains various utility functions for analytics processing, including date range generation (`defaultDateRange`), query value replacement (`replaceValueInQuery`), user object retrieval (`getUserObject`), filter query generation based on display preferences (`getFilterQueryByDisplayValue`), and column selection for Excel exports (`getExcelSelectedColumns`).
    *   `dateFilterOnExport` function is present to filter and format date strings, treating "0000-01-01T00:00:00.000Z" as invalid dates and formatting valid ones to "MM/DD/YYYY". The `dateKeys` array for this function included a broad set of date fields and `booking_status`.
    *   The `EXCEL_EXPORT_COLUMNS` array, listing all available export columns, is defined.
*   **Timestamp: 12/15/2025, 12:12:54 PM**
    *   The `dateKeys` array in `dateFilterOnExport` was expanded to include several new date-related fields such as `rail_ata`, `rail_etd`, `t49_raildeparted_date`, `vessel_actualarrived_date`, `vessel_actualdeparted_date`, `inland_rail_move`, `rail_destination`, and `rail_departed_pod`.
*   **Timestamp: 12/15/2025, 12:24:57 PM**
    *   The `otherColumns` string within `getExcelSelectedColumns` was updated to explicitly include `m.rail_ata` for selection in generic non-booking reports.
*   **Timestamp: 12/15/2025, 1:57:33 PM**
    *   The `dateKeys` array in `dateFilterOnExport` was modified. `booking_status` and `rail_destination` were removed from this array, indicating they are no longer treated as date fields needing specific validation or formatting in exports.
*   **Subsequent timestamps between 2:27:08 PM and 2:51:02 PM on 12/15/2025**
    *   No functional code changes were introduced in this file.

**File: `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.export.controller.ts`**
*   **Timestamp: 12/15/2025, 2:02:29 PM**
    *   This controller implements the `getExportData` endpoint, which orchestrates the data retrieval and export process. It dynamically handles report-specific queries using `bookingQuery` and `nonBookingQuery`.
    *   It retrieves parameters like `fromDate`, `toDate`, `reportName`, `mblwise`, and `booking` from the request.
    *   It uses `getUserObject` to get company context and applies `columnsQuery` and `saveColumns` (functions not provided in this log).
    *   A `replaceColumnHandler` function is defined to unify port names, specifically converting 'LONG BEACH' to 'LOS ANGELES' for `pod` and `place_of_delivery` fields.
    *   It includes special handling for "TEU MBLwise" reports to deduplicate container entries.
*   **Timestamp: 12/15/2025, 2:08:57 PM**
    *   A `console.log("excelExportData",excelExportData)` statement was added after fetching data for booking reports, likely for debugging purposes.
*   **Timestamp: 12/15/2025, 2:09:40 PM**
    *   The debugging log was slightly adjusted to `console.log("excelExportData",excelExportData.rows)`, making it more specific by logging only the `rows` property of the query result.

**Patterns and Recurring Elements:**
*   **Date-Based Reporting:** A significant portion of the changes revolves around handling and querying data based on dates (booking date, departure date, arrival date, cargo ready date), often involving calculations of day differences and filtering by date ranges.
*   **Report Customization:** The system supports various report types (`type` and `reportName` parameters), allowing for different data aggregations (e.g., average, stacked bar) and views (container-wise, MBL-wise).
*   **Data Cleaning and Transformation:** There's a consistent effort to clean data, such as handling invalid date values (like '0001-01-01 00:00:00 BC') and unifying common port names (e.g., "Long Beach" to "Los Angeles").
*   **Dynamic SQL Generation:** SQL queries are constructed dynamically based on user input and report type, which allows for flexible reporting but also necessitates careful management of conditional logic within the queries themselves.
*   **Debugging:** The addition of `console.log` statements points to active debugging during these development iterations.
*   **Modular Design:** The separation of concerns into query generation files (`export.query.ts`, `booking.query.ts`) and utility functions (`analytics.ts`) is evident.