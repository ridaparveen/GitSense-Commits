...
## 5:59:23 PM
The provided code changes primarily revolve around enhancing the backend's analytics and reporting capabilities, with a new feature for customer/agent autosearch options and significant updates to SQL query generation for various reports.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\customerAutosearchOptions.ts`**
    *   **Timestamp:** 12/18/2025, 3:54:52 PM
    *   This file introduces a new service function `getAllCustomerOptions`. It is designed to fetch distinct customer or agent names for autosearch functionalities. It dynamically queries either `agent_po_account_master` for agents (`cname`) or `mtr_tracking_data` for customers (`account_name`), based on the `role` parameter in the request. The customer search also supports pagination. Results are formatted as `{ value: name, label: name }`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\constants\Paths.ts`**
    *   **Timestamp:** 12/18/2025, 3:55:32 PM
    *   This file, defining API endpoints, has been updated to include a new base path for `CustomerOptions: { Base: "/getCustomerOptions" }`. This directly supports the `customerAutosearchOptions` service. Additionally, the `Analytics` module's paths are very extensive, covering endpoints for dashboards, metrics, excel exports, saving sequences, sizes, CBM, KGS, TEU, containers, transit, and table data, indicating a rich analytical feature set.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\routes\index.ts`**
    *   **Timestamp:** 12/18/2025, 3:56:44 PM
    *   This central routing file now imports `getAllCustomerOptions` and mounts it to the new `Paths.CustomerOptions.Base` endpoint. This completes the integration of the new customer/agent autosearch feature into the API. The file also continues to integrate various other route modules like Auth, Shipment, Dashboard, Reports, Booking, User, Notification, Analytics, Planboard, Menu, and CO2Emission, alongside direct GET endpoints for global options, company details, and audit logs.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\export.query.ts`**
    *   **Timestamp:** 12/19/2025, 7:53:25 AM
    *   This utility file provides a `bookingQuery` function that dynamically generates complex SQL queries based on a given `reportName` and various parameters (company name, month, week, quarter, display options). It includes a helper function `getMonthValue` to parse month strings. The queries primarily calculate day differences between booking dates, actual/estimated departure dates, and POD arrival dates using `mtr_tracking_data` and `agent_booking_entry` tables. Many queries include conditional logic for `stackedExport` to categorize time differences into descriptive labels like "Less than 1 week", "1 - 2 weeks", etc., based on `displayBy` (month, week, quarter) and `stackedGroup`.

### Timestamps of Significant Changes:

The changes occurred over two consecutive days:
*   **December 18, 2025 (late afternoon):** Introduction of the customer/agent autosearch functionality, including its service logic, API path definition, and integration into the main router.
*   **December 19, 2025 (early morning):** Updates to the `export.query.ts` file, indicating a focus on enhancing the backend's reporting and data export capabilities, particularly for booking-related analytics.

### Patterns and Recurring Elements:

*   **Analytics and Reporting Focus:** A major theme is the expansion of analytics and reporting features. This is evident from the new `customerAutosearchOptions` service, the extensive `Analytics` paths in `Paths.ts`, and the detailed SQL query generation in `export.query.ts`.
*   **Database Interaction:** Both `customerAutosearchOptions.ts` and `export.query.ts` heavily interact with database tables, specifically `mtr_tracking_data` and `agent_booking_entry`, often joining them on `mblno`.
*   **Dynamic Querying:** The code demonstrates a pattern of generating SQL queries dynamically based on request parameters (e.g., `role`, `reportName`, `CompanyName`, `stackedMonth`).
*   **Date Calculations in SQL:** Many SQL queries involve calculating differences between dates (e.g., `EXTRACT(DAY FROM date1 - date2)`) and filtering by date components (month, quarter, week).
*   **Data Formatting for UI:** Autosearch results are consistently formatted into `{ value: string, label: string }` objects, a common pattern for dropdowns or selection components in frontends.
*   **Robust Error Handling:** Services include `try...catch` blocks to log errors and return a 500 status code with a generic error message.
*   **Consistent Path Naming:** The `Paths.ts` file shows a structured approach to defining API endpoints for various modules.
*   **Parameter-driven Logic:** Many functions utilize query parameters (`req.query`) to drive their behavior, such as `role`, `pagination`, `page`, `perPage`, and reporting parameters like `stackedMonth`, `startWeek`, `endWeek`, `quarter`, `stackedGroup`, `stackedExport`, `displayBy`.

## 7:29:10 PM
The `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\dashboard\ShipmentTrackingScreen.jsx` file, last updated on 12/23/2025, 10:08:25 AM, defines the `ShipmentTrackingScreen` component, a detailed page for displaying shipment information.

Key updates and features in this file include:

*   **Data Fetching:** The component retrieves shipment data using `useFetchShipmentTrackingQuery`, which takes `mblno` and `fileno` extracted from URL query parameters.
*   **Layout and Structure:** The screen is divided into two main columns: a wider section (74%) for detailed tracking and a narrower section (26%) for milestones.
*   **Core Tracking Components:**
    *   Integrates `MapTrackingContainer` and `TrackingStepper` to visualize shipment progress and location.
    *   A primary card displays essential shipment details like `File No`, `BL`, `Supplier`, and `MBL No` using a `CardField` component.
*   **Expandable "More Details" Section:** A significant addition is an expandable section that, when toggled, reveals additional shipment information such as `Customer Ref No.`, `Notify`, `Commodity`, `Consignee`, `Service`, `Piece count`, and `Weight`. This functionality is controlled by a `showMore` state and uses `ExpandMoreIcon`/`ExpandLessIcon`.
*   **Information Containers:** Dedicated `TrackingInfoContainer` and `MilestonesContainer` components are used to organize specific tracking and milestone details, respectively.
*   **Navigation and Loading:** A `ScreenToolbar` includes an `ArrowBackIcon` for navigation, and a `Loader` component is displayed while data is being fetched.
*   **Styling Patterns:** Several components exhibit consistent styling, including rounded borders (`borderRadius: "8px"`) and subtle box shadows (`boxShadow: '0px 1px 8px 0px #0000001A'`), indicating a uniform design language across the application's cards and containers.
*   **Utility:** A `fieldFormater` function ensures data is displayed correctly, particularly for the `status` field.
*   **Debugging:** `console.log` statements for `PARAMS` and `shipment page info` are present, suggesting active development or recent debugging.

## 7:29:14 PM
The provided log details a series of changes made to two files: `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\tracking-stepper.tsx` and `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-details-page.tsx`.

### File-Specific Updates:

#### `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\tracking-stepper.tsx` (Multiple updates from 12/23/2025, 10:41:03 AM to 12/23/2025, 6:20:19 PM)

**Key Changes:**

*   **Refactoring and Simplification:**
    *   Initial versions of the `TrackingStepper` component contained complex conditional logic within `getTrackingDetails` and `polToPodcompletedValidation` functions to determine shipment status, icons, and progress percentages based on various `trackingInfo` properties like `inland_rail_move`, `trucker`, `delivery_customer_date`, `arrived_rail_ramp`, etc.
    *   A significant refactor occurred around `12/23/2025, 11:09:06 AM` where new helper functions `getScope` and `getFinalDates` were introduced to centralize the logic for determining the shipment scope (PORT_PORT, PORT_RAIL, PORT_DOOR) and final ETA/ATA. This made `getTrackingDetails` much cleaner and more modular.
    *   The `polToPodcompletedValidation` function was simplified considerably in a few steps, removing the `ataDate` and `currentDate` variables, and condensing the conditional logic.
    *   The `getTravelProgress` function appears to have been removed or commented out from the main logic flow as of `12/23/2025, 11:09:06 AM`, though it briefly reappears in a later commit (`12/23/2025, 3:39:02 PM`) along with older `polToPodcompletedValidation` logic, suggesting some back-and-forth or merge conflicts.
*   **Iconography:**
    *   Initially, icons (VesselSVGIcon, TruckSVGIcon, TrainSVGIcon) were imported from `assets/svg-list`.
    *   Later, starting around `12/23/2025, 11:50:11 AM`, there was a move towards using `@iconify/react` for icons, with a new `TransportIcon` component mapping `type` (VESSEL, PORT, RAIL) to specific `Icon` components. The direct SVG imports were gradually replaced.
    *   At `12/23/2025, 12:13:04 PM`, the `TrainSVGIcon` and `TruckSVGIcon` were redefined using the `@iconify/react` `Icon` component.
    *   By `12/23/2025, 12:18:40 PM`, more specific `Icon` components like `ShipIcon`, `AnchorIcon`, and `TrainIcon` were defined directly using `Icon` from `@iconify/react` and a new `LocationPinIcon` component was introduced, using `mdi:map-marker` icon and accepting an `active` prop for color. The existing `LocationPinIcon` (SVG based) was still present in some subsequent updates, indicating a transition.
    *   Further refined icon imports and usage on `12/23/2025, 6:04:26 PM`, introducing `FaShip`, `FaAnchor`, and `MdDirectionsRailway` from `react-icons/fa6` and `react-icons/md`, and renaming `TruckSVGIcon` to use `FaShip` and `Terminal` for `FaAnchor`. The `Railway` icon was introduced using `MdDirectionsRailway`.
*   **Styling & Structure:**
    *   Early versions used inline `sx` props for dynamic styling.
    *   Styles were extracted into a `s` object and applied consistently.
    *   Specific `Box` widths for lines (`data.length === 2 ? "508px" : "242px"`) were simplified and made more consistent (`width: "242px"`) around `12/23/2025, 11:09:06 AM`.
    *   The `vehicleBox` styling saw adjustments to its `right` and `top` properties and `padding` multiple times.
    *   Around `12/23/2025, 2:30:32 PM`, the styling for the progress line was enhanced to show a "completed part" (`lineCompleted`) and "remaining dotted" (`lineRemaining`), replacing a single `line` style. A `departedText` typography was also introduced outside the main `root` box.
    *   The `marginTop` for `infoRow` and `lineWrapper` were adjusted in several commits, indicating layout fine-tuning.
    *   The `CalendarMonthOutlinedIcon` in the subtitle `Typography` was commented out at `12/23/2025, 12:24:25 PM` and remained so in subsequent logs.
    *   The `vehicleBox` positioning and styling underwent several tweaks, notably becoming dynamic with `left: calc(${item.percentage}% - 12px)` at `12/23/2025, 6:20:04 PM`.
*   **Timeline Logic:**
    *   The structure of the `timeline` array within `getTrackingDetails` evolved significantly.
    *   Initially, it handled different paths (no rail, rail move) with extensive conditional logic directly adding items.
    *   After refactoring with `getScope` and `getFinalDates` (`12/23/2025, 11:09:06 AM`), the timeline construction became more structured into "POL", "POD", "RAIL", and "FINAL (DOOR ONLY)" sections, using the `scope` variable.
    *   The "FINAL (DOOR ONLY)" section was temporarily commented out at `12/23/2025, 11:11:45 AM` and then reintroduced with new logic using `getFinalInlandDates` and a generic "Final Destination" label at `12/23/2025, 12:04:08 PM`.
    *   The `Final Inland ATA` subtitle in the last timeline item was changed from `trackingInfo.delivery_customer_date` to `finalInlandATA` object values around `12/23/2025, 3:51:33 PM` and subsequent commits.
    *   Placeholder dates (`'2025-04-29'`) were temporarily added to `ETA` and `ATA` values within `formatDate` calls in the timeline at `12/23/2025, 2:45:11 PM` for testing or default display, then removed or adjusted.
    *   The `POD` timeline entry's icon logic for rail shipments changed from `TrainSVGIcon` to `TruckSVGIcon` briefly, then reverted to `TrainSVGIcon` or `Terminal` based on intermediate changes, indicating testing or re-evaluation of iconography.
    *   The percentage calculation logic was updated for rail moves at `12/23/2025, 5:01:30 PM` to force `100%` when `trackingInfo.arrived_rail_ramp` is true.

#### `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\shipment\shipment-details-page.tsx` (Updates from 12/23/2025, 2:37:36 PM to 12/23/2025, 2:38:05 PM)

**Key Changes:**

*   **Layout Adjustment:** The `<TrackingStepper>` component was moved within the `ShipmentDetailsPage` component.
    *   Initially, it was rendered after the `MapContainer` and within its own `div` (`<div className="py-4">`).
    *   At `12/23/2025, 2:37:50 PM`, the `<TrackingStepper>` component was moved to be nested inside the `p-4 flex justify-between items-center` div, specifically after the `getPriorityDateLabel` typography.
    *   Then, at `12/23/2025, 2:38:05 PM`, it was moved back outside that `p-4` div, but remained within the main `bg-white border ...` container, suggesting it should occupy its own section rather than being inline with the date label.
*   **Redundant Console Logs:** There were several `console.log` statements for `shipmentDeatils`, `isSuccess`, `isLoading`, `isFetching`, and `shipmentAirDeatils` that appeared in these logs.

### Patterns and Recurring Elements:

*   **Date Formatting:** The `formatDate` helper function is consistently used across all versions to display dates in a uniform `"${label} ${appDateFormat(date)}"` or `"${label} ---"` format.
*   **Tracking Info Logic:** The core logic revolves around a `trackingInfo` object to determine the state of a shipment. Functions like `polToPodcompletedValidation` and `getTrackingDetails` are central to processing this information.
*   **Visual Elements:** The use of `LocationPinIcon` and dynamic `line` elements (representing progress) with vehicle icons (Ship, Truck, Train) is a consistent visual pattern for the tracking stepper.
*   **Styling Object `s`:** Styles are generally defined within a constant `s` object at the bottom of the `tracking-stepper.tsx` file and applied using MUI's `sx` prop.
*   **Console Logging:** Frequent `console.log` statements are present throughout the `tracking-stepper.tsx` file, indicating active debugging during development.
*   **Evolution of Icon Imports:** There's a clear trend from directly imported SVGs to using the `@iconify/react` library, and then selectively importing icons from `react-icons/fa6` and `react-icons/md` for better maintainability and consistency.
*   **Shipment Scope Handling:** The concept of `shipment_scope` (PORT_PORT, P-R, P-D) is crucial for tailoring the tracking timeline, and its implementation was refined over time.
*   **Percentage Calculation:** An `useEffect` hook consistently calculates and reports the completion percentage of the tracking steps using `trackingDataFromStepper`.