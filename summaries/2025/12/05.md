...
## 2:54:18 PM
The code changes primarily focus on the development and refinement of House Bill of Lading (HBL) forms and related components within a React frontend application, utilizing Material-UI, Formik, and Redux Toolkit Query for state and API management.

**File-specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HblPoForm.jsx`**
    *   **12/3/2025, 12:25:57 PM - 12:26:15 PM:** The component responsible for PO details (`PODetailsExtend`) was initially created, then quickly renamed to `HblPODetails`. It dynamically selects between "Basic PO" and "Linked PO" tabs based on the `client_name` from form data.
    *   **12/3/2025, 12:27:09 PM:** The `formik` prop was changed to `values`, indicating a shift in how form data is received by this component (passing raw values rather than the full Formik object).
    *   **12/3/2025, 3:25:22 PM:** The `BasicPoExtend` sub-component was replaced with a new, specialized `BasicPoForHbl` component, suggesting a refactoring to handle HBL-specific PO logic.
    *   **12/3/2025, 3:49:19 PM:** The logic for automatically switching tabs was updated. Instead of `values.client_name`, it now checks `values.basicPoList?.[0]?.customer` to determine if the "SP RICHARDS COMPANY" client is selected, which then defaults to the "Linked PO" tab.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLform.jsx`**
    *   **12/3/2025, 12:26:33 PM:** This commit marks the initial comprehensive implementation of the `HBLForm`. It includes extensive state management using React hooks (`useState`, `useReducer`), form handling with Formik, Redux integration for various lists (routing, containers, basic POs), and API mutations for saving and updating HBL bookings. It also sets up UI components for header details, party management (Shipper, Consignee, Notify), and defines keyboard shortcuts for common actions.
    *   **12/3/2025, 1:38:27 PM - 1:55:49 PM:** Multiple small, iterative changes were made to the `initialValues` structure within Formik, particularly for `agent_booking_id` and `booking_no`. These changes reflect ongoing adjustments to how these fields are initialized based on the `formAction` (e.g., "add", "copy", "edit") and available data from `initialValues` or `location.state`. There was a brief, quickly reverted change to `enableReinitialize` at 1:46:11 PM.
    *   **12/3/2025, 2:37:43 PM - 2:53:46 PM:** Introduced `useFetchPoFormDetailsQuery` to dynamically fetch PO details (e.g., `poDetails?.data?.basicPoList`) and integrate them into the Formik `initialValues`. This involved several refinements to the conditional logic for populating `basicPoList` and handling potential missing data. A temporary bug was introduced at 2:49:21 PM due to incorrect destructuring of `poDetails` which was later fixed at 2:53:46 PM.
    *   **12/3/2025, 4:56:44 PM:** Added `linkedPoList` to the `initialValues` object, fetching its data similarly from `poDetails?.data?.linkedPoList`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\form\sub-components\BasicPoExtend.jsx`**
    *   **12/3/2025, 12:33:44 PM:** This component was created to manage a list of basic PO items, allowing users to add, edit, and delete rows. It integrated Redux for state management and an autocomplete for fetching supplier names.
    *   **12/3/2025, 3:24:24 PM:** No functional changes, suggesting it might have been deprecated or copied to a new specialized component.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\store\api\agentBookingApi.js`**
    *   **12/3/2025, 2:38:52 PM:** This was a major update, adding comprehensive API endpoints and Redux Toolkit Query (RTK Query) mutations/queries specifically for HBL (House Bill of Lading) functionality. New endpoints include `saveHblBooking`, `fetchHblBookingFormDetails`, `fetchPoFormDetails`, `getHblDashboard`, `updateHblBooking`, and `deleteHblBooking`. It also updated existing mutations like `updateDo` for more precise cache invalidation.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLListDetails.jsx`**
    *   **12/3/2025, 1:43:57 PM:** Initial implementation of a dashboard component to display HBL records in a table, with features for deleting HBLs, navigating to the HBL form (for copying), and viewing audit logs.
    *   **12/3/2025, 1:54:42 PM:** Modified the navigation logic for adding a new HBL; the `formAction` parameter was changed from "copy" to "add".

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HblHeaderForms.jsx`**
    *   **12/3/2025, 1:42:46 PM:** Initial development of the HBL header form, including fields for transportation, booking details, dates, client, MBL/HBL numbers, ports, vessel, and voyage. It also introduced an "Add Vessel" modal with a keyboard shortcut.
    *   **12/3/2025, 1:51:14 PM - 1:51:28 PM:** A minor, quickly reverted change was made to the `onChange` prop of the "Agent Booking #" input box, switching between `handleChange` and `onchange`.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\BasicPoForHbl.jsx`**
    *   **12/3/2025, 3:25:02 PM:** This component was introduced as a specialized version of `BasicPoExtend`.
    *   **12/3/2025, 3:27:29 PM - 3:27:50 PM:** Key interactive elements (onChange handlers for inputs, Add/Delete buttons) were commented out, suggesting a temporary intention to make it read-only or to work on displaying data first.
    *   **12/3/2025, 3:29:13 PM - 3:30:11 PM:** The component's data source for rendering PO items shifted from a Redux selector (`selector`) to directly using `formik?.basicPoList`, indicating that PO data is now expected to be part of the Formik's `values` prop.
    *   **12/3/2025, 3:37:58 PM:** Imports for `CustomDeleteIcon`, `AddCircleOutlined`, `ApiManager`, and `recoverValue` were removed, aligning with the commenting out of interaction logic.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\form_extend\CargoFormExtend.jsx`**
    *   **12/3/2025, 4:58:45 PM:** This component was introduced for managing cargo details within a table, with functionality to add, edit, and delete cargo items using Redux and a modal form.
    *   **12/3/2025, 4:59:42 PM - 12/5/2025, 2:27:08 PM:** All subsequent commits for this file are identical, indicating no further functional changes were implemented after its initial setup.

**Patterns and Recurring Elements:**

*   **HBL Feature Development:** The changes show a clear focus on building out HBL-related features, including forms for creating/editing HBLs, associated lists (PO, routing, containers), and a dashboard view.
*   **RTK Query and Redux:** Consistent use of RTK Query for API calls and `react-redux` for managing client-side state across multiple components. New API endpoints and hooks were added for HBL operations.
*   **Formik Integration:** Formik is the primary library for form management, with complex `initialValues` logic being a recurring area of adjustment to handle various form actions and data sources.
*   **Material-UI:** Heavy reliance on MUI components for building the user interface, including Grid, Card, Tabs, and custom input/autocomplete components.
*   **Debugging:** Frequent `console.log` statements with distinct markers are present across several files, indicating an active development and debugging process.
*   **Refactoring and Specialization:** The introduction of `BasicPoForHbl.jsx` to replace `BasicPoExtend.jsx` within the HBL context demonstrates a pattern of component specialization.
*   **Data Flow Adjustments:** A significant pattern is the evolution of how list data (specifically `basicPoList`) is managed: initially directly from Redux state, then shifted to be part of the `Formik` values, potentially simplifying how form data is passed around.
*   **Keyboard Shortcuts:** Implementation of `useKeyboardShortcut` for improved user experience and efficiency in form interaction.

## 3:25:47 PM
**File: c:\Users\ridap\OneDrive\Documents\cng-batches\cng-batches\modules\login-req-notify\nofifyAdmin.js**

This file, `nofifyAdmin.js`, is responsible for notifying administrators about new user registrations and sending follow-up emails to eligible users requiring additional information.

*   **Timestamp: 12/5/2025, 2:09:33 PM**
    *   The initial version introduces the `NotifyAdmin` class with an `async notifyAdmin()` method.
    *   It fetches new users, generates an email with their details, and sends it to `config.email.all`.
    *   It updates an `emailcounter` for registered users.
    *   Crucially, it includes logic to send a second email *only* to users whose `emailcounter` is less than or equal to 2, requesting "Shipment Number" and "Container Number" for registration approval. The HTML content for this second email is inlined directly within the `notifyAdmin` method.
    *   It defines `fetchNewUsers` (status 'NEW'), `updateEmailCounter`, and `generateEmailContent` methods. A `generateUserRegisterApprovalEmailContent` method is present but not utilized in this version.
    *   Debugging `console.log` statements like `user90`, `emailcounter`, and `kkkkkk90` are present.

*   **Timestamp: 12/5/2025, 2:10:02 PM**
    *   The primary change is refactoring the second email's HTML content. The inlined HTML for the "Additional Information Required" email is replaced with a call to `this.generateUserRegisterApprovalEmailContent()`. This improves code organization and reusability of the email template.

*   **Timestamp: 12/5/2025, 2:15:39 PM**
    *   A minor change in the `generateUserRegisterApprovalEmailContent()` method: the placeholder "Dear [Customer Name]," is updated to use a dynamic value from the configuration, "Dear ${config.email.userRegistrationApproval.customer}".

*   **Timestamp: 12/5/2025, 2:16:45 PM**
    *   The dynamic customer name in `generateUserRegisterApprovalEmailContent()` is hardcoded to "Dear Rajeev,".

*   **Timestamps: 12/5/2025, 2:20:06 PM and 12/5/2025, 2:20:20 PM**
    *   These updates primarily involve replacing several `console.log` debugging statements with `logger.info` calls (e.g., `logger.info(New Users: ...`, `logger.info(Email Counters after update: ...`, `logger.info(Eligible Users: ...`, `logger.info(Eligible Users Count: ...)`). This indicates a shift from development debugging to more formal logging. The code content remains identical between these two specific timestamps, suggesting a minor save or formatting change rather than a functional one.

**Patterns and Recurring Elements:**

*   **Email-centric functionality:** The core purpose of this file revolves around fetching new user registrations and sending two distinct types of emails: an initial notification to administrators and a follow-up request for additional details to new users.
*   **Status-based processing:** Users are identified for processing based on their `status = 'NEW'` in the `mtr_user_registration` table.
*   **Counter-based logic:** An `emailcounter` is used to limit the number of follow-up emails sent to new users (specifically, up to 2).
*   **Configuration reliance:** The module heavily relies on a `config` object (imported from `../../config/config`) for email addresses, URLs, and potentially other settings.
*   **Logging vs. debugging:** There's an observable pattern of transitioning from direct `console.log` statements to `logger.info` calls for better application monitoring.
*   **Templating:** While one email body (for admin notification) is dynamically generated within the `generateEmailContent` method, the "Additional Information Required" email uses a pre-defined HTML template via `generateUserRegisterApprovalEmailContent()`, illustrating a preference for separate, reusable email templates.

## 4:30:22 PM
The logs highlight recent developments across two core files: `config.js` and `nofifyAdmin.js`, primarily focusing on configuration updates and user registration notification logic. All recorded changes occurred on 12/5/2025, indicating a period of active development and refinement.

### File-Specific Updates:

**`c:\Users\ridap\OneDrive\Documents\cng-batches\cng-batches\config\config.js`**
This file, which manages application settings for both `development` and `production` environments, underwent several updates:
*   **Timestamp: 12/5/2025, 2:06:15 PM:**
    *   Development environment file paths were updated, including `googleTokenPath` and the addition of `anchorJsonFilePath`/`anchorJsonArcFilePath`.
    *   Extensive updates to email recipient lists across various report types (e.g., `isf24HrReport`, `isfNotMatchReport`), and the introduction of new email configurations like `anchor24HrJsonReport`, `doBatchReport`, `openFileEmail`, `air_data_pulling_failure`, and `userRegistrationApproval` for both development and production. The production `email.all`, `email.office`, `email.execptionMailList`, and `email.isfTeam` recipient lists were significantly expanded.
*   **Timestamp: 12/5/2025, 2:13:27 PM:**
    *   The `development.email.userRegistrationApproval` configuration saw recipient email addresses moved between the `to` and `cc` fields.
*   **Timestamp: 12/5/2025, 2:14:56 PM:**
    *   A `customer` field was added to `development.email.userRegistrationApproval`.
*   **Timestamp: 12/5/2025, 2:18:39 PM:**
    *   The `cc` list within `development.email.userRegistrationApproval` was further modified to include additional email addresses, and the `support_team` list was also updated.
*   **Timestamp: 12/5/2025, 2:21:30 PM:**
    *   Database connection details for the `development` environment were updated, including changes to the database name (`Rida_Database` to `tmodal`) and password.
    *   The `googleTokenPath` for development was changed to a different local directory.
    *   The general `development.email.all` recipient list was reduced.
*   **Timestamp: 12/5/2025, 2:24:48 PM:**
    *   New file paths, `cngInvXmlPath` and `sprInvXmlPath`, were added to both `development.paths` and `production.paths` (though production paths were initialized as empty).
    *   An additional, potentially misplaced `to` field was introduced directly under the main `development.email` object.
    *   The `production.email.air_data_pulling_failure` recipient list was reordered.

**`c:\Users\ridap\OneDrive\Documents\cng-batches\cng-batches\modules\login-req-notify\nofifyAdmin.js`**
This module is responsible for notifying administrators about new user registrations and requesting additional information from new users.
*   **Timestamp: 12/5/2025, 2:09:33 PM:**
    *   Initial implementation of the `NotifyAdmin` class, which fetches new users with `status = 'NEW'`, sends a general registration notification, updates an `emailcounter`, and sends a follow-up "Additional Information Required" email to users whose `emailcounter` is less than or equal to 2.
    *   The "Additional Information Required" email initially attempted to dynamically inject recipient email IDs into the "Dear [Customer Name]" greeting.
*   **Timestamp: 12/5/2025, 2:10:02 PM:**
    *   The "Additional Information Required" email generation was refined, moving its content into the dedicated `generateUserRegisterApprovalEmailContent()` method, correcting the previous dynamic recipient issue.
*   **Timestamp: 12/5/2025, 2:15:39 PM:**
    *   The `generateUserRegisterApprovalEmailContent()` method was updated to use `config.email.userRegistrationApproval.customer` for the recipient's name in the email greeting.
*   **Timestamp: 12/5/2025, 2:16:45 PM:**
    *   The greeting in `generateUserRegisterApprovalEmailContent()` was changed to a hardcoded "Dear Rajeev,".
*   **Timestamp: 12/5/2025, 2:20:06 PM:**
    *   Debugging output was standardized, replacing `console.log` statements with `logger.info` calls for better structured logging.

### Patterns and Recurring Elements:
*   **Dual Environment Configuration:** A consistent pattern is the separation of configurations for `development` and `production` environments within `config.js`, requiring updates to be applied to both or selectively to one.
*   **Email Management:** Frequent modifications to email recipient lists and the introduction of new email notification types (e.g., `isf24HrReport`, `anchor24HrJsonReport`, `doBatchReport`, `openFileEmail`, `air_data_pulling_failure`, `userRegistrationApproval`) indicate an evolving notification system. The `nofifyAdmin.js` module specifically focuses on automated email communications for user onboarding.
*   **File Path Management:** The `paths` configuration in `config.js` is regularly updated, suggesting ongoing adjustments to file storage locations for various processes.
*   **Iterative Refinement of Email Content:** Changes in `nofifyAdmin.js` demonstrate an iterative process of refining the HTML content and recipient addressing for automated emails, moving from inline HTML generation to dedicated methods and then adjusting dynamic placeholders to hardcoded values.
*   **Logging Practices:** The shift from `console.log` to `logger.info` in `nofifyAdmin.js` suggests a move towards more robust and production-ready logging within the application.
*   **Credential/Configuration Changes:** There are recurring updates to database credentials and email/SFTP server details in the development configuration, along with integration passwords, which are sensitive elements managed within the `config.js` file.

## 8:57:28 PM
The changes log details updates to two core files: `config.js` and `nofifyAdmin.js`, all occurring on 12/5/2025 within a timeframe of approximately 18 minutes.

**File-specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\cng-batches\cng-batches\config\config.js`**
    *   **12/5/2025, 2:06:15 PM**: Initial configuration updates. In the `development` environment, the `googleTokenPath` was modified, and a `userRegistrationApproval` email configuration was added. The `anchor24HrJsonReport` email recipient list was updated. In the `production` environment, several email recipient lists were extensively modified, including `isf24HrReport`, `isfNotMatchReport`, `email.all`, `email.office`, `email.execptionMailList`, and `email.isfTeam`.
    *   **12/5/2025, 2:13:27 PM**: The `userRegistrationApproval` `to` and `cc` email lists in the `development` environment were reordered, with `ridaparveen679@gmail.com` moved to the `cc` list.
    *   **12/5/2025, 2:14:56 PM**: A `customer` field was added to the `userRegistrationApproval` email configuration in the `development` environment.
    *   **12/5/2025, 2:18:39 PM**: The `userRegistrationApproval.cc` email list in the `development` environment was updated to include both `rida@dyneinfotech.com` and `ridaparveen679@gmail.com`.
    *   **12/5/2025, 2:21:30 PM**: Significant changes were made to the `development` database connection, with `db.database` changing from "Rida_Database" to "tmodal" and `db.password` from "12345678" to "12345". The `googleTokenPath` was also updated, and the `email.all` list was shortened.
    *   **12/5/2025, 2:24:48 PM**: New path configurations, `cngInvXmlPath` and `sprInvXmlPath`, were added to the `paths` object in both `development` (with concrete paths) and `production` (with empty paths). A potentially erroneous `to` field was added directly under the `email` object in `development`, and the `air_data_pulling_failure.to` list in `production` was reordered.

*   **`c:\Users\ridap\OneDrive\Documents\cng-batches\cng-batches\modules\login-req-notify\nofifyAdmin.js`**
    *   **12/5/2025, 2:09:33 PM**: The initial version of the `notifyAdmin` method contained an inlined HTML template for an "Additional Information Required" email, directly embedding configuration values.
    *   **12/5/2025, 2:10:02 PM**: The inlined email content was refactored into a new dedicated method, `generateUserRegisterApprovalEmailContent()`, improving modularity.
    *   **12/5/2025, 2:15:39 PM**: The `generateUserRegisterApprovalEmailContent()` method was updated to dynamically use `config.email.userRegistrationApproval.customer` for the recipient's name instead of a static placeholder.
    *   **12/5/2025, 2:16:45 PM**: The dynamic customer name in `generateUserRegisterApprovalEmailContent()` was replaced with a hardcoded "Dear Rajeev,".
    *   **12/5/2025, 2:20:06 PM**: Debugging `console.log` statements were replaced with `logger.info` calls for better logging of new users, email counters, and eligible users, indicating a shift towards more structured application monitoring.

**Patterns and Recurring Elements:**

*   **Configuration Management:** The `config.js` file is a central point of change, frequently updated with email recipient lists, file paths, and database connection details. This highlights ongoing adjustments to deployment environments and communication channels.
*   **Email System Development:** A significant theme is the development and refinement of email notification logic, particularly for user registration approvals. This includes defining new email lists, structuring email content, and making it dynamic.
*   **Refactoring and Code Quality:** The move of inline HTML to a dedicated function in `nofifyAdmin.js` and the switch from `console.log` to `logger.info` demonstrate an effort to improve code readability, maintainability, and logging practices.
*   **Environment-Specific Settings:** Both `development` and `production` configurations are present and are subject to distinct updates, emphasizing the need to manage different operational environments.
*   **Rapid Iteration:** All changes occurred on the same day within a short timeframe, suggesting a focused session of development, debugging, or configuration tuning.

## 9:24:39 PM
The provided log shows changes across three files: `booking.controller.ts`, `hblEntry.controller.ts`, and `export.query.ts`, as well as `booking.query.ts` and `hblEntry.route.ts`. The timestamps range from December 3, 2025, to December 5, 2025.

**File-Specific Updates:**

**1. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\booking.controller.ts`**
   - **Timestamp of Significant Changes:** The file was modified multiple times on **December 3, 2025**, between 12:06:34 PM and 12:15:30 PM.
   - **Key Updates:** The primary function `saveBooking` is responsible for creating and updating agent bookings.
     - It retrieves existing booking data based on `agent_booking_id`.
     - It constructs a `BookingPayloadTypes` object from the request body, sanitizing string and integer inputs and formatting dates.
     - For updates, it first deletes existing entries across several tables (`agent_booking_entry`, `agent_booking_cargolist`, `agent_booking_container_pre`, `agent_booking_vessel`, `agent_booking_po`, `agent_booking_linked_po`) associated with the `agent_booking_id` before inserting new data.
     - For new bookings, it generates a new `agent_booking_id`.
     - It inserts the main booking details into `agent_booking_entry`.
     - It then iteratively inserts related data into `agent_booking_Cargolist`, `agent_booking_vessel`, `agent_booking_container_pre`, `agent_booking_po`, and `agent_booking_linked_po` if the respective lists are not empty.
     - The `linkedPoList` processing also includes an `updateQuery` to modify `remain_packs` in the `dso_order` table.
     - The `saveMaster` function handles master account creation for various booking account types. It validates the `account_type` against a predefined `poAccountMastertype` enum.
   - **Recurring Elements/Patterns:**
     - Extensive use of `sanitizeString` and `sanitizeIntString` for input validation.
     - Consistent date formatting using `AppDateFormate(..., db_format)`.
     - Repetitive SQL `DELETE` operations followed by `insertMany` for lists in update scenarios, indicating a "delete-all-and-reinsert" strategy for sub-records.
     - Database operations (`query`, `insertQuery`, `insertMany`, `updateQuery`) are wrapped in a `try-catch` block with `BEGIN` and `ROLLBACK` comments, although the `BEGIN` and `COMMIT` calls are commented out in the provided snippets, suggesting potential transactional issues or incomplete implementation.
     - Logging (`logger.info`, `logger.error`) is used at various stages.

**2. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\hblEntry.controller.ts`**
   - **Timestamp of Significant Changes:** The file was modified frequently on **December 3, 2025**, from 2:04:59 PM to 3:44:53 PM, and again on **December 5, 2025**, at 1:56:46 PM and 1:56:56 PM.
   - **Key Updates:** This file manages House Bill of Lading (HBL) master bookings (`createHblMasterBooking` and `updateHblMasterBooking`).
     - `createHblMasterBooking`:
       - Constructs a `hblEntryPayloadTypes` object from the request body, sanitizing various shipper, consignee, notify, and booking details.
       - Starts a database transaction (`await query('BEGIN')`).
       - Fetches `bookingSerialId` from `agent_booking_entry`.
       - Inserts the new HBL record into `hbl_master`, then records an audit log using `simpleAudit`.
       - Inserts vessel/voyage details into `agent_booking_vessel`.
       - Inserts HBL specific details into `agent_booking_hbl`.
       - Inserts `routingList`, `containerList`, and `basicPoList` into their respective tables (`agent_booking_routing`, `agent_booking_container`, `agent_booking_po`).
       - Commits the transaction (`await query('COMMIT')`).
     - `updateHblMasterBooking`:
       - Requires `serial_id` for identifying the HBL to update.
       - Updates the `hbl_master` table using a parameterized SQL `UPDATE` statement for a comprehensive set of fields.
       - Updates `agent_booking_vessel` with new vessel and voyage information.
       - Uses an `insertAudit` function for `hbl_master` changes, filtering the payload to include only relevant columns.
       - Updates `agent_booking_hbl` with new HBL details.
   - **Recurring Elements/Patterns:**
     - Similar payload construction and sanitization as `booking.controller.ts`.
     - Consistent date formatting with `AppDateFormate(..., db_format)`.
     - Database transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) are explicitly used.
     - Extensive use of `insertMany` for handling list-based data (routing, containers, POs) in `createHblMasterBooking`.
     - `logger.info` and `console.log` statements are frequently used for debugging.
     - Audit logging (`simpleAudit`, `insertAudit`) is integrated for tracking changes in `hbl_master`.

**3. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\hblEntry.route.ts`**
   - **Timestamp of Significant Changes:** A single update on **December 3, 2025, 2:36:19 PM**.
   - **Key Updates:** This file defines API routes for HBL operations.
     - Added routes for `createHblMasterBooking` (POST `/create`) and `updateHblMasterBooking` (POST `/update`).
     - Added an authentication middleware to the `deleteHblForm` (DELETE `/delete`) route.
     - The commented-out `router.post("/",saveHblBooking);` suggests a previous or alternative HBL saving mechanism.
   - **Recurring Elements/Patterns:** Standard Express.js router setup.

**4. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts`**
   - **Timestamp of Significant Changes:** Numerous small changes on **December 4, 2025**, from 11:59:53 AM to 3:35:27 PM. These appear to be minor modifications or saving artifacts as the code content within the changes is identical.
   - **Key Updates:** This file contains SQL query builders specifically for booking-related analytics reports.
     - Defines an `QueryBuilderI` interface for report parameters.
     - The `bookingQueryBuilder` function constructs complex PostgreSQL queries based on the `type` of the report.
     - Reports include `BookingDatetoActualDeparture` (container-wise and MBL-wise, both standard and stacked bar charts).
     - Queries involve joining `mtr_tracking_data` with `agent_booking_entry` on `mblno`.
     - They calculate date differences (`EXTRACT(DAY FROM ... - ...)`) and aggregate data.
     - Stacked bar chart queries categorize day differences into "Less than 1 week", "1 - 2 weeks", etc.
     - Filters for `account_name` (CompanyName) and date ranges are consistently applied.
     - Uses `getColumnsByCategory` and `getFilterQueryByDisplayValue` from `../../utils/analytics`.
   - **Recurring Elements/Patterns:**
     - All queries join `mtr_tracking_data` and `agent_booking_entry` tables.
     - Consistent filtering by `LOWER(account_name) = '${CompanyName}'` and date ranges (`departure_date >= '${fromDate}'::timestamp AND departure_date < '${toDate}'::timestamp + INTERVAL '1 day'`).
     - Checks for `NULL` and `0001-01-01 00:00:00 BC` in date columns to ensure valid date calculations.
     - `customizer` object is used to configure chart and date-related settings for the frontend.

**5. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\others.controller.ts`**
   - **Timestamp of Significant Changes:** A single update on **December 4, 2025, 12:02:39 PM**.
   - **Key Updates:** The `getTableDataReports` function is the main entry point for generating analytics reports.
     - It parses query parameters like `fromDate`, `toDate`, `type`, `displayBy`, etc.
     - Sets default date ranges if not provided.
     - Dynamically calls different query builders (`chassisQueryBuilder`, `bookingQueryBuilder`, `containerQueryBuilder`, `departureQueryBuilder`, `generalQueryBuilder`) based on the `type` parameter.
     - Executes the generated SQL query and sends back the data along with customizer settings.
     - Includes a `codeCustomization` flag for `TotalShipments` reports, indicating a different processing path.
     - Logging of the generated SQL query (`mainQuery`) is present.
   - **Recurring Elements/Patterns:**
     - Centralized dispatcher for various report types.
     - Consistent handling of date range defaults.
     - Relies on `getUserObject` for user and company details.

**6. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\export.query.ts`**
   - **Timestamp of Significant Changes:** Multiple changes on **December 4, 2025**, from 3:39:54 PM to 5:52:20 PM, and again on **December 5, 2025**, from 10:58:20 AM to 1:57:27 PM. Similar to `booking.query.ts`, many of these appear to be minor modifications or saving artifacts where the code content remains largely unchanged.
   - **Key Updates:** This file provides SQL query snippets for various analytics reports specifically tailored for export functionality.
     - The `getMonthValue` helper function maps month names (e.g., 'Jan') to numerical values.
     - The `bookingQuery` function returns a dictionary of SQL string fragments, each corresponding to a specific report type (e.g., "Booking Date to Actual Departure", "Carrier On-time delivery By carrier").
     - These snippets primarily define the `SELECT` and `WHERE` clauses for different reports, focusing on calculating date differences or specific metrics.
     - Includes conditional logic for `stackedExport` and `displayBy` to add date filtering based on month, week, or quarter.
     - The "Carrier On-time delivery By carrier" query was refined to include `GROUP BY LOWER(m.scac_code)` and explicitly check for non-empty string values in `eta`, `actual_arrived_date`, and `scac_code`.
   - **Recurring Elements/Patterns:**
     - Consistent joins between `mtr_tracking_data` and `agent_booking_entry`.
     - Filtering by `LOWER(account_name) = '${CompanyName}'`.
     - Checks for `NULL` and `0001-01-01 00:00:00 BC` for date columns.
     - Conditional logic for date-based grouping (`stackedExport`, `displayBy`).
     - `EXTRACT(DAY FROM ...)` for calculating date differences.

**7. `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\core.export.controller.ts`**
   - **Timestamp of Significant Changes:** The file was modified twice on **December 5, 2025**, at 10:52:08 AM and 11:40:58 AM, and again at 11:53:48 AM, 11:55:28 AM, 1:56:46 PM, and 1:56:56 PM.
   - **Key Updates:** This controller handles the export of analytics data.
     - It receives numerous query parameters to customize the export.
     - Uses `columnsQuery` to construct the `SELECT` clause based on requested columns.
     - Calls `saveColumns` to persist column selections.
     - It checks `isBooking` to determine whether to use `bookingQuery` or `nonBookingQuery` from `export.query.ts` to get the base SQL snippet.
     - **A significant change at 11:53:48 AM** introduces conditional logic within the `isBooking` block: if `bookingQuery` returns an empty `customQuery`, it falls back to `nonBookingQuery`, indicating a more robust handling of different report types for booking data.
     - It constructs the final `mainQuery` by combining selected columns, the custom query snippet, and date filters.
     - Executes the query and applies `dateFilterOnExport` and `replaceColumnHandler` for post-processing the data.
     - The `replaceColumnHandler` is a utility to replace 'LONG BEACH' with 'LOS ANGELES' for `pod` and `place_of_delivery` fields, suggesting a data standardization requirement.
     - Includes a special filter for "TEU MBLwise" reports to deduplicate containers and set `teu` to '-' for duplicates.
     - Several `console.log` and `logger.info` statements were added/modified, likely for debugging during development.
   - **Recurring Elements/Patterns:**
     - Extensive use of query parameters for dynamic report generation.
     - Layered query construction using helper functions.
     - Post-processing of query results for filtering and data transformation.
     - Debugging statements are noticeable.

**Overall Patterns and Recurring Elements:**

- **Date Formatting and Handling:** Dates are consistently formatted to "YYYY-MM-DD" using `AppDateFormate`. Queries often include checks for `NULL` values or '0001-01-01 00:00:00 BC' for date fields to prevent calculation errors.
- **Input Sanitization:** `sanitizeString` and `sanitizeIntString` are widely used across controllers to prevent SQL injection and ensure data integrity.
- **Database Transactions:** Both `booking.controller.ts` and `hblEntry.controller.ts` involve multiple database operations and appear to intend to use transactions (`BEGIN`, `COMMIT`, `ROLLBACK`), although the `BEGIN` and `COMMIT` in `saveBooking` are commented out.
- **Modular Query Generation:** The analytics controllers (`analytics/query/booking.query.ts`, `analytics/query/export.query.ts`) employ a modular approach to build complex SQL queries based on report types and user-defined parameters, using utility functions like `getColumnsByCategory` and `getFilterQueryByDisplayValue`.
- **Audit Logging:** The `hblEntry.controller.ts` explicitly uses `simpleAudit` and `insertAudit` to log changes, indicating a requirement for tracking data modifications.
- **Data Standardization/Transformation:** The `replaceColumnHandler` in `core.export.controller.ts` demonstrates a need for harmonizing location names ('LONG BEACH' to 'LOS ANGELES').
- **Debugging Statements:** Frequent `console.log` and `logger.info` statements, especially in analytics and HBL controllers, suggest active development and debugging.
- **MBL and Booking ID Association:** A common pattern in analytics queries is joining `mtr_tracking_data` with `agent_booking_entry` on `mblno` to link tracking data with booking information.
- **Container and Shipment Tracking:** Many analytics reports focus on metrics related to containers and shipments, such as transit times, departure dates, and on-time delivery.