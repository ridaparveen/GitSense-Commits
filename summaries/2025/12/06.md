...
This file manages configuration settings for both `development` and `production` environments, including database connections, numerous file system paths, integration credentials, and extensive email distribution lists.

*   **12/5/2025, 2:06:15 PM**: The initial or baseline configuration provided defines database settings (username, host, database name, port), numerous local file paths (e.g., for XML, Excel, CSV files, Google token paths), OTM integration credentials, and a wide array of email configurations with specific recipients for various reports (e.g., `isf24HrReport`, `anchor24HrJsonReport`, `doBatchReport`, `userRegistrationApproval`).
*   **12/5/2025, 2:13:27 PM**: The email recipient list for `development.email.userRegistrationApproval.to` was modified, moving an email address (`ridaparveen679@gmail.com`) from the 'to' field to the 'cc' field.
*   **12/5/2025, 2:14:56 PM**: A new `customer` field was added to the `development.email.userRegistrationApproval` object, set to `rajeev.kumar@transmodal.net`.
*   **12/5/2025, 2:18:39 PM**: The `development.email.userRegistrationApproval.cc` list was updated, with two email addresses removed and `rida@dyneinfotech.com` added.
*   **12/5/2025, 2:21:30 PM**: Significant changes were made to the `development` environment:
    *   The database name was updated from "Rida_Database" to "tmodal", and the database password was changed.
    *   The `googleTokenPath` was updated from a user-specific OneDrive path to a more general `D:/Projects/cng-batches/config`.
    *   An email address (`rida@dyneinfotech.com`) was removed from the `development.email.all` list.
*   **12/5/2025, 2:24:48 PM**: New XML path configurations, `cngInvXmlPath` and `sprInvXmlPath`, were added to both `development.paths` (with specific local paths) and `production.paths` (initialized as empty strings). Additionally, a seemingly misplaced and redundant `to` field was added directly under `development.email`.

**File: `c:\Users\ridap\OneDrive\Documents\cng-batches\cng-batches\modules\login-req-notify\nofifyAdmin.js`**

This file contains logic for notifying administrators about new user registrations and requesting additional information from users.

*   **12/5/2025, 2:09:33 PM**: The `notifyAdmin` function was implemented to:
    1.  Fetch new users with a 'NEW' status from the `mtr_user_registration` table.
    2.  Send an email to all configured recipients (`config.email.all`) containing a table of newly registered users and a link to the admin panel.
    3.  Increment an `emailcounter` for each notified user in the database.
    4.  Conditionally send a second email to eligible users (those with `emailcounter <= 2`) requesting additional information (Shipment Number, Container Number). The HTML for this second email was initially hardcoded within the `sendEmail` call, and the support email dynamically constructed from `newUsers`.
*   **12/5/2025, 2:10:02 PM**: The hardcoded HTML content for the "Additional Information Required" email was refactored into a dedicated method, `generateUserRegisterApprovalEmailContent()`. The support email contact within this template was updated to consistently use `config.email.userRegistrationApproval.support_team`.
*   **12/5/2025, 2:15:39 PM**: The greeting in the `generateUserRegisterApprovalEmailContent()` template was updated to include dynamic customer name placeholder using `config.email.userRegistrationApproval.customer`.
*   **12/5/2025, 2:16:45 PM**: The dynamic customer name placeholder `Dear ${config.email.userRegistrationApproval.customer}` was changed to a hardcoded `Dear Rajeev,` in the `generateUserRegisterApprovalEmailContent()` method.
*   **12/5/2025, 2:20:06 PM**: Multiple `console.log` statements within the `notifyAdmin` function were replaced with or supplemented by `logger.info` calls to provide more structured and detailed logging about new users, email counters, and eligible users.

**Patterns and Recurring Elements:**

*   **Email-centric updates**: A dominant pattern across both files is the continuous refinement of email configurations and email sending logic. This includes updating recipient lists, adding new email categories, modifying email content templates, and improving dynamic email generation.
*   **Iterative configuration changes**: The `config.js` file shows frequent, small adjustments to various settings (paths, email lists, database details) over a short period, suggesting active development or debugging cycles.
*   **Refactoring and logging improvements**: In `nofifyAdmin.js`, changes indicate an evolution towards more maintainable code (refactoring HTML content into methods) and better operational visibility (enhancing logging).
*   **Timestamps**: All recorded changes occurred on December 5, 2025, within a relatively narrow timeframe (approximately 2:06 PM to 2:24 PM), suggesting a focused work session.

## 1:44:27 PM
The provided log details changes across several JavaScript files within a React frontend application, primarily concerning House Bill of Lading (HBL) forms and related components. The timestamps span from December 3rd to December 5th, 2025.

### File-Specific Updates:

#### `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HblPoForm.jsx`

*   **12/3/2025, 12:25:57 PM - 12:26:15 PM:** The component name was changed from `PODetailsExtend` to `HblPODetails`. The `formik` prop passed to `PODetailsExtend` was changed to `formik.values`. This component handles the display of "Basic PO" and "Linked PO" tabs, with the default tab ('1' for Basic PO) dynamically changing to '2' (Linked PO) if `formik.values.client_name` is 'SP RICHARDS COMPANY'.
*   **12/3/2025, 12:27:09 PM:** The `formik` prop passed to `HblPODetails` was changed to `values`, and subsequently, `formik.values.client_name` within the `useEffect` hook became `values.client_name`. The `formik` prop passed to `BasicPoExtend` and `LinkPoExtend` was also updated to `values`.
*   **12/3/2025, 1:01:38 PM:** A `console.log` statement was added to debug the `values` prop.
*   **12/3/2025, 3:23:41 PM:** Another `console.log` statement was added (`values===901234`).
*   **12/3/2025, 3:25:22 PM:** The `BasicPoExtend` component within the "Basic PO" tab was replaced with a new component `BasicPoForHbl`, indicating a refactoring or specialization for HBL-specific basic PO details.
*   **12/3/2025, 3:49:19 PM:** The logic for setting the default `tabValue` in the `useEffect` hook was changed. Instead of `values.client_name`, it now checks `values.basicPoList?.[0]?.customer`. This suggests a change in how the client name is stored or accessed, possibly moving it into the `basicPoList` array.

#### `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HBLform.jsx`

*   **12/3/2025, 12:26:33 PM:** This is a large update introducing significant functionality for HBL forms.
    *   Imports for several components (e.g., `HblContainerDetailsForm`, `HblRoutingForm`, `HblHeaderForms`, `HblPODetails`) and Redux hooks (`useDispatch`, `useSelector`) and RTK Query mutations (`useSaveHblBookingMutation`, `useUpdateHblBookingMutation`) were added.
    *   A `reducer` function was defined for managing options and row data.
    *   Initial values for the `Formik` component were expanded significantly to include many HBL-related fields (shipment details, party names, addresses, ports, dates, vessel, voyage, marks, description, booking numbers). These initial values are conditionally populated based on `initialValues` and `location.state`.
    *   The `handleSave` function was implemented to process form data, transform values (e.g., splitting `ship_name` into `ship_name` and `ship_code`), and call either `saveBooking` or `updateHblBooking` mutations based on `formAction`. Error handling and success navigation with `toast` notifications were also added.
    *   Keyboard shortcuts (`ctrl+u`, `ctrl+d`, `ctrl+a`, `ctrl+s`, `ctrl+p`) were implemented for party management and saving the form.
    *   The form structure includes `HblHeaderForms` and `ThemeTabs` for HBL Parties, indicating a multi-section form. Party-specific actions (Edit, View, Add New Party) are present.
    *   Redux state (`routingList`, `containerList`, `basicPoList`) is integrated for managing lists within the form.
*   **12/3/2025, 1:38:27 PM - 1:41:10 PM:** Minor adjustments to the `agent_booking_id` initialization logic within `initialValues` for `formAction` being "copy" or "add".
*   **12/3/2025, 1:41:58 PM:** Further refinement of the `agent_booking_id` initial value logic, specifically when `formAction` is "copy" or "add", referencing `location.state.agent_booking_id`.
*   **12/3/2025, 1:46:11 PM:** The `enableReinitialize` prop for `Formik` was changed from `formAction !== "add"` to `formAction == "add"`, then immediately reverted. This suggests testing or a momentary change in reinitialization behavior.
*   **12/3/2025, 1:46:51 PM:** `HblPODetails` import was removed and `BasicPoForm` was retained, indicating a possible change in how PO details are rendered directly in `HBLform.jsx`. (However, later commits show `HblPODetails` being re-imported, so this was likely temporary or a rollback).
*   **12/3/2025, 1:47:04 PM - 1:55:49 PM:** Numerous minor, mostly whitespace or console.log related, changes. The initial value for `booking_no` was tweaked multiple times, particularly for "copy" and "add" actions, trying to correctly derive it from `initialValues` or `location.state`.
*   **12/3/2025, 2:37:43 PM:** A `useFetchHblBookingFormDetailsQuery` was introduced, possibly for fetching HBL form details for editing, though it was quickly changed in the next commit.
*   **12/3/2025, 2:40:01 PM:** The `useFetchHblBookingFormDetailsQuery` was replaced with `useFetchPoFormDetailsQuery` and its parameters were updated to conditionally fetch based on `location.state?.agent_booking_id`. `skipToken` from `@reduxjs/toolkit/query` was imported. The `poDetails` data from this query is logged.
*   **12/3/2025, 2:42:38 PM - 2:53:46 PM:** The `basicPoList` initial value was updated to prioritize data from `poDetails?.data?.basicPoList` if available, otherwise falling back to `initialValues?.basicPoList`, and finally to `poDetails?.data?.basicPoList` (seemingly redundant, possibly reflecting intermediate stages of logic refinement).
*   **12/3/2025, 2:49:44 PM:** `basicPoList` initial value was commented out, indicating a temporary removal or different approach, then uncommented shortly after.
*   **12/3/2025, 4:56:44 PM:** `linkedPoList` was added to `initialValues`, also prioritizing data from `poDetails?.data?.linkedPoList`. This suggests that the `useFetchPoFormDetailsQuery` now returns both basic and linked PO lists.

#### `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\HblHeaderForms.jsx`

*   **12/3/2025, 1:42:46 PM - 1:51:28 PM:** This component received updates for handling form input changes.
    *   `clientOptions` state and an `useEffect` to fetch them using `ApiManager.getAutocomplete("booking_customer")` were added.
    *   An "Add Vessel" button and corresponding `ThemedModal` and `AddVesselForm` were introduced, with a keyboard shortcut `ctrl+x` for opening the modal.
    *   The `onChange` handler for `booking_no` was changed from `onChange` to `handleChange`.
...
*   **12/3/2025, 1:43:57 PM:**
    *   `useDeleteHblBookingMutation` and `useGetHblDashboardQuery` were imported and utilized.
    *   A `handleDelete` function was implemented for deleting HBLs with user confirmation and `toast` notifications.
    *   The `COLUMNS_GRID` for `ThemedGrid` now includes navigation and delete functionality.
    *   The "Add HBL" button's `onClick` handler was updated to navigate to the HBL form screen with `formAction: "copy"`.
*   **12/3/2025, 1:54:42 PM:** The "Add HBL" button's `formAction` was changed from "copy" to "add", indicating a switch to creating new HBLs rather than copying existing ones.
...
*   **12/3/2025, 12:33:44 PM:** This component was updated to manage a list of Basic PO items using Redux (`addList`, `removeList`, `updateList`). It includes fields for PO number, date, supplier, quantity, packages, weight, weight unit, and volume. It also fetches supplier options using `ApiManager`.
*   **12/3/2025, 3:24:24 PM:** No functional changes, possibly a formatting or save operation.

#### `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\store\api\agentBookingApi.js`

*   **12/3/2025, 2:38:52 PM:**
    *   New endpoints were added for HBL management: `saveHblBooking` (mutation), `fetchHblBookingFormDetails` (query), `fetchPoFormDetails` (query), `getHblDashboard` (query), `updateHblBooking` (mutation), `deleteHblBooking` (mutation).
    *   New `tagTypes` ("hbl") were introduced for cache invalidation.
    *   The `updateDo` mutation was enhanced with more precise tag invalidation logic for "BOOKING_CONFIRM" using an `id`.

#### `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\hbl\BasicPoForHbl.jsx`

*   **12/3/2025, 3:25:02 PM:** This new file was created, seemingly as a specialized version of `BasicPoExtend`. It uses `useDispatch` and `useSelector` to manage a `basicPoList` from the Redux store. It includes input fields for PO details (number, date, packages, quantity, weight, volume, supplier) and action buttons (Add, Delete), though the `onChange` and API call logic for updating rows and fetching supplier options are commented out.
*   **12/3/2025, 3:27:29 PM:** The imports were adjusted to point to common components within `../../../common`. `ApiManager` and `recoverValue` imports were removed, and `onChange` handlers for inputs were commented out, indicating that the form inputs in this component might be intended for display or read-only purposes, or the update logic is handled externally. The `IconButton` for add/delete row was commented out. `Grid` item sizes were adjusted (`lg={1}` to `lg={1} md={6} sm={8} xs={12}`).
*   **12/3/2025, 3:27:50 PM:** No functional changes, likely formatting.
*   **12/3/2025, 3:28:41 PM:** A `console.log` for `formik` prop was added.
*   **12/3/2025, 3:29:13 PM - 3:37:58 PM:** The `selector.map` loop was changed to `formik?.basicPoList?.map`, indicating that this component now directly consumes the `basicPoList` from the `formik` prop rather than relying on the Redux selector for rendering the list itself (though the Redux selector `selector` is still present, its usage for iteration was removed).

#### `c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\booking\form_extend\CargoFormExtend.jsx`

*   **12/3/2025, 4:58:45 PM - 12/5/2025, 2:27:08 PM:** This file shows consistent content across multiple timestamps. It's a component for managing a list of cargo items using an `AppTable`. It integrates with Redux to `addList`, `updateList`, and `removeList` for the `cargoList` state. A modal (`ThemedModalExtend`) is used for adding and editing cargo details via `AddCargoFormExtend`. The table columns include action buttons for editing and deleting rows.

### Patterns and Recurring Elements:

1.  **Redux Integration:** Heavy reliance on Redux (specifically `@reduxjs/toolkit/query/react` and `react-redux`'s `useDispatch`, `useSelector`) for state management and API interactions. Mutations (`useSaveHblBookingMutation`, `useUpdateHblBookingMutation`, `useDeleteHblBookingMutation`) and queries (`useGetHblDashboardQuery`, `useFetchPoFormDetailsQuery`) are extensively used for data fetching and persistence.
2.  **Formik for Forms:** `Formik` is consistently used for form management, including initial values, validation schemas, and submission handling.
3.  **Material-UI (MUI):** Extensive use of MUI components (`Box`, `Grid`, `Stack`, `Card`, `CardHeader`, `CardContent`, `Tab`, `TabContext`, `TabList`, `TabPanel`, `IconButton`, `Accordion`, `Typography`, `CircularProgress`) for UI layout and elements.
4.  **Modular Components:** The application is structured with many sub-components (e.g., `HblHeaderForms`, `BasicPoExtend`, `LinkPoExtend`, `AddVesselForm`, `AddCargoFormExtend`) indicating a modular design approach.
5.  **Dynamic UI/Conditional Rendering:** Logic to dynamically change UI elements or data based on conditions, such as `formAction` (add/edit/copy), `client_name`, or the presence of data.
6.  **"Extend" Suffix:** Several components have an "Extend" suffix (e.g., `LinkPoExtend`, `BasicPoExtend`, `InputBoxExtend`, `AppDatePickerExtend`, `AppAutocompleteExtend`, `SelectBoxExtend`, `ThemedModalExtend`), suggesting a pattern of extending or customizing base components for specific functionalities or styling.
7.  **Console Logging:** Frequent use of `console.log` statements throughout the code for debugging, especially to inspect `values`, `poDetails`, and `location`.
8.  **Keyboard Shortcuts:** Implementation of global keyboard shortcuts (`useKeyboardShortcut` hook) for common actions like saving, editing parties, and adding new parties/vessels.
9.  **Data Transformation:** A `recoverValue` helper function is used to parse string values (e.g., "name~code") into their individual parts, indicating a common data format for certain fields.
10. **Refactoring/Specialization:** The creation of `BasicPoForHbl.jsx` from `BasicPoExtend.jsx` and the subsequent change in `HblPoForm.jsx` to use the specialized component indicates a move towards more HBL-specific functionality, potentially to simplify or isolate logic. The commenting out of `onChange` and action buttons in `BasicPoForHbl` further supports a possible shift towards a more read-only or externally controlled display for that component within the HBL context.