...
## 11:15:03 PM
The `uploadDocuments.ts` file was updated on 9/9/2025, 11:13:28 PM.  The update involved several functions related to document management:

* **`uploadDocument`**: This function handles uploading documents.  It now explicitly requires a file (`req.file`), logs user info,  and inserts document data into the `document_upload` table.  The `uploaded_by` field is hardcoded as "system" but could be improved to use user information from `req.user`.  Error handling is in place using `HttpError` and logging.

* **`fetchUploadedDocuments`**: This function retrieves uploaded documents based on `ref_type` and `ref_id`. It constructs download links using `process.env.BACKEND_BASE_URL` and replaces backslashes with forward slashes in file paths for consistency.  It returns an empty array if no documents are found, improving the response.

* **`deleteDocument`**: This function deletes a document by `serial_id`. It first checks for the document's existence, then deletes it from the database and the file system if the file path exists.   Comprehensive error handling and logging are implemented.

* **`downloadDocument`**: This function handles document downloads. It retrieves the document from the database, validates the file's existence, and then uses `res.download` to initiate the download process.  It includes robust error handling and logging.


The code uses a common pattern of error handling and logging throughout all functions, employing `try...catch` blocks and the `logger` utility.  Database interactions are consistently handled using `query` and `insertQuery` functions. The code also shows a reliance on environment variables (`process.env.BACKEND_BASE_URL`, `process.env.DOC_UPLOAD_PATH`) for paths and URLs, indicating a setup designed for deployment.  The update appears to improve error handling and response consistency across the different document management functions.
