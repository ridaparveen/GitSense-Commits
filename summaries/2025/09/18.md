...
## 6:40:04 PM
The log shows a series of modifications to the `vesselAndVoyageDataApi.ts` file and a final update to the `store.ts` file.  The changes in `vesselAndVoyageDataApi.ts` are focused on refactoring API endpoints related to vessel and voyage data.

Initially (5:53:35 PM),  `vesselAndVoyageDataApi.ts` contained endpoints for fetching vessel dashboard data, saving vessels, fetching vessel details, and getting vessel card data, all using `/api/vessel/` base URLs.

A subsequent update (5:56:29 PM) changed all API endpoints to use `/api/vessel-voayge/` base URLs, suggesting a merging of vessel and voyage data handling into a single API section.  The `tagTypes` array was also updated, replacing `"vesselDashboardData"` with `"voyageDashboardData"`.

The final modification to `vesselAndVoyageDataApi.ts` (5:59:57 PM) renamed several endpoints, replacing "Vessel" with "Voyage" in their names (e.g., `fetchVesselDashboardData` became `fetchVoyageDashboardData`). A new `updateVoyage` mutation was added.  The core functionality remained consistent – fetching dashboard data, creating, updating, and fetching details – but now explicitly focused on voyages.

The `store.ts` file (6:01:08 PM) update integrated the final version of `vesselAndVoyageDataApi` into the Redux store, along with other API reducers and slices. This suggests a complete integration of the refactored API into the application's state management.  The consistent use of `getAppHeaders()` across all API calls suggests a centralized authentication or authorization mechanism.  The overall pattern reveals a shift from a separate vessel and voyage data handling to a unified approach.


## 6:40:13 PM
The log shows a series of code changes made to the `vesselAndVoyageService.ts` file and related files (`Paths.ts`, `vesselAndVoyageRoutes.ts`) between 4:23 PM and 6:37 PM on September 18, 2025.  The changes primarily focus on enhancing the functionality for managing vessels and voyages.

**`vesselAndVoyageService.ts` Changes:**

The `vesselAndVoyageService.ts` file underwent multiple revisions.  Key changes include:

* **Initial Commit (4:23 PM):** This commit introduced a `VesselDashboardQueryBuilder` class designed for constructing complex SQL queries to fetch vessel and voyage data.  The class handles pagination, sorting (using `orderBy` parameter), filtering by status (active/inactive), and search/filter clauses. The `getAll` function fetches voyage data including related routing information.  The `CreateVesselAndVoyage` function handles creating new voyage entries, including transaction management (`BEGIN` and `COMMIT`). The `saveVessel` function is incomplete in this initial commit.

* **Subsequent Commits (4:26 PM, 4:31 PM, 4:32 PM):** These commits incrementally completed the `saveVessel` function, adding more fields from the request body to the payload for database insertion or update, using nullish coalescing (`??`) to handle optional parameters and a date formatting function (`AppDateFormate`).

* **Commit at 4:50 PM:**  Added a new `fetchVesselAndVoyageData` function which retrieves data for a specific voyage based on `serial_id`.  The `CreateVesselAndVoyage` function was updated to handle both creating new voyages and updating existing ones based on `entryMode` determined by checking the existence of `vesselVoyage.serial_id`.  The `status` field was added to the `voyagePayload`.


* **Commits at 4:52 PM, 4:55 PM, 4:55 PM:** These commits refined the code.  Minor improvements in formatting and variable assignments occurred. There were some additions to the `fields` array in `fetchVesselAndVoyageData` function.  The most important change is adding "vessel_id" in fields array in `fetchVesselAndVoyageData` function.


* **Commit at 5:41 PM - 5:49 PM:**  This is a major update adding an `UpdateVesselAndVoyage` function to handle updates for existing voyage entries. It includes transaction management and uses `updateQuery` to update database rows. The function handles the deletion of existing routing entries before inserting new ones.  These commits improved error handling and added more robust data validation in `UpdateVesselAndVoyage`.


* **Final Commit (5:49 PM):** The `getVesselAndVoyageDashboardCardInfo` function was added to fetch dashboard card information (active, inactive, and total voyages) with optional filtering by date range or time period (weekly, monthly, all).

**`Paths.ts` Changes:**

The `Paths.ts` file, which defines API routes, was updated at 5:44 PM and 6:37 PM to include new endpoints corresponding to the new functions added in `vesselAndVoyageService.ts`:

* Addition of `/update-vesselVoayge` endpoint for the new UpdateVesselAndVoyage function.  The  `Voyage` section in Paths.ts also got addition of `/vesselVoayge-card-details`

* The final commit added a `Vendor` section to the Paths object to reflect the inclusion of vendor management routes in the project.


**`vesselAndVoyageRoutes.ts` Changes:**

This file, which registers routes for vessel and voyage management, was modified at 5:45 PM, 5:47 PM and 5:48 PM to incorporate the new API endpoints defined in Paths.ts:

* New routes were added for `UpdateVesselAndVoyage` using PUT HTTP method and improved error handling.


**Overall Pattern:**

The changes follow a pattern of iterative development.  New features (query building, voyage creation, fetching specific voyage data, updating voyages, and dashboard card info) were added in stages.  There is consistent use of async/await for database operations, transaction management, and clear error handling. The code consistently uses logging for debugging and monitoring. The changes demonstrate a well-structured approach to building and maintaining an API.
