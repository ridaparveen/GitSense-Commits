...
## 6:53:42 PM
The log shows multiple revisions of the `invoice.controller.ts` file between 5:03 PM and 5:35 PM on September 4th, 2025.  The core functionality remains consistent throughout: retrieving invoice data based on various parameters (page, perPage, orderBy, fromDate, toDate, invoice_no, mblno) from either `cng_invoice` or `spr_invoice` tables depending on the `req.companyname`.  The data retrieved includes invoice details through joins with `cng_invoice_details` or `spr_invoice_details` and `cng_xml_batchlog`.

The primary changes across revisions are minor and focus on the `orderBy` parameter:

* **Initial versions (5:03 PM, 5:06 PM):** The code used `orderBy = "serial_id*DESC"` with a capitalized `DESC`.  There's also a difference in the response JSON where `allRecords` is renamed to `count` in one commit.  One version also included `allRecords` in the response JSON, which was later removed.


* **Subsequent versions (5:13 PM onwards):**  The `orderBy` parameter in the `req.query` destructuring is changed to lowercase `"serial_id*desc"`.  This necessitated no other changes in the code, as the code already handles case-insensitive ordering.


The code consistently uses conditional logic to dynamically select tables and join keys based on the `req.companyname`.  Error handling is included to catch database syntax errors and other exceptions, logging API usage with the `api_key_id` for each request and response. The SQL query is consistently logged using `logger.info`.  The overall structure and functionality of the API endpoint remain unchanged.
