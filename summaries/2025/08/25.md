...
## 6:07:50 PM
The log shows multiple revisions to the `dashboardService.ts` file between 5:23 PM and 6:04 PM on August 25, 2025.  The primary function being modified is `getDeliveryData`, which retrieves delivery information based on query parameters `type` ('delivery' or 'vessel') and `mode` (Door Delivery, Port Delivery, or Rail Delivery).

The `getDashboardCardInfo` function remains largely unchanged throughout the revisions. It constructs a complex SQL query to fetch dashboard card information, including counts and TEU (Twenty-foot Equivalent Unit) calculations for various shipment statuses (Pending Departure, In Transit, ETA within 1 week, Out for Delivery).  The query uses `mtr_tracking_data` table and incorporates date range filtering based on `fromDate` and `toDate` query parameters.  The consistent use of `t49_` prefixed columns suggests the integration of data from a system or table identified as 'T49'.

The most significant changes occur in the `getDeliveryData` function, specifically within the `extraFilter` variable which is used to refine queries based on the `mode` parameter.  The initial versions contained inconsistencies or omissions in the filtering logic for "Door Delivery" and "Port Delivery" modes. These were refined in subsequent commits.  Specifically:

* **5:24:36 PM:** A minor correction was made to `extraFilter` within the "Door Delivery" condition to include `delivery_customer_date`.
* **5:31:19 PM and 5:31:24 PM:**  Further refinements to the Port Delivery filtering logic using `t49_vessel_actualarrived_date` and `eta_pod`.
* **5:33:01 PM and 5:33:16 PM:** The "Door Delivery" filter was changed from using `delivery_customer_location` to `place_of_delivery`, and subsequently minor adjustment was done removing `eta_pod` from port delivery filter. 
* **5:56:13 PM and 5:56:33 PM:** The Rail Delivery filter was improved by adding `rail_ata` and modifying the vessel query to include `rail_ata` in the selection.
* **6:00:32 PM and 6:04:02 PM:**  Final adjustments to the filtering conditions for "Door Delivery" and "Rail Delivery" modes were made, reverting back to the previous logic for "Door Delivery" to use `delivery_customer_location` and simplifying condition for "Rail Delivery".

The recurring pattern is the iterative refinement of the SQL queries within `getDeliveryData` to correctly filter results based on user-specified delivery modes.  The overall structure and logic of both functions remain relatively stable throughout the revision history.


## 6:07:52 PM
The log shows multiple revisions to two files: `dashboard-page.tsx` and `Calender.tsx` on August 25th, 2025.  The changes primarily focus on enhancing the dashboard's calendar component (`Calender.tsx`) and integrating it with data fetching and display in the `dashboard-page.tsx`.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\pages\dashboard\dashboard-page.tsx`:**

This file underwent several revisions between 5:09 PM and 5:18 PM.  The main updates involved:

* **API Integration:**  The component makes multiple API calls (`useFetchShipmentDataQuery`, `useFetchDeliveryDataQuery`, `useFetchCartDataQuery`) to retrieve data for the dashboard.  The logic for handling loading states (`isLoading`, `isFetching`) and conditional rendering based on API responses has also been added.  Data filtering and pagination are also implemented using Redux actions (`dashboardSetFilters`, `dashboardSetPagination`).

* **Calendar Data Preparation:**  A `useMemo` hook processes the `deliveryData` from the API and transforms it into a format suitable for the `Calendar` component. This involved filtering deliveries based on the selected date range and calendar type ("vessel" or "delivery").

* **Date Selection and Filtering:** Improvements in how date range is selected and updated in `DatePeriodSelector` are seen.

* **UI Enhancements:**  The code includes various UI improvements such as loading placeholders and conditional rendering for different states, like when data is loading or no data is available.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\common\Calender.tsx`:**

This file was modified between 5:09 PM and 5:59 PM. The key updates are:


* **Delivery Mode Selection:** A select element for choosing delivery modes ("Door Delivery", "Port Delivery", "Rail Delivery") was added, particularly for the "vessel" calendar type.  This element was initially a hardcoded select, then improved to be dynamic and use  `onDeliveryModeChange` prop for handling changes.

* **Delivery Mode Color Coding:** The background color of each delivery item in the calendar is updated based on selected `deliveryMode`, using the `cn` utility for conditional class naming.

* **Prop Updates:** The `Calender.tsx` component's props (`deliveryMode`, `onDeliveryModeChange`) were refined for better handling of delivery mode selection and updates.  The  `iFetching` prop was renamed to `isFetching`.  The `onTitleChange` prop's type was also corrected.

Overall, the changes reflect a focused effort to improve the dashboard's calendar functionality.  The initial versions had basic functionality, followed by several iterations focusing on data handling, UI enhancements, and user interaction improvements, particularly around delivery mode selection. The final version includes the `cn` utility for more efficient conditional class handling.
