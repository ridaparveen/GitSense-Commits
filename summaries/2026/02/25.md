...
4.  **`c:\Users\ridap\OneDrive\Documents\Envosyes\frontend\envosys-frontend\src\components\common\note\NotesDrawer.tsx`**
    ...
    *   **Timestamp: 2/25/2026, 12:59:10 PM:** A `console.log("📝 [NotesDrawer] Rendered with props:", { ... });` was added at the component's top level.
    *   **Timestamp: 2/25/2026, 12:59:37 PM:** A `console.log("📝 [NotesDrawer] Local notes from Redux:", localNotes);` was added after fetching local notes.
...
*   **Debugging via `console.log`:** Extensive `console.log` statements are present across almost all changed files. They are used for monitoring component rendering, user state, API responses, form data, and specific data structures. The `NotesDrawer` component uses a distinct `📝 [NotesDrawer]` prefix for its logs.
...
## 6:10:28 PM
The provided log indicates a significant refactoring effort, primarily focusing on the `employeeServices.ts` file, along with related Prisma schema updates, occurring between February 24th and 25th, 2026. The core theme of these changes is a migration from raw SQL queries to Prisma ORM for enhanced data management and multi-tenancy support.

**File-specific updates:**

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\routes\employeeRoutes.ts` (2/24/2026, 3:23:23 PM):**
    *   This file defines the API routes for employee management. The primary change is the inclusion of `getEmployeeCardInfo` and `getNewEmployeeId` services.
    *   It also shows the use of `uploadMiddleware` for `create` and `update` employee routes, indicating support for document uploads.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\prisma\models\employee.prisma` (2/24/2026, 4:10:34 PM, and subsequent changes on 2/25/2026 at 3:45:08 PM, 5:42:26 PM, 5:42:35 PM, 5:44:15 PM):**
    *   The `employee` model schema is defined, including fields like `serial_id`, `employee_code`, `first_name`, `email`, `designation`, `department`, `joining_date`, `status`, `address` (multiple lines), emergency contacts, blood group, employee type, and location details.
    *   **Multi-tenancy and Location Isolation:** The model explicitly includes `tenant_id` (UUID) and `loc_code` (VarChar) fields with relations to `tenants` and `company_locations` tables, and corresponding `@@index` annotations for performance. This is a critical pattern.
    *   **Audit Fields:** `created_by`, `updated_by`, `created_at`, and `updated_at` fields are present for tracking changes.
    *   **Timestamp (2/25/2026, 3:45:08 PM):** The `status` field's type was changed from `String? @default("ACTIVE") @db.VarChar(20)` to `employeeStatus @default(Active)`, introducing a new `enum employeeStatus { Active Inactive }`.
    *   **Timestamp (2/25/2026, 5:42:26 PM):** The enum name `employeeStatus` was renamed to `status` in the `enum` definition but the `employee` model still referred to `employeeStatus`. This was likely a temporary inconsistency.
    *   **Timestamp (2/25/2026, 5:42:35 PM):** The `employee` model's `status` field was correctly updated to reference the `status` enum (`status status @default(Active)`).
    *   **Timestamp (2/25/2026, 5:44:15 PM):** The enum `status` was reverted back to `employeeStatus`. This indicates an experimental change and subsequent rollback/re-evaluation regarding the enum naming.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\employeeServices.ts` (Multiple timestamps from 2/24/2026, 4:10:45 PM to 2/25/2026, 6:04:30 PM):**
    *   This is the most heavily modified file, showing an iterative migration from raw SQL to Prisma for the `getAll` employee service.
    *   **Initial Migration (2/24/2026, 4:10:45 PM):** The `getAll` function was refactored to use `prisma.employee.findMany` and `prisma.employee.count` for fetching employees and total records. Filtering by `tenant_id`, `loc_code`, `search` term (insensitive `OR` on multiple fields), `status`, `department`, and `employee_type` was implemented using Prisma's `where` clause. It continued to use raw SQL queries for fetching `document_upload` and `notes`. An `getAll_OLD` function, containing the previous raw SQL implementation, was commented out.
    *   **Refinement and Debugging (2/24/2026, 4:36:34 PM to 2/25/2026, 11:40:20 AM):** These timestamps show minor changes, primarily adding `console.log` statements for debugging `req.user?.tenant_id` and temporarily commenting out the filter options query (2/25/2026, 4:42:38 PM) before re-enabling it. The structure and logic remained largely consistent: Prisma for employee data, raw SQL for related documents and notes, and then merging.
    *   **Partial Prisma Migration for Related Data (2/25/2026, 12:06:35 PM):** This is a significant update. The `notes` and `document_upload` fetching were also migrated to Prisma (`prisma.notes.findMany`, `prisma.document_upload.findMany`). This includes applying `tenant_id` filtering for documents and notes, indicating a more complete adoption of Prisma for data retrieval. Status codes were hardcoded to `200` and `401`/`500` instead of `HttpStatusCodes` constants in some places (a potential regression). The `notes` and `document_upload` serial_id are explicitly converted to strings.
    *   **Minor Adjustments in Prisma Migration (2/25/2026, 12:08:13 PM - 2/25/2026, 1:09:51 PM):** These changes include commenting out and then re-enabling `document_upload` fetching using Prisma, and minor refactoring of status code usage (e.g., reverting to `HttpStatusCodes` constants in some instances, then back to literal numbers). `generateUniqId` was removed from imports in favor of `generateSeqCode` (12:39:40 PM) for consistency in ID generation. A `fromDate` and `toDate` filter was added to `getAll` at 2/25/2026, 2:42:22 PM for filtering by `created_at`.
    *   **Removal of `location` field from select for `options` (2/25/2026, 3:57:32 PM and subsequent):** The `location` field was commented out from the `prisma.employee.findMany` call used to fetch distinct filter options. This might indicate that location filtering options are no longer derived directly from the `employee` table, or that this was an intentional removal.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\prisma\models\tanent.prisma` (2/24/2026, 4:15:56 PM):**
    *   The `tenants` model is updated to include an `employee employee[]` relation, linking employees to their respective tenants. This reinforces the multi-tenancy structure.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\prisma\models\core.prisma` (2/24/2026, 4:21:38 PM):**
    *   This file defines core models like `users`, `roles`, `companies`, and `company_locations`.
    *   The `company_locations` model is updated to include an `employee employee[]` relation, linking employees to specific company locations, supporting location isolation.
    *   All these core models (`users`, `roles`, `companies`, `company_locations`, `user_locations`, `user_menu`, `user_sequence`, `user_management_audit`, `role_menu`, `menu_permissions`, `menu_permissions_audit`) consistently include a `tenant_id` field and a `tenant` relation, affirming a strong multi-tenancy architecture.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\prisma\schema.prisma` (2/25/2026, 11:56:25 AM, and subsequent changes):**
    *   This is the main Prisma schema file, consolidating all model definitions.
    *   It contains core tables like `notes` and `Sequence` (for ID generation), both with `tenant_id` for multi-tenancy.
    *   Extensive commented-out sections for various other models (`booking_sequence`, `carting`, `consignee_master`, `customer`, `customer_alerts`, etc.) suggest a modular development approach where schemas are added as modules are built out. This also highlights that `document_upload` is an implicit model from a raw SQL query.
    *   **Timestamp (2/25/2026, 12:16:31 PM):** No significant content change from the previous version, primarily a regeneration.
    *   **Timestamp (2/25/2026, 12:22:53 PM):** No significant content change from the previous version, primarily a regeneration.
    *   **Timestamp (2/25/2026, 12:28:01 PM):** No significant content change from the previous version, primarily a regeneration.
    *   **Timestamp (2/25/2026, 12:39:01 PM):** No significant content change from the previous version, primarily a regeneration.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\common\util\globalUpload.ts` (2/25/2026, 12:42:22 PM):**
    *   The `saveDoc` function now explicitly supports `tenant_id` in its parameters and passes it during the `prisma.document_upload.create` call, ensuring uploaded documents are correctly associated with a tenant.
    *   It also includes explicit conversion of `BigInt` `serial_id` to `string` for JSON serialization.

*   **`c:\Users\ridap\OneDrive\Documents\Envosyes\envosys-backend\src\services\notesService.ts` (2/25/2026, 2:06:47 PM and 2:07:16 PM):**
    *   The `getAllNotes`, `addNote`, `updateNote`, and `deleteNote` functions were migrated from raw SQL to Prisma.
    *   All these operations now include `tenant_id` in their `where` clauses (for reads and checks) or `data` objects (for creates), ensuring notes are managed within the correct tenant context.
    *   `DOMPurify.sanitize` is used for `description` and `title` fields to prevent XSS vulnerabilities, showing a focus on security.
    *   BigInt `serial_id` values are explicitly converted to strings for JSON responses.
    *   Old raw SQL implementations are commented out, similar to `employeeServices.ts`.

**Timestamps of Significant Changes:**

*   **2/24/2026, 3:23:23 PM:** Initial setup of `employeeRoutes.ts` with basic CRUD and middleware.
*   **2/24/2026, 4:10:34 PM:** `employee.prisma` model defined, establishing multi-tenancy and location isolation, and audit fields.
*   **2/24/2026, 4:10:45 PM:** First major refactor in `employeeServices.ts` to use Prisma for main employee queries, marking the start of the ORM migration.
*   **2/25/2026, 12:06:35 PM:** Significant step in `employeeServices.ts` where fetching of `notes` and `document_upload` is also migrated to Prisma from raw SQL, moving towards full ORM adoption for `getAll`.
*   **2/25/2026, 12:42:22 PM:** `globalUpload.ts` is updated to include `tenant_id` in `saveDoc` parameters and Prisma `create` calls, confirming multi-tenancy support for document uploads.
*   **2/25/2026, 2:06:47 PM:** `notesService.ts` fully migrates to Prisma, including multi-tenancy and security sanitization.
*   **2/25/2026, 2:42:22 PM:** `fromDate` and `toDate` filters are introduced in `employeeServices.ts` for `created_at` field, adding date range filtering capability.
*   **2/25/2026, 3:45:08 PM - 5:44:15 PM:** Iterative changes in `employee.prisma` related to the `status` field, specifically experimenting with and reverting enum naming (`employeeStatus` vs `status`).
*   **2/25/2026, 3:57:32 PM:** The "location" filter option generation is commented out in `employeeServices.ts`, suggesting a change in how location-based filtering options are intended to be presented or derived.

**Patterns and Recurring Elements:**

1.  **Prisma Migration:** The most prominent pattern is the extensive migration from raw SQL queries to Prisma ORM. This is evident in `employeeServices.ts` and `notesService.ts`, where `query` functions are replaced with `prisma.model.findMany`, `prisma.model.count`, `prisma.model.create`, `prisma.model.update`, and `prisma.model.delete`. Old SQL code is consistently commented out rather than removed.
2.  **Multi-Tenancy (`tenant_id`):** `tenant_id` is a crucial recurring element across all changed files (`employee.prisma`, `tanent.prisma`, `core.prisma`, `schema.prisma`, `employeeServices.ts`, `globalUpload.ts`, `notesService.ts`). It's consistently used in Prisma `where` clauses, model definitions, and function parameters to ensure data isolation per tenant. `loc_code` is also frequently used for location-based data isolation within a tenant.
3.  **Audit Fields:** Models like `employee`, `users`, `roles`, `companies`, `company_locations`, `user_menu`, `user_management_audit`, `notes`, `JobMaster`, etc., consistently include `created_at`, `updated_at`, `created_by`, and `updated_by` (or similar) fields, indicating a strong emphasis on auditability and tracking changes.
4.  **Pagination and Filtering:** The `getAll` functions (e.g., in `employeeServices.ts`, `notesService.ts`) consistently implement pagination (`page`, `perPage`, `take`, `skip`) and various filtering options (`search`, `status`, `department`, `employee_type`). Sorting (`orderBy`, `orderField`, `orderDir`) is also a common feature.
5.  **Error Handling and Logging:** `try...catch` blocks with `logger.error` and `console.error` are consistently used for error handling, returning standardized JSON error responses.
6.  **Data Serialization:** The explicit conversion of `BigInt` `serial_id` to `string` in responses (e.g., `note.serial_id.toString()`) is a repeated pattern, likely addressing issues with BigInt serialization in JSON.
7.  **`process.env.DB_SCHEMA`:** The database schema is consistently referenced via `process.env.DB_SCHEMA` in raw SQL queries (before migration) and in schema comments, suggesting a configurable schema approach.
8.  **Security Measures:** The `notesService.ts` includes `DOMPurify.sanitize` for user-provided text, indicating a focus on preventing XSS vulnerabilities.
9.  **ID Generation:** Functions like `generateUniqId` and `generateSeqCode` are imported and used for generating unique IDs, typically based on a `tableName` and `prefix`.