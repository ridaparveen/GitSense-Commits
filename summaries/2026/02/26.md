...
## 3:02:51 PM
The file `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\booking.query.ts` was last updated on `2/26/2026, 12:43:16 PM`.

This file defines a `bookingQueryBuilder` asynchronous function responsible for constructing SQL queries based on various booking report types. It imports utility functions like `getColumnsByCategory` and `getFilterQueryByDisplayValue` from `@src/utils/analytics` for dynamic column and filter generation.

**Key components and functionality include:**

*   **Interfaces:** Several interfaces (`CalColumn`, `CalColumnConfig`, `Customizer`, `QueryBuilderI`) are defined to structure the configuration and input parameters for the query builder, allowing for dynamic customization of reports, including date ranges, selected columns, calculation types, and chart types.
*   **`bookingQueryBuilder` Function:**
    *   Requires a `type` parameter and throws an error if missing.
    *   Dynamically populates `customizer.selectedColumns` based on user ID and report name, specifically including `bookingdate`, `booking_status_action_date`, `est_cargo_ready_date`, `departure_date` and ignoring certain columns.
    *   Generates specific SQL queries based on the provided `type`:
        *   **`BookingDatetoActualDeparture`:** Calculates the average difference in days between `Live Actual Departed Date` (`t49_vessel_actualdeparted_date`) and `Booking Date` (`bookingdate`) per `fileno` and `contno`. It then expresses this difference as a percentage of the maximum difference. The `customizer` is set for an average calculation, with date range and specific date columns configured.
        *   **`BookingDatetoActualDepartureMblWise`:** Similar to the above, but calculates the average day difference per `fileno` and `mblno` (Master Bill of Lading), marked by `customizer.mblwise = true`.
        *   **`BookingDatetoActualDepartureStackedBar`:** Configures the `customizer` for a "StackedBar" chart type. It calculates the day difference and categorizes it into predefined time buckets (e.g., "Less than 1 week", "1 - 2 weeks"). The query then aggregates these counts, grouping them by a dynamic `categories_y` derived from `departure_date` using `DATE_TRUNC`, facilitating a stacked bar chart visualization.
        *   **`BookingDatetoActualDepartureMblWiseStackedBar`:** Similar to the `StackedBar` type, but likely performs the categorization and aggregation on an MBL-wise basis, indicated by `customizer.mblwise = true`. (The provided log content for this section is truncated).

**Recurring Patterns and Elements:**

*   **Date Difference Calculation:** A consistent calculation of `EXTRACT(DAY FROM t49_vessel_actualdeparted_date - bookingdate)` is central to all query types, reflecting a focus on transit or lead times.
*   **Data Source and Joins:** All queries involve joining `mtr_tracking_data` (aliased as `m`) with `agent_booking_entry` (aliased as `a`) on `LOWER(m.mblno) = LOWER(a.mblno)`.
*   **Date and Data Validity Checks:** Repeated `WHERE` clauses ensure valid date entries (`IS NOT NULL` and `!= '0001-01-01 00:00:00 BC'`) and filter by `CompanyName` and `departure_date` range.
*   **Customizer Configuration:** The `customizer` object is extensively used across all `type` conditions to consistently configure report display options such as `dateRange`, `calType`, `dateColumns`, `booking`, `mblwise`, `chartType`, and `calColumnConfig`.
*   **Container vs. MBL Wise Logic:** There's a clear distinction in logic and SQL queries between container-level (`fileno`, `contno`) and Master Bill of Lading-level (`fileno`, `mblno`) analyses, with corresponding `mblwise` flag settings in the customizer.
*   **Dynamic Query Components:** Utilization of `getFilterQueryByDisplayValue` for dynamic `SELECT`, `GROUP BY`, `ORDER BY`, and `DATE_TRUNC` clauses in the stacked bar chart queries, allowing flexibility in how data is aggregated and displayed.
*   **`DISTINCT ON` Clause:** The `DISTINCT ON` clause for container-related queries (`m.voyage, m.contno, m.t49_cont_type` or `cont_type`) is used to ensure unique container-voyage combinations, preventing duplicate calculations.