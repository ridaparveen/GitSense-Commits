...
## 3:47:06 PM
The provided log details changes across several frontend JavaScript files in a project named "T360_Api/t360-frontend" over the dates 1/2/2026 and 1/5/2026.
...
1.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\App.js`**
    ...
    *   **Summary:** These two entries are almost identical, indicating a minor or no functional change between them. The primary content is a large block of React imports and a complex `App` component rendering numerous `Routes` for a single-page application.
    *   **Key Changes:**
        *   An `Auth0LogoutHandler` component was imported and a `/logout` route was added for it.
        *   The root path (`/`) route was commented out (`<Route path="/" element={ <AuthGuard> <Auth0Login /> </AuthGuard> } />`) and replaced with a route to `LoginScreen` (`<Route path="/" element={ <AuthGuard> <LoginScreen /> </AuthGuard> } />`). This suggests a change in the default landing page or authentication flow.
        *   A new `console.log("🚀 ~ file: App.js:2 ~ TEST on Flow");` was added.
        *   Many new screen imports and corresponding routes were added, including:
            *   `BookingDashbaordScreenExtend`
            *   `AgentBookingFormScreenExtend`
            *   `AgentPoFormExtend`
            *   `Dashboard7501Screen`
            *   `ShipmentsScreen`
            *   `CngBookingScreen`, `CngBookingFormScreen`, `CngBookingScreenExtend`
            *   `CityMaster`
            *   `UploadOTMBOL`
            *   `PortmasterScreen`
            *   `AirTrackingScreen`
            *   `Auth0LogoutHandler`
            *   `HblListDetails`
        *   Several existing routes were updated to use `_Extend` versions of components (e.g., `BookingDashbaordScreen` to `BookingDashbaordScreenExtend`, `AgentBookingFormScreen` to `AgentBookingFormScreenExtend`).
        *   New protected routes were introduced for `/spr/portmaster`, `/spr/city_master`, `/spr/otm_bol/upload`, `/booking/hbl_form_screen`, and `/isf_dashboard/form`.
        *   The `ProtectedRoute` roles were extensively expanded for many routes, adding `UserRoleEnum.SPR_CUSTOMER` and `UserRoleEnum.ADMIN_V2`.

2.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\booking\AgentBookingConfirmation.jsx`**
    *   **Timestamp:** 1/2/2026, 2:59:06 PM.
    *   **Summary:** This file deals with agent booking confirmation, handling file uploads, XML generation/upload, and packing list selection.
    *   **Key Changes:**
        *   A significant logic block was added within the `onSubmit` handler to explicitly check for `initialValues?.agent_booking_id` being `null` or `undefined` before proceeding with form submission, returning an error toast if missing.
        *   The handling of warnings from the `updateDo` mutation was enhanced. If `result.warnings` are present, a `WarningToast` is displayed for 6 seconds, and an `Error` is thrown to trigger the `catch` block for a more robust error display.
        *   A new `setAlertConfig` state was introduced and used in the `catch` block to handle thrown warning errors with a `PopupAlert`.

3.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\code\customer\CustomerForm.jsx`**
    *   **Timestamps:** 1/2/2026, 3:46:26 PM, 1/2/2026, 3:48:00 PM, 1/2/2026, 3:48:14 PM, 1/2/2026, 3:54:30 PM, 1/5/2026, 10:24:58 AM, 1/5/2026, 10:44:05 AM, 1/5/2026, 10:45:28 AM, 1/5/2026, 11:13:26 AM, 1/5/2026, 4:48:32 PM, 1/5/2026, 5:35:15 PM.
    *   **Summary:** This component is a form for managing customer data, including mappings and file uploads. Multiple small changes were made over time.
    *   **Key Changes:**
        *   **Initialization of `customer_mapping`:** The `customer_mapping` field in `formik.initialValues` was adjusted to properly handle cases where it might be empty or undefined, initializing it as an empty array or an array with a default empty customer object. This was done across several commits, demonstrating an iterative refinement for robust state initialization.
        *   **Logging:** Numerous `console.log` statements were added (e.g., `initialValues123456`, `customerArray`, `values.mapping`, `initialValues@@@`) for debugging purposes related to form values and mappings.
        *   **`customer_mapping` payload modification:** For form submission, the `customer_mapping` array was transformed. Initially, it was stringified as `JSON.stringify(values.customer_mapping)`. Later, it was changed to `JSON.stringify(customerArray)`, where `customerArray` is `values.customer_mapping.map(item => item.customer).filter(Boolean)`. This indicates a change in how customer mappings are sent to the API, specifically extracting only the `customer` property from each item in the array and filtering out any empty values. The `mapping` field was also affected by this change in one instance, then reverted or re-aligned.

4.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\components\screen\code\customer\CustomerMapping.jsx`**
    *   **Timestamps:** 1/2/2026, 3:47:04 PM, 1/2/2026, 4:03:31 PM, 1/2/2026, 4:04:17 PM, 1/2/2026, 4:09:07 PM, 1/2/2026, 4:21:23 PM, 1/2/2026, 4:21:38 PM, 1/2/2026, 4:34:47 PM, 1/2/2026, 4:35:01 PM, 1/2/2026, 4:35:58 PM, 1/5/2026, 10:42:52 AM, 1/5/2026, 5:15:15 PM, 1/5/2026, 5:15:23 PM, 1/5/2026, 5:16:17 PM, 1/5/2026, 5:34:05 PM, 1/5/2026, 5:34:40 PM, 1/5/2026, 5:34:55 PM, 1/5/2026, 5:47:00 PM, 1/5/2026, 5:47:34 PM, 1/5/2026, 5:51:09 PM, 1/5/2026, 5:51:30 PM, 1/5/2026, 5:52:01 PM, 1/5/2026, 5:52:15 PM, 1/5/2026, 5:52:45 PM, 1/5/2026, 5:53:40 PM.
    *   **Summary:** This component handles the UI for mapping customers, including adding, deleting, and updating customer entries in a list. It underwent several significant revisions, particularly around deletion logic and initialization.
    *   **Key Changes:**
        *   **Delete Logic Evolution:**
            *   Initially, `deleteRow` simply filtered the array and updated formik state.
            *   It then included API calls (`ApiManager.deleteCustomerOption`) to delete the customer from the backend *if* `rowToDelete?.customer` existed. This was then changed to `ApiManager.deleteCustomerOptions` (note the plural), then reverted, and finally settled on checking `row.customer_id` for backend deletion.
            *   Error handling for API deletion (`toast.error`) was added.
            *   Eventually, the backend deletion logic was removed from `deleteRow`, making it purely a UI operation (splicing the array and updating formik state).
        *   **Initialization of `customer_mapping`:** An `useEffect` hook was added to check if `mapping.length` is 0 and, if so, initialize `customer_mapping` with a default empty customer object (`[{ customer: "" }]`). This was further refined to handle cases where the incoming `mapping` from initial values might be an array of strings instead of objects, converting them to the `[{ customer: "value" }]` format. Another `useEffect` was also added to ensure it's always an array and has at least one item.
        *   **Console Logs:** Numerous `console.log` statements were added for debugging `formik` and `customer_mapping` values.
        *   **`AppAutocomplete` `value` prop:** The `value` prop for `AppAutocomplete` was briefly changed to `row.customer || []` and then reverted to `row.customer`, suggesting experimentation with handling null/undefined values for the autocomplete.

5.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\services\Endpoints.js`**
    ...
    *   **Summary:** This file defines API endpoints for the frontend application.
    *   **Key Changes:**
        *   **`GET_MENU` Endpoint:** The `GET_MENU` endpoint was enhanced to accept an additional optional parameter `ismenudrawer` for more flexible menu fetching.
        *   **`DELETE_CUSTOMER_OPTION` Endpoint:** This endpoint underwent significant changes and re-evaluations:
            *   Initially defined to take `accountName` and include it as a query parameter (`/deleteCustomerOption?account_name=...`).
            *   Changed to `GET_CUSTOMER_OPTIONS` with a query parameter (effectively duplicating a GET endpoint's URL for a DELETE operation).
            *   Corrected to use a path parameter (`/deleteCustomerOption/{customerId}`).
            *   Modified to `/delete/{customerId}`.
            *   Finalized as `/getCustomerOptions/delete/{customerId}`, indicating a specific path under customer options for deletion. This endpoint was consistently changed to accept `customerId` as a path parameter.

6.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\services\ApiManager.js`**
    ...
    *   **Summary:** This file centralizes API calls using the `ENDPOINTS` definitions.
    *   **Key Changes:**
        *   **`deleteCustomerOptions` Method:** The name of the method for deleting customer options was changed from `deleteCustomerOptions` to `deleteCustomerOption` (singular).
        *   **API Method for Deletion:** The `deleteCustomerOption` method was updated to use `ApiMethods.delete(url)` instead of `ApiMethods.get(url)`, aligning with the intended DELETE operation and standard RESTful practices.
        *   **Endpoint Parameter:** The parameter passed to `ENDPOINTS.DELETE_CUSTOMER_OPTION` was updated from `search` (which might have been a name) to `accountName` and then consistently to `customerId`, matching the evolving definition in `Endpoints.js`.

7.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\dashboard\ShipmentTrackingScreen.jsx`**
    *   **Timestamp:** 1/5/2026, 5:37:02 PM.
    *   **Summary:** This component displays shipment tracking details, including a map, stepper, and various information fields.
    *   **Key Changes:**
        *   A new `showMore` state and associated UI ( "More Details" button with `ExpandLessIcon`/`ExpandMoreIcon`) were added to toggle the visibility of additional shipment details (Customer Ref No., Notify, Commodity, Consignee, Service, Piece count, Weight).
        *   The hardcoded `state` object was removed, and route parameters (`params`) are now correctly extracted from `useLocation().search`.
        *   The `Typography` title for the shipment was updated from "Shipment" to "Shipment123".
        *   The layout for displaying shipment details was changed to use `Stack` and `CardField` components for better organization and expandability.

8.  **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-frontend\src\pages\code\CustomerFormScreen.jsx`**
    *   **Timestamps:** 1/5/2026, 4:56:34 PM, 1/5/2026, 4:57:21 PM, 1/5/2026, 4:57:30 PM, 1/5/2026, 5:03:43 PM, 1/5/2026, 5:03:56 PM, 1/5/2026, 5:04:23 PM, 1/5/2026, 5:04:52 PM, 1/5/2026, 5:21:21 PM.
    *   **Summary:** This screen component is responsible for rendering the customer form and managing its initial state, particularly customer mappings.
    *   **Key Changes:**
        *   **Initial `customer_mapping` state:** The initial state for `customer_mapping` was modified several times.
            *   Initially set to `state.initialValues?.customer_map || ''`.
            *   Changed to `state.initialValues?.customer_map || []`.
            *   Later, the `useEffect` hook that sets initial values after fetching `mappingData` was refined. It now explicitly checks if `apiRow.customer_mapping` is an array and, if so, maps string elements to objects of the format `({ customer: c })`, falling back to `[{ customer: "" }]` if empty or not an array. This addresses the common pattern seen in `CustomerMapping.jsx` to ensure consistent object structure.
        *   **Console Logs:** `console.log("mappingData : ",mappingData);` and `console.log("statedata : ",state);` were added for debugging purposes.

### Patterns and Recurring Elements:

*   **Refinement of Customer Mapping Logic:** There's a clear and recurring pattern of refining how "customer mapping" data is handled:
    *   **Initialization:** Consistent efforts across `CustomerForm.jsx` and `CustomerMapping.jsx` to ensure `customer_mapping` is initialized correctly (as an array of objects) even when data is missing or in an unexpected format (e.g., array of strings).
    *   **Deletion:** Iterative changes in `CustomerMapping.jsx` regarding the deletion of mapped customers, starting from simple UI removal, then attempting backend API calls, and finally settling on UI removal locally, suggesting a decoupled approach or a change in API design.
    *   **API Endpoint Evolution:** The `DELETE_CUSTOMER_OPTION` endpoint in `Endpoints.js` and its corresponding usage in `ApiManager.js` saw multiple changes, indicating a development process where the API design for deleting customer mappings was being actively refined.
*   **Debugging with `console.log`:** Numerous `console.log` statements are present across various files, especially `CustomerForm.jsx`, `CustomerMapping.jsx`, and `CustomerFormScreen.jsx`, indicating active debugging during development.
*   **Role-Based Access Control:** `App.js` shows a strong reliance on `ProtectedRoute` and `UserRoleEnum` for managing access to different application routes, which is a consistent pattern for authorization.
*   **Formik Usage:** `useFormik` is consistently used in form components (`AgentBookingConfirmation.jsx`, `CustomerForm.jsx`) for managing form state, validation, and submission.
*   **API Manager Centralization:** `ApiManager.js` and `Endpoints.js` are used consistently to centralize API endpoint definitions and interaction logic.
*   **MUI Components:** The frequent use of `@mui/material` components (Grid, Box, Stack, IconButton, Typography, Card, etc.) suggests a consistent design system and component library is in use across the frontend.

## 4:46:46 PM
**File: `c:\Users\ridap\OneDrive\Documents\javascript-learning\index.html`**
This file, initially created around 1/4/2026, 3:18:32 PM, contained a basic HTML structure with a `<h1>Hello world!</h1>` and an empty `<button>`. Shortly after, at 3:18:41 PM, the button was updated to display the text "Click me", while retaining its `onclick` event to show an alert.

**File: `c:\Users\ridap\OneDrive\Documents\javascript-learning\0_variables.js`**
Development on this JavaScript file began around 1/4/2026, 3:36:06 PM with a `console.log` statement containing a syntax error (an extra semicolon). This error was corrected within a minute at 3:36:47 PM. Subsequent updates demonstrated JavaScript variable declaration. At 3:38:49 PM, `var myAge = 25;` was added, along with a comment which was slightly refined to "camel case" at 3:38:57 PM. The file saw significant expansion around 3:45:07 PM, introducing more variable examples like `my_firtsName`, `_lastName`, and `$myAge`. This update also included attempts to declare invalid variable names (`123data` and `my@email`), which were then commented out at 3:45:32 PM, indicating an exploration of JavaScript's naming conventions. The last recorded change at 3:45:41 PM was identical to the previous entry, showing no new functional modifications.

**Patterns and Recurring Elements:**
*   **Learning and Experimentation:** The changes across both files, particularly `0_variables.js`, demonstrate a clear learning curve, from basic syntax correction to exploring valid and invalid variable naming conventions.
*   **Progressive Code Development:** Changes within `0_variables.js` show a step-by-step approach to learning JavaScript fundamentals, starting with basic output and moving to variable declaration examples.
*   **Focused Development Session:** All recorded changes occurred on the same day, 1/4/2026, within a relatively short period (from 3:18 PM to 3:45 PM), suggesting an active and concentrated coding session.

## 4:47:37 PM
The provided log details a series of code changes primarily focused on enhancing analytics and reporting functionalities within the `t360-backend-v2` project. The modifications span several service and utility files, indicating an active development cycle for data handling, query generation, and user-specific data access.

The log entry for `c:\Users\ridap\.dbclient\storage\1747201553765@@127.0.0.1@5432@Rida_Database@public\public.sql` is excluded from this summary as it contains raw database commands, including schema manipulation and data insertions, which could reveal sensitive internal database details.
...
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\customerAutosearchOptions.ts`**
    *   **Timestamp:** 1/5/2026, spanning from 10:14:20 AM to 5:30:33 PM.
    *   **Key Changes:** This file underwent extensive refactoring and iteration for customer search functionality.
        *   An initial change added a `search` parameter and implemented `ILIKE` search for "AGENT" roles.
        *   For "CUSTOMER" roles, the search logic fluctuated significantly:
            *   Initially used `Account_Master_Customer_MAP` (10:14:20 AM), then experimented with `PO_AccountMaster_Customer_Master` by unnesting `customer_map` for broader search (11:32:39 AM).
            *   Further developed to select both `cname` and `customer_map` from `PO_AccountMaster_Customer_Master` and perform client-side processing for uniqueness and robust parsing of array-like strings (11:59:52 AM, 12:10:43 PM).
            *   Briefly reverted to a `UNION` query for `PO_AccountMaster_Customer_Master` (12:13:07 PM), then back to `Account_Master_Customer_MAP` (12:15:06 PM), and even `mtr_tracking_data` (12:42:32 PM).
            *   **Significant Change (1/5/2026, 5:30:33 PM):** The "CUSTOMER" search logic was finalized using `PO_AccountMaster_Customer_Master`. The SQL query concatenates `cname` with distinct `customer_map` values into a `customer_array`, with robust JavaScript logic for parsing and mapping to `label`/`value` pairs, supporting both `cname` and `customer_map` searching with `ILIKE`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\general.query.ts`**
    *   **Timestamp:** 1/5/2026, 1:56:51 PM to 1/6/2026, 10:33:28 AM.
    *   **Key Changes:** Refined how company names are handled for filtering in analytics queries.
        *   Transitioned from `regexp_split_to_array` to a more direct `IN (${companyList})` clause for `account_name` filtering across "TotalShipments" and "TotalShipmentsMblWise" queries (1/5/2026, 2:13:45 PM - 2:18:07 PM).
        *   **Significant Change (1/6/2026, 10:33:28 AM):** The delimiter used to split the `CompanyName` string was changed from a comma (`,`) to double pipe (`||`), affecting the expected input format for multiple company names.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\booking.query.ts`**
    *   **Timestamp:** 1/5/2026, 2:11:12 PM to 1/6/2026, 10:27:27 AM.
    *   **Key Changes:** Corrected SQL syntax and updated company name handling.
        *   Fixed a missing `AND` keyword in the `WHERE` clause for stacked bar charts (1/5/2026, 2:35:38 PM).
        *   **Significant Change (1/6/2026, 10:27:27 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\chasis.query.ts`**
    *   **Timestamp:** 1/5/2026, 2:38:19 PM to 1/6/2026, 10:32:38 AM.
    *   **Key Change (1/6/2026, 10:32:32 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\container.query.ts`**
    *   **Timestamp:** 1/5/2026, 2:42:51 PM to 1/6/2026, 10:32:49 AM.
    *   **Key Change (1/6/2026, 10:32:49 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\departure.query.ts`**
    *   **Timestamp:** 1/5/2026, 2:45:27 PM to 1/6/2026, 10:32:55 AM.
    *   **Key Change (1/6/2026, 10:32:55 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\export.query.ts`**
    *   **Timestamp:** 1/5/2026, 3:20:59 PM to 1/6/2026, 4:35:17 PM.
    *   **Key Changes:** Standardized `CompanyName` filtering and updated delimiter.
        *   Updated filtering for several "Booking Date to Estimated Departure" related queries to use `LOWER(account_name) IN (${companyList})` (1/5/2026, 3:23:49 PM).
        *   **Significant Change (1/6/2026, 10:33:04 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\cbmService.ts`**
    *   **Timestamp:** 1/5/2026, 8:38:09 PM to 1/6/2026, 10:33:48 AM.
    *   **Key Change (1/6/2026, 10:33:48 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\containerService.ts`**
    *   **Timestamp:** 1/5/2026, 8:38:59 PM to 1/6/2026, 10:33:57 AM.
    *   **Key Change (1/6/2026, 10:33:57 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\kgsService.ts`**
    *   **Timestamp:** 1/5/2026, 8:40:08 PM to 1/6/2026, 10:34:04 AM.
    *   **Key Change (1/6/2026, 10:34:04 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\teuService.ts`**
    *   **Timestamp:** 1/5/2026, 8:41:46 PM to 1/6/2026, 10:34:11 AM.
    *   **Key Change (1/6/2026, 10:34:11 AM):** The `CompanyName` splitting delimiter was changed from `","` to `"||"`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\shipmentService.ts`**
    *   **Timestamp:** 1/6/2026, 10:59:39 AM to 1/6/2026, 1:28:48 PM.
    *   **Key Changes:** Refined `CompanyName` filtering in `SqlQueryBuilder`.
        *   Initially, `CompanyName` was handled as a single string.
        *   **Significant Change (1/6/2026, 11:00:59 AM):** Introduced `companyList` splitting by `"||"` and used `LOWER(REGEXP_REPLACE(TRIM(mtr.account_name), '[,]', '', 'g')) IN ($$)` for ocean and a similar `IN` clause for air.
        *   **Reversion (1/6/2026, 11:03:31 AM):** The `companyList` delimiter was reverted to `","`, and the `REGEXP_REPLACE` was removed, implying a simpler comma-separated list of company names. This indicates an inconsistency in `CompanyName` parsing between this service and other analytics services.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\planbaordService.ts`**
    *   **Timestamp:** 1/6/2026, 4:12:58 PM to 1/6/2026, 4:13:40 PM.
    *   **Key Change (1/6/2026, 4:13:40 PM):** The handling of `CompanyName` for filtering was simplified. It reverted from supporting multiple companies (split by `"||"`) to only supporting a single `CompanyName` value for direct `UPPER(master.customer) = ...` comparison.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\utils\analytics.ts`**
    *   **Timestamp:** 1/6/2026, 4:20:07 PM to 1/6/2026, 4:39:56 PM.
    *   **Key Changes:** Expanded columns for export and improved date handling.
        *   `m.rail_ata` was added to the `otherColumns` selected for Excel exports.
        *   `inv_date` was added to the list of `dateKeys` in `dateFilterOnExport` to ensure proper date validation and formatting during export (1/6/2026, 4:22:07 PM).

### Patterns and Recurring Elements:

1.  **`CompanyName` Delimiter Shift and Inconsistency:** A prominent pattern is the widespread change of the `CompanyName` query parameter splitting delimiter from `","` to `"||"` across most analytics query builders (general, booking, chassis, container, departure, CBM, KGS, TEU, export). This change occurred between **1/6/2026, 10:27:27 AM** and **1/6/2026, 10:34:11 AM**. However, `shipmentService.ts` and `planbaordService.ts` later reverted or modified this to use `","` or single-company logic, indicating potential divergence or ongoing refinement in how multi-company filtering is handled.
2.  **Date Filtering and Normalization:** Consistent use of `fromDate`, `toDate`, and SQL date range clauses across various analytics reports. The `analytics.ts` utility centralizes `defaultDateRange`, `isValidDate`, and `dateFilterOnExport` to maintain uniform date handling and formatting. Invalid PostgreSQL `0001-01-01` dates are specifically handled by `isValidDate` and set to `null` on export.
3.  **Dynamic Column Selection:** The `getColumnsByCategory` function is heavily utilized across services to dynamically determine which columns to include in reports, often ignoring certain fields while mandating others.
4.  **Port Name Canonicalization:** The `replaceValueInQuery` utility is consistently applied to normalize port names like 'LONG BEACH' to 'LOS ANGELES' in SQL queries, improving data consistency.
5.  **SQL Query Structure:** Many analytics queries follow a pattern of using Common Table Expressions (`WITH t AS (...)`, `max_value AS (...)` or similar) to calculate intermediate results and maximum values for percentage calculations. `DISTINCT ON` is frequently used to ensure unique records.
6.  **Extensive Logging:** `logger.info` and `logger.error` statements are pervasive, indicating a strong emphasis on debugging and monitoring query generation and execution flow.