...
## 8:05:50 PM
The provided log details a series of code changes, all timestamped on January 2, 2026, primarily within a timeframe from 4:59 PM to 5:47 PM, suggesting a focused development session on analytics query generation.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\transit.query.ts` (Timestamp: 1/2/2026, 4:59:40 PM):** This file was updated to handle various transit-related analytics. The `transitQueryBuilder` function now supports different report types, including `country_of_origin`, `ScheduledTransitVsActualTransit`, `ScheduledTransitVsActualTransitMblWise`, and a default `port_pair` view. It dynamically configures chart types, calculation methods, and selects relevant columns. The code generates complex SQL queries to calculate estimated, actual, and estimated-actual transit times, often involving date differences and aggregation by `fileno`/`contno` or `fileno`/`mblno`. A `queryGenerator` helper function is used to modularize SQL query construction based on the report type.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\teuService.ts` (Timestamp: 1/2/2026, 5:00:42 PM and 5:00:53 PM):** This service handles fetching TEU (Twenty-foot Equivalent Unit) reports. The `getTEUReports` function dynamically constructs SQL queries to calculate TEU based on user-specified `group_by_column` (e.g., `country_of_origin`, `country_of_discharge`, or other fields). It includes logic to extract numeric container types using `REGEXP_REPLACE` and to normalize certain geographical names (e.g., 'LONG BEACH' to 'LOS ANGELES'). The `mblwiseCondition` object, which contains a `DISTINCT ON` clause, ensures unique container counts for TEU calculations. The second timestamp for this file shows no functional changes, indicating a save without modifications.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\departure.query.ts` (Timestamp: 1/2/2026, 5:01:49 PM):** This file focuses on generating queries for departure-related analytics. It includes builders for reports like `BLReleaseKPI` (calculating lead times for Bill of Lading release and categorizing them into time buckets for a stacked bar chart), `DelayedPOLDepartures` (by container or MBL), and `OriginalETAvsActualETA` (by container or MBL). The queries consistently calculate day differences between various departure/ETA dates and aggregate them, often including percentage calculations and specific handling for negative values or division by zero.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\general.query.ts` (Timestamp: 1/2/2026, 5:03:05 PM):** This file provides query builders for general shipment counts. The `generalQueryBuilder` function supports `TotalShipments` (counting distinct `fileno` and `voyage`) and `TotalShipmentsMblWise` (counting distinct `fileno` and `mblno`). It uses SQL `UNION` to combine different counts and performs post-processing in TypeScript to calculate percentages relative to the maximum count.
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\container.query.ts` (Timestamp: 1/2/2026, 5:03:44 PM):** This file is dedicated to container lifecycle analytics. It includes query builders for `ContainerDischargevsContainerAvailable`, `ContainerDischargeVsFinalDeliveryDate`, `ContainerGateOutVsGateIn`, and `ContainerOrVesselDischargeVsGateOut`. These reports calculate time differences between key container events, such as discharge, availability, final delivery, and gate in/out, using complex SQL queries with `CASE WHEN` statements to prioritize live tracking data (`t49_` prefixed columns).
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\booking.query.ts` (Timestamps: 1/2/2026, 5:09:05 PM, 5:09:30 PM, 5:11:03 PM, 5:46:22 PM, 5:47:08 PM):** This file generates queries for booking-related analytics. It defines functions for reports such as `BookingDatetoActualDeparture` (calculating average day differences between booking date and actual departure, by container or MBL) and `BookingDatetoActualDepartureStackedBar` (categorizing these differences into time ranges like "Less than 1 week", "1-2 weeks" for a stacked bar chart). Multiple timestamps for this file show identical code, indicating repeated saves without functional changes.

**Patterns and Recurring Elements:**

*   **Modular Query Builders:** Each `.query.ts` file encapsulates query generation for a specific analytics domain (transit, departure, general, container, booking).
*   **Dynamic Column Selection:** The utility `getColumnsByCategory` from `@src/utils/analytics` is universally used to dynamically select relevant database columns for reports, often specifying columns to `ignore` or `must` include, based on `user_id` and `reportName`.
*   **Date Range Filtering:** All SQL queries consistently filter data by `fromDate` and `toDate` using `departure_date` or other relevant timestamps, ensuring `timestamp` casting and inclusive lower bounds with exclusive upper bounds (`< '${toDate}'::timestamp + INTERVAL '1 day'`).
*   **Company-Specific Data:** Data is always filtered by `account_name` using `LOWER(account_name) = ANY ( regexp_split_to_array(LOWER('${CompanyName}'), '\s*,\s*') )`, allowing reports for single or multiple companies.
*   **Robust Date Handling in SQL:** A significant recurring pattern is the explicit validation and filtering of date columns in SQL queries, checking for `IS NOT NULL` and `!= '0001-01-01 00:00:00 BC'`, to prevent issues with invalid or default date values in the underlying data.
*   **`DISTINCT ON` for Uniqueness:** Many queries utilize `DISTINCT ON (voyage,contno, CASE WHEN t49_cont_type IS NOT NULL AND TRIM(t49_cont_type) <> '' AND TRIM(t49_cont_type) != '0' THEN t49_cont_type ELSE cont_type END )` within subqueries. This indicates a consistent approach to ensure calculations are based on unique container instances per voyage, prioritizing live tracking container types (`t49_cont_type`) over standard ones.
*   **`customizer` Object for Report Configuration:** The `customizer` object is central to configuring report metadata such as `chartType`, `calType` (average, total, N/A), `dateColumns` (used in calculations), and `selectedColumns`, allowing the frontend to interpret and render the analytics data correctly.
*   **Percentage Calculations:** Many reports include a percentage calculation, typically `ROUND(((ABS(t.value) / max_value.max_total) * 100)::numeric, 2)`, with specific `CASE` logic to handle division by zero or negative values.
*   **"MblWise" Variants:** Several report types have "MblWise" versions, indicating a common need to aggregate data both by container number (`contno`) and by master bill of lading number (`mblno`).
*   **Complex SQL with CTEs:** The SQL queries are often complex, making extensive use of Common Table Expressions (`WITH t AS (...)`) for clarity and multi-stage data processing.