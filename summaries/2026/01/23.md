...
## 3:44:57 PM
The log details various code changes across analytics components and their Redux slice.

**File-specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\.env`**: Changes to environment variables defining `VITE_MODE`, `VITE_GEOLOCATION_URL`, `VITE_GMAP_API_KEY`, `VITE_API_BASE_URL`, and `VITE_UI_V1_BASE_URL` for UAT, PROD, and DEV environments were recorded on **1/22/2026, 7:05:53 PM** and **1/23/2026, 9:54:12 AM**. Notably, the first entry enabled UAT variables while commenting out PROD and DEV, and the second entry enabled DEV variables while commenting out UAT and PROD.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\MetricToolbar.tsx`**:
    *   **1/23/2026, 11:25:18 AM - 11:26:57 AM**: Initial component setup with filter `SelectBox` and `SelectBoxLight`. Temporary debug text (`kk`, `kkkkk`) and label changes (`View By` to `View By123`) were introduced and then removed/reverted.
    *   **1/23/2026, 11:27:53 AM - 11:38:14 AM**: Multiple `console.log` statements were added or modified to debug `chartType`, `toolState`, and `toolState?.viewBy[index]` values. The "View By123" label was reverted to "View By".
    *   **1/23/2026, 11:44:48 AM**: The "View By123" label was finally reverted to "View By".
    *   **1/23/2026, 11:52:11 AM - 11:53:15 AM**: A temporary change was made to the `SelectBoxLight` for `AnalyticsChartType.Transit`, attempting to pass a specific `key: "transit_air"` to `setViewFilter` and use `toolState.viewBy.transit_air` for its value, suggesting a brief shift to object-based `viewBy` state access. This was quickly reverted, indicating a return to array-based `viewBy` state access.
    *   **1/23/2026, 12:02:08 PM - 12:06:11 PM**: Debugging efforts continued with commenting out and re-adding `SelectBoxLight` for `AnalyticsChartType.Transit` with extensive `console.log` statements within its `onChange` handler and new `useEffect` hooks to monitor `index`, `toolState?.viewBy`, and `toolState?.viewBy?.length`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\analytics-slice.ts`**:
    *   **1/23/2026, 11:51:18 AM**: The `initialState.viewBy` was structured as an object (`{ transit_air: "actual", ... }`), and the `setViewFilter` reducer expected `key` and `value` in its payload.
    *   **1/23/2026, 11:53:23 AM**: **Significant change**: `initialState.viewBy` was refactored from an object to an array (`Array(9).fill("actual")`). The `setViewFilter` reducer was updated to accept `index` and `value` and directly update the array element (`state.viewBy[index] = value;`).
    *   **1/23/2026, 12:01:46 PM**: The `setViewFilter` reducer was further refined to ensure immutability by creating a new `viewBy` array using `map` when updating an element.
    *   **1/23/2026, 12:05:57 PM**: Debugging `console.log` statements were added within the `setViewFilter` reducer to track state changes.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\metricBox.tsx`**:
    *   **1/23/2026, 12:13:06 PM - 12:13:51 PM**: Introduced conditional `chartKey` generation for `Transit` chart type and then applied it to the `BarChart` component to aid in re-rendering.
    *   **1/23/2026, 12:16:40 PM**: Modified `chartKey` generation, incorrectly attempting to access `mainState?.toolState?.viewBy?.[index]`.
    *   **1/23/2026, 1:34:55 PM**: Introduced `useMemo` for `chartData` to optimize data slicing, ensuring only the first 9 items are shown in the regular view, but this was quickly reverted.
    *   **1/23/2026, 1:39:48 PM - 1:40:06 PM**: Briefly changed the default display behavior to show *all* chart data instead of slicing to the first 9 elements, then reverted it.
    *   **1/23/2026, 1:52:55 PM**: Re-introduced the `useMemo` for `chartData` to manage data slicing for regular vs. enlarged views efficiently.
    *   **1/23/2026, 1:58:09 PM**: Changed the Redux state selector from `state.analyticSlice` to `state.analytics` (incorrect, later reverted), removed a `Tooltip` wrapper, and adjusted `ThemedModal`'s `overflow` prop.
    *   **1/23/2026, 2:03:02 PM - 2:03:07 PM**: Reverted the Redux state selector and `ThemedModal` prop changes, and re-added the `Tooltip` component.
    *   **1/23/2026, 2:05:54 PM - 2:06:52 PM**: Introduced and then reverted temporary changes that caused the regular chart view to render with all data and enlarged height, indicating debugging or accidental commits. The final state at `2:06:52 PM` restored the correct slicing for the regular view.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\Charts\BarChart.tsx`**:
    *   **1/23/2026, 12:20:20 PM**: Initial `BarChart` setup with ApexCharts, defining animations and data label formatting.
    *   **1/23/2026, 12:22:07 PM**: Modified `useEffect` dependencies, removing `data` (which could cause stale data issues).
    *   **1/23/2026, 1:33:06 PM**: **Major Refactor**: Introduced `useMemo` for `categories` and `seriesData`, adjusted animation/label enabling logic based on data length (`<= 100` for animations, `<= 50` for labels), and added a `key` to `ReactApexChart` for forced redraws. `useEffect` dependencies were also updated comprehensively.
    *   **1/23/2026, 1:35:26 PM**: **Significant Revert**: Most of the `1:33:06 PM` refactor was undone, reverting to an earlier, less optimized state. `plotOptions.bar.borderRadius` was changed to `4`.
    *   **1/23/2026, 1:36:16 PM**: Added a `console.log` for debugging `data`.
    *   **1/23/2026, 1:51:46 PM - 1:52:06 PM**: A more stable re-refactor occurred. `useEffect` dependencies were correctly set to `[data]`, functional `setState` updates were used, and a robust `key` prop was added to `ReactApexChart` to ensure re-renders based on data changes.
    *   **1/23/2026, 1:58:54 PM - 2:03:36 PM**: Repeated changes to chart styling properties, specifically `colors` (added/removed) and `plotOptions.bar.borderRadius` (toggled between `0` and `4`). A `Tooltip` for the chart title was briefly re-introduced.
    *   **1/23/2026, 2:15:36 PM**: **Another Major Refactor**: Re-introduced `useMemo` hooks for data, categories, series data, and chart ID. Separated `useEffect` hooks for updating chart data/labels and forcing chart resize. Also ensured `width="100%"` for `ReactApexChart` and removed the manual title `Tooltip`.
    *   **1/23/2026, 2:29:13 PM**: **Complete Revert**: The entire `2:15:36 PM` refactor was undone, reverting the component to a state similar to earlier versions, losing the optimizations and structured updates.

**Patterns and Recurring Elements:**

*   **Extensive Debugging:** Frequent additions and removals of `console.log` statements (`chartType===`, `toolState===`, `Reducer called with:`, `Current value:`, `ViewBy array length:`, etc.) and temporary UI elements (`kk`, `kkkkk`) indicate ongoing development and debugging, particularly for analytics filters and chart rendering logic.
*   **Redux State Instability/Refinement:** The `analytics-slice.ts` demonstrates an evolution in how the `viewBy` filter state is managed (from object to array, and then ensuring immutable array updates), reflecting a common challenge in Redux development.
*   **Chart Re-rendering Issues:** The repeated additions and removals of `key` props and `ApexCharts.exec(chartId, 'resize')` calls in `BarChart.tsx`, along with the frequent refactors and reverts, suggest persistent problems with ApexCharts not updating reliably or efficiently when underlying data or size changes.
*   **Data Slicing Logic for Charts:** `metricBox.tsx` repeatedly adjusted whether to slice data for display in the main chart view, indicating an ongoing refinement of how much data is displayed by default versus in an enlarged view.
*   **Refactor-Revert Cycles:** `BarChart.tsx` experienced multiple cycles of significant refactoring for performance and stability followed by complete reverts to an earlier state. This pattern suggests difficulties in making the new, more robust implementations work as intended or integrating them without introducing new issues.
*   **Styling Tweaks:** Minor stylistic changes like `borderRadius` and `colors` in `BarChart.tsx` were repeatedly toggled, possibly to fine-tune visual appearance.
*   **Redux State Access Errors:** A temporary change in `metricBox.tsx` from `state.analyticSlice` to `state.analytics` indicates a brief period of incorrect Redux store access.