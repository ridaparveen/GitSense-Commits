...
## 8:01:27 PM
The log captures a series of code changes on **January 2, 2026**, primarily focused on customer management features and analytics query enhancements.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\analytics\query\booking.query.ts`** (Timestamp: 1/2/2026, 2:35:49 PM)
    *   This file introduces a comprehensive `bookingQueryBuilder` function.
    *   It's designed to construct dynamic SQL queries for various booking-related analytics reports, including "BookingDatetoActualDeparture" (for average day difference between booking and actual departure), "BookingDatetoActualDepartureMblWise" (MBL-wise calculation), and their "StackedBar" chart variations.
    *   The function integrates with `getColumnsByCategory` and `getFilterQueryByDisplayValue` utilities.
    *   It customizes a `customizer` object with report-specific configurations such as `dateRange`, `calType`, `dateColumns`, `booking`, `mblwise`, `chartType`, and `calColumnConfig`.
    *   The SQL queries involve complex joins, date calculations (`EXTRACT(DAY FROM ...)`, `DATE_TRUNC`), aggregations (e.g., `AVG`), and percentage calculations.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\getCustomerOptions.ts`**
    *   **1/2/2026, 3:58:49 PM**: Introduces `getAllCustomerOptions` to fetch distinct customer names (`account_name`) with a search filter and `deleteCustomerOption` to delete customer records from `mtr_tracking_data` based on `account_name` provided in the request body.
    *   **1/2/2026, 4:17:54 PM**: Refactors `deleteCustomerOption` to accept `customer_id` from `req.params` instead of `account_name` from `req.body`, updating the SQL query to delete from `mtr_tracking_data` where `customer_id = $1`.
    *   **1/2/2026, 4:22:36 PM**: Corrects the SQL query in `deleteCustomerOption` to use the `id` column (`DELETE FROM mtr_tracking_data WHERE id = $1`), implying the actual column name for the customer ID is `id`.
    *   **1/2/2026, 4:28:41 PM, 4:28:50 PM, 4:29:40 PM**: Minor debugging additions to `deleteCustomerOption`, including `console.log` statements to inspect `customer_id` and `req.params`. No functional code changes.
    *   **1/2/2026, 4:31:23 PM**: Makes a critical correction in `deleteCustomerOption` by changing the target table for deletion from `mtr_tracking_data` to `Account_Master_Customer_MAP` (`DELETE FROM Account_Master_Customer_MAP WHERE id = $1`). This indicates that the intention was to delete a customer mapping entry, not the core tracking data.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\routes\getCustomerOptionsRoute.ts`**
    *   **1/2/2026, 3:59:33 PM**: Defines initial routes: a GET route for `getAllCustomerOptions` at `/` and a DELETE route for `deleteCustomerOption` also at `/`.
    *   **1/2/2026, 3:59:43 PM**: Modifies the DELETE route path from `/` to `/delete`.
    *   **1/2/2026, 4:18:20 PM**: Updates the DELETE route path to include a dynamic parameter `/delete/:customer_id`, aligning with the controller's change to use `req.params` for deletion.
    *   **1/2/2026, 4:28:07 PM**: This entry is identical to the previous one, indicating a re-save or minor, non-functional commit.

*   **`c:\Users\ridap\OneDrive\Documents\T360_Api\t360-backend\src\controllers\code.controller.ts`**
    *   **1/2/2026, 4:12:38 PM**: Introduces new functionalities for customer management:
        *   `getAllCustomer`: Fetches customer details from `PO_AccountMaster_Customer_Master`, including related shipper and customer mappings, and document uploads. It supports pagination, sorting, and search based on query parameters.
        *   `createCustomer`: Handles creation and modification of customer records. It uses an `upload` middleware, performs database transactions, checks for existing customers, generates new `acode`s if needed, inserts/updates customer master data, handles customer types, and processes shipper mappings by querying related tables (e.g., `portmaster`, `agent_po_account_master`) before inserting into `Account_Master_Shipper_MAP`.
    *   **1/2/2026, 4:14:13 PM**: Enhances `getAllCustomer` by including the `id` of the `Account_Master_Customer_MAP` entry in the `customer_mapping` data returned.

### Patterns and Recurring Elements:

*   **Customer Management Focus**: A significant portion of the changes revolves around customer-related functionalities, including fetching customer options, deleting specific customer mappings, and comprehensive customer master data management (create, read, update, delete).
*   **API Endpoint Refinement**: There's a clear pattern of refining the `deleteCustomerOption` API endpoint and its corresponding controller logic. This involved:
    1.  Switching from using a body parameter (`account_name`) to a URL parameter (`customer_id`).
    2.  Correcting the column name used for deletion (`customer_id` to `id`).
    3.  Finally, correcting the target database table for deletion (`mtr_tracking_data` to `Account_Master_Customer_MAP`). This iterative process suggests a debugging and correction phase for this specific feature.
*   **Dynamic SQL and Database Interaction**: Multiple files (especially `booking.query.ts` and `code.controller.ts`) extensively use dynamic SQL generation and interaction with various database tables (`mtr_tracking_data`, `agent_booking_entry`, `PO_AccountMaster_Customer_Master`, `Account_Master_Shipper_MAP`, `document_upload_details`, `agent_po_account_master_type`, `portmaster`, `Account_Master_Customer_MAP`).
*   **Modularity**: The codebase utilizes utility functions (e.g., `getColumnsByCategory`, `getFilterQueryByDisplayValue`, `chkStr`, `getAccountCode`, `getAccountSubCode`) for common tasks, promoting code reusability.
*   **Timestamp Concentration**: All recorded changes occurred on the same date, indicating a concentrated development effort or a single commit of multiple accumulated changes.

## 8:01:42 PM
This log details updates to several query builder and service files within a `t360-backend-v2` project, primarily focused on analytics and reporting functionalities. The changes occurred on January 2nd, 2026, within a span of roughly 48 minutes.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\transit.query.ts` (Timestamp: 1/2/2026, 4:59:40 PM):**
    *   The `transitQueryBuilder` function was significantly updated to handle different transit report types.
    *   For `country_of_origin` reports, it now customizes chart type (`transitBar`), sets `calType` to "NA", dynamically selects columns, defines calculation configurations for "actual," "estimated," and "estimated_actual" transit times, and executes separate queries for each using a `queryGenerator` function.
    *   For `ScheduledTransitVsActualTransit` and `ScheduledTransitVsActualTransitMblWise` (Master Bill of Lading-wise) reports, it configures date range, sets `calType` to "avg", specifies date columns, and constructs complex SQL queries. These queries calculate the average difference between actual and estimated transit times (e.g., `Live Actual Arrived Date - Live Actual Departed Date` vs `Inland ETA - Departure Date`) and then compute a percentage relative to the maximum absolute value. The MBL-wise variant groups results by `fileno || ',' || mblno`.
    *   A default `else` block handles `port_pair` reports with similar logic to `country_of_origin`.
    *   A `queryGenerator` helper function was introduced to abstract the common parts of transit query construction.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\teuService.ts` (Timestamps: 1/2/2026, 5:00:42 PM and 1/2/2026, 5:00:53 PM):**
    *   The `getTEUReports` service function was updated to calculate TEU (Twenty-foot Equivalent Unit) reports.
    *   It initializes a `customizer` object for report display, setting `calType` to "total" and defining `mblwiseCondition` for distinctness on `voyage`, `contno`, and container type.
    *   When grouping by `country_of_origin` or `country_of_discharge`, it uses `portmaster` to resolve country names and sums `total_volume` (derived from `t49_cont_type` or `cont_type`) to calculate TEU, displaying it as a percentage of the maximum TEU.
    *   For other `group_by_column` values, it constructs a query that groups by the specified column, handles null/empty column values by labeling them 'OTHERS', and specifically replaces 'LONG BEACH' with 'LOS ANGELES' for 'pod' or 'place_of_delivery' columns.
    *   The content across both timestamps is identical, suggesting a non-functional save or reformatting.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\departure.query.ts` (Timestamp: 1/2/2026, 5:01:49 PM):**
    *   The `departureQueryBuilder` function was expanded to handle various departure-related KPIs.
    *   For `BLReleaseKPI`, it sets up a stacked bar chart to show the difference between "Vessel Actual Departure Date" and "Batch Run Date," categorizing delays into "Equal/Less than 3 days" and "More than 3 days."
    *   `DelayedPOLDepartures` and `DelayedPOLDeparturesMblWise` reports calculate the sum of day differences between "Live Actual Departed Date" and "Departure Date," grouping by `fileno || ',' || contno` or `fileno || ',' || mblno` respectively.
    *   `OriginalETAvsActualETA` and `OriginalETAvsActualETAMblWise` reports calculate the sum of day differences between "Updated ETA Port Date" and "ETA POD," grouping by container or MBL. These queries include logic to handle division by zero and negative values in percentage calculations.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\general.query.ts` (Timestamp: 1/2/2026, 5:03:05 PM):**
    *   The `generalQueryBuilder` function was updated to provide basic shipment and container counts.
    *   For `TotalShipments`, it generates a `UNION` query to count distinct `fileno` (Shipment Count) and distinct `voyage` (Container Count).
    *   For `TotalShipmentsMblWise`, it counts distinct `fileno` and distinct `mblno` (MBL Count).
    *   The function then calculates percentages for these counts based on the maximum observed value.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\container.query.ts` (Timestamp: 1/2/2026, 5:03:44 PM):**
    *   The `containerQueryBuilder` function was enhanced for container tracking analytics.
    *   `ContainerDischargevsContainerAvailable` calculates the day difference between container availability (prioritizing `t49_cont_available_date`) and discharge date.
    *   `ContainerDischargeVsFinalDeliveryDate` calculates the day difference between "Delivery Customer Date" and "Container Discharge Date," specifically for `type_of_move` being 'FDC' or 'FDN'.
    *   `ContainerGateOutVsGateIn` calculates the day difference between "Empty Return Date" (prioritizing `t49_empty_return_date`) and "Container Outgate + 1" (prioritizing `t49_contno_out_gated`).
    *   `ContainerOrVesselDischargeVsGateOut` calculates the day difference between "Container Outgate" and "Container Discharge Date."
    *   All these reports group by `fileno || ',' || contno`, calculate sums of day differences, and provide percentage values with error handling for division by zero or negative results.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\booking.query.ts` (Timestamps: 1/2/2026, 5:09:05 PM, 5:09:30 PM, 5:11:03 PM, 5:46:22 PM, 5:47:08 PM):**
    *   The `bookingQueryBuilder` function was updated for booking-related reports.
    *   `BookingDatetoActualDeparture` and `BookingDatetoActualDepartureMblWise` reports calculate the average day difference between "Live Actual Departed Date" and "Booking Date" by joining `mtr_tracking_data` with `agent_booking_entry`. The MBL-wise variant groups results by MBL.
    *   `BookingDatetoActualDepartureStackedBar` and `BookingDatetoActualDepartureMblWiseStackedBar` implement stacked bar charts. These categorize the `day_diff` (Live Actual Departed Date - Booking Date) into predefined time ranges ("Less than 1 week", "1 - 2 weeks", "2 - 3 weeks", "3 - 5 weeks", "Above 5 weeks").
    *   Multiple timestamps for this file with identical content indicate frequent saving or reformatting without functional changes.

**Patterns and Recurring Elements:**

1.  **Centralized Column Management:** All query builders consistently use `getColumnsByCategory` to dynamically select columns for reports, often ignoring a common set of date/type columns (`bookingdate`, `booking_status_action_date`, `est_cargo_ready_date`, `t49_cont_type`, `country_name`, `networkdays`, `bl_release_date`) while specifying "must-have" columns for each report.
2.  **`customizer` Object for Configuration:** A `customizer` object is universally employed to configure report display properties such as `dateRange`, `calType` (e.g., "avg", "total", "NA"), `chartType` ("StackedBar", "transitBar"), `dateColumns`, `booking`, and `mblwise` flags.
3.  **Consistent Date and Company Filtering:** All generated SQL queries include standard filtering criteria:
    *   `departure_date >= '${fromDate}'::timestamp AND departure_date < '${toDate}'::timestamp + INTERVAL '1 day'` for date ranges.
    *   `LOWER(account_name) = ANY ( regexp_split_to_array(LOWER('${CompanyName}'), '\s*,\s*') )` for filtering by company name(s).
4.  **Robust Date Handling in SQL:** Queries frequently check for `IS NOT NULL` and `!= '0001-01-01 00:00:00 BC'` on date fields, indicating an attempt to handle missing or default/invalid date values in the database.
5.  **Day Difference Calculations:** The `EXTRACT(DAY FROM date1 - date2)` SQL function is a dominant pattern for calculating durations and differences between various date fields across transit, departure, container, and booking reports.
6.  **Percentage Calculation Logic:** Many reports calculate a `percentage` value. This often involves a common table expression (`WITH max_value AS (SELECT MAX(...) FROM t)`) to find the maximum value, used as the denominator, and includes `CASE` statements to prevent division by zero or handle negative results.
7.  **`DISTINCT ON` Clause for Uniqueness:** A complex `DISTINCT ON (m.voyage, m.contno, CASE WHEN m.t49_cont_type IS NOT NULL AND TRIM(m.t49_cont_type) <> '' AND TRIM(m.t49_cont_type) != '0' THEN m.t49_cont_type ELSE m.cont_type END)` clause appears in several queries when selecting from `mtr_tracking_data`, indicating a standardized approach to identifying unique container/voyage combinations while normalizing container types.
8.  **MBL-Wise Reporting Variants:** Several reports offer `MblWise` versions (e.g., `DelayedPOLDeparturesMblWise`, `BookingDatetoActualDepartureMblWise`), signifying a recurring need to present data aggregated by Master Bill of Lading (`mblno`) alongside container/file number.
9.  **Type-Driven Conditional Logic:** The query builders primarily use `if/else if` statements based on a `type` parameter to determine which specific report logic and SQL query to apply, enabling a single entry point for multiple report types.
10. **Container Type Preference:** When dealing with container types, there's a consistent preference for `t49_cont_type` if it's not null, empty, or '0', falling back to `cont_type` otherwise.