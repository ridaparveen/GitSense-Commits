...
## 3:31:05 PM
The code changes primarily focus on enhancing the analytics dashboard functionalities, including new data views and reporting, alongside a refactoring of how the default dashboard is managed.

**File-Specific Updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx`**
    *   **Timestamp: 1/20/2026, 1:49:22 PM**: Initial state and rendering for custom views supported "Container Wise" and "MBL Wise" tabs.
    *   **Timestamp: 1/21/2026, 12:55:46 PM - 1:21:49 PM**: A new "AIR Wise" view capability was gradually introduced and integrated. This involved:
        *   Updating the `selectedTab` state type to include `"airWise"`.
        *   Modifying the rendering logic to explicitly handle `"airWise"` as a separate conditional rendering block for `TreeViewContainer`.
        *   Updating the `initializeCheckedMetrics` function to iterate over `airWise` options.
        *   Finally, adding `"AIR Wise"` as a selectable option in the `tabData` array for the UI's `RadioNav` component.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`**
    *   **Timestamp: 1/20/2026, 1:50:39 PM**: The toolbar displayed "Manage View" tooltip and `ChipsTabView` options, with the default dashboard referred to as `_Default_`.
    *   **Timestamp: 1/20/2026, 6:38:27 PM**: The naming convention for the default dashboard was updated in the UI logic. References to `_Default_` were changed to `Default` for displaying the "Manage View" tooltip and mapping `ChipsTabView` options.
    *   **Timestamp: 1/21/2026, 1:19:45 PM**: A `console.log` statement was added for debugging `showDrawer` state.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\analytics-slice.ts`**
    *   **Timestamp: 1/20/2026, 1:56:23 PM**: The Redux slice's initial state set `selectedDashobard` to `_Default_`. The `setDashboard` reducer included logic to handle `_Default_` when inserting dashboards, but `setCustomer` would set `selectedDashobard` to "Default".
    *   **Timestamp: 1/20/2026, 1:57:02 PM**: `initialState.selectedDashobard` was changed from `_Default_` to `"Default"`.
    *   **Timestamp: 1/20/2026, 6:35:22 PM**: The `setDashboard` reducer's logic for `type === "insert"` was updated to consistently set `state.selectedDashobard = "Default"` even when `_Default_` was detected in the input.
    *   **Timestamp: 1/20/2026, 6:39:02 PM**: The `setDashboard` reducer's logic was fully updated to replace all references to `_Default_` with `"Default"` when filtering and setting dashboard values during an "insert" operation, standardizing the default dashboard name.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts`**
    *   **Timestamp: 1/20/2026, 6:41:28 PM**: A new constant `AIR_SHIPMENT_STATUS` was added, defining various statuses for air shipments (e.g., "New", "In Progress", "Booked", "EnRoute", "Landed", "Delivered", "Untracked") along with their associated color codes.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\config\analytics.ts`**
    *   **Timestamp: 1/21/2026, 2:31:38 PM**: The `ANALYTICS_REPORT_HEADER` object was significantly expanded. New report headers were introduced for:
        *   "KGS" (Kilograms) metrics, both Container Wise and MBL Wise, grouped by various attributes (Shipment Type, POL, Country of Discharge, SCAC, Country of Origin, POD, Supplier).
        *   Detailed "Transit Time" analyses, including reports by Port Pair, Country of Origin, and Scheduled vs. Actual Transit Time for both Container and MBL Wise.
        *   A new category for "Chassis MBL Wise" reports, covering metrics such as Chassis Pick-Up Date vs Final Delivery, Average Days Chassis Used, Container Discharge vs Availability/Final Delivery/Outgate Date, Container Gate-Out vs Gate-In, Container Departure to Door Delivery Date, and Containers Outgated but Not Returned.

**Patterns and Recurring Elements:**

*   **Expansion of Analytics Capabilities**: There's a clear and consistent pattern of expanding the application's analytics features. This is evident through the introduction of the "AIR Wise" view in `ViewContainer.tsx`, the new `AIR_SHIPMENT_STATUS` in `global.ts`, and the substantial addition of new "KGS", "Transit Time", and "Chassis MBL Wise" reports in `analytics.ts`.
*   **Default Dashboard Naming Refactoring**: A recurring theme is the refactoring of the default dashboard's identifier from `_Default_` to `"Default"`. This change was systematically applied across both the Redux state management (`analytics-slice.ts`) and the UI components (`AnalyticsToolbar.tsx`) to standardize its usage.
*   **Incremental Feature Development**: The introduction of the "AIR Wise" view in `ViewContainer.tsx` demonstrates an incremental development approach, where the feature was gradually implemented in stages (state definition, rendering logic, then UI tab).
*   **Timestamp Cluster**: Multiple changes occurred around **1/20/2026** (1:49 PM - 6:41 PM) and **1/21/2026** (12:55 PM - 2:31 PM), indicating focused development periods on analytics features and the dashboard default naming.

## 3:31:31 PM
The primary focus of these code changes revolves around enhancing and integrating analytics reporting functionalities, particularly introducing and refining "Air Wise" reports.

### File-Specific Updates:

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\booking.query.ts`** (Timestamp: 1/19/2026, 6:44:58 PM & 7:29:32 PM)
    *   Both visible code entries appear identical, suggesting no functional changes were made to the visible parts between these timestamps, or it was a non-substantive save.
    *   The file defines the `bookingQueryBuilder` function, which constructs complex SQL queries for various booking and cargo-ready reports (e.g., "Booking Date to Actual Departure", "Booking Date to Actual Departure Mbl Wise", "Booking Date to Actual Departure Stacked Bar"). These queries calculate time differences between dates, group data by `fileno`/`contno` or `mblno`, and calculate percentage values relative to a maximum. It also configures a `customizer` object with report-specific settings like date ranges, calculation types, and columns.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\cbmService.ts`** (Timestamp: 1/20/2026, 5:28:09 PM & 5:28:35 PM)
    *   A minor change occurred at 1/20/2026, 5:28:35 PM, adding a `console.log` statement (`console.log(mainQuery,"test==========");`) for debugging purposes.
    *   The `getCBMReports` function dynamically builds SQL queries to fetch CBM-related analytics. It handles grouping by different columns (e.g., `country_of_origin`, `country_of_discharge`) and calculates percentages based on CBM values. It uses `getColumnsByCategory` to select relevant data for display.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\export.query.ts`** (Multiple timestamps on 1/21/2026)
    *   The visible code across all timestamps is identical, indicating no functional changes in the provided content.
    *   This file contains the `bookingQuery` function, which serves as a centralized source for SQL query fragments used in data export functionalities for various booking reports. It includes logic to filter data based on month, week, or quarter and allows for filtering by "stacked group" categories (e.g., "Less than 1 week").

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\models\User.ts`** (Timestamp: 1/21/2026, 1:02:38 PM)
    *   No changes shown. This file defines the `UserType` interface (including `CompanyName`, `usercode`, `firstname`, `lastname`, `role`, `userid`, `emailid`, `companyname`, `location`) and the `UserRole` enum, along with user validation utilities.

*   **`c:\Users\ridap\.dbclient\storage\1747201553765@@127.0.0.1@5432@Rida_Database@public\public.sql`** (Timestamp: 1/21/2026, 1:14:04 PM)
    *   No changes shown. This appears to be a log of various direct SQL queries executed in a database client. The queries include `SELECT` statements for dashboards and metrics, an `INSERT` statement for metrics, and several complex `WITH` clause queries for calculating average transit times and percentages, filtering by `account_name` and date ranges.
...
*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\coreMetricService.ts`** (Timestamp: 1/21/2026, 1:16:07 PM)
    *   This file was updated to integrate "Air_Wise" metrics into the `getMetricList` function, categorizing them alongside "Container_Wise" and "MBL_Wise" groups. This indicates the initial setup for supporting air freight analytics within the dashboard system.
    *   It also contains functions for fetching dashboard endpoints, saving metric sequences, and saving display sizes for dashboard items.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\constants\Paths.ts`** (Timestamp: 1/21/2026, 1:33:09 PM)
    *   No changes shown. This file defines various API routes, including those for analytics such as `/analytics/dashboards`, `/analytics/dashboardEndPoints`, `/analytics/metricsList`, `/analytics/cbm`, `/analytics/kgs`, `/analytics/teu`, `/analytics/containers`, `/analytics/transit`, and `/analytics/Tables/tableData`.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\air.query.ts`** (Extensive changes between 1/21/2026, 1:33:53 PM and 3:20:38 PM)
    *   **Initial State (1:33:53 PM)**: The file `air.query.ts` initially contained query builders for chassis-related reports (`Averagechassisuseddays`, `ChassisPUdateVsFinalDeliveryDate`, `ChassisPUdateVsFinalDeliveryDateMblWise`). It returned a raw SQL query string.
    *   **Introduction of Air Wise (2:15:39 PM)**: The `TotalShipmentAirWise` report type was added, specifically designed to count distinct AWB numbers from `mtr_air_data`. The function's return type shifted towards a processed `analysticsData` object for this type.
    *   **Specialization (2:55:57 PM)**: Chassis-related report types were completely removed, making `air.query.ts` exclusively responsible for air-related queries.
    *   **Refinement & Robustness (2:57:41 PM onwards)**:
        *   The logic for populating `analysticsData` for `TotalShipmentAirWise` was fixed and improved, including explicit handling for division by zero when calculating percentages.
        *   The `ignore` list for `getColumnsByCategory` was significantly expanded across multiple commits (3:18:27 PM, 3:20:38 PM) to exclude numerous sea/container-specific columns (e.g., `fileno`, `mblno`, `contno`, `pol`, `pod`, various ETA/discharge dates), ensuring that the `airQueryBuilder` strictly requests air-relevant data.
        *   Debugging `console.log` statements were added and removed throughout this period.
        *   There was a temporary change to return `analysticsData.rows` (3:06:53 PM), which was quickly reverted back to returning the full `analysticsData` object (3:07:18 PM).

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\services\analytics\tableService.ts`** (Multiple timestamps on 1/21/2026)
    *   **Integration of Air Analytics (1:38:12 PM)**: `airQueryBuilder` was imported, and an `else if` block was added to dispatch to it. Initially, this dispatch had a typo (`type === "TotalShipmentsMblWise"`).
    *   **Correction (2:07:22 PM)**: The typo was fixed, correctly routing requests for `TotalShipmentAirWise` to `airQueryBuilder`, enabling the new air freight reports.
    *   This file acts as the central dispatcher for different types of analytics reports, routing requests to specific query builder functions based on the `type` parameter.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\utils\analytics.ts`** (Timestamp: 1/21/2026, 3:21:18 PM & 3:23:52 PM)
    *   Both visible code entries appear identical, suggesting no functional changes were made to the visible parts.
    *   This utility file provides helper functions crucial for analytics, including:
        *   `defaultDateRange`: Provides a default date range (current date minus 365 days).
        *   `replaceValueInQuery`: A utility to replace specific port names in queries.
        *   `getUserObject`: Retrieves user information, potentially adjusting `usercode` and `companyname` based on access level.
        *   `getFilterQueryByDisplayValue`: Generates SQL `GROUP BY`, `ORDER BY`, `DATE_TRUNC`, and `SELECT` clauses based on how data should be displayed (month, week, quarter).
        *   `getExcelSelectedColumns`: Selects columns for Excel reports based on report name and type (booking or other).
        *   `dateFilterOnExport`: Filters and formats date values in data for export, handling invalid dates.
        *   `EXCEL_EXPORT_COLUMNS`: A predefined list of columns for Excel exports.

### Patterns and Recurring Elements:

*   **Analytics Report Development**: A clear and ongoing development effort to build out a comprehensive analytics platform, evident in the creation and refinement of numerous report types (booking, CBM, chassis, container, departure, general, and newly introduced air freight).
*   **Modular Query Builders**: A consistent architectural pattern is observed where specific utility files (`*.query.ts`) are created to house the SQL query generation logic for different analytics categories (e.g., `booking.query.ts`, `chassis.query.ts`, `air.query.ts`). These builders encapsulate the complexity of dynamic SQL.
*   **Dynamic SQL Generation**: SQL queries are almost entirely constructed dynamically using template literals, incorporating user-defined parameters like company names, date ranges, and report types.
*   **Date and Company Filtering**: All reports consistently filter data by `account_name` (converted to a list of lowercased, single-quoted company names) and a date range (`fromDate` to `toDate`), often checking for valid/non-null date values and using `::timestamp` and `INTERVAL '1 day'`.
*   **`customizer` Object**: A `customizer` object is universally used across query builders to pass report-specific display and calculation configurations (e.g., `chartType`, `dateRange`, `calType`, `selectedColumns`).
*   **`getColumnsByCategory` Utility**: This function from `@src/utils/analytics` is heavily relied upon by all query builders to dynamically select or ignore columns based on the report context, often featuring very long `ignore` lists to filter out irrelevant data.
*   **Percentage Calculations**: Many reports include logic to calculate percentage values relative to a maximum, often incorporating robust checks for division by zero or negative values.
*   **Debugging Practices**: Frequent use of `console.log` statements for `mainQuery` and `analysticsData` during development, indicating an iterative and debug-heavy development process, especially apparent in the `air.query.ts` changes.
*   **MBL-Wise/Container-Wise and Stacked Bar Variations**: Many report types offer "MBL Wise" variations (aggregating by Master Bill of Lading) and "Stacked Bar" chart variations, suggesting a need for different data granularity and visualization options.

## 4:33:08 PM
The provided logs detail recent development efforts focused on enhancing the analytics dashboard and its custom view capabilities within the `t360-frontend-v2` project.

**File-Specific Updates:**

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx`**
This file, responsible for managing custom analytics views, underwent several updates, primarily to introduce support for "AIR Wise" metrics.
*   **1/20/2026, 1:49:22 PM:** The initial logged state shows support for "Container Wise" and "MBL Wise" tabs, allowing users to create, edit, save, delete, and audit custom dashboards. Metric selection is limited to a maximum of 9 items.
*   **1/21/2026, 12:55:46 PM:** The `selectedTab` state's type was expanded to include `"airWise"`, preparing for the new category.
*   **1/21/2026, 12:57:42 PM:** Conditional rendering logic was updated to display `TreeViewContainer` for `viewOptions?.airWise` when "AIR Wise" is selected.
*   **1/21/2026, 12:58:45 PM:** A `console.log` for `viewOptions` was added, likely for debugging purposes.
*   **1/21/2026, 1:18:10 PM:** The `initializeCheckedMetrics` function was modified to iterate over `airWise` metrics alongside `containerWise` and `mblWise`, ensuring proper initialization for the new category.
*   **1/21/2026, 1:21:49 PM:** The `tabData` array was updated to include `{ label: "AIR Wise", value: "airWise" }`, making the new tab visible in the UI.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx`**
This component handles the analytics dashboard's toolbar, including filters, dashboard selection, and export functions.
*   **1/20/2026, 1:50:39 PM:** The initial version shows functionality for date filtering (local/global, custom ranges), PDF export of the dashboard (including a logo), and managing custom views via a "New" button and editable view chips.
*   **1/20/2026, 6:38:07 PM & 1/20/2026, 6:38:27 PM:** The logic for checking the "Default" dashboard was standardized. Specifically, `"_Default_"` was replaced with `"Default"` in comparisons and display labels related to `selectedDashobard` and the `ChipsTabView` options.
*   **1/21/2026, 1:19:45 PM:** A `console.log` for `showDrawer` was added, likely for debugging.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\analytics-slice.ts`**
This Redux slice manages the analytics-related state.
*   **1/20/2026, 1:56:23 PM:** The initial state for `selectedDashobard` was `"_Default_"`. The `setDashboard` reducer showed some inconsistency in handling `"Default"` vs `"_Default_"` when inserting or selecting dashboards.
*   **1/20/2026, 1:57:02 PM:** The `initialState.selectedDashobard` was changed from `"_Default_"` to `"Default"`.
*   **1/20/2026, 6:35:22 PM:** Inside the `setDashboard` reducer, the assignment `state.selectedDashobard = "_Default_"` was changed to `state.selectedDashobard = "Default"` when `_Default_` was detected in the incoming value.
*   **1/20/2026, 6:36:16 PM, 1/20/2026, 6:36:30 PM, 1/20/2026, 6:37:08 PM:** No functional code changes were observed in these timestamps; they likely represent minor saves.
*   **1/20/2026, 6:39:02 PM:** The `setDashboard` reducer completed the standardization of the "Default" dashboard name. It now consistently checks for `value.includes("Default")` and prepends `"Default"` (without underscores) to the `dashboards` array, also setting `selectedDashobard` to `"Default"`.

**`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\data\options\global.ts`**
This file defines various constants and static data for the application.
*   **1/20/2026, 6:41:28 PM:** The log indicates a comprehensive set of static configurations, including various status definitions (e.g., `SHIPMENT_STATUS`, `AIR_SHIPMENT_STATUS`), role definitions, filter options (e.g., `DATE_FILTER_OPTIONS`), inventory types, default analytics API endpoints, Excel export column definitions, and an extensive `ANALYTICS_REPORT_HEADER` mapping for different analytics reports (CBM, TEU, KGS, Transit Time for Container-Wise and MBL-Wise views).

**Patterns and Recurring Elements:**

*   **Analytics Feature Enhancement:** A primary focus of these changes is the continued development and expansion of the analytics dashboard, particularly the ability to create and manage custom views.
*   **Standardization of "Default" Dashboard Name:** Across `AnalyticsToolbar.tsx` and `analytics-slice.ts`, there was a consistent effort (around **1/20/2026, 6:30 PM**) to standardize the naming of the default dashboard from `"_Default_"` to `"Default"` for both internal logic and UI display.
*   **Introduction of "AIR Wise" Analytics Category:** A new category for air freight-specific analytics ("AIR Wise") was progressively integrated into the `ViewContainer.tsx` component on **1/21/2026** (between 12:55 PM and 1:21 PM), expanding the types of custom views users can create.
*   **Redux for State Management:** The `analytics-slice.ts` Redux slice plays a crucial role in managing the application's analytics state, with updates reflecting new features and logic changes in the UI.
*   **Debugging Practices:** Several `console.log` statements were added in the UI components, indicating active debugging during these development periods.