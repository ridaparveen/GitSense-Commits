...
## 3:07:10 PM
The log entries cover a single file, `c:\Users\ridap\OneDrive\Documents\T360-V2\t360-backend-v2\src\common\util\booking.query.ts`, across two timestamps: `1/19/2026, 6:44:58 PM` and `1/19/2026, 7:29:32 PM`.

**File-Specific Updates & Content Analysis:**
The code content for both timestamps is identical in the provided log snippets. This indicates no functional changes were introduced between `6:44:58 PM` and `7:29:32 PM` on January 19, 2026.

The file `booking.query.ts` defines the `bookingQueryBuilder` asynchronous function responsible for constructing various SQL queries related to booking analytics. Key functionalities include:

*   **Query Types:** It handles four distinct query types, primarily focused on calculating the time difference between `Booking Date` and `Live Actual Departed Date`:
    1.  `BookingDatetoActualDeparture`: Calculates the average day difference per `fileno` and `contno`, and provides its percentage relative to the maximum value.
    2.  `BookingDatetoActualDepartureMblWise`: Similar to the above, but aggregates per `fileno` and `mblno`.
    3.  `BookingDatetoActualDepartureStackedBar`: Generates data for a stacked bar chart, categorizing the day differences into predefined ranges (e.g., "Less than 1 week", "1 - 2 weeks", "Above 5 weeks") based on `departure_date`.
    4.  `BookingDatetoActualDepartureMblWiseStackedBar`: Similar to the stacked bar chart, but performs the analysis master bill of lading (MBL) wise.

*   **Customization:** The function dynamically updates a `customizer` object based on the query `type`, setting properties like `dateRange`, `calType` (e.g., "avg"), `dateColumns`, `booking`, `mblwise`, and `chartType` (e.g., "StackedBar").
*   **Data Sources:** All queries join `mtr_tracking_data` and `agent_booking_entry` tables using the `mblno` field.
*   **Filtering:** Queries are filtered by a list of `CompanyName` values, `fromDate`, `toDate`, and ensure that date fields (`t49_vessel_actualdeparted_date`, `bookingdate`) are not `NULL` or invalid ('0001-01-01 00:00:00 BC').
*   **Utility Functions:** It imports and utilizes `getColumnsByCategory` to populate `selectedColumns` for the `customizer` and `getFilterQueryByDisplayValue` to construct dynamic `SELECT`, `DATE_TRUNC`, `GROUP BY`, and `ORDER BY` clauses for the stacked bar charts.

**Timestamps of Significant Changes:**
Based on the provided log, there are no visible code changes between `1/19/2026, 6:44:58 PM` and `1/19/2026, 7:29:32 PM`. Both log entries contain the exact same code snippet.

**Patterns or Recurring Elements:**
*   **Consistent Date Range Filtering:** All queries include `departure_date >= '${fromDate}'::timestamp AND departure_date < '${toDate}'::timestamp + INTERVAL '1 day'`.
*   **Handling Invalid Dates:** All queries explicitly check for `IS NOT NULL` and `!= '0001-01-01 00:00:00 BC'` for relevant date columns.
*   **Company Filtering:** The `companyList` derived from `CompanyName` is consistently used in `LOWER(account_name) IN (...)` clauses.
*   **Customizer Configuration:** The `customizer` object is extensively used and updated with `dateRange: true`, `calType: "avg"`, `dateColumns: ["Live Actual Departed Date", "Booking Date"]`, and a `calColumnConfig` specifying `day_diff`.
*   **DISTINCT ON Clause:** The subqueries for `day_diff` calculations frequently use `DISTINCT ON (m.voyage, m.contno, CASE WHEN m.t49_cont_type IS NOT NULL AND TRIM(m.t49_cont_type) <> '' AND TRIM(m.t49_cont_type) != '0' THEN m.t49_cont_type ELSE m.cont_type END)` to ensure unique container or voyage entries.

## 3:07:21 PM
**File-specific updates:**

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\custom-view\ViewContainer.tsx` (Timestamp: 1/20/2026, 1:49:22 PM)**
    This React component, responsible for creating and managing custom analytics views, underwent significant updates. It now handles:
    *   **View Lifecycle:** Logic for "add", "edit", and "new" actions, including initializing the view name and checkbox states based on the action type.
    *   **Metric Selection:** Manages a `checkBoxManager` to track selected metrics, enforcing a maximum limit of 9 metrics per view.
    *   **API Integration:** Utilizes `useFetchDashboardOptionsQuery` to retrieve available metrics and `useSaveDasboardMutation` to persist (POST/PUT) or delete (DELETE) custom dashboard configurations. API calls include `apiKey` and `userid`.
    *   **User Feedback & State Updates:** Employs `react-hot-toast` for success/error notifications and dispatches Redux actions (`setRefetchAll`, `setDashboard`) to update the application's overall analytics state.
    *   **Audit Functionality:** Includes a button to open an `AuditViewDrawer`.
    *   **UI Elements:** Incorporates `RadioNav` for tab switching (Container Wise/MBL Wise), `InputBox` for view naming, and `PopupAlert` for confirmation dialogs (e.g., for deletion). A commented-out section shows a previous iteration of the `handleSave` logic.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\components\screen\analytics\AnalyticsToolbar.tsx` (Timestamp: 1/20/2026, 1:50:39 PM)**
    The analytics dashboard toolbar component received updates to enhance user interaction and reporting features:
    *   **Date Filtering:** Improved date filtering options with `RadioNav` for "Local" or "Global" scope, `SelectBox` for predefined date ranges, and a `DateRange` component within a `PopoverPanel` for custom date selections.
    *   **PDF Export:** A new feature to download the current dashboard as a PDF, leveraging `jspdf` and `html2canvas-pro`. This includes logic to add a company logo to the exported PDF and displays a `CircularProgress` during the export process.
    *   **Dashboard Management:** Integrates with the `ViewContainer` component via an `AppDrawer`, allowing users to "Create" new custom views or "Manage" (edit) existing ones.
    *   **Chart Interaction:** Provides `RadioNav` options to switch between "move" and "resize" operations for dashboard widgets.
    *   **Dashboard Selection:** Uses `ChipsTabView` to display and select different custom dashboards.

*   **`c:\Users\ridap\OneDrive\Documents\T360-V2\t360-frontend-v2\src\store\fretures\analytics-slice.ts` (Timestamp: 1/20/2026, 1:56:23 PM and 1/20/2026, 1:57:02 PM)**
    The Redux slice managing the analytics state was updated in two quick succession commits:
    *   **Initial State Expansion (1:56:23 PM):** New properties were added to the `initialState` including `selectedCustomer`, `filterType` (global/local), `periodFilter`, `viewBy`, and a `refetchAll` toggle. The `selectedDashobard` was initialized to `"_Default_"`.
    *   **`setDashboard` Reducer Refinement:** The `setDashboard` reducer was updated to handle both selecting and inserting dashboards, with specific logic to manage "_Default_" and "Default" views. An older version of this reducer was commented out.
    *   **New Reducers:** Added `setRefetchAll` (toggles `refetchAll` boolean), `setCustomer`, and `setSelectedColumns` (for managing column visibility within metrics).
    *   **`selectedDashobard` Initialization Correction (1:57:02 PM):** A minor, immediate follow-up change adjusted the `initialState` value of `selectedDashobard` from `"_Default_"` to `"Default"`.

**Timestamps of significant changes:**
All documented code changes occurred on **January 20, 2026**, within a concentrated timeframe between approximately 1:49 PM and 1:57 PM. This suggests a period of active development or bug fixing specifically on the analytics dashboard features, including custom view creation/management, toolbar functionalities, and the underlying Redux state logic.

**Patterns or recurring elements:**
*   **Analytics Feature Development:** The changes consistently focus on enhancing the analytics dashboard's functionality, particularly around user-defined views, data filtering, and reporting.
*   **Redux State Management:** There is a strong reliance on Redux for managing complex application state, with clear actions and reducers to handle different aspects of the analytics data and UI.
*   **Modularity and Reusability:** The use of dedicated React components (e.g., `ViewContainer`, `AnalyticsToolbar`) and common UI components (e.g., `RadioNav`, `SelectBox`, `AppDrawer`) promotes a modular and reusable architecture.
*   **API-driven UI:** Both `ViewContainer` and `AnalyticsToolbar` interact with backend APIs (implied by API hooks) for data fetching and persistence, indicating a data-driven approach to the dashboard.
*   **Date Filter Logic:** A recurring theme is the implementation of flexible date filtering, supporting global, local, and custom ranges, often utilizing a `dateFilterHandle` utility.
*   **Commented-out Code:** The presence of commented-out code in both `ViewContainer.tsx` and `analytics-slice.ts` suggests an iterative development process where previous logic or alternatives were preserved during refinement.